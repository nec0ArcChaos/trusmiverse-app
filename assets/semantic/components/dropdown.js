...
(function ($, window, document) {
  ...
  $.fn.dropdown_se = function (parameters) {
    var $allModules = $(this),
      $document = $(document),
      time = Date.now(),
      performance = [],
      ...
      contextCheck = function (context, win) {
        ...
        if ([window, document].indexOf(context) >= 0) {
          ...
          if ($context.length === 0) {
            $context = win.frameElement
              ? contextCheck(context, win.parent)
              : window;
          }
        }
    ...
    $allModules.each(function (elementIndex) {
      var settings = $.isPlainObject(parameters)
          ? $.extend(true, {}, $.fn.dropdown_se.settings, parameters)
          : $.extend({}, $.fn.dropdown_se.settings),
      ...
      module = {
        ...
        setup: {
          ...
          layout: function () {
            ...
            if (module.is.clearable() && !module.has.clearItem()) {
              ...
              $clear = $("<i />").addClass("remove icon").insertAfter($icon);
            }
        ...
        queryRemote: function (query, callback, callbackParameters) {
          ...
          apiSettings = $.extend(
            ...
            apiCallbacks,
            tempDisableApiCache ? { cache: false } : {}
          );
          ...
          if (module.has.query()) {
            ...
            $item.each(function () {
              ...
              if (settings.match === "both" || settings.match === "text") {
                ...
                if (
                  text.search(beginsWithRegExp) !== -1 ||
              ...
              if (settings.match === "both" || settings.match === "value") {
                ...
                if (
                  value.search(beginsWithRegExp) !== -1 ||
                  (settings.fullTextSearch === "exact" &&
            ...
            $divider
              ...
              .filter(function () {
                ...
                return (
                  ...
                  ($lastDivider.length > 0 ? $lastDivider : $(this))
        ...
        event: {
          paste: function (event) {
            ...
            if (notFoundTokens.length > 0) {
              ...
              $search.val(
                orgText.slice(0, startPos) + pasteText + orgText.slice(endPos)
              );
          ...
          test: {
            toggle: function (event) {
              ...
              if (
                module.determine.eventOnElement(event, toggleBehavior) &&
                event.type !== "touchstart"
              ) {
          ...
          keydown: function (event) {
            var pressedKey = event.which,
              isShortcutKey =
                module.is.inObject(pressedKey, keys) ||
                event.key === settings.delimiter;
            if (isShortcutKey) {
              var $currentlySelected = $item
                  .not(selector.unselectable)
                  .filter("." + className.selected)
                ...
                delimiterPressed =
                  event.key === settings.delimiter && module.is.multiple(),
              ...
              if (module.is.visible()) {
                ...
                if (pressedKey === keys.upArrow) {
                  ...
                  if (
                    ...
                    module.is.single() &&
                    !$nextItem.hasClass(className.actionable)
                  ) {
          ...
          activate: function (text, value, element) {
            ...
            if (module.can.activate($(element))) {
              module.set.selected(
                ...
                $(element),
                false,
                settings.keepSearchTerm
              );
              if (
                !module.is.multiple() &&
                !(
                  !settings.collapseOnActionable &&
                  $(element).hasClass(className.actionable)
                )
              ) {
                module.hideAndClear();
              }
                ...
                if (isMultiple) {
                  if (
                    $.inArray(
                      ...
                      value.map(String).map(module.escape.htmlEntities)
                    ) !== -1
                  ) {
        ...
        scrollPage: function (direction, $selectedItem) {
          ...
          if ($nextSelectedItem.length > 0) {
            ...
            if (
              settings.selectOnKeydown &&
              module.is.single() &&
              !$nextSelectedItem.hasClass(className.actionable)
            ) {
          ...
          selected: function (
            ...
            $selectedItem.each(function () {
              var $selected = $(this),
                ...
                shouldAnimate =
                  isMultiple && $selectedItem && $selectedItem.length === 1;
              if (isActionable) {
                if (
                  (!isMultiple || !isActive || isUserValue) &&
                  settings.apiSettings &&
                  settings.saveRemoteData
                ) {
                  module.save.remoteData(selectedText, selectedValue);
                }
                settings.onActionable.call(
                  element,
                  selectedValue,
                  selectedText,
                  $selected
                ...
                if (
                  !keepSearchTerm &&
                  !$selected.hasClass(className.actionable)
                ) {
        ...
        add: {
          ...
          userSuggestion: function (value) {
            var $addition = $menu.children(selector.addition),
              ...
              alreadyHasValue =
                $existingItem &&
                $existingItem.not(selector.addition).length > 0,
          ...
          value: function (
            ...
            if (Array.isArray(currentValue)) {
              newValue =
                $selectedItem && $selectedItem.hasClass(className.actionable)
                  ? currentValue
        ...
        is: {
          ...
          allFiltered: function () {
            return (
              ...
              !(!settings.hideAdditions && module.has.userSuggestion()) &&
              !module.has.message() &&
          ...
          noApiCache: function () {
            return (
              tempDisableApiCache ||
              (settings.apiSettings && !settings.apiSettings.cache)
            );
          },
          single: function () {
          ...
          selectMutation: function (mutations) {
            ...
            $.each(mutations, function (index, mutation) {
              if (
                $(mutation.target).is("option, optgroup") ||
          ...
          searchSelection: function (deep) {
            return (
              module.has.search() &&
              (deep
                ? $search.parents(selector.dropdown)
                : $search.parent(selector.dropdown)
              ).length === 1
            );
          },
          selection: function () {
          ...
          verticallyScrollableContext: function () {
            var overflowY =
              $context[0] !== window ? $context.css("overflow-y") : false;
            return overflowY === "auto" || overflowY === "scroll";
          },
          horizontallyScrollableContext: function () {
            var overflowX =
              $context[0] !== window ? $context.css("overflow-X") : false;
          ...
          htmlEntities: function (string, forceAmpersand) {
            ...
            if (shouldEscape.test(string)) {
              string = string.replace(
                forceAmpersand ? /&/g : /&(?![\d#a-z]{1,12};)/gi,
                "&amp;"
              );
  ...
  /* Templates */
  $.fn.dropdown_se.settings.templates = {
    ...
    dropdown: function (select, fields, preserveHTML, className) {
      var placeholder = select.placeholder || false,
        html = "",
        escape = $.fn.dropdown_se.settings.templates.escape,
        deQuote = $.fn.dropdown_se.settings.templates.deQuote;
      ...
      html += $.fn.dropdown_se.settings.templates.menu(
        select,
        fields,
        preserveHTML,
        className
      ...
      $.each(values, function (index, option) {
        ...
        if (itemType === "item" || isMenu) {
          ...
          html +=
            ...
            deQuote(
              maybeActionable +
                ...
                (option[fields.class] || className.item)
            ) +
            '" data-value="' +
          ...
          if (option[fields.image]) {
            html +=
              '<img class="' +
              deQuote(option[fields.imageClass] || className.image) +
              '" src="' +
          ...
          if (option[fields.icon]) {
            html +=
              ...
              deQuote(
                option[fields.icon] +
                  " " +
                  (option[fields.iconClass] || className.icon)
              ) +
              '"></i>';
          }
          if (hasDescription) {
            html +=
              '<span class="' +
              deQuote(className.description) +
              '">' +
              escape(option[fields.description] || "", preserveHTML) +
              "</span>";
            html += !isMenu
              ? '<span class="' + deQuote(className.text) + '">'
              : "";
          }
          if (isMenu) {
          ...
          if (groupName !== "" || groupIcon !== "") {
            html +=
              '<div class="' +
              deQuote(option[fields.class] || className.header) +
              '">';
            if (groupIcon !== "") {
              html +=
                '<i class="' +
                deQuote(
                  groupIcon + " " + (option[fields.iconClass] || className.icon)
                ) +
                '"></i>';
            }
            html += groupName;
    ...
    label: function (value, text, preserveHTML, className) {
      var escape = $.fn.dropdown_se.settings.templates.escape,
        deQuote = $.fn.dropdown_se.settings.templates.deQuote;
      return (
        escape(text, preserveHTML) +
        '<i class="' +
        deQuote(className.delete) +
        ' icon"></i>'
      );
    },
