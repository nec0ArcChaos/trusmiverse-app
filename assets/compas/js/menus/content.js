...
window.LoadInit['menus']['content'] = function (container) {
...
function initDateRange(view) {
    ...
    function cb(start, end) {
        ...
        loadContent(view, start.format('YYYY-MM-DD'), end.format('YYYY-MM-DD'));
    }
...
async function loadContent(view, start, end) {
    $.ajax({
        url: `${BASE_URL}compas/content/load`,
        type: "POST",
        data: {
            start: start,
            end: end
        ...
        success: function (response) {
            ...
            switch (view) {
                case 'kanban':
                    loadContentKanban(response.data);
                    break;
                case 'list':
                    loadContentList(response.data);
                    break;
                case 'calendar':
                    loadContentCalendar(response.data);
                    break;
                default:
                    loadContentKanban(response.data);
                    break;
...
function loadContentKanban(data) {
    ...
    if (data) {
        ...
        data.forEach(item => {
            ...
            // Calculate Progress
            let actual = parseFloat(item.content_actual) || 0;
            let target = parseFloat(item.content_target) || 1;
                ...
                <div class="card-footer bg-transparent border-top-dashed pt-3 pb-3">
                    <div class="d-flex justify-content-between align-items-center">
                        ...
                        <div class="d-flex align-items-center">
                            ...
                            <div class="text-muted small" title="Content Production">
            ...
            $(`#${KANBAN_STATUS[item.content_status].kanban_status_name.toLowerCase().replace(' ', '')}column`).append(card);
        });
    }
}
...
function loadContentList(data) {
    ...
    // Status config: maps content_status int → badge config
    const STATUS_CONFIG = {
    ...
    const rowsData = data.map(item => {
        const statusKey = item.content_status || 1;
...
function loadContentCalendar(data) {
    // ── Status config ──────────────────────────────────────────────────────────
    const STATUS_CONFIG = {
...
function loadContentStats() {
    ['#content-stat-avg-lead', '#content-stat-avg-ai', '#content-stat-total', '#content-stat-approved'].forEach(id => $(id).text('—'));
    const fd = new FormData();
    fetch(`${BASE_URL}compas/content/get_content_stats`, { method: 'POST', body: fd })
        .then(r => r.json())
        .then(result => {
            ...
            $('#content-stat-avg-lead').text(lead > 0 ? lead + 'd' : '0d');
            countUp('#content-stat-avg-ai', d.avg_ai_score || 0);
            countUp('#content-stat-total', d.total_submissions || 0);
            countUp('#content-stat-approved', d.approved_plans || 0);
        })
        .catch(err => console.warn('loadContentStats error:', err));
}