...
window.LoadInit['menus']['talent'] = function (container) {
...
function initDateRange(view){
	...
	function cb(start, end) {
		...
		loadTalent(view, start.format('YYYY-MM-DD'), end.format('YYYY-MM-DD'));
	}
...
async function loadTalent(view, start, end) {
    $.ajax({
			url: `${BASE_URL}compas/talent/load`,
			type: "POST",
			data: {
			    start: start,
			    end: end
			...
			success: function (response) {
                switch (view) {
                    case 'kanban':
                        loadTalentKanban(response.data);
                        break;
                    case 'list':
                        loadTalentList(response.data);
                        break;
                    case 'calendar':
                        loadTalentCalendar(response.data);
                        break;
                    default:
                        loadTalentKanban(response.data);
                        break;
...
function loadTalentKanban(data){
    ...
    if(data){
        ...
        data.forEach(item => {
            ...
            // Calculate Progress
            let actual = parseFloat(item.talent_actual) || 0;
            let target = parseFloat(item.talent_target) || 1;
                ...
                <div class="card-footer bg-transparent border-top-dashed pt-3 pb-3">
                    <div class="d-flex justify-content-between align-items-center">
                        ...
                        <div class="d-flex align-items-center">
                            ...
                            <div class="text-muted small" title="Talent">
            ...
            $(`#${KANBAN_STATUS[item.talent_status].kanban_status_name.toLowerCase().replace(' ', '')}column`).append(card);
        });
    }
}
...
function loadTalentList(data){
    ...
    // Status config: maps activation_status int → badge config
    const STATUS_CONFIG = {
    ...
    const rowsData = data.map(item => {
        const statusKey  = item.talent_status || 1;
...
function loadTalentCalendar(data) {
    // ── Status config ──────────────────────────────────────────────────────────
    const STATUS_CONFIG = {
...
function loadTalentStats() {
    ['#talent-stat-unique', '#talent-stat-avg-ai', '#talent-stat-total', '#talent-stat-approved'].forEach(id => $(id).text('—'));
    
    const fd = new FormData();
    
    fetch(`${BASE_URL}compas/talent/get_talent_stats`, { 
        method: 'POST', 
        body: fd 
    })
    .then(r => r.json())
    .then(result => {
        ...
        countUp('#talent-stat-unique', d.unique_talents || 0);
        countUp('#talent-stat-avg-ai', d.avg_ai_score || 0);
        countUp('#talent-stat-total', d.total_submissions || 0);
        countUp('#talent-stat-approved', d.approved_plans || 0);
    })
    .catch(err => console.warn('loadTalentStats error:', err));
}
