...
function initDateRange(view){
	...
	function cb(start, end) {
		...
		loadActivation(view, start.format('YYYY-MM-DD'), end.format('YYYY-MM-DD'));
	}
...
async function loadActivation(view, start, end) {
...
function loadActivationKanban(data){
    ...
    if(data){
        ...
        data.forEach(item => {
            ...
            // Calculate Progress
            let actual = parseFloat(item.activation_actual) || 0;
            let target = parseFloat(item.activation_target) || 1;
                ...
                <div class="card-footer bg-transparent border-top-dashed pt-3 pb-3">
                    <div class="d-flex justify-content-between align-items-center">
                        ...
                        <div class="d-flex align-items-center">
                            ...
                            <div class="text-muted small" title="Event Activation">
            ...
            $(`#${KANBAN_STATUS[item.activation_status].kanban_status_name.toLowerCase().replace(' ', '')}column`).append(card);
        });
    }
}
...
function loadActivationList(data){
    ...
    const rowsData = data.map(item => {
        const statusKey  = item.activation_status || 1;
...
function loadActivationStats() {
    ...
    fetch(`${BASE_URL}compas/activation/get_activation_stats`, {
        method: 'POST',
        body: formData,
    })
    .then(r => r.json())
    ...
    .catch(err => {
        console.warn('loadActivationStats error:', err);
    });
}
// ── / Activation Stat Cards ─────────────────────────────────────────────────
