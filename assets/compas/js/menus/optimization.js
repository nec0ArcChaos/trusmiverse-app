...
window.LoadInit['menus']['optimization'] = function (container) {
...
function initDateRange(view){
	...
	function cb(start, end) {
		...
		loadOptimization(view, start.format('YYYY-MM-DD'), end.format('YYYY-MM-DD'));
	}
...
async function loadOptimization(view, start, end) {
    $.ajax({
        url: `${BASE_URL}compas/optimization/load`,
        type: "POST",
        data: {
            start: start,
            end: end
        ...
        success: function (response) {
            ...
            switch (view) {
                case 'kanban':
                    loadOptimizationKanban(response.data);
                    break;
                case 'list':
                    loadOptimizationList(response.data);
                    break;
                case 'calendar':
                    loadOptimizationCalendar(response.data);
                    break;
                default:
                    loadOptimizationKanban(response.data);
                    break;
...
function loadOptimizationKanban(data){
    ...
    if(data){
        ...
        data.forEach(item => {
            ...
            // Calculate Progress
            let actual = parseFloat(item.optimization_actual) || 0;
            let target = parseFloat(item.optimization_target) || 1;
                ...
                <div class="card-footer bg-transparent border-top-dashed pt-3 pb-3">
                    <div class="d-flex justify-content-between align-items-center">
                        ...
                        <div class="d-flex align-items-center">
                            ...
                            <div class="text-muted small" title="Optimization">
            ...
            $(`#${KANBAN_STATUS[item.optimization_status].kanban_status_name.toLowerCase().replace(' ', '')}column`).append(card);
        });
    }
}

function loadOptimizationList(data){
    ...
    // Status config: maps optimization_status int → badge config
    const STATUS_CONFIG = {
    ...
    const rowsData = data.map(item => {
        const statusKey  = item.optimization_status || 1;
...
function loadOptimizationCalendar(data) {
    // ── Status config ──────────────────────────────────────────────────────────
    const STATUS_CONFIG = {
...
function loadOptimizationStats() {
    ['#opt-stat-efficiency','#opt-stat-avg-ai','#opt-stat-total','#opt-stat-approved'].forEach(id => $(id).text('—'));
    const fd = new FormData();
    fetch(`${BASE_URL}compas/optimization/get_optimization_stats`, { method: 'POST', body: fd })
    .then(r => r.json())
    .then(result => {
        ...
        $('#opt-stat-avg-lead').text(lead > 0 ? lead + 'd' : '0d');
        countUp('#opt-stat-avg-ai', d.avg_ai_score || 0);
        countUp('#opt-stat-total', d.total_submissions || 0);
        countUp('#opt-stat-approved', d.approved_plans || 0);
    })
    .catch(err => console.warn('loadOptimizationStats error:', err));
}