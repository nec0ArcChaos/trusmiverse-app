...
window.LoadInit["tabs"]["tasklist_project"] = function (container) {
	...
	$("#filter_month").on("change", function () {
		const val = $(this).val();
		const m = moment(val, "YYYY-MM");
		const newStart = m.clone().startOf("month");
		const newEnd = m.clone().endOf("month");

		if ($(".range").data("daterangepicker")) {
			$(".range").data("daterangepicker").setStartDate(newStart);
			$(".range").data("daterangepicker").setEndDate(newEnd);
		}

		cb(newStart, newEnd);
	});

	// cb(start, end); // Moved to end
	window.ssFiltersUser = window.ssFiltersUser || {};
	/* ───── Gantt Chart ───── */
	(function initGantt() {
		/* ══════════════════════════════════════════════════════════════════
        DATA STORE
		...
		function populateFilters() {
			...
			function initNewSlimSelect(id, data) {
				window.ssFiltersUser[id] = new SlimSelect({
					select: id,
					settings: {
						showSearch: true,
		...
		function getInitials(fullName) {
			...
			var l = parts.length > 1 ? parts[parts.length - 1].charAt(0) : "";
		...
		window.selectGanttDropdownVal = function (val, e) {
			...
			if (
				...
				if (
					...
					window.ganttOpenPop(dId, dType, e, true, true);
					return;
				}
				...
				function (res) {
					...
					if (data.status === "success") {
						...
						proj.tasks.push({
							...
							type: "task",
							text: "New Task",
							pic: getInitials(CURRENT_USER_FULLNAME),
							pic_id: CURRENT_USER_ID,
							status: "Not Started",
							status_icon: sFound ? sFound.status_icon : "bi-search",
							status_style: sFound ? sFound.style : "",
							category: "-",
		...
		// Execute data loading immediately
		cb(start, end);
		bindSplitter();
	})();
};
