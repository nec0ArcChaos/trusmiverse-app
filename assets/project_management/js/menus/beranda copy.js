...
window.LoadInit["menus"]["beranda"] = function (container) {
    ...
    function renderTaskByStatusChart(data) {
        ...
        if (_chartTaskByStatus) { _chartTaskByStatus.destroy(); }
        ...
        $({ n: 0 }).animate({ n: total }, {
            duration: 1400,
            easing: 'swing',
            step: function () { _displayTotal = Math.round(this.n); },
            complete: function () { _displayTotal = total; }
        });

        _chartTaskByStatus = new Chart(ctx, {
            type: 'doughnut',
            data: {
                labels: ['Not Started', 'Progress', 'Done', 'QA', 'Need Data', 'Cancelled'],
                datasets: [{
                    data: ds,
                    backgroundColor: ['#9ca3af', '#3b82f6', '#10b981', '#a855f7', '#eab308', '#ef4444'],
                    borderWidth: 2,
                    borderColor: '#ffffff',
                    hoverOffset: 6
                }]
    ...
    function loadTaskOverview(d) {
        ...
        var avgPct = weeks.reduce(function(sum, w) {
            return sum + (w.total > 0 ? (w.done / w.total) * 100 : 0);
        }, 0) / weeks.length;

        // ── Weekly avg bar & color ──
        var avgColor = avgPct >= 80 ? 'var(--green)' : (avgPct >= 50 ? '#f59e0b' : '#ef4444');
        var avgBar = avgPct >= 80 ? 'linear-gradient(90deg,#22c55e,#16a34a)' : (avgPct >= 50 ? 'linear-gradient(90deg,#f59e0b,#d97706)' : 'linear-gradient(90deg,#ef4444,#dc2626)');
        
        setTimeout(function() {
            $('#bar-weekly-avg').css({'width': avgPct.toFixed(1) + '%', 'background': avgBar});
        }, 50);
        
        $({ n: 0 }).animate({ n: avgPct }, {
            duration: 1400, easing: 'swing',
            step: function() {
                $('#val-weekly-avg').text(parseFloat(this.n).toFixed(1) + '%').css('color', avgColor);
            },
            complete: function() {
        ...
        weeks.forEach(function(w, i) {
            ...
            var rowColor = pctVal === 100 ? 'var(--green)' : (pctVal >= 50 ? '#f59e0b' : '#ef4444');
            var key = w.name;

            weekHtml += '<div class="d-flex align-items-center justify-content-between" style="font-size:11px; line-height:1.2; padding: 1px 0;">' +
                        '<span class="sm-label mb-0" style="width:52px; font-weight:600;">' + key + '</span>' +
                        '<span style="display: inline-block; width: 65px; background:#f1f3f5; color:#555; padding: 2px 6px; border-radius: 4px; font-size: 10px; margin-right: 4px;">Total: <strong>' + w.total + '</strong></span>' +
                        '<span style="display: inline-block; width: 65px; background:#1a1a1a; color:#fff; padding: 2px 6px; border-radius: 4px; font-size: 10px;">Done: <strong>' + w.done + '</strong></span>' +
                        '<span class="cpr-value animate-pct-val" data-val="'+pctVal+'" id="val-' + key.toLowerCase() + '-pct" style="min-width:48px; text-align:right; font-weight:700; color:'+rowColor+';">0%</span>' +
                        '</div>';
        });
        ...
        $('.animate-pct-val').each(function() {
            ...
            $({ n: 0 }).animate({ n: toVal }, {
                duration: 1400, easing: 'swing',
                step: function() { $el.text(parseFloat(this.n).toFixed(1) + '%'); },
                complete: function() { $el.text(toVal.toFixed(1) + '%'); }
            });
        });

        // ── Ringkasan Pekerjaan ──
        var ontimePct = totalTasks > 0 ? ((d.ontime / totalTasks) * 100).toFixed(1) : '0.0';
        var latePct   = totalTasks > 0 ? ((d.late   / totalTasks) * 100).toFixed(1) : '0.0';
        ...
        $({ n: 0 }).animate({ n: parseFloat(ontimePct) }, {
            duration: 1400, easing: 'swing',
            step: function() {
        ...
        $({ n: 0 }).animate({ n: d.ontime }, {
            duration: 1400, easing: 'swing',
            step: function() { $('#val-task-ontime').text(Math.round(this.n)); },
            complete: function() { $('#val-task-ontime').text(d.ontime); }
        });

        $({ n: 0 }).animate({ n: d.late }, {
            duration: 1400, easing: 'swing',
            step: function() {
                $('#val-task-late').text(Math.round(this.n));
                $('#val-task-late-pct').text(parseFloat(latePct * (this.n / d.late)).toFixed(1) + '%');
            },
            complete: function() {
    ...
    function cb(start, end) {
        ...
        $.post(BASE_URL + 'project_management/beranda/ajax_dashboard_stats', {
            ...
            if (res) {
                if (window.Chart) {
                    ...
                    s.onload = function() { renderTaskByStatusChart(res.statuses); };
        ...
        if (window.loadGanttDataWithDates) {
            ...
            window.loadGanttDataWithDates(start.format('YYYY-MM-DD'), end.format('YYYY-MM-DD'));
    ...
    (function initGantt() {
        ...
        var nextId = 50;
        var projects = [];
        var ganttLookups = { companies: [], statuses: [], categories: [], employees: [] };
        ...
        var GANTT_COLS_DEFAULT = [
            { key: 'text',           label: 'Project / Task Name', width: 250, minWidth: 250, maxWidth: 300, sortField: 'text',           fixed: true,  frozen: true,  visible: true,  align: 'left'   },
            { key: 'company',        label: 'Company',             width: 100, minWidth: 60,  sortField: 'company',         frozen: false, visible: true,  align: 'center' },
            { key: 'pic',            label: 'PIC',                 width: 120, minWidth: 60,  sortField: 'pic',             frozen: false, visible: true,  align: 'center' },
            { key: 'requester',      label: 'Requester / PO',      width: 120, minWidth: 80,  sortField: 'requester',       frozen: false, visible: true,  align: 'center' },
            { key: 'evidence',       label: 'Evidences',           width: 100, minWidth: 70,  sortField: null,              frozen: false, visible: true,  align: 'center' },
            { key: 'status',         label: 'Status',              width: 100, minWidth: 70,  sortField: 'status',          frozen: false, visible: true,  align: 'center' },
            { key: 'category',       label: 'Category',            width: 120, minWidth: 70,  sortField: 'category',        frozen: false, visible: true,  align: 'center' },
            { key: 'week',           label: 'Week',                width: 70,  minWidth: 50,  sortField: 'week',            frozen: false, visible: true,  align: 'center' },
            { key: 'progress',       label: 'Progress',            width: 90,  minWidth: 60,  sortField: 'progress',        frozen: false, visible: true,  align: 'center' },
            { key: 'start',          label: 'Tgl Mulai',           width: 140, minWidth: 100, sortField: 'start',           frozen: false, visible: true,  align: 'center' },
            { key: 'estimasi',       label: 'Estimasi',            width: 70,  minWidth: 50,  sortField: 'estimasi',        frozen: false, visible: true,  align: 'center' },
            { key: 'end',            label: 'Tgl Deadline',        width: 140, minWidth: 100, sortField: 'end',             frozen: false, visible: true,  align: 'center' },
            { key: 'tglSelesai',     label: 'Tgl Selesai',         width: 140, minWidth: 100, sortField: 'tglSelesai',      frozen: false, visible: true,  align: 'center' },
            { key: 'leadtime_pct',   label: 'Leadtime (%)',        width: 90,  minWidth: 60,  sortField: 'leadtime_pct',    frozen: false, visible: true,  align: 'center' },
            { key: 'leadtime_status',label: 'Status Leadtime',     width: 110, minWidth: 80,  sortField: 'leadtime_status', frozen: false, visible: true,  align: 'center' },
            { key: 'leadtime_diff',  label: 'Diff LT',             width: 70,  minWidth: 50,  sortField: 'leadtime_diff',   frozen: false, visible: true,  align: 'center' },
            { key: 'leadtime_days',  label: 'LT Days',             width: 60,  minWidth: 40,  sortField: 'leadtime_days',   frozen: false, visible: true,  align: 'center' }
        ];

        // Deep clone helper
        function cloneCols(arr) { return arr.map(function(c){ return $.extend({}, c); }); }
        ...
        window.loadGanttDataWithDates = function(start_str, end_str) {
            ...
            $.when(
                ...
                $.getJSON(BASE_URL + 'project_management/beranda/get_gantt_data', { start_date: start_str, end_date: end_str }),
        ...
        function loadGanttData() {
            var start = $('#start_date').val() || moment().startOf('month').format('YYYY-MM-DD');
            var end   = $('#end_date').val() || moment().endOf('month').format('YYYY-MM-DD');
        ...
        function updateSortIcons() {
            ...
            $('#gantt-col-header [data-sort="'+ganttSort.field+'"] .sort-icon').html('<i class="bi '+iconClass+'" style="margin-left:4px; font-size:12px; color:var(--accent);"></i>');
        }

        $(document).on('click', '.g-sortable', function(e) {
            if ($(e.target).closest('.gcol-resize-handle, .gcol-freeze-btn').length) return;
        ...
        function applyViewConfig(cfg) {
            ...
            var order     = typeof cfg.column_order   === 'string' ? JSON.parse(cfg.column_order)   : cfg.column_order;
            var hidden    = typeof cfg.hidden_columns  === 'string' ? JSON.parse(cfg.hidden_columns)  : (cfg.hidden_columns  || []);
            var widths    = typeof cfg.column_widths   === 'string' ? JSON.parse(cfg.column_widths)   : (cfg.column_widths   || {});
            var frozen    = typeof cfg.frozen_columns  === 'string' ? JSON.parse(cfg.frozen_columns)  : (cfg.frozen_columns  || []);

            if (cfg.sort_field) { ganttSort.field = cfg.sort_field; ganttSort.dir = cfg.sort_dir || 'asc'; }
            if (cfg.grid_width) {
            ...
            order.forEach(function(key) {
                var col = ganttColumns.find(function(c){ return c.key === key; });
                if (col) ordered.push(col);
            });
            // Append any unsaved new columns that weren't in saved order
            ganttColumns.forEach(function(c) {
                if (!ordered.find(function(o){ return o.key === c.key; })) ordered.push(c);
            });
        ...
        function getViewState() {
            var widths = {}, frozen = [];
            var order = ganttColumns.map(function(c){ return c.key; });
            var hidden = ganttColumns.filter(function(c){ return !c.visible; }).map(function(c){ return c.key; });
            ganttColumns.forEach(function(c) {
        ...
        ══════════════════════════════════════════════════════════════════ */
        var _dragCol = null, _dragFromIdx = -1;
        ...
        function renderColHeader() {
            ...
            var stickyLeft = 0;
            var visibleCols = ganttColumns.filter(function(c){ return c.visible; });
            ...
            visibleCols.forEach(function(col, idx) {
                ...
                var stickyStyle = isFrozen ? 'position:sticky; left:'+stickyLeft+'px; z-index:3; background:#f8fafc;' : '';
                ...
                var textStyle = (col.key === 'text' && !isFrozen) ? 'min-width:'+col.minWidth+'px; max-width:'+col.maxWidth+'px; flex:1;' : 'width:'+w+'px;';
                var alignStyle = col.align === 'center' ? 'justify-content:center;' : '';
                var freezeIcon = !isFixed ? '<button class="gcol-freeze-btn" title="'+(isFrozen?'Unfreeze':'Freeze')+' column" data-key="'+col.key+'" style="background:none;border:none;padding:0;cursor:pointer;color:'+(isFrozen?'var(--accent)':'#d1d5db')+';font-size:11px; flex-shrink:0;"><i class="bi bi-pin'+(isFrozen?'-fill':'')+'"></i></button>' : '';
                ...
                var $col = $('<div>')
                    ...
                    .css({ 
                        ...
                        'min-width': (col.key === 'text' && !isFrozen) ? col.minWidth+'px' : w+'px', 
                    ...
                    .html('<span class="gcol-label" style="flex:1; overflow:hidden; text-overflow:ellipsis; white-space:nowrap;">'+col.label+'</span><span class="sort-icon"></span>'+freezeIcon+ (!isFixed ? '<span class="gcol-resize-handle" data-key="'+col.key+'" style="width:5px;cursor:col-resize;position:absolute;right:0;top:0;height:100%;z-index:4;background:transparent;"></span>' : ''));
                ...
                if (!isFixed) {
                    $col.attr('draggable', true)
                        ...
                        .on('drop', function(e) {
                            ...
                            var fromIdx = ganttColumns.findIndex(function(c){ return c.key === _dragCol; });
                            var toIdx   = ganttColumns.findIndex(function(c){ return c.key === toKey; });
            ...
            $hdr.find('.gcol-freeze-btn').on('click', function(e) {
                ...
                var fixed   = ganttColumns.filter(function(c){ return c.fixed; });
                var frozen  = ganttColumns.filter(function(c){ return c.frozen && !c.fixed; });
                var rest    = ganttColumns.filter(function(c){ return !c.frozen && !c.fixed; });
        ...
        function renderSettingsPanel() {
            ...
            // Bulk toggle row
            var configurableCols = ganttColumns.filter(function(c) { return !c.fixed; });
            var visibleConfigurable = configurableCols.filter(function(c) { return c.visible; });
            var isAllChecked = (visibleConfigurable.length === configurableCols.length && configurableCols.length > 0);
            
            var $bulkRow = $('<div>').css({
                display: 'flex', alignItems: 'center', gap: '12px', padding: '10px 16px',
                background: '#f8fafc', borderBottom: '1px solid #e2e8f0', marginBottom: '4px',
                cursor: 'pointer'
            });
            var $bulkChk = $('<input type="checkbox">').prop('checked', isAllChecked).css({ cursor: 'pointer', width: '15px', height: '15px' });
            var $bulkLbl = $('<span>').text(isAllChecked ? 'Unselect All' : 'Select All')
                .css({ fontSize: '11px', fontWeight: '800', textTransform: 'uppercase', letterSpacing: '0.5px', color: '#64748b', flex: 1 });
            ...
            $bulkChk.on('change', function(e) {
                ...
                ganttColumns.forEach(function(c) { if (!c.fixed) c.visible = checked; });
                renderColHeader(); render(); renderSettingsPanel();
            });
            ...
            ganttColumns.forEach(function(col, idx) {
                ...
                var $row = $('<div>')
                    ...
                    .css({ display:'flex', alignItems:'center', gap:'10px', padding:'8px 16px', cursor:'grab', borderBottom:'1px solid #f8fafc', background:'#fff', transition:'background 0.1s' });
                
                var $drag  = $('<i>').addClass('bi bi-grip-vertical').css({ color:'#d1d5db', fontSize:'14px', cursor:'grab', flexShrink:0 });
                var $chk   = $('<input type="checkbox">').prop('checked', col.visible).css({ cursor:'pointer', width:'15px', height:'15px', flexShrink:0 });
                var $pin   = $('<i>').addClass('bi bi-pin'+(col.frozen?'-fill':'')).css({ color: col.frozen?'var(--accent)':'#d1d5db', fontSize:'13px', cursor:'pointer', flexShrink:0, marginLeft:'auto' }).attr('title', col.frozen?'Unfreeze':'Freeze');
                var $label = $('<span>').text(col.label).css({ fontSize:'12px', flex:1, color:'#374151', userSelect:'none' });

                $chk.on('change', function() { col.visible = this.checked; renderColHeader(); render(); renderSettingsPanel(); });
                ...
                $row.on('drop', function(e) {
                    e.preventDefault();
                    $(this).css('border-top', '');
                    if (_dragFromIdxInSettings === undefined || _dragFromIdxInSettings === idx) return;
        ...
        $('#g-settings-close, #gantt-view-settings-overlay').on('click', function() {
        ...
        $('#g-save-view').on('click', function() {
            saveView();
            $(this).html('<i class="bi bi-check-lg"></i> Saved!').css('background','#10b981');
            setTimeout(function(){ $('#g-save-view').html('<i class="bi bi-floppy"></i> Save').css('background',''); }, 1500);
        });
        ...
        function populateFilters() {
            ...
            var filterIds = ['#g-filter-company', '#g-filter-status', '#g-filter-category', '#g-filter-pic'];
            var dataSets = {
            ...
            if (ganttLookups.companies) {
                ganttLookups.companies.forEach(function(c) {
                    if (c.alias) dataSets['#g-filter-company'].push({ text: c.alias, value: c.alias });
                });
            }
            if (ganttLookups.statuses) {
                ganttLookups.statuses.forEach(function(s) {
                    if (s.status_name) dataSets['#g-filter-status'].push({ text: s.status_name, value: s.status_name });
                });
            }
            if (ganttLookups.categories) {
                ganttLookups.categories.forEach(function(c) {
                    if (c.category) dataSets['#g-filter-category'].push({ text: c.category, value: c.category });
                });
            }
            if (ganttLookups.employees) {
                ganttLookups.employees.forEach(function(e) {
                    var name = ((e.first_name||'') + ' ' + (e.last_name||'')).trim();
                    dataSets['#g-filter-pic'].push({ text: name, value: String(e.user_id) });
                });
            }
                ...
                if (window.SlimSelect && window.ssFilters[id]) {
                    ...
                    try {
                        window.ssFilters[id].setData(data);
                         ...
                         try { window.ssFilters[id].destroy(); } catch (err) {}
            ...
            projects.forEach(function(proj) {
                if (proj.tasks && proj.tasks.length > 0) {
                    ...
                    proj.tasks.forEach(function(t) {
                        ...
                        var pics = (t.pic || '').split(',').map(function(s){return s.trim();}).filter(Boolean);
                        pics.forEach(function(p){ picSet[p] = true; });
                        
                        var picIds = (t.pic_id || '').toString().split(',').map(function(s){return s.trim();}).filter(Boolean);
                        picIds.forEach(function(pid){ picIdSet[pid] = true; });
                    });
        ...
        function parseDate(str) {
            ...
            var yr = parseInt(ds[0], 10), mo = parseInt(ds[1]||1, 10)-1, dy = parseInt(ds[2]||1, 10);
            var d = new Date(yr, mo, dy);
            if (parts[1]) {
                var ts = parts[1].split(':');
                d.setHours(parseInt(ts[0]||0, 10), parseInt(ts[1]||0, 10), parseInt(ts[2]||0, 10), 0);
            } else {
                d.setHours(0, 0, 0, 0);
            }
            return d;
        ...
        function weekNum(d) {
            ...
            var getWorkDays = function(w) {
                ...
                for (var i = 1; i <= lastDay.getDate(); i++) {
                    ...
                    if (calcW(curr) === w && curr.getDay() !== 0) { // 0 is Sunday
                        count++;
                    }
                }
                return count;
        ...
        function updateWeekLabel() {
            ...
            $('#g-week-range').text(fmtShort(isoDate(viewStart)) + ' – ' + fmtShort(isoDate(end)));
        }
        ...
        function avHtml(idStr, size, id, fieldType, itemType, hasTasks) {
            ...
            var canEdit = (id !== null && id !== undefined && itemType && (fieldType != 'pic' || !hasTasks));
            if (!idStr) {
                if (!canEdit) return '<span style="color:#a1a1aa;font-size:11px;">—</span>';
                return '<div onclick="openGanttDropdown(event, '+id+', \''+itemType+'\', \''+fieldType+'\')" style="width:'+size+'px;height:'+size+'px;border-radius:50%;border:1px dashed #cbd5e1;display:flex;align-items:center;justify-content:center;color:#94a3b8;cursor:pointer;font-size:10px;"><i class="bi bi-plus"></i></div>';
            }
            var uids = idStr.toString().split(',').map(function(s){ return s.trim(); }).filter(function(s){return s;});
            var clickAttrs = canEdit ? 'onclick="openGanttDropdown(event, '+id+', \''+itemType+'\', \''+fieldType+'\')" style="cursor:pointer; padding:2px; margin:-2px; border-radius:12px; transition:background 0.2s;" onmouseover="this.style.background=\'#f1f5f9\'" onmouseout="this.style.background=\'transparent\'"' : 'style="padding:2px; margin:-2px; border-radius:12px;"';
            var h = '<div class="av-group" ' + clickAttrs + '>';
            var extraNames = [];
            uids.forEach(function(uid, i) {
                var allUsers = [].concat(ganttLookups.employees || [], ganttLookups.product_owners || []);
                var emp = allUsers.find(function(e) { return e.user_id.toString() === uid; });
                var name = emp ? ((emp.first_name||'') + ' ' + (emp.last_name||'')).trim() : 'Unknown';
                if (i > 2) {
                ...
                var imgSrc = '';
                var uiAv = 'https://ui-avatars.com/api/?name=' + encodeURIComponent(name) + '&background=random&color=fff&size='+size+'&rounded=true&bold=true';
                if (emp && emp.profile_picture) {
                    imgSrc = 'https://trusmiverse.com/hr/uploads/profile/' + emp.profile_picture;
                } else {
                    imgSrc = uiAv;
                }
                var safeName = name.replace(/'/g, "\\'");
                h += '<div class="av-img" style="width:'+size+'px;height:'+size+'px;'+margin+'">' +
                    '<img src="'+imgSrc+'" onmouseenter="ganttShowAvTip(event, \''+safeName+'\')" onmouseleave="ganttHideAvTip()" onerror="this.onerror=null; this.src=\''+uiAv+'\';" style="object-fit:cover; cursor:pointer;">' +
                    '</div>';
            });
            if (uids.length > 3) {
                var extraStr = extraNames.join(', ').replace(/'/g, "\\'");
                h += '<div class="av-more" style="width:'+size+'px;height:'+size+'px;margin-left:-5px;cursor:pointer;" onmouseenter="ganttShowAvTip(event, \''+extraStr+'\')" onmouseleave="ganttHideAvTip()">+' + (uids.length-3) + '</div>';
            }
            h += '</div>';
            return h;
        }
        ...
        function avBarHtml(idStr) {
            ...
            uids.forEach(function(uid, i) {
                ...
                var uiAv = 'https://ui-avatars.com/api/?name='+encodeURIComponent(name)+'&background=random&color=fff&size=20&rounded=true&bold=true';
                var imgSrc = (emp && emp.profile_picture) ? 'https://trusmiverse.com/hr/uploads/profile/' + emp.profile_picture : uiAv;
                var safeName = name.replace(/'/g, "\\'");
                
                h += '<div class="av-img" style="width:20px;height:20px;border-width:1.5px;'+ml+'">' +
                    '<img src="'+imgSrc+'" onmouseenter="ganttShowAvTip(event, \''+safeName+'\')" onmouseleave="ganttHideAvTip()" onerror="this.onerror=null; this.src=\''+uiAv+'\';" style="object-fit:cover; cursor:pointer;">' +
                    '</div>';
            });
            if (uids.length > 3) {
                var extraStr = extraNames.join(', ').replace(/'/g, "\\'");
                h += '<div class="av-more" style="width:20px;height:20px;font-size:9px;border-width:1.5px;margin-left:-4px;cursor:pointer;" onmouseenter="ganttShowAvTip(event, \''+extraStr+'\')" onmouseleave="ganttHideAvTip()">+' + (uids.length-3) + '</div>';
            }
            h += '</div>';
            return h;
        }
        ...
        function coHtml(company, id, isProject, style) {
            if (!isProject) return '<span style="color:#a1a1aa;font-size:11px;">—</span>';
            var itemType = isProject ? 'project' : 'task';
            var txt = company || '—';
            var st = style || 'background-color:#f8fafc; color:#64748b; border-color:#e2e8f0;';
            return '<button type="button" class="chip-btn" style="'+st+'" onclick="openGanttDropdown(event, '+id+', \''+itemType+'\', \'company\')">' +
        ...
        function stHtml(status, id, icon, style, itemType) {
            ...
            return '<button type="button" class="chip-btn" style="'+st+'" onclick="openGanttDropdown(event, '+id+', \''+itemType+'\', \'status\')">' +
        ...
        function catHtml(category, id, style, itemType) {
            var txt = category || '—';
            var st = style || 'background-color:#f0f9ff; color:#0369a1; border-color:#bae6fd;';
            return '<button type="button" class="chip-btn" style="'+st+'" onclick="openGanttDropdown(event, '+id+', \''+itemType+'\', \'category\')">' +
                   '<i class="bi bi-tag"></i>' +
                   '<span>'+txt+'</span>' +
                   '</button>';
        }
        ...
        function renderHeader() {
            ...
            for (var di = 0; di < viewDays; di++) {
                ...
                var wn = weekNum(d);
                if (wn !== prevWeek) {
                    if (prevWeek !== -1) wGroups.push({ wn: prevWeek, w: wGroupWidth, label: 'W'+prevWeek+' · '+fmtShort(isoDate(wGroupStart))+' – '+fmtShort(isoDate(addDays(d,-1))) });
                    wGroupStart = d;
                    wGroupWidth = 0;
                    prevWeek = wn;
                }
            ...
            wGroups.push({ wn: prevWeek, w: wGroupWidth, label: 'W'+prevWeek+' · '+fmtShort(isoDate(wGroupStart))+' – '+fmtShort(isoDate(addDays(viewStart, viewDays-1))) });

            wGroups.forEach(function(g) {
                weeksH += '<div class="gantt-th-week-band" style="width:'+g.w+'px;">'+g.label+'</div>';
            });
            ...
            for (var di2 = 0; di2 < viewDays; di2++) {
                ...
                daysH += '<div class="gantt-day-cell'+cls+'" style="width:'+DAY_W+'px;">' +
                        '<span class="dn">'+d2.getDate()+'</span>' +
                        '<span class="dd">'+DAY_NAMES[dow].slice(0,2)+'</span>' +
                        '</div>';
            }
        ...
        function gridRowHtml(item, isTask, parentItem, isHidden) {
            ...
            if (!isTask) {
                ...
                expandBtn = '<button class="expand-btn'+rotateStyle+'" data-id="'+item.id+'" data-type="'+item.type+'" onclick="ganttToggle(this)"><i class="bi bi-chevron-right"></i></button>';
            } else {
                expandBtn = '<span style="width:14px;flex-shrink:0;color:#d1d5db;font-size:12px;text-align:center;">&middot;</span>';
            }
            var dotColor = item.status_style ? (item.status_style.split(' color:')[1] || '').split(';')[0] : '#94a3b8';
            var dot = '<span class="status-dot" style="flex-shrink:0; background:'+dotColor+';"></span>';
            var indent = isTask ? (parentItem ? 'padding-left:45px;' : 'padding-left:25px;') : 'font-weight:700;';

            var dateHtmlFn = function(field) {
                var val = item[field] ? item[field].split(' ')[0] : '';
                var hl = (field === 'tglSelesai' && !val) ? 'background:#fff1f2;border:1px solid #fda4af;color:#e11d48;' : '';
                return '<input type="text" class="due-input gantt-datetimepicker" style="width:90px;text-align:center;'+hl+'" value="'+val+'" data-id="'+item.id+'" data-type="'+item.type+'" data-field="'+field+'" autocomplete="off">';
            };
            var numHtmlFn = function(field, w, ph) {
                return '<input type="number" class="due-input" style="width:'+w+'px;text-align:center;" placeholder="'+ph+'" value="'+(item[field]||'')+'" data-id="'+item.id+'" data-type="'+item.type+'" data-field="'+field+'" onchange="ganttUpdateField(this)">';
            };

            var week = '<span style="color:#a1a1aa;font-size:11px;">&#8212;</span>';
            if (isTask && item.end) { var sD = parseDate(item.end); if (sD) week = 'W' + weekNum(sD); }

            // KPI
            var leadtimePctStr = '<span style="color:#a1a1aa;font-size:11px;">&#8212;</span>';
            var statusLtStr    = '<span style="color:#a1a1aa;font-size:11px;">&#8212;</span>';
            var diffLtStr      = '<span style="color:#a1a1aa;font-size:11px;">&#8212;</span>';
            var ltDaysStr      = '<span style="color:#a1a1aa;font-size:11px;">&#8212;</span>';
            var startM = item.start ? moment(item.start) : null;
            var endM   = item.end   ? moment(item.end)   : null;
            var finishM= item.tglSelesai ? moment(item.tglSelesai) : null;
            var todayM = moment().startOf('day');
            if (startM && endM && startM.isValid() && endM.isValid()) {
                ...
                var elapsedDays = compareDate.diff(startM, 'days');
                if (elapsedDays < 0) {
                    leadtimePctStr = '<span style="font-weight:600;font-size:11px;">0%</span>';
                    ltDaysStr      = '<span style="font-weight:600;font-size:11px;">0</span>';
                    statusLtStr    = pct === 0 ? '<span class="sbadge" style="background:#f1f5f9;color:#64748b;border:1px solid #e2e8f0;">Pending</span>' : '<span class="sbadge" style="background:#ecfdf5;color:#059669;border:1px solid #a7f3d0;">Ontime</span>';
                    diffLtStr      = pct === 0 ? '<span style="font-weight:600;font-size:11px;">0</span>' : '<span style="font-weight:600;font-size:11px;color:#10b981;">+'+pct+'</span>';
                } else {
                    var ltPct      = Math.round((elapsedDays / totalDays) * 100);
                    var diffVal    = pct - ltPct;
                    var diffColor  = diffVal < 0 ? '#ef4444' : (diffVal > 0 ? '#10b981' : '#64748b');
                    leadtimePctStr = '<span style="font-weight:600;font-size:11px;">'+ltPct+'%</span>';
                    ltDaysStr      = '<span style="font-weight:600;font-size:11px;">'+elapsedDays+'</span>';
                    diffLtStr      = '<span style="font-weight:600;font-size:11px;color:'+diffColor+';">'+(diffVal>0?'+':'')+diffVal+'</span>';
                    statusLtStr    = diffVal < 0 ? '<span class="sbadge" style="background:#fef2f2;color:#dc2626;border:1px solid #fecaca;">Late</span>' : '<span class="sbadge" style="background:#ecfdf5;color:#059669;border:1px solid #a7f3d0;">Ontime</span>';
                }
            } else if (pct > 0) {
                statusLtStr = '<span class="sbadge" style="background:#ecfdf5;color:#059669;border:1px solid #a7f3d0;">Ontime</span>';
            }

            var prgFillClr = pct === 100 ? 'rgba(16,185,129,.15)' : 'rgba(59,130,246,.15)';
            var prgTextClr = pct === 100 ? '#047857' : '#1d4ed8';
            var prgHtml = hasTasks
                ? '<div style="position:relative;width:75px;height:26px;background:#fff;border:1px solid #e4e4e7;border-radius:6px;overflow:hidden;display:flex;align-items:center;justify-content:center;"><div style="position:absolute;left:0;top:0;height:100%;background:'+prgFillClr+';width:'+pct+'%;transition:width .3s;"></div><span style="position:relative;z-index:2;font-weight:600;font-size:11px;color:'+prgTextClr+';">'+pct+'<small style="margin-left:2px;font-size:9px;">%</small></span></div>'
                : '<div class="grid-prg-row" style="cursor:pointer;position:relative;width:75px;height:26px;background:#fff;border:1px solid #e4e4e7;border-radius:6px;overflow:hidden;display:flex;align-items:center;justify-content:center;" onclick="window.ganttOpenPop('+item.id+', \''+item.type+'\', event)">' +
                '<div class="grid-prg-fill" style="position:absolute;left:0;top:0;height:100%;background:'+prgFillClr+';width:'+pct+'%;transition:width .3s;"></div>' +
                '<span class="grid-prg-text" style="position:relative;z-index:2;font-weight:600;font-size:11px;color:'+prgTextClr+';">'+pct+'<small style="margin-left:2px;font-size:9px;">%</small></span></div>';

            var noteVal = (item.note || '').replace(/"/g, '&quot;');
            var noteH   = '<button title="Update Progress & Note Log" class="gantt-note-btn" style="background:none;border:none;cursor:pointer;color:'+(noteVal?'var(--blue)':'#cbd5e1')+';font-size:14px;position:relative;" onclick="window.ganttOpenPop('+item.id+', \''+item.type+'\', event)"><i class="bi bi-chat-text'+(noteVal?'-fill':'')+'"></i></button>';
            var evCount = item.evidence_count || 0;
            var evH     = '<button style="background:'+(evCount>0?'#eff6ff':'transparent')+';border:'+(evCount>0?'1px solid #bfdbfe':'1px solid transparent')+';border-radius:12px;font-size:11px;font-weight:600;color:'+(evCount>0?'var(--blue)':'#cbd5e1')+';padding:2px 8px;cursor:pointer;" onclick="window.ganttOpenEvidence('+item.id+',\''+(isTask?'task':'project')+'\')"><i class="bi bi-paperclip"></i> '+evCount+'</button>';

            var cellMap = {
                text:            null,
                company:         coHtml(item.company, item.id, !isTask, item.company_style),
                pic:             avHtml(item.pic_id, 24, item.id, 'pic', item.type, hasTasks),
                requester:       item.type === 'project' ? avHtml(item.requester_id, 24, item.id, 'requester', 'project', hasTasks) : '<span style="color:#a1a1aa;font-size:11px;">&#8212;</span>',
                evidence:        evH,
                status:          stHtml(item.status, item.id, item.status_icon, item.status_style, item.type),
                category:        item.type === 'task' ? catHtml(item.category, item.id, item.category_style, 'task') : '<span style="color:#a1a1aa;font-size:11px;">&#8212;</span>',
                week:            week,
                progress:        prgHtml,
            ...
            var parentAttr = isTask && parentItem ? ' data-parent="'+parentItem.id+'"' : '';
            ...
            ganttColumns.forEach(function(col) {
                ...
                var isFrozen    = !!col.frozen;
                var w           = col.width;
                var stickyStyle = isFrozen ? 'position:sticky;left:'+stickyLeft+'px;z-index:2;border-right:2px solid #cbd5e1;' : '';
                if (isFrozen) stickyLeft += w;

                if (col.key === 'text') {
                    var cellStyle = (isFrozen) ? 'width:'+w+'px; flex:0 0 '+w+'px;'+indent+stickyStyle : 'min-width:'+col.minWidth+'px; flex:1;'+indent+stickyStyle;
                    cells += '<div class="gcell gcell-name" style="'+cellStyle+'">' +
                        expandBtn + dot +
                        '<span class="row-name" contenteditable="true" spellcheck="false" ' +
                            'onblur="ganttUpdateName('+item.id+', \''+item.type+'\', this.innerText)" ' +
                            'onkeydown="if(event.key===\'Enter\'){event.preventDefault();this.blur();}">'+item.text+'</span>' +
                        '</div>';
                } else {
                    var flexCols = ['pic','requester'];
                    var flexStyle = flexCols.indexOf(col.key) !== -1 ? 'display:flex;' : '';
                    cells += '<div class="gcell" style="width:'+w+'px; flex:0 0 '+w+'px; justify-content:'+(col.align==='center'?'center':'flex-start')+';'+flexStyle+stickyStyle+'">' + cellMap[col.key] + '</div>';
                }
            });

            return '<div class="gantt-row '+(isTask?'is-task':'is-project')+'" data-id="'+item.id+'"'+parentAttr+hiddenAttr+' oncontextmenu="ganttCtxMenu(event, '+item.id+', \''+item.type+'\')">' + cells + '</div>';
        }
        ...
        function tlRowHtml(item, isTask, parentItem, isHidden) {
            ...
            for (var di = 0; di < viewDays; di++) {
                ...
                cols += '<div class="day-col'+(isWE?' is-weekend':'')+(isTo?' is-today':'')+'" style="width:'+DAY_W+'px;"></div>';
            }
            ...
            if (sD && eD && eD >= sD) {
                ...
                bar = '<div class="gantt-bar-wrap" style="left:'+(x+2)+'px;width:'+barW+'px;">' +
                    '<div class="gantt-bar '+bCls+(isLate?' is-late':'')+'" data-id="'+item.id+'" data-type="'+item.type+'" ' +
                        'onmouseenter="ganttShowTip(event, '+item.id+', \''+item.type+'\')" ' +
                        'onmouseleave="ganttHideTip()" ' +
                        'ondblclick="ganttOpenPop('+item.id+', \''+item.type+'\', event)">' +
                        '<div class="gantt-bar-fill" style="width:'+pct+'%;"></div>' +
                        '<span class="gantt-bar-text">'+item.text+'</span>' +
                        '<span class="bar-pct">'+pct+'%</span>' +
                        avBarHtml(item.pic_id) +
                        '<div class="resize-handle" data-id="'+item.id+'" data-type="'+item.type+'"></div>' +
                    '</div>' +
                '</div>';
            }
            ...
            var parentAttr = isTask && parentItem ? ' data-parent="'+parentItem.id+'"' : '';
            var hiddenAttr = isHidden ? ' style="display:none;"' : '';
            return '<div class="'+trowClass+'" data-id="'+item.id+'"'+parentAttr+hiddenAttr+'>'+cols+bar+'</div>';
        }
        ...
        function renderDescription() {
            ...
            projects.forEach(function(proj) {
                var projMatchC = fCompany === 'all' || (proj.company && proj.company === fCompany);
                var projMatchS = fStatus  === 'all' || (proj.status && proj.status === fStatus);
                var projMatchCat = fCategory === 'all' || (proj.category && proj.category === fCategory);
                var projMatchPic = fPic === 'all' || (proj.pic_id && proj.pic_id.toString().split(',').map(function(s){return s.trim();}).includes(fPic.toString()));
                var projMatchQ = !search || (proj.text && proj.text.toLowerCase().includes(search));
                
                var projMatchesDirectly = projMatchC && projMatchS && projMatchCat && projMatchPic && projMatchQ;

                var matchingTasks = [];
                if (proj.tasks && proj.tasks.length) {
                    proj.tasks.forEach(function(task) {
                        var tMatchC = fCompany === 'all' || (task.company && task.company === fCompany);
                        var tMatchS = fStatus  === 'all' || (task.status && task.status === fStatus);
                        var tMatchCat = fCategory === 'all' || (task.category && task.category === fCategory);
                        var tMatchPic = fPic === 'all' || (task.pic_id && task.pic_id.toString().split(',').map(function(s){return s.trim();}).includes(fPic.toString()));
                        
                        // Task satisfies search if task matches, OR if parent project matches text
                        var tMatchQ = !search || (task.text && task.text.toLowerCase().includes(search)) || projMatchQ;
        ...
        function render() {
            ...
            if (todayX >= 0 && todayX <= viewDays * DAY_W) {
                var lineHtml = '<div class="today-vline" style="left:'+(todayX + DAY_W/2 - 1)+'px;">' +
        ...
        $(document).on('focus', '.gantt-datetimepicker', function() {
            ...
            if (!$this.hasClass('xdan-initialized')) {
                $this.datetimepicker({
                    ...
                    scrollMonth: false,
                    scrollInput: false,
                    onSelectDate: function(ct, $i) { ganttUpdateDate($i.data('id'), $i.data('type'), $i.data('field'), $i.val()); },
                    onSelectTime: function(ct, $i) { ganttUpdateDate($i.data('id'), $i.data('type'), $i.data('field'), $i.val()); }
                }).addClass('xdan-initialized').datetimepicker('show');
            }
        });
        ...
        function syncScroll() {
            ...
            $(window).off('scroll.gmain').on('scroll.gmain', function() {
                hideAllOverlays();
            });
        }
        ...
        function persistUpdate(id, type, field, value, cb, note) {
            $.post(BASE_URL + 'project_management/beranda/ajax_update_item', {
                id: id,
                type: type,
                field: field,
                value: value,
        ...
        function bindDrag() {
            $('#gantt-timeline-rows').find('.gantt-bar').each(function() {
                var $bar  = $(this);
                var id    = +$bar.data('id'), type = $bar.data('type');
                ...
                $bar.off('mousedown.gmove').on('mousedown.gmove', function(e) {
                    ...
                    $(document).one('mouseup.gmove', function(ev) {
                        ...
                        if (delta) {
                            ...
                            if (f) {
                                f.item.start = isoDate(addDays(parseDate(f.item.start), delta));
                                f.item.end   = isoDate(addDays(parseDate(f.item.end),   delta));

                                persistUpdate(id, f.item.type, 'start', f.item.start);
                                persistUpdate(id, f.item.type, 'end', f.item.end);
                                
                                var wNode = weekNum(parseDate(f.item.end));
                                f.item.week = 'W' + wNode;
            ...
            $('#gantt-timeline-rows').find('.resize-handle').each(function() {
                var $rh   = $(this);
                var id    = +$rh.data('id'), type = $rh.data('type');
                ...
                $rh.off('mousedown.gresize').on('mousedown.gresize', function(e) {
                    ...
                    $(document).on('mousemove.gresize', function(ev) {
                        $wrap.css('width', Math.max(DAY_W, origW + ev.clientX - startX) + 'px');
                    });
        ...
        window.ganttToggle = function(btn) {
            ...
            if (f && f.item.type === 'project') {
                ...
                var $gridRows = $('#gantt-grid-rows .gantt-row.is-task[data-parent="'+id+'"]');
                var $tlRows   = $('#gantt-timeline-rows .gantt-trow.is-task[data-parent="'+id+'"]');

                if (isExpanding) {
                    $gridRows.css('display', 'flex').hide().slideDown(250, function() {
                        $(this).css('display', ''); // clear inline styles
                    });
                    $tlRows.css('display', 'flex').hide().slideDown(250, function() {
                        $(this).css('display', '');
                    });
                } else {
        ...
        window.ganttUpdateDate = function(id,type, field, val) {
            ...
            if (field === 'tglSelesai' && !oldTgl && val) {
                ...
                f.item.progress = 100;
                f.item.status = 'Done';
                var sFound = (ganttLookups.statuses || []).find(function(v){ return v.status_name === 'Done'; });
                if (sFound) {
                ...
                var eventMock = { stopPropagation: function(){}, clientX: window.innerWidth/2, clientY: window.innerHeight/2 };
            ...
            if (field === 'start' && f.item.estimasi && val) {
                ...
                if (startDate) {
                    let estimasi = parseInt(f.item.estimasi, 10)-(f.item.estimasi > 0 ? 1 : 0);
            ...
            // keep start <= end
            if (f.item.start && f.item.end && parseDate(f.item.end) < parseDate(f.item.start)) {
                f.item.start = val;
            }
        ...
        window.ganttUpdateField = function(sel) {
            var id = +$(sel).data('id'), field = $(sel).data('field'), type = $(sel).data('type'), val = $(sel).val();
            var f  = findItem(id,type);
            if (f) {
                f.item[field] = val;
                persistUpdate(id, f.item.type, field, val);
        ...
        $(document).on('click', function(e) {
            if (!$(e.target).closest('#gantt-inline-dropdown').length && !$(e.target).closest('.chip-btn').length) {
                $('#gantt-inline-dropdown').hide();
                activeDdState = null;
            }
        });
        ...
        function renderGanttDropdownOptions(filter) {
            ...
            if (activeDdState.type === 'company') {
                opts = ganttLookups.companies.map(function(c){ return {v: c.alias, i: 'bi-building', s: c.style}; });
                selectedVals = [f.item.company];
            } else if (activeDdState.type === 'status') {
                opts = ganttLookups.statuses.map(function(s){ return {v: s.status_name, i: s.status_icon || 'bi-circle', s: s.style}; });
                selectedVals = [f.item.status];
            } else if (activeDdState.type === 'category') {
                opts = ganttLookups.categories.map(function(c){ return {v: c.category, i: 'bi-tag', s: c.style}; });
                selectedVals = [f.item.category];
            } else if (activeDdState.type === 'requester') {
                opts = (ganttLookups.product_owners || []).map(function(e){ 
                    var name = ((e.first_name||'') + ' ' + (e.last_name||'')).trim();
                    return {v: e.user_id.toString(), i:'bi-person-circle', label: name};
                });
                var cur = (f.item.requester_id || '').toString();
                selectedVals = cur.split(',').map(function(p){ return p.trim(); }).filter(function(s){return s;});
            } else if (activeDdState.type === 'pic' || activeDdState.type === 'spv' || activeDdState.type === 'pm') {
            ...
            opts.forEach(function(o) {
                if (search && !o.v.toLowerCase().includes(search) && !(o.label && o.label.toLowerCase().includes(search))) return;
                var isSel = selectedVals.includes(o.v);
                var dotStyle = o.s ? ('background:' + o.s.split('background-color:')[1].split(';')[0]) : '';
                html += '<div class="gdd-item '+(isSel?'selected':'')+'" onclick="selectGanttDropdownVal(\''+o.v+'\', event)">' +
                        '<div>' + (o.s ? '<span class="status-dot" style="'+dotStyle+'"></span>' : (o.imgHtml ? o.imgHtml : '<i class="bi '+o.i+'"></i>')) + (o.label || o.v) + '</div>' +
                        (isSel ? '<i class="bi bi-check2 text-primary" style="margin-right:0;"></i>' : '') +
                        '</div>';
            });
        ...
        window.selectGanttDropdownVal = function(val, e) {
            ...
            if (activeDdState.type === 'pic' || activeDdState.type === 'requester' || activeDdState.type === 'spv' || activeDdState.type === 'pm') {
                var isSingle = (activeDdState.type === 'spv' || activeDdState.type === 'pm');
                ...
                if (isSingle) {
                    ...
                    var newInitials = [];
                    var emp = ganttLookups.employees.find(function(e){ return e.user_id.toString() === val.toString(); });
                    if (emp) {
                         newInitials.push(((emp.first_name||'').slice(0,1) + (emp.last_name||'').slice(0,1)).toUpperCase());
                    }
                    f.item[activeDdState.type] = newInitials.join('');

                    persistUpdate(activeDdState.id, f.item.type, activeDdState.type, newIds);
                    $('#gantt-inline-dropdown').hide();
                    activeDdState = null;
                    if (f.item.type === 'task') preprocessData();
                    render();
                } else {
                    var curIds = (f.item[activeDdState.type + '_id'] || '').toString().split(',').map(function(s){return s.trim();}).filter(function(s){return s;});
                    ...
                    curIds.forEach(function(idStr) {
                        var emp = ganttLookups.employees.find(function(e){ return e.user_id.toString() === idStr; });
                        if (emp) {
                             newInitials.push(((emp.first_name||'').slice(0,1) + (emp.last_name||'').slice(0,1)).toUpperCase());
                        }
                    });
                ...
                var syncMap = {
                    status:   { list: 'statuses',   match: 'status_name', props: { status_style: 'style', status_icon: 'status_icon', color: 'color' } },
                    company:  { list: 'companies',  match: 'alias',       props: { company_style: 'style' } },
                    category: { list: 'categories', match: 'category',    props: { category_style: 'style' } }
                };
                
                var cfg = syncMap[activeDdState.type];
                if (cfg) {
                    var itemFound = ganttLookups[cfg.list].find(function(x){ return x[cfg.match] === val; });
                    if (itemFound) {
                ...
                if (activeDdState.type === 'status' && val === 'Done' && oldStatus !== 'Done') {
                    f.item.progress = 100;
                    f.item.tglSelesai = isoDate(new Date());
                    
                    var dId = activeDdState.id, dType = activeDdState.itemType;
                    $('#gantt-inline-dropdown').hide();
                    activeDdState = null;
                    render();
        ...
        window.ganttShowTip = function(e, id, type) {
            ...
            var f = findItem(id, type); if (!f) return; var item = f.item;
            ...
            var tw = $tt.outerWidth(), th = $tt.outerHeight();
        ...
        window.ganttHideTip = function() { $('#gantt-tooltip').hide().css('visibility', 'visible'); };
        ...
        window.ganttOpenPop = function(id, type, e, skipRefresh) {
            ...
            activePopId = id;
            activePopType = type;
            var f = findItem(id,type); if (!f) return;
            ...
            $('#gp-slider').val(pct); $('#gp-val').text(pct);
            $('#gp-note').val(''); $('#gp-msg').hide();
            $('#gp-word-count').text('0 / 3 words').css('color','#94a3b8');
            $('#gantt-progress-pop').css({ display:'block', top: e.clientY-10, left: e.clientX+10, zIndex: 9999 });
        };
        ...
        window.ganttCtxMenu = function(e, id, type) {
            ...
            var anyCollapsed = projects.some(function(p) { return !p.expanded; });
            var $toggle = $('#gctx-toggle-all');
            if (anyCollapsed) {
                $toggle.html('<i class="bi bi-arrows-expand"></i> Expand All Projects');
            } else {
                $toggle.html('<i class="bi bi-arrows-collapse"></i> Collapse All Projects');
            }

            $('#gantt-ctx-menu').css({ display:'block', top: e.clientY-10, left: e.clientX+10, zIndex: 9999 });
        };
        ...
        $('#g-today-btn').on('click', function() {
            ...
            if (tx >= 0 && tx <= viewDays * DAY_W) {
                ...
                viewStart = new Date(today.getFullYear(), today.getMonth(), today.getDate());
        ...
        // Filters
        $('#g-search, #g-filter-company, #g-filter-status, #g-filter-category, #g-filter-pic').on('input change', function() { render(); });
        ...
        $('#g-reset-filters').on('click', function() { window.resetGanttFilters(); });
        ...
        function doAddProject() {
            ...
            $.post(BASE_URL + 'project_management/beranda/ajax_add_project', {
                text: 'New Project',
                start: start,
                end: end,
                color: color,
                ...
                if (data.status === 'success') {
                    ...
                    var sFound = (ganttLookups.statuses || []).find(function(v){ return v.status_name === 'Not Started'; });
                    projects.push({
                        id: data.id, type: 'project', text: 'New Project',
                        company: '-', company_style: '', pic: '', pic_id: '', requester: '-', requester_id: '',
                        status: 'Not Started',
                        status_icon: sFound ? sFound.status_icon : 'bi-search',
                        status_style: sFound ? sFound.style : '',
                        category: '-', progress: 0,
                        start: start, end: end, tglSelesai: '', estimasi: 1, leadtime: 0, diffLt: 0, hours: 0, statusLt: 'Pending',
                    ...
                    var newId = data.id;
                    $('.gantt-row[data-id="'+newId+'"], .gantt-trow[data-id="'+newId+'"]').addClass('highlight-flash');
                    setTimeout(function() { $('.highlight-flash').removeClass('highlight-flash'); }, 3000);
                    setTimeout(function() {
                        $('#gantt-grid-scroll').animate({ scrollTop: $('#gantt-grid-scroll').prop("scrollHeight") }, 400);
                        $('#gantt-tl-scroll').animate({ scrollTop: $('#gantt-tl-scroll').prop("scrollHeight") }, 400);
                    }, 50);
                }
            });
        }
        ...
        $('#gctx-toggle-all').on('click', function() {
            $('#gantt-ctx-menu').hide();
            var anyCollapsed = projects.some(function(p) { return !p.expanded; });
            projects.forEach(function(p) { p.expanded = anyCollapsed; });
            render();
        });
        $('#gctx-details').on('click', function() {
            $('#gantt-ctx-menu').hide();
            if (!ctxItemId) return;
            var f = findItem(ctxItemId, ctxItemType); if (!f) return;
            var item = f.item;

            $('#det-text').text(item.text);
            $('#det-company').text(item.company || '-');
            $('#det-category').text(item.category || '-');
            $('#det-status-badge').text(item.status || '-').attr('class', 'badge-status ' + (item.status_style || ''));
        ...
        $('#gctx-add-task').on('click', function() {
            ...
            $.post(BASE_URL + 'project_management/beranda/ajax_add_task', {
                project_id: proj.id,
                text: 'New Task',
                start: taskStart,
                end: taskEnd,
                ...
                if (data.status === 'success') {
                    ...
                    var sFound = (ganttLookups.statuses || []).find(function(v){ return v.status_name === 'Not Started'; });
                    proj.tasks.push({
                        id: data.id, type: 'task', text: 'New Task', pic: '', pic_id: '',
                        status: 'Not Started',
                        status_icon: sFound ? sFound.status_icon : 'bi-search',
                        status_style: sFound ? sFound.style : '',
                        category: '-', category_style: '', progress: 0,
                        start: taskStart, end: taskEnd, tglSelesai: '', estimasi: 1, leadtime: 0, diffLt: 0, hours: 0, statusLt: 'Pending',
                        week: 'W' + wNode,
                        note: '',
                        evidence_count: 0,
                        color: proj.color
        ...
        $('#gctx-delete').on('click', function() {
            ...
            $.post(BASE_URL + 'project_management/beranda/ajax_delete', {
                id: f.item.id,
                type: type
            }, function(res) {
                var data = JSON.parse(res);
        ...
        $('#gctx-duplicate').on('click', function() {
            ...
            $.post(BASE_URL + 'project_management/beranda/' + url, payload, function(res) {
                var data = JSON.parse(res);
                if (data.status === 'success') {
                    var newItem = Object.assign({}, item, { id: data.id, text: item.text + ' (Copy)' });
        ...
        $('#gp-slider').on('input', function() {
            ...
            if (activePopId !== null) {
                ...
                if (f) {
                    ...
                    // grid update
                    var fillClr = v === 100 ? 'rgba(16,185,129,.15)' : 'rgba(59,130,246,.15)';
                    var textClr = v === 100 ? '#047857' : '#1d4ed8';
                    var $g = $('#gantt-grid-rows .gantt-row[data-id="'+activePopId+'"]');
                    $g.find('.grid-prg-fill').css({ 'width': v+'%', 'background': fillClr });
                    $g.find('.grid-prg-text').css({ 'color': textClr }).html(v + '<small style="margin-left:2px;font-size:9px;">%</small>');
                    ...
                    if (v === 100) {
                        ...
                        $g.find('.status-chip').attr('class', 'status-chip ' + f.item._origSStyle);
                    }
                }
            }
        });
        ...
        $('#gp-save').on('click', function() {
            ...
            var words = n.split(/\s+/).filter(Boolean);
            if (words.length < 3) {
                $('#gp-note').css({ 'border-color': '#ef4444', 'background': '#fff1f2' }).focus();
                Swal.fire({
                    icon: 'warning',
            ...
            persistUpdate(activePopId, f.item.type, 'progress', v, function(res) {
                $btn.prop('disabled', false).text('Save Progress');
                
                // Persist side-effects
                if (isCompletion) {
                    persistUpdate(f.item.id, f.item.type, 'status', 'Done');
                    persistUpdate(f.item.id, f.item.type, 'tglSelesai', f.item.tglSelesai);
                }
        ...
        $(document).on('click.gantt', function(e) {
            if (!$(e.target).closest('#gantt-ctx-menu').length) $('#gantt-ctx-menu').hide();
            if (!$(e.target).closest('#gantt-progress-pop,.gantt-bar').length && $('#gantt-progress-pop').is(':visible')) {
                window.ganttClosePop();
            }
        });
        ...
        $(document).on('mousedown', function(e) {
            if ($('#gantt-note-pop').is(':visible') && !$(e.target).closest('#gantt-note-pop, .gantt-note-btn').length) {
                window.ganttCloseNotePop();
            }
        });
        ...
        function loadEvidenceList(id, itemType) {
            ...
            $con.html('<div style="padding:40px; text-align:center; color:#94a3b8; font-size:12px; font-weight:500;"><div class="spinner-border spinner-border-sm" role="status" style="color:var(--accent);"></div> Loading evidence...</div>');
            
            $.post(BASE_URL + 'project_management/beranda/ajax_get_evidence', {
                id: id,
                type: itemType
            }, function(res) {
                var data = JSON.parse(res);
                if (data.status === 'success') {
                    ...
                    if (evs.length === 0) {
                        $con.html('<div style="padding:30px; text-align:center; color:#a1a1aa; font-size:12px;"><i class="bi bi-folder-x" style="font-size:24px; color:#e2e8f0; display:block; margin-bottom:8px;"></i> No evidence attached yet</div>');
                        return;
                    }
                    ...
                    for (var i=0; i<evs.length; i++) {
                        var e = evs[i];
                        var icon = e.type === 'url' ? '<i class="bi bi-link-45deg"></i>' : '<i class="bi bi-file-earmark-text"></i>';
                        ...
                        html += '<li style="padding:12px 16px; border-bottom:1px solid #f1f5f9; display:flex; justify-content:space-between; align-items:center;">' +
                                    '<div style="display:flex; align-items:center; gap:12px; overflow:hidden;">' +
                                        '<div style="width:32px; height:32px; border-radius:6px; background:#f0f9ff; color:#0284c7; display:flex; justify-content:center; align-items:center; flex-shrink:0; font-size:14px;">'+icon+'</div>' +
                                        '<div style="min-width:0;">' +
                                            '<a href="'+link+'" target="_blank" style="font-size:12px; font-weight:600; color:#0f172a; text-decoration:none; display:block; white-space:nowrap; overflow:hidden; text-overflow:ellipsis;">'+fileName+'</a>' +
                                            '<span style="font-size:10px; color:#94a3b8;">'+date+'</span>' +
                                        '</div>' +
                                    '</div>' +
                                    '<button class="btn-delete-evidence" data-id="'+e.id+'" data-item-id="'+id+'" data-item-type="'+itemType+'" style="background:none; border:none; color:#ef4444; font-size:12px; cursor:pointer; padding:8px; flex-shrink:0; border-radius:4px;" onmouseover="this.style.background=\'#fef2f2\'" onmouseout="this.style.background=\'none\'"><i class="bi bi-trash3"></i></button>' +
                                '</li>';
                    }
                    html += '</ul>';
                    $con.html(html);
                } else {
                    $con.html('<div style="padding:20px; color:red; font-size:12px;">Failed to load evidence.</div>');
                }
            });
        }
        ...
        $(document).on('click', '.btn-delete-evidence', function() {
            ...
            $.post(BASE_URL + 'project_management/beranda/ajax_delete_evidence', {id: evId}, function(res) {
                var data = JSON.parse(res);
                if(data.status==='success') {
        ...
        function bindSplitter() {
            ...
            $(document).off('mousemove.gsplit').on('mousemove.gsplit', function(e) {
            ...
            $(document).off('mouseup.gsplit').on('mouseup.gsplit', function(e) {
                if (isResizing) {
                    isResizing = false;
