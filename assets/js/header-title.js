...
$(window).on("load", function () {
  ...
  if (getCookie("WinDOORSusername") != "") {
    $("#usernamedisplay, #usernamedisplay2 span, .username").html(
      getCookie("WinDOORSusername")
    );
  }

  /* temperature data */
  var cityname = "Indonesia";
  ...
  function dataload() {
    fetch(
      "https://api.openweathermap.org/data/2.5/weather?q=" +
        cityname +
        "&APPID=ce2008ef871845f77c7f03aafe2d54eb&units=metric"
    )
  ...
  function appendData(data) {
    ...
    $("#tempimage").attr(
      "src",
      "assets/img/openweather-icon/light/" + data.weather[0].icon + "@2x.png"
    );
  }
  ...
  $("#titltfilterlist, #searchfilterlist").chosen({
    no_results_text: "Oops, nothing found!",
    max_selected_options: 2,
  });
  $("#titltfilterlist, #searchfilterlist").bind(
    "chosen:maxselected",
    function () {
      $(this).closest(".input-group").next(".invalid-feedback").show();
    }
  );
  $("#titltfilterlist, #searchfilterlist")
    .chosen()
    .change(function () {
