...
$(document).ready(function () {
  ...
  if (getCookie("TrusmiVERSElayoutmode") === "dark-mode") {
    html.addClass("dark-mode");
    $("#btn-layout-modes-dark").prop("checked", true);
    $(".ui .dropdown").addClass("inverted");
    $(".ui .dropdown .search").addClass("inverted");
    $(".ui .input").addClass("inverted");
  } else {
    html.removeClass("dark-mode");
    $("#btn-layout-modes-dark").prop("checked", false);
    $(".ui .dropdown").removeClass("inverted");
    $(".ui .dropdown .search").removeClass("inverted");
    $(".ui .input").removeClass("inverted");
  }

  $("#btn-layout-modes-dark").on("click", function () {
    if ($(this).is(":checked")) {
      setCookie("TrusmiVERSElayoutmode", "dark-mode", 1);
      html.attr("class", getCookie("TrusmiVERSElayoutmode"));
    } else {
      setCookie("TrusmiVERSElayoutmode", "light-mode", 1);
      html.attr("class", getCookie("TrusmiVERSElayoutmode"));
    }
  });
  ...
  /* layout modes dark-light */
  if (getCookie("TrusmiVERSElayoutmode") === "dark-mode") {
    $("#btn-layout-modes-dark-nav").prop("checked", true);
    html.addClass("dark-mode");
  } else {
    $("#btn-layout-modes-dark-nav").prop("checked", false);
    html.removeClass("dark-mode");
  }

  $("#btn-layout-modes-dark-nav").on("click", function () {
    if ($(this).is(":checked")) {
      setCookie("TrusmiVERSElayoutmode", "dark-mode", 1);
      html.attr("class", getCookie("TrusmiVERSElayoutmode"));
    } else {
  ...
  if (
    $.type(getCookie("WinDOORStheme")) != "undefined" &&
    getCookie("WinDOORStheme") != ""
  ) {
    ...
    .each(function () {
      $(this).on("click", function () {
        ...
        if ($(this).hasClass("active") != true && setstyle != "") {
          ...
          $(this).addClass("active");
          body
            .removeClass(curentstyle)
            .addClass(setstyle)
            .attr("data-theme", setstyle);
          setCookie("WinDOORStheme", setstyle, 1);
          curentstyle = setstyle;
        }
      });
  ...
  /* background images */
  if (
    $.type(getCookie("WinDOORSsetimagepath")) != "undefined" &&
    getCookie("WinDOORSsetimagepath") != ""
  ) {
    $(".main-bg").each(function () {
      $(this).find("img").attr("src", getCookie("WinDOORSsetimagepath"));
      $(this).css(
        "background-image",
        'url("' + getCookie("WinDOORSsetimagepath") + '")'
      );
    });
  ...
  /* sidebar type */
  if (
    $.type(getCookie("WinDOORSsidebarfilled")) != "undefined" &&
    getCookie("WinDOORSsidebarfilled") != ""
  ) {
  ...
  $(".personalise-preview-sidebar").on("click", function () {
    ...
    if (setSidebarfill != "") {
      body
        .removeClass(getCookie("WinDOORSsidebarfilled"))
        .addClass(setSidebarfill);
  ...
  if (
    $.type(getCookie("WinDOORSsidebarStyle")) != "undefined" &&
    getCookie("WinDOORSsidebarStyle") != ""
  ) {
    body
      .removeClass(currentstyle)
      .addClass(getCookie("WinDOORSsidebarStyle"))
      .attr("data-sidebarstyle", getCookie("WinDOORSsidebarStyle"));
  ...
  $(".sidebarstyle").on("click", function () {
    ...
    if (setSidebarStyle != "") {
      body
        .removeClass(getCookie("WinDOORSsidebarStyle"))
        .addClass(setSidebarStyle);
  ...
  if ("WinDOORSlogopath" in sessionStorage) {
    $(".header .navbar-brand img").attr(
      "src",
      sessionStorage.getItem("WinDOORSlogopath")
    );
  }
  if ("WinDOORSuserimg1" in sessionStorage) {
    $("#userprofiledd img, #userphotoonboarding3, .ususerphotoonboarding")
      .parent()
      .css("background-image", sessionStorage.getItem("WinDOORSuserimg1"));
  }
