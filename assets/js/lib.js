...
window.ProductTourJS = {
  ...
  start: function () {
    if (this.baseCheckOptions() && this.totalSteps > 0) {
      ...
      this.options.darkLayerPersistence &&
        this.$darkenLayer.addClass("is-always-visible");
      this.darken();

      this.$current = $(
        ".product-tour-js-single-step.js-product-tour-index-0"
  ...
  baseCheckOptions: function () {
    let ok =
      this.tips && Array.isArray(this.tips) && "object" === typeof this.tips[0];
    if (!ok) {
  ...
  navigationInit: function () {
    let html =
      '<div class="product-tour-js-nav"><span class="product-tour-js-nav-numeric"><b class="product-tour-js-actual-step">1</b> of ' +
      this.totalSteps +
      '</span><ul class="product-tour-js-tour-nav"><li><a href="#0" class="product-tour-js-prev">' +
      this.options.prev +
      '</li><li><a href="#0" class="product-tour-js-next">' +
      this.options.next +
      '</a></li></ul></div><a href="#0" class="product-tour-js-close js-product-tour-js-tour-close">Close</a>';
    let self = this;

    $.each(this.tips, function (tipIndex) {
      let elem = $(
        ".product-tour-js-single-step.js-product-tour-index-" + tipIndex
      );
      let nextIndex = tipIndex + 1;
      let nextClass = nextIndex < self.totalSteps ? "" : "inactive";
      let prevClass = 1 === nextIndex ? "inactive" : "";
      html =
        tipIndex + 1 === self.tips.length
          ? html +
            '<div class="product-tour-js-single-step-final-btn js-product-tour-js-tour-close ">' +
            self.options.finish +
            "</div>"
          : html;
      let $html = $(html);
  ...
  changeStep: function (direction) {
    if (this.isStarted()) {
      ...
      this.darken();
      this.$current = $(
        ".product-tour-js-single-step.js-product-tour-index-" + nextStep
      ).addClass("is-selected");
  ...
  darken: function () {
    ...
    this.$darkenLayer
      .addClass("is-visible")
      .on("webkitAnimationEnd msAnimationEnd animationend", function () {
  ...
  setPosition: function (index, tip) {
    ...
    let positions = this.getRelativePosition(
      elem,
      tip.x,
      tip.y,
      tip.offx,
  ...
  htmlInit: function ($wrapper) {
    ...
    $.each(this.tips, function (index, tip) {
      html +=
        "<div class='product-tour-js-single-step js-product-tour-index-" +
        index +
        "'><span class='product-tour-js-pulse'></span><div class='product-tour-js-single-step-inner is-" +
        tip.position +
        "'><div class='product-tour-js-progress'><span class='product-tour-js-progress-inner' style='width: " +
        Math.round(((index + 1) / self.totalSteps) * 100) +
        "%'></span></div><div class='product-tour-js-image coverimg' style='background-image: url(" +
        tip.image +
        ")'></div><div class='product-tour-js-title'>" +
        tip.title +
        "</div><div class='product-tour-js-description'>" +
        tip.description +
        "</div>" +
        (tip.additionalHtml || "") +
        "</div></div>";
