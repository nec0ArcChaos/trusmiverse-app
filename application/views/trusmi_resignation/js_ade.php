...
<script>
    ...
    function get_my_resignation() {
        window.open(`<?= base_url() ?>trusmi_resignation/detail_resignation?id=${id_resignation}`, '_blank');
    }

    function check_pengajuan() {
        user_id = "<?= $this->session->userdata("user_id"); ?>";
        $.ajax({
            url: '<?= base_url('trusmi_resignation/check_pengajuan') ?>',
            type: 'POST',
            dataType: 'json',
            data: {
                user_id: user_id
            ...
            success: function(response) {
                ...
                if (check_pengajuan == true) {
                    ...
                    btn_my_resignation = `<a href="<?= base_url() ?>trusmi_resignation/detail_resignation?id=${id_resignation}" class="btn btn-md btn-outline-warning"><i class="bi bi-eye"></i> My Resignation</a>`;
    ...
    function list_waiting_resignation() {
        ...
        $('#dt_trusmi_list_waiting_resignation').DataTable({
            ...
            "ajax": {
                ...
                "url": "<?= base_url(); ?>trusmi_resignation/list_waiting_resignation",
            },
            "columns": [{
                    "data": "notice_date",
                },
                    ...
                    "render": function(data, type, row, meta) {
                        if (row['id_status_resignation'] == 0) {
                            ...
                            if (arr_pic.includes(user_id)) {
                                ...
                                detail = `<a href="<?= base_url() ?>trusmi_resignation/detail_resignation?id=${row['resignation_id']}"><span class="badge bg-danger data-resignation"> ${data}</span></a>`;
                            }
                            return `${detail}${send_wa_again}`;
                        } else {
                            print = '';
                            if (user_role_id == 1 || user_id == 2063 || user_id == 61) {
                                print = `<a href="<?= base_url() ?>trusmi_resignation/print_paklaring/${row['resignation_id']}" target="_blank"><span class="badge bg-success data-resignation";
    ...
    function dt_report_detail() {
        ...
        $('#dt_report_detail').DataTable({
            ...
            "ajax": {
                ...
                "url": "<?= base_url(); ?>trusmi_resignation/get_report_detail",
            },
            "columns": [{
                    "data": "created_at",
                },
    ...
    function list_waiting_resignation_hrd() {
        ...
        $('#dt_trusmi_list_waiting_resignation_hrd').DataTable({
            ...
            "ajax": {
                ...
                "url": "<?= base_url(); ?>trusmi_resignation/list_waiting_resignation_hrd",
            },
            "columns": [{
                    "data": "notice_date",
                },
                    ...
                    "render": function(data, type, row, meta) {
                        if (row['id_status_resignation'] == 0) {
                            if (user_role_id == 1 || user_id == 2063 || user_id == 61 || user_id == 979) {
                                v_hrd = `<a href="<?= base_url() ?>trusmi_resignation/verify_hrd?id=${row['resignation_id']}"><span class="badge bg-danger data-resignation"> ${data}</span></a>`;
                            } else {
                                v_hrd = ``;
                            }
                            return `${v_hrd}`;
    ...
    function get_profil_user() {
        user_id = "<?= $this->session->userdata("user_id"); ?>";
        $.ajax({
            url: '<?= base_url('trusmi_resignation_ade/get_profile') ?>',
            type: 'POST',
            dataType: 'json',
            data: {
                user_id: user_id
            ...
            success: function(response) {
                ...
                let designation_name = response.designation_name == null ? 'no-data' : response.department_name;
                // add by Ade
                // let reason =
    ...
    $('#category').change(function() {
        ...
        $.ajax({
            url: '<?= base_url() ?>trusmi_resignation_ade/get_data_reason',
            type: 'POST',
            dataType: 'json',
            data: {
    ...
    function get_data_reason() {
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                //     list_reasons += `<option value="${response[index].id_reason}">${response[index].reason}</option>`;
                // }
                // console.log(list_reasons);
                // $("#reason").empty().append(list_reasons);
                // n_reasons.update();
    ...
    function dt_trusmi_resignation(start, end) {
        ...
        $('#dt_trusmi_resignation').DataTable({
            ...
            buttons: [{
                extend: 'excelHtml5',
                text: 'Export to Excelx',
                footer: true
            }],
            "ajax": {
                "dataType": 'json',
                "type": "POST",
                "url": "<?= base_url(); ?>trusmi_resignation_ade/dt_trusmi_resignation",
                "data": {
                    start: start,
                    end: end,
                    user_id: user_id,
                ...
                },
                {
                    ...
                    "render": function(data, type, row, meta) {
                        if (row['id_status_resignation'] == 0) {
                            ...
                            //     detail = `<a href="<?= base_url() ?>trusmi_resignation/verify_resignation?id=${row['resignation_id']}"><span class="badge bg-success data-resignation"> ${data}</span></a>`;
                            // } else {
                            detail = `<a href="<?= base_url() ?>trusmi_resignation/detail_resignation?id=${row['resignation_id']}"><span class="badge bg-success data-resignation"> ${data}</span></a>`;
                            // }
                            return `${detail}
                            ${print}
                            `;
    ...
    function store_resignation() {
        ...
        var form_data = form.serialize() + '&category_name=' + category_name + '&reason_title=' + reason_title;

        $.ajax({
            url: '<?= base_url('trusmi_resignation_ade/store') ?>',
            type: 'POST',
            dataType: 'json',
...
https://trusmiverse.com/apps/login/verify_resignation?u=${pic}&id=${id_resignation}`;
                        ...
                        // send_wa(array_contact, msg); // komen by Ade
                    }
                    ...
                    window.setTimeout(function() {
                        window.location = `<?= base_url() ?>trusmi_resignation_ade/detail_resignation?id=${id_resignation}`;
                    }, 1500);
    ...
    function resend_wa() {
        ...
        $.ajax({
            url: '<?= base_url('trusmi_resignation/send_wa_again') ?>',
            type: 'POST',
            dataType: 'json',
            data: {
