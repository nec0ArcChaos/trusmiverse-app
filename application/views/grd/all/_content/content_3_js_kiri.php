...
<script>
    $(document).ready(function() {
        ...
        $('#select_divisi').change(function(e) {
            ...
            // console.info("id_company " + id_company);
            // console.info("divisi " + divisi);
            if(divisi == 'Marketing Communication' && id_company == 2){
                document.getElementById("div_part_support").style.display = "none";
            } else {
                document.getElementById("div_part_support").style.display = "block";
    ...
    function reload_data() {
        console.info("=== START reload_data call ===");
        var divisi = $('#select_divisi').val();
        ...
        console.info("=== END reload_data call ===");

    }

    function updateProgressBar(goals) {

        console.info("=== START updateProgressBar call ===");
...
<script>
    function get_all_list_grd(divisi, start, end) {
        console.info("=== START get_all_list_grd call ===");
        console.info("divisi : " + divisi);
                ...
                response.forEach((dt_goal, goalindex) => {
                    ...
                    dt_goal.sos.forEach((so, index) => {
                        ...
                        so.data_si.forEach((subtask, index) => {
                            // console.info("subtask.evidence_file " + subtask.evidence_file);
                            if(subtask.evidence_file == '' || subtask.evidence_file == null){
                                evidence_file = ``;
                            } else {
                                // link_file = "<?= base_url(''); ?>uploads/grd/evidence/" + subtask.evidence_file;
                                // console.info("subtask.evidence_file link_file :" + link_file);
                                evidence_file = `<span class="badge bg-light-blue text-black" style='cursor:pointer'><a href="<?= base_url(''); ?>uploads/grd/evidence/`+subtask.evidence_file+`" target="_blank">
                                        <i class="bi bi-file-earmark"></i>${subtask.evidence_file}
                                    </a></span>`;
                            }

                            if(subtask.evidence_link == '' || subtask.evidence_link == null){
                                evidence_link = ``;
                            } else {
                                // link_file = "<?= base_url(''); ?>uploads/grd/evidence/" + subtask.evidence_file;
                                // console.info("subtask.evidence_file link_file :" + link_file);
                                evidence_link = `<span class="badge bg-light-blue text-black" style='cursor:pointer'><a href="`+subtask.evidence_link+`" target="_blank">
                            ...
                            <tr class="bg-light-yellow">
                                ...
                                <td class="text-center">`+evidence_file +` ` + evidence_link + `</td>
                            ...
                            subtask.tasklist.forEach(detail => {
                                    ...
                                    if(detail.evidence == ''){
                                        detail_evidence_file = ``;
                                    } else {
                                        // detail_link_file = "<?= base_url(''); ?>uploads/grd/evidence/" + detail.evidence;
                                        // console.info("detail.evidence_file link_file :" + detail_link_file);
                                        detail_evidence_file = `<span class="badge bg-light-blue text-black" style='cursor:pointer'><a href="<?= base_url(''); ?>uploads/grd/evidence/`+detail.evidence+`" target="_blank">
                                                <i class="bi bi-file-earmark"></i>${detail.evidence}
                                            </a></span>`;
                                    }

                                    if(detail.evidence_link == '' ){
                                        detail_evidence_link = ``;
                                    } else {
                                        link_evidence_link = detail.evidence_link;
                                        // console.info("detail.evidence_file evidence_link :" + link_evidence_link);
                                        detail_evidence_link = `<span class="badge bg-light-blue text-black" style='cursor:pointer'><a href="`+detail.evidence_link+`" target="_blank">
                                                <i class="bi bi-link-45deg"></i>Link
                                            </a></span>`;
                                    }
                                ...
                                <tr>
                                    ...
                                    <td class="text-center">`+detail_evidence_file +` ` + detail_evidence_link + `</td>
                                        ...
                                        <div id="accordion-${so.id_so}" class="mb-2">
                                            <div class="row" id="heading-${so.id_so}">
                                                ...
                                                <p style="font-size: 14px; font-weight: bold;">
                                                    ...
                                                    SO ${index + 1}. ${so.so} <br>
                                                    <span class="text-grey small fw-bold mt-0 ml-4" data-bs-toggle="tooltip"
                                                        title="timeline dari tasklist grd paling awal ke paling akhir">
                                                    <i class="bi bi-stopwatch"></i> ${so.start} <i class="bi bi-arrow-right-short"></i> ${so.end}</span>
                                                </p>
        ...
        console.info("=== END get_all_list_grd call ===");
