    ...
    $query_ops = mysqli_query(
        $conn,
                "SELECT
                    ...
                    'Kehadiran' AS nama_target,
                    100 AS target,
                    ROUND
                        (
                            CASE
                                WHEN SUM(x.harus_hadir) = 0 THEN 0
                            ELSE (SUM(x.total_hadir) / SUM(x.harus_hadir)) * 100
                            END
                            ,1
                        ) AS actual
                FROM
                    (
                        SELECT
                            COALESCE ( trusmi_harus_hadir.harus_hadir,
                            CASE
                                ...
                                WHEN COUNT(IF( rekap.`status` = 'R', 1, NULL )) > 0
                                    THEN COUNT(IF( rekap.`status` NOT IN ( '', 'R' ), 1, NULL )) -
                                    COUNT(IF( rekap.libur_aktif = 1, 1, NULL ))
                            ELSE COUNT( rekap.`status` ) -
                                IF(rekap.libur > 0,rekap.libur,COUNT(IF( rekap.hari = 'Sun' AND rekap.date >= rekap.date_of_joining, 1, NULL ))) -
                                COUNT(IF( rekap.`status` = '', 1, NULL )) 
                            END 
                            ) AS harus_hadir,
                        ...
                        FROM
                            (
                                SELECT
                                    ...
                                    IF ( xin_employees.date_of_leaving != '' AND xin_employees.date_of_leaving < all_dates.date, NULL,
                                        IF ( DATE_FORMAT( all_dates.date, '%a' ) = 'Sun' AND IF ( all_dates.date < xin_employees.date_of_joining, '', NULL ) != '', 1,NULL )
                                    ) AS libur_aktif,
                                    CASE-- Karyawan baru join tidak absen
                                        WHEN all_dates.date < xin_employees.date_of_joining THEN '' -- Karyawan baru absen
                                        WHEN xin_employees.date_of_joining = xin_attendance_time.attendance_date THEN 'P' --  Resign
                                        WHEN xin_employees.date_of_leaving != '' AND xin_employees.date_of_leaving < all_dates.date THEN 'R' --  Hari Esok
                                        ...
                                        WHEN xin_leave_applications.leave_id IS NOT NULL AND xin_leave_applications.leave_type_id = 7 THEN
                                            CASE
                                                WHEN xin_leave_applications.tgl_ph < CONCAT(DATE_FORMAT(CURRENT_DATE(), '%Y-%m'), '-20') THEN
                                                    CASE
                                                        WHEN xin_employees.user_role_id = 8 THEN '' ELSE 'H' 
                                                    END 
                                        ...
                                        WHEN xin_leave_applications.leave_id IS NOT NULL AND xin_leave_applications.leave_type_id IN ( 1, 14 ) THEN 'C' --  Cuti Menikah

                                        WHEN xin_leave_applications.leave_id IS NOT NULL AND xin_leave_applications.leave_type_id = 2 THEN 'M' --  Izin Sakit

                                        WHEN xin_leave_applications.leave_id IS NOT NULL AND xin_leave_applications.leave_type_id = 5 THEN 'S' --  Cuti Bersalin
                                        ...
                                        WHEN xin_leave_applications.leave_id IS NOT NULL AND ( xin_leave_applications.leave_type_id = 15 OR xin_leave_applications.leave_type_id = 16 ) THEN 'PR' --  Khitan Anak

                                        WHEN xin_leave_applications.leave_id IS NOT NULL AND xin_leave_applications.leave_type_id = 25 THEN 'KA' --  Keluarga sakit

                                        WHEN xin_leave_applications.leave_id IS NOT NULL AND ( xin_leave_applications.leave_type_id = 21 OR xin_leave_applications.leave_type_id = 22 ) THEN 'SK' --  Kematan Keluarga

                                        WHEN xin_leave_applications.leave_id IS NOT NULL AND ( xin_leave_applications.leave_type_id = 9 || xin_leave_applications.leave_type_id = 17 ) THEN 'KL' --  Wisuda/skripsi

                                        WHEN xin_leave_applications.leave_id IS NOT NULL AND xin_leave_applications.leave_type_id = 18 THEN 'SW' --  DLK non driver

                                        WHEN xin_leave_applications.leave_id IS NOT NULL AND xin_leave_applications.leave_type_id = 13 
                                            AND IF
                                                ( IF ( xin_leave_applications.to_date = all_dates.date, xin_leave_applications.end_time, NULL ) <
                                                    IF ( xin_leave_applications.to_date = all_dates.date, CONCAT( xin_attendance_time.shift_in, ':00' ), NULL ), 1, 0 
                                                ) = 0 THEN 'DL' --  DLK driver

                                        WHEN xin_leave_applications.leave_id IS NOT NULL AND xin_leave_applications.leave_type_id = 23 THEN 'DD' --  Skripsi/wisuda

                                        WHEN xin_leave_applications.leave_id IS NOT NULL AND xin_leave_applications.leave_type_id = 18 THEN 'SW' --  Cuti thun lalu

                                        WHEN xin_leave_applications.leave_id IS NOT NULL AND xin_leave_applications.leave_type_id = 19 THEN 'CL' --  Telat (lebih 40% termasuk absen)

                                        WHEN DATE_FORMAT( xin_attendance_time.clock_in, '%Y-%m-%d %H:%i:00' ) IS NOT NULL 
                                            AND IF ( DATE_FORMAT( xin_attendance_time.clock_in, '%Y-%m-%d %H:%i:00' ) > CONCAT( all_dates.date, ' ', xin_attendance_time.shift_in, ':00' ), 1, 0 ) = 1 
                                            AND xin_leave_applications.leave_id IS NULL THEN
                                                CASE
                                                    WHEN DATE_FORMAT( xin_attendance_time.clock_in, '%Y-%m-%d %H:%i:00' ) >
                                                        CONCAT( all_dates.date, ' ', xin_attendance_time.shift_in, ':00' ) + INTERVAL ROUND(TIMESTAMPDIFF( MINUTE, CONCAT( all_dates.date, ' ', xin_attendance_time.shift_in, ':00' ),
                                                        IF ( xin_attendance_time.shift_in > '17:00', CONCAT( all_dates.date, ' ', xin_attendance_time.shift_out, ':00' ) + INTERVAL 1 DAY, CONCAT( all_dates.date, ' ', xin_attendance_time.shift_out, ':00' ) 
                                                        ...
                                                        CASE
                                                            WHEN TIMESTAMPDIFF( MINUTE, xin_attendance_time.clock_in, xin_attendance_time.clock_out ) > 479 
                                                                AND xin_employees.office_shift_id IN ( 7, 11, 12, 13, 14, 18, 21 ) THEN 'P' -- Jam masuk dan jam pulang sama
                                                            WHEN TIMESTAMPDIFF( MINUTE, xin_attendance_time.clock_in, xin_attendance_time.clock_out ) < 11 THEN 'F' ELSE
                                                        CASE
                                                            WHEN xin_employees.user_role_id = 8 THEN ''
                                                            ELSE 'A' 
                                                            END 
                                                END
                                                ELSE -- Kondisi Shift Store
                                                CASE	
                                                    WHEN TIMESTAMPDIFF( MINUTE, xin_attendance_time.clock_in, xin_attendance_time.clock_out ) > 479 
                                                        AND xin_employees.office_shift_id IN ( 7, 11, 12, 13, 14, 18, 21 ) THEN 'P'
                                                    ELSE
                                                        CASE
                                                        ...
                                                        CASE
                                                            WHEN DATE_FORMAT( xin_attendance_time.clock_in, '%Y-%m-%d %H:%i:00' ) IS NOT NULL 
                                                                AND xin_attendance_time.clock_out IS NULL AND locked.employee_id IS NOT NULL THEN
                                                                    CASE
                                            ...
                                            (
                                                CASE
                                                    WHEN xin_attendance_time.clock_in IS NOT NULL
                                                        AND ( xin_employees.office_shift_id = 166 OR xin_designations.trusmi_shift = 31 ) THEN 480 
                                                    WHEN xin_attendance_time.clock_in IS NOT NULL 
                                                        AND ( xin_employees.office_shift_id = 167 OR xin_designations.trusmi_shift = 32 ) THEN 540 
                                                    WHEN xin_attendance_time.clock_in IS NOT NULL 
                                                        AND ( xin_employees.office_shift_id = 168 OR xin_designations.trusmi_shift = 33 ) THEN 600 
                                                    WHEN xin_attendance_time.clock_in IS NOT NULL 
                                                        AND ( xin_employees.office_shift_id = 169 OR xin_designations.trusmi_shift = 34 ) THEN 660 
                                                    WHEN xin_attendance_time.clock_in IS NOT NULL 
                                                        AND ( xin_employees.office_shift_id = 170 OR xin_designations.trusmi_shift = 35 ) THEN 720
                                                ELSE NULL 
                                                END 
                                            ) > TIMESTAMPDIFF( MINUTE, xin_attendance_time.clock_in, xin_attendance_time.clock_out ) THEN 'NP'
                                        ...
                                        WHEN
                                            (
                                                ...
                                                ELSE NULL 
                                                END 
                                            ) < TIMESTAMPDIFF( MINUTE, xin_attendance_time.clock_in, xin_attendance_time.clock_out ) THEN 'P' --  Pulang Cepat (kurang 60% termasuk absen)

                                        WHEN xin_attendance_time.clock_out IS NOT NULL 
                                            AND IF ( xin_attendance_time.clock_out < IF ( xin_attendance_time.shift_in > '17:00',
                                            ...
                                            ), 1, 0  ) = 1 AND xin_leave_applications.leave_id IS NULL THEN
                                            CASE
                                                WHEN xin_attendance_time.clock_out < CONCAT( all_dates.date, ' ', xin_attendance_time.shift_in, ':00' ) + INTERVAL ROUND(TIMESTAMPDIFF(MINUTE, IF ( xin_attendance_time.shift_in > '17:00',
                                                    CONCAT( all_dates.date, ' ', xin_attendance_time.shift_out, ':00' ) + INTERVAL 1 DAY,
                                                    CONCAT( all_dates.date, ' ', xin_attendance_time.shift_out, ':00' ) ), IF ( xin_attendance_time.shift_in > '17:00',
                                                    CONCAT( all_dates.date, ' ', xin_attendance_time.shift_out, ':00' ) + INTERVAL 1 DAY,
                                                    CONCAT( all_dates.date, ' ', xin_attendance_time.shift_out, ':00' ) 
                                                    ))* 0.6 
                                                    ) MINUTE THEN
                                                ...
                                                WHEN SUBSTR( CONCAT( all_dates.date, ' ', xin_attendance_time.shift_in, ':00' ) + INTERVAL ROUND( TIMESTAMPDIFF(MINUTE, CONCAT( all_dates.date, ' ', xin_attendance_time.shift_in, ':00' ),
                                        ...
                                        WHEN DATE_FORMAT( xin_attendance_time.clock_in, '%Y-%m-%d %H:%i:00' ) IS NOT NULL AND xin_attendance_time.clock_out IS NULL 
                                            ...
                                            CASE
                                                ...
                                                WHEN locked.jenis = 'eaf' THEN 'LE' 
                                                WHEN locked.jenis = 'training' THEN 'LR' 
                                                WHEN locked.jenis IS NULL THEN 'LA' ELSE '' 
                            ...
                            FROM
                                ...
                                JOIN all_dates ON all_dates.date BETWEEN CONCAT(DATE_FORMAT(DATE_SUB(CURRENT_DATE(), INTERVAL 1 MONTH), '%Y-%m'), '-21') 
                                AND CONCAT(DATE_FORMAT(CURRENT_DATE(), '%Y-%m'), '-20')
                                ...
                                WHERE
                                    trusmi_history_lock.id IN (
                                        ...
                                        FROM
                                            trusmi_history_lock 
                                        WHERE
                                            DATE( trusmi_history_lock.created_at ) BETWEEN CONCAT(DATE_FORMAT(DATE_SUB(CURRENT_DATE(), INTERVAL 1 MONTH), '%Y-%m'), '-21') 
                                            AND CONCAT(DATE_FORMAT(CURRENT_DATE(), '%Y-%m'), '-20')
                                        GROUP BY
                                            trusmi_history_lock.employee_id,
                                ...
                                FROM
                                    ...
                                    WHERE
                                        ...
                                        AND (
                                            ( DATE( trusmi_lock_absen_manual.created_at ) BETWEEN CONCAT(DATE_FORMAT(DATE_SUB(CURRENT_DATE(), INTERVAL 1 MONTH), '%Y-%m'), '-21') AND CONCAT(DATE_FORMAT(CURRENT_DATE(), '%Y-%m'), '-20') ) 
                                            OR ( COALESCE ( DATE( trusmi_lock_absen_manual.updated_by ), CURDATE( ) ) BETWEEN CONCAT(DATE_FORMAT(DATE_SUB(CURRENT_DATE(), INTERVAL 1 MONTH), '%Y-%m'), '-21') AND CONCAT(DATE_FORMAT(CURRENT_DATE(), '%Y-%m'), '-20') ) 
                                        ) 
                                    GROUP BY
                                        trusmi_lock_absen_manual.employee_id 
                                    ) AS lock_manual
                                ...
                                WHERE
                                    all_dates.date BETWEEN CONCAT(DATE_FORMAT(DATE_SUB(CURRENT_DATE(), INTERVAL 1 MONTH), '%Y-%m'), '-21') 
                                    AND CONCAT(DATE_FORMAT(CURRENT_DATE(), '%Y-%m'), '-20')
                            ...
                            WHERE
                                ...
                                AND (
                                    ( xin_employees.is_active = 1 AND xin_employees.date_of_joining <= CONCAT(DATE_FORMAT(CURRENT_DATE(), '%Y-%m'), '-20') ) 
                                    OR (
                                    ...
                                    IF
                                        (
                                        IF
                                            ( xin_employees.date_of_leaving = '' OR xin_employees.date_of_leaving IS NULL, CONCAT(DATE_FORMAT(DATE_SUB(CURRENT_DATE(), INTERVAL 1 MONTH), '%Y-%m'), '-21') - INTERVAL 1 DAY, xin_employees.date_of_leaving ) > CONCAT(DATE_FORMAT(CURRENT_DATE(), '%Y-%m'), '-20'),
                                            CONCAT(DATE_FORMAT(CURRENT_DATE(), '%Y-%m'), '-20'),
                                        IF
                                        ( xin_employees.date_of_leaving = '' OR xin_employees.date_of_leaving IS NULL,CONCAT(DATE_FORMAT(DATE_SUB(CURRENT_DATE(), INTERVAL 1 MONTH), '%Y-%m'), '-21') - INTERVAL 1 DAY, xin_employees.date_of_leaving )) BETWEEN CONCAT(DATE_FORMAT(DATE_SUB(CURRENT_DATE(), INTERVAL 1 MONTH), '%Y-%m'), '-21') 
                                        AND CONCAT(DATE_FORMAT(CURRENT_DATE(), '%Y-%m'), '-20')
                                    ) 
                                ) 
                            GROUP BY
                            ...
                            AND trusmi_harus_hadir.periode = SUBSTR(CURRENT_DATE(),1,7) 
                            GROUP BY
                            rekap.user_id 
                    ) AS x
            ");
