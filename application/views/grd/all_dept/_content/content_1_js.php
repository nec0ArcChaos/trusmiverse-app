<script>
    $(document).ready(function() {
        ...
        get_data_prs_budget(startCalendar, endCalendar, divisi);


    });

    function get_data_prs_budget(start,end, divisi) {
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                if(divisi == 'Soft Launch' || divisi == 'Grand Opening'){
                    ...
                    $('#div_eaf_produksi_bar').show();
                    document.getElementById("div_eaf_sdm").style.display = "flex";
                    document.getElementById("div_eaf_produksi").style.display = "flex";
                    document.getElementById("eaf_sdm_p").style.display = "flex";
                    document.getElementById("eaf_produksi_p").style.display = "flex";
                    ...
                    pie_budget_divisi(response.prs_bt_budget);
                    document.getElementById("div_eaf_sdm").style.display = "none";
                    document.getElementById("div_eaf_produksi").style.display = "none";
                    document.getElementById("eaf_sdm_p").style.display = "none";
                    document.getElementById("eaf_produksi_p").style.display = "none";

                    budget_awal_pe = parseInt(response.prs_bt_budget['budget_awal']);
                    budget_sisa_pe = response.prs_bt_budget['act_budget'];

                    budget_pemakaian_pe = parseInt(budget_awal_pe) - parseInt(budget_sisa_pe);
                    $('#eaf_pengeluaran').text('Rp. ' + formatAngka(budget_sisa_pe) + ' / ' + formatAngka(budget_awal_pe));
                    $('#eaf_pengeluaran_p').text(response.prs_bt_budget['budget_all_p'] + '%');
                    document.getElementById("eaf_pengeluaran_bar").style.width = response.prs_bt_budget['budget_all_p'] + "%";
                    document.getElementById("eaf_pengeluaran_bar").style.fontSize = "12px";
    ...
    function pie_budget_divisi(data_budget) {
        $('#chart_pie_budget').empty().append(`
            <canvas id="pie_budget" style="width: 105px;"></canvas>`);
        const ctx = document.getElementById('pie_budget');
        ...
        new Chart(ctx, {
            ...
            data: {
                ...
                datasets: [{                       
                        data: [
                            data_budget.budget_all_p,
                            data_budget.budget_all_p_sisa,
                        ],                       
                        borderColor: ['#081226', '#0B3281'],
                        backgroundColor: ['#081226', '#0B3281'],
                        cutout: '70%',
                    },
    ...
    function reload_milestone() {
        ...
        fetch(base_url+"/getMilestones/"+id_company)
...
<script>
    function get_company() {
        $.ajax({
            url: base_url+'get_company',
            type: 'POST',
            data: {
                id_divisi: '<?= $divisi_select ?>',
...
<script>
    function save_task() {
        let val_id_company              = $('#add_id_company').val();
        let val_id_divisi               = $('#add_id_divisi').val();
        let val_id_goal                 = $('#add_id_goal').val();
        let val_id_so                   = $('#add_id_so').val();
        let val_id_si                   = $('#add_id_si').val();
        let val_id_pic                  = $('#add_id_pic').val();
        let val_id_jenis                = $('#add_id_jenis').val();
        let val_tasklist_target_batas   = $('#tasklist_target_batas').val();
        ...
        if (val_id_company == "" || val_id_company == null || val_id_company == undefined) {
            $.confirm({
                icon: 'fa fa-close',
                title: 'Oops!',
                theme: 'material',
        ...
        } else if (val_id_divisi == "" || val_id_divisi == null || val_id_divisi === undefined) {
            $.confirm({
                icon: 'fa fa-close',
                title: 'Oops!',
                theme: 'material',
        ...
        } else if (val_id_goal == "" || val_id_goal == null || val_id_goal === undefined) {
            $.confirm({
                icon: 'fa fa-close',
                title: 'Oops!',
                theme: 'material',
        ...
        } else if (val_id_so == "" || val_id_so == null || val_id_so === undefined) {
            $.confirm({
                icon: 'fa fa-close',
                title: 'Oops!',
                theme: 'material',
        ...
        } else if (val_id_si == "" || val_id_si == null || val_id_si === undefined) {
            $.confirm({
                icon: 'fa fa-close',
                title: 'Oops!',
                theme: 'material',
        ...
        } else if (val_id_pic == "" || val_id_si == null) {
            $.confirm({
                icon: 'fa fa-close',
                title: 'Oops!',
                theme: 'material',
        ...
        } else if (val_id_jenis == "" || val_id_jenis === null || val_id_jenis === undefined) {
            $.confirm({
                icon: 'fa fa-close',
                title: 'Oops!',
                theme: 'material',
        ...
        } else if (val_tasklist_target_batas == "" || val_tasklist_target_batas === null || val_tasklist_target_batas === undefined) {
            $.confirm({
                icon: 'fa fa-close',
                title: 'Oops!',
                theme: 'material',
                type: 'red',
                content: 'Oops, Batas tasklist must be filled',
                buttons: {
                    close: {
                        actions: function() {}
                    },
            ...
            $.confirm({
                ...
                buttons: {
                    confirm: function() {
                        ...
                        form_data_tasklist.append("id_jenis", val_id_jenis);
                        ...
                        form_data_tasklist.append("tasklist_target_batas", val_tasklist_target_batas);
...
<script>
    function save_task_milestone() {
        ...
        let val_milestone_target_batas = $('#milestone_target_batas').val();
        ...
        } else if (val_milestone_target_batas == "" || val_milestone_target_batas === null || val_milestone_target_batas == undefined) {
            $.confirm({
                icon: 'fa fa-close',
                title: 'Oops!',
                theme: 'material',
                type: 'red',
                content: 'Oops, Batas melistone must be filled',
                buttons: {
                    close: {
                        actions: function() {}
                    },
            ...
            $.confirm({
                ...
                buttons: {
                    confirm: function() {
                        ...
                        $.ajax({
                            url: base_url+'save_task_milestone',
                            cache: false,
                            contentType: false,
                            processData: false,
                            data: form_data_tasklist, // Setting the data attribute of ajax with file_data
...
<script>
    function save_so() {
        ...
        let val_target_so_batas = $('#so_target_so_batas').val();

        console.log('id pic = ' + val_id_pic);
        console.log('id divisi = ' + val_id_divisi);
        console.log('val_target_type = ' + val_target_type);
        console.log('val_target_so_batas = ' + val_target_so_batas);
        ...
        } else if (val_id_pic == "" || val_id_pic == null || val_id_pic == undefined) {
            $.confirm({
                icon: 'fa fa-close',
                title: 'Oops!',
                theme: 'material',
        ...
        } else if (val_target_type == "" || val_target_type === null || val_target_type === undefined) {
            $.confirm({
                icon: 'fa fa-close',
                title: 'Oops!',
                theme: 'material',
        ...
        } else if (val_target_so_batas == "" || val_target_so_batas === null || val_target_so_batas === undefined) {
            $.confirm({
                icon: 'fa fa-close',
                title: 'Oops!',
                theme: 'material',
                type: 'red',
                content: 'Oops, jenis target so must be filled',
                buttons: {
                    close: {
                        actions: function() {}
                    },
            ...
            $.confirm({
                ...
                buttons: {
                    confirm: function() {
                        ...
                        form_data_so.append("so_target_so_batas", val_target_so_batas);

                        $.ajax({
                            url: base_url+'save_so',
                            cache: false,
                            contentType: false,
                            processData: false,
                            ...
                            console.log("response.save_so : " + response.save_m_so);
                            if (response.save_m_so == true) {
...
<script>
    function save_si() {
        ...
        let val_target_batas = $('#si_target_si_batas').val();
        ...
        if (val_id_company == "" || val_id_company == null || val_id_company == undefined) {
            ...
            $.confirm({
                ...
                theme: 'material',
                type: 'red',
                content: 'Oops, type target SI must be filled',
                buttons: {
                    close: {
                        actions: function() {}
                    },
        ...
        } else if (val_target_batas == "" || val_target_batas === null || val_target_batas === undefined) {
            $.confirm({
                icon: 'fa fa-close',
                title: 'Oops!',
                theme: 'material',
                type: 'red',
                content: 'Oops, jenis target si must be filled',
                buttons: {
                    close: {
                        actions: function() {}
                    },
        ...
        } else if (val_target == "" || val_target == 0) {
            $.confirm({
                icon: 'fa fa-close',
                title: 'Oops!',
                theme: 'material',
            ...
            $.confirm({
                ...
                buttons: {
                    confirm: function() {
                        ...
                        form_data_si.append("si_target_si_batas", val_target_batas);

                        $.ajax({
                            url: base_url+'save_si',
                            cache: false,
                            contentType: false,
                            processData: false,
                            ...
                            console.log("response.save_si " + response.save_m_si);
                            if (response.save_m_si == true) {
                                ...
                                setTimeout(() => {
                                    ...
                                    $.confirm({
                                        ...
                                        onOpen: function () {
                                            reload_data(); // ✅ ini dipanggil otomatis saat alert muncul
                                        },
                                        autoClose: 'close|2000',
                                        buttons: {
                                            close: {
