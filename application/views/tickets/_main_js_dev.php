        ...
        $.ajax({
            ...
            for (let index_t = 0; index_t < response.length; index_t++) {
                if (parseInt(response[index_t].id_pic) == parseInt(my_ticket_id)) {
                    list_pic_ticket += `<option value="${response[index_t].id_pic}" selected>My Tickets (${response[index_t].ticket})</option>`;
                }
            }
            for (let index = 0; index < response.length; index++) {
...
<script>
    ...
    function get_filter_status() {
        $.ajax({
            ...
            for (let index = 0; index < response.length; index++) {
                list_filter_pic += `<option value="${response[index].id_status}">${response[index].status}</option>`;
            }
...
<script>
    ...
    function e_get_type(id_type = '') {
        $.ajax({
            ...
            for (let index = 0; index < response.length; index++) {
                e_list_type += `<option value="${response[index].id_type}" ${response[index].id_type == id_type ? 'selected':''}>${response[index].type}</option>`;
            }
            if (user_role_id == 1) {
                // list_type += '<option value="add_new">+ Add Type</option>';
            }
    ...
    function e_get_object(id_object = '') {
        $.ajax({
            ...
            if (response != null) {
                for (let index = 0; index < response.length; index++) {
                    e_list_object += `<option value="${response[index].id_object}" ${response[index].id_object == id_object ? 'selected':''} data-id_category="${response[index].id_category}" data-id_type="${response[index].id_type}">${response[index].object}</option>`;
                }
            }
            ...
            onOpen: function() {
                $.ajax({
                    ...
                    if (response.status == true) {
                        ...
                        $('#e_requested_by_text').html(response.detail.requested_by + `<br>${response.detail.requested_contact_no} <a class="badge bg-light-green text-green" href="https://api.whatsapp.com/send?phone=${response.detail.requested_contact_no}" target="_blank"><i class="bi bi-whatsapp text-success"></i> Chat</a>`);
    ...
    function update_task() {
        ...
        if (val_e_id_task == "") {
            ...
            $.confirm({
                ...
                onOpen: function() {
                    $.ajax({
                        url: `<?= base_url() ?>tickets/main_dev/update_task`,
                        type: 'POST',
                        dataType: 'json',
                        data: {
                            id_task: val_e_id_task,
...
<?php if ($this->uri->segment(4) != "") {
    $this->load->view('tickets/' . $this->uri->segment(4) . '/js_dev');
} ?>