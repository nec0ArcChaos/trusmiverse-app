                ...
                if (response != null) {
                    ...
                    for (let index = 0; index < response.length; index++) {
                        ...
                        if (parseInt(response[index].sub_type) == 1) {
                            ...
                            if (parseInt(response[index].sub_type) == 1 && department_session != 68 && new Date().getDate() >= tgl_open_ticket_dev) {
...
<script>
    ...
    function get_sub_type_od() {
        ...
        $.ajax({
            url: '<?= base_url() ?>tickets/main/get_sub_type_od',
            type: 'POST',
            dataType: 'json',
            beforeSend: function() {
                $('#id_sub_type').empty().append(
...
<script>
        ...
        $.ajax({
            ...
            if (response != null) {
                for (let index = 0; index < response.length; index++) {
                    ...
                    list_pic +=
                        `<option value="${response[index].id_pic}">${response[index].pic} ${ticket_onprogress}</option>`;
                }
            }
    ...
    function e_get_pic(e_id_type = '', e_id_pic = '') {
        ...
        $.ajax({
            url: '<?= base_url() ?>tickets/main/' + target_func, // oddev
            type: 'POST',
            dataType: 'json',
            data: {
                id_type: e_id_type
...
<script>
    ...
    function generate_status_history(status) {
        if (status == 'Not Started') {
        ...
        } else if (status == 'Stuck' || status == 'Cancel') {
            ...
            badge = `<span class="btn btn-link btn-sm bg-orange text-white">${status}</span>`
        } else if (status == 'Waiting List') {
            badge = `<span class="btn btn-link btn-sm bg-pink text-white">${status}</span>`
        } else {
            badge = ``
        }
        return badge;
