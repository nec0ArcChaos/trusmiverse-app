...
<script>
    function timer(countDownDate) {
        ...
        var x = setInterval(function() {
            ...
            if (distance < 0) {
                clearInterval(x);
                document.getElementById("timer").innerHTML = "Your Ticket is Overdue.";
            }
        }, 1000);
    }
</script>
...
<script>
                ...
                $.ajax({
                    ...
                    if (response.status == true) {
                        setTimeout(() => {
                            ...
                            $('#e_type_text').html(response.detail.type).addClass('badge bg-light-pink text-dark');
                            $('#e_category_text').html(response.detail.category).addClass('badge bg-light-yellow text-dark');
                            $('#e_priority_text').html(response.detail.priority).addClass(response.detail.priority_color);
                            $('#e_level_text').html(response.detail.level).addClass(response.detail.level_color);
                            $('#e_status_text').html(response.detail.status).addClass(response.detail.status_color);
                            ...
                            // console.log(response.detail.location)
                        ...
                        body_log_history += `<tr>
                            <td>
                                <div class="row text-start">
                                    <div class="col-auto">
                                        <small>${calculate_time_history_log_detail(value.datetime)}</small><br>
                                    ...
                                    <div class="col">
                                        <div class="col">
                                            <span class="mb-2">${get_jenis_log_detail(value.jenis)}</span>
                                        </div>
                                        <small>${generate_history_change_detail(value)}<small>
                                    </div>
                                </div>
                            </td>
                        </tr>`
    ...
    function get_jenis_log_detail(jenis) {
        if (jenis == 'created') {
            jenis_log = `<span class="badge bg-light-blue text-dark"><i class="bi bi-pen"></i> Created</span>`
        } else if (jenis == 'progress') {
            jenis_log = `<span class="badge bg-light-yellow text-dark"><i class="bi bi-percent"></i> Progress</span>`
        } else if (jenis == 'status') {
            jenis_log = `<span class="mb-2 badge bg-light-green text-dark"><i class="bi bi-flag"></i> Status</span>`
        } else if (jenis == 'evaluasi') {
            jenis_log = `<span class="badge bg-light-purple text-dark"><i class="bi bi-clipboard-data"></i> Evaluasi</span>`
        } else if (jenis == 'note') {
            jenis_log = `<span class="badge bg-light-pink text-dark"><i class="bi bi-chat-right-text"></i> Note</span>`
        } else {
            jenis_log = ``
        }
        return jenis_log;
