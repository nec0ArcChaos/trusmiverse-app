...
<script type="text/javascript">
  ...
  $(document).ready(function() {
    ...
    $('#btn_add_pengajuan').click(function() {
      ...
      // Event handler khusus untuk jenis_komisi
      $('#jenis_komisi').on('change', function() {
          var selectedValue = $(this).val();
          
          if (selectedValue === 'komisi_driver') {
              $('.batik-tourleader-fields').hide();
              setBatikTourLeaderFieldsRequired(false);
              
              $('#status_penerima_rekening').val('').removeClass('is-invalid');
              $('#no_reservasi').val('').removeClass('is-invalid');
              $('#pic_marketing').val('').removeClass('is-invalid');
          }
          else if (selectedValue === 'komisi_tl') {
      ...
      $('#save_eaf').on('click', function() {
        ...
        if (nama_penerima.val() == "") {
          ...
          var isBatikTourLeader =
            ...
            jenisKomisi === 'komisi_tl';

          if (isBatikTourLeader) {
            if (!validateBatikTourLeaderFields()) {
                swal("Perhatian!", "Harap lengkapi semua field tambahan untuk Integrasi Tour Leader.", "error");
                return;
            }
          }
          ...
          swal({
            ...
            .then((willDelete) => {
              if (willDelete) {
                $.ajax({
                  url: '<?php echo base_url() ?>eaf/pengajuan_dev/insert_pengajuan_dev',
                  type: 'post',
                  dataType: 'json',
                  ...
                  success: function(response) {
                    ...
                    if (response.warning == "") {
                      ...
                      $('#jenis_komisi_container').hide();
                      $('#jenis_komisi').val('').prop('required', false);
        ...
        $('#status_penerima_rekening').on('change', function () {
  ...
  function toggleBatikTourLeaderFields() {
    ...
    // Cek apakah memenuhi syarat untuk menampilkan jenis_komisi
    var showJenisKomisi = 
        companyId == 5 && 
        keperluanValue && 
        keperluanValue.includes('55|7|Fee Tour Leader Store');
    
    // Cek apakah harus menampilkan batik-tourleader-fields
    var showTourLeaderFields = showJenisKomisi && jenisKomisi === 'komisi_tl';
    
    if (showJenisKomisi) {
        $('#jenis_komisi_container').show();
        $('#jenis_komisi').prop('required', true);
        
        if (jenisKomisi === '' || jenisKomisi === 'komisi_tl') {
            if (jenisKomisi === 'komisi_tl') {
                $('.batik-tourleader-fields').show();
                setBatikTourLeaderFieldsRequired(true);
    ...
    if (showTourLeaderFields) {
        $('.batik-tourleader-fields').show();
        setBatikTourLeaderFieldsRequired(true);
        loadPicMarketingData();
        loadNoReservasiData();
    } else if (jenisKomisi === 'komisi_driver') {
        $('.batik-tourleader-fields').hide();
        setBatikTourLeaderFieldsRequired(false);
        resetBatikTourLeaderFields();
    }
  ...
  function setBatikTourLeaderFieldsRequired(isRequired) {
    $('#status_penerima_rekening').prop('required', false);
  ...
  function resetBatikTourLeaderFields() {
    $('#jenis_komisi').removeClass('is-invalid');
  ...
  // Function untuk validasi field Batik Tour Leader
  function validateBatikTourLeaderFields() {
      var jenisKomisi = $('#jenis_komisi').val();
    
      if (jenisKomisi !== 'komisi_tl') {
          return true; // Tidak perlu validasi field batik tour leader
      }  

      var isValid = true;
      
      // Validasi Status Penerima Rekening
      if ($('#status_penerima_rekening').val() === "") {
          $('#status_penerima_rekening').addClass('is-invalid');
          isValid = false;
      }
      
      // Validasi No Reservasi
      if ($('#no_reservasi').val() === "") {
          $('#no_reservasi').addClass('is-invalid');
          isValid = false;
      }
      
      // Validasi PIC Marketing
  ...
  function loadPicMarketingData() {
    $.ajax({
      url: '<?php echo base_url() ?>eaf/pengajuan/get_pic_marketing',
      type: 'GET',
      dataType: 'json',
      success: function (response) {
  ...
  function loadNoReservasiData() {
    $.ajax({
      url: '<?php echo base_url() ?>eaf/pengajuan/get_no_reservasi',
      type: 'GET',
      dataType: 'json',
      success: function (response) {
        ...
        response.forEach(function (reservasi) {
          $('#no_reservasi').append(
            $('<option>', {
              ...
              text: reservasi.no_reservasi + ' | ' + reservasi.kode_kasir,
