...
<script type="text/javascript">
  ...
  function toggleBatikTourLeaderFields() {
    ...
    var isBatikTourLeader = keperluanMap[companyId] && keperluanMap[companyId].includes(keperluanValue);

    if (isBatikTourLeader) {
      $('.batik-tourleader-fields').show();
      $('#jenis_komisi').val('');
      $('#no_reservasi').val('').prop('disabled', true);
      $('#kode_kasir').val('');
  ...
  function resetBatikTourLeaderFields() {
    ...
    $('#no_reservasi').val('').removeClass('is-invalid').prop('disabled', true);
    $('#kode_kasir').val('');
    ...
    $('.fa_wait_foto_rekening, .fa_done_foto_rekening').hide();
    $('.fa_wait_lampiran_reservasi, .fa_done_lampiran_reservasi').hide();
