...
<script type="text/javascript">
  ...
  $(document).ready(function() {
    ...
    $('#upload_nota_wrap').hide();

    // Kalau kategori berubah
    $('#kategori').on('change', function() {
        if ($(this).val() == "17") {
            $('#upload_nota_wrap').show();
            $('#rupiah').val('').prop('disabled', true);  // langsung disable saat pilih kategori 17
        } else {
            $('#upload_nota_wrap').hide();

            // reset biar gak kebawa
            $('#nota').val('');
            $('#preview-nota').html('');
            $('#rupiah').val('').prop('disabled', false); // enable lagi kalau bukan 17
            $('.fa_wait').hide();
            $('.fa_done').hide();
        }
    });
    ...
    function check_nominal() {
        let nominal = $('#rupiah').val().replace(/[^0-9]/g, '');
    ...
    $('#btn_add_pengajuan').click(function() {
      ...
      $('#kategori').on('change', function() {
        ...
        let dvs = $('#id_divisi').val();

        if (ktg == 17) {
            $('#attch').text('Bukti Nota');
            $('.tgl_hide').show();
            $('#upload_nota_wrap').show();   // tampilkan upload nota
        } else if (ktg == 18) {
            $('#attch').text('Lampiran');
            $('.tgl_hide').hide();
            $('#upload_nota_wrap').hide();   // sembunyikan
        }
      ...
      $('#company').on('change', function get_keperluan() {
        ...
        if (id_hr != null && id_hr != '-- Pilih Yang Mengajukan --' && b_nominal != '' && companyId != '-- Pilih Company --') {
          $.ajax({
            url: '<?php echo base_url() ?>eaf/pengajuan_fel/jenis_biaya_by_company/',
            type: 'POST',
            data: {
          ...
          $('.dlk_makan').remove();
          $.ajax({
            url: '<?php echo base_url(); ?>eaf/pengajuan_fel/cek_dlk/' + id_hr,
            type: 'GET',
            dataType: 'json',
            success: function(response) {
      ...
      $('#save_eaf').on('click', function() {
        ...
        //   console.log("tanggal kosong");
        } else if (kategori.val() == "17" && $('#temp_nota_id').val() == "" && pilihan_ba == 'file') {
          swal("Perhatian!", "Nota belum diupload dan dibaca.", "error");
          console.log("temp_nota_id kosong");
        } else if (kategori.val() == "17" && tanggal.val() == "" && pilihan_ba == 'file') {
          tanggal.addClass('is-invalid');
          tanggal.focus();
          console.log("tanggal kosong");
          ...
          swal({
            ...
            .then((willDelete) => {
              if (willDelete) {
                $.ajax({
                  url: '<?php echo base_url() ?>eaf/pengajuan_fel/insert_pengajuan',
                  type: 'post',
                  dataType: 'json',
                  ...
                  success: function(response) {
                    if (response.warning == "") {
                        ...
                        $("#string").val("");
                        $("#temp_nota_id").val("");  // reset id temp_nota
                        $('#preview-nota').html(''); // kosongkan preview OCR
   ...
   $('#upload_nota_wrap').on('click', function() {
      // let file = $('#nota')[0].files[0];            
      const file = document.querySelector('#nota').files[0];
      console.log(file);
      

      if (!file) {
          swal("Perhatian!", "Silahkan pilih file nota terlebih dahulu.", "error");
          return;
      }
      var form = $(`#form_keperluan`)[0];
      ...
      $('.fa_wait').show();
      $('.fa_done').hide();
      $('#preview-nota').html('Mengunggah...');

      $.ajax({
          url: '<?= base_url("eaf/pengajuan_fel/upload_nota") ?>',
          type: 'POST',
          data: formData,
          processData: false,
          contentType: false,
        ...
        //       $('#preview-nota').html('');
        //   } else {
        //       swal("Success", "Nota berhasil diupload.", "success");

        //       $('#temp_nota_id').val(res.id);
        //       console.log("temp_nota_id terisi:", $('#temp_nota_id').val());

        //       // isi otomatis dari OCR
        //       $('#rupiah').val(formatRupiah(res.total));

        //       // pastikan tetap disable kalau kategori 17
        //       if ($('#kategori').val() == "17") {
        ...
        //       // 🔥 Tambahan di sini supaya Company bisa dipilih walau total otomatis
        //       check_nominal();
        ...
        success: function(res) {
          ...
          if (res.status === 'error') {
              ...
              // simpan temp_nota_id dari response
              $('#temp_nota_id').val(res.id);
              console.log("temp_nota_id terisi:", $('#temp_nota_id').val());

              // isi otomatis field rupiah dengan grand_total
              $('#rupiah').val(formatRupiah(res.total));

              // kalau kategori 17 → disable input
              if ($('#kategori').val() == "17") {
                  $('#rupiah').prop('disabled', true);
              } else {
                  $('#rupiah').prop('disabled', false);
              ...
              // jalankan pengecekan nominal supaya company bisa dipilih
              check_nominal();
    ...
    $('#dt_list_eaf').on('click', '.detail_pengajuan', function() {
      ...
      $.ajax({
        url: '<?php echo base_url() ?>eaf/pengajuan_fel/data_detail_pengajuan',
        type: 'POST',
        dataType: 'html',
        data: {
    ...
    function tabel_list_eaf(start, end) {
      $('#dt_list_eaf').DataTable({
        ...
        "ajax": {
          ...
          "url": "<?= base_url('eaf/pengajuan_fel/get_list_eaf') ?>",
        },
        "columns": [{
            "data": "id_pengajuan",
            "render": function(data, type, row) {
              ...
              //   print = '<a target="_blank" href="<?php echo base_url() ?>eaf/pengajuan_fel/save_pengajuan_fel/' + data + '/' + row['id_kategori'] + '/' + row['flag'] + '" class="label label-info"><i class="ti-printer"></i></a>'
              // } else {
              //   print = ''
              // }
  ...
  function notif_pengajuan(id) {
    $.ajax({
      url: '<?php echo base_url() ?>eaf/pengajuan_fel/get_pengajuan_for_wa/' + id,
      type: 'GET',
      dataType: 'json',
      success: function(res) {
        // console.log(id_user);        
