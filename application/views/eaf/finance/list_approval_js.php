...
<script type="text/javascript">
    ...
    $('#dt_list_eaf').DataTable({
      ...
      "columns": [{
          ...
          "render": function(data, type, row) {
            if (row['status'] == 3) {
              if(row['ju_id_eces'] != ''){
                return `<a href="javascript:void(0)" class="badge bg-primary" onclick="approval('${data}','${row['tgl_input']}','${row['name']}','${row['admin_company_name']}','${row['admin_dept_name']}','${row['admin_desg_name']}','${row['nama_divisi']}','${row['nama_kategori']}','${row['nama_penerima']}','${row['status']}')">${data}</a><a target="_blank" href="<?= base_url() ?>eaf/finance/save_pengajuan/${data}/${row['id_kategori']}/${row['flag']}" class="badge bg-info"><i class="bi-printer"></i></a><a target="_blank" href="<?= base_url() ?>eaf/finance/print_nota_eces/${row['ju_id_eces']}" class="label label-warning"><i class="bi-printer"></i></a>`
              } else {
                return `<a href="javascript:void(0)" class="badge bg-primary" onclick="approval('${data}','${row['tgl_input']}','${row['name']}','${row['admin_company_name']}','${row['admin_dept_name']}','${row['admin_desg_name']}','${row['nama_divisi']}','${row['nama_kategori']}','${row['nama_penerima']}','${row['status']}')">${data}</a><a target="_blank" href="<?= base_url() ?>eaf/finance/save_pengajuan/${data}/${row['id_kategori']}/${row['flag']}" class="badge bg-info"><i class="bi-printer"></i></a>`
              }
            } else {
              return `<a href="javascript:void(0)" class="badge custom-bg-outline-primary" onclick="approval_lpj('${data}','${row['tgl_input']}','${row['name']}','${row['admin_company_name']}','${row['admin_dept_name']}','${row['admin_desg_name']}','${row['nama_divisi']}','${row['nama_kategori']}','${row['nama_penerima']}')">${data}</a><a target="_blank" href="<?= base_url() ?>eaf/finance/save_pengajuan/${data}/${row['id_kategori']}/${row['flag']}" class="badge bg-info"><i class="bi-printer"></i></a>`
            }
          }
        },
        {
  ...
  function approval(id, tgl, name, admin_company_name, admin_dept_name, admin_desg_name, divisi, kategori, penerima, status) {
    ...
    if((id_user == 1 || id_user == 162 || id_user == 188 || id_user == 5821 || id_user == 488) && status == 3){
