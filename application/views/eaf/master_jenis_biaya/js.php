...
<script>
  ...
  function mdl_add_jenis_biaya() {
    ...
    selectBoxcompanyid.addEventListener('change', function() {
      ...
      budget.enable();
      // console.info('ffff: ' + selectBoxcompanyid.value);
  ...
  $('#dt_list_jenis_biaya').on('click', '.delete_jenis_biaya', function() {
    ...
    // Gunakan jquery-confirm (karena librarynya sudah diload di file js.php Anda)
    $.confirm({
      title: 'Konfirmasi Non-Aktif',
      content: 'Apakah Anda yakin ingin menonaktifkan data ini? (Data akan ditandai XXX)',
      theme: 'bootstrap',
      buttons: {
        ya: {
          ...
          action: function() {
            $.ajax({
              url: "<?= base_url('eaf/master_jenis_biaya/delete_jenis_biaya') ?>",
              type: "POST",
              dataType: "JSON",
              data: {
