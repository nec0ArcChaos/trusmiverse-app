...
<script>
    ...
    function dt_resume_tasklist(start, end, status) {
        $('#dt_resume_tasklist').DataTable({
            ...
            "ajax": {
                "dataType": 'json',
                "method": "POST",
                "url": "<?= base_url(); ?>jkhpj_dev/dt_resume_tasklist",
                "data": {
                    start: start,
                    end: end,
                    status: status
            ...
            "columns": [{
                    ...
                    "render": function(data, type, row, meta) {
                        return `<a class="badge text-bg-success" href="<?= base_url(); ?>jkhpj_dev/list_task?id_task=${data}">${data}</a>`;
                    },
                },
                {
                    "data": 'created_by',
                ...
                },
                {
                    ...
                    "render": function(data, type, row, meta) {
                        if (data != null) {
                            return `<a href="<?= base_url(); ?>files/jkhpj_dev/${data}"><i class="bi bi-file-earmark-arrow-down"></i></a>`
                        } else {
                            return '';
                        }
    ...
    function dt_resume_tasklist_feedback(start, end, status) {
        $('#feedback_jkhpj').DataTable({
            ...
            "ajax": {
                "dataType": 'json',
                "method": "POST",
                "url": "<?= base_url(); ?>jkhpj_dev/dt_resume_tasklist_feedback",
                "data": {
                    start: start,
                    end: end,
                    status: status
    ...
    function add_task() {
        $.ajax({
            url: "<?= base_url(); ?>jkhpj_dev/check_insert_task",
            type: "GET",
            dataType: "json",
            success: function(response) {
                if (response.message === "Success insert task") {
                    swal("Success", response.message, "success");
                    window.location.href = "<?= base_url(); ?>jkhpj_dev/list_task?id_task=" + response.id_task;
    ...
    function cek_detail_task(id_task_item) {
        ...
        $.ajax({
            url: '<?= base_url('jkhpj_dev/detail_task_item') ?>',
            type: 'POST',
            dataType: 'json',
            data: {
                id: id_task_item
    ...
    function update_task_item() {
        ...
        if (is_file == 1 && old_photo == '' && photo == undefined) {
            ...
            $.ajax({
                url: '<?= base_url('jkhpj_dev/update_task_item') ?>',
                type: 'POST',
                dataType: 'json',
                cache: false,
                contentType: false,
    ...
    function upload_file_progress(id, g_string, title) {
        ...
        url = "<?= base_url() . 'jkhpj_dev/upload_foto_task' ?>";
    ...
    function add_feedback(id) {
        ...
        $.ajax({
            url: "<?= base_url('jkhpj_dev/dt_list_detail_task') ?>",
            method: "POST",
            data: {
                id: id
            },
            ...
            success: function(res) {
                ...
                $.each(data, function(i, val) {
                    history += `<tr>
                        ...
                        <td>${val.file != null ? `<a href="https://trusmiverse.com/files/jkhpj_dev/${val.file}" target="_blank">Download File</a>` : ''}</td>
        ...
        // } 
            ...
            swal({
                ...
                .then((simpan) => {
                    if (simpan) {
                        $.ajax({
                            'url': "<?= base_url('jkhpj_dev/feedback_jkhpj') ?>",
                            'type': "POST",
                            'data': form_data,
                            'dataType': "JSON",
    ...
    function load_kanban_jkhpj() {
        ...
        $.ajax({
            url: "<?= base_url(); ?>jkhpj_dev/get_list_task_kanban",
            type: "POST",
            dataType: "json",
            data: {
                id_task: id_task
