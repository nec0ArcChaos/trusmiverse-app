...
<script type="text/javascript">
    ...
    function save_pph21() {
        ...
        console.log(attachment, note, kategori);
        ...
        if (attachment == "") {
            ...
            $.confirm({
                ...
                theme: 'material',
                type: 'red',
                content: 'Kategori belum dipilih!',
                buttons: { close: {} },
            });
        }
        ...
        // } 
            ...
            form_file.append("kategori", kategori);
            $.confirm({
    ...
    function loadKategori() {
        $.ajax({
            url: "<?= base_url('pajak/get_category_pph') ?>",
            method: "GET",
            dataType: "JSON",
            success: function (data) {
                let option = '';
                $.each(data, function (i, row) {
                    // kalau id = 1, otomatis selected
                    if (row.id == 1) {
                        option += `<option value="${row.id}" selected>${row.category}</option>`;
                    } else {
                        option += `<option value="${row.id}">${row.category}</option>`;
                    }
                });
                $('#kategori').html(option);
            },
            error: function (err) {
                console.error("Gagal ambil kategori:", err.responseText);
                $('#kategori').html('<option disabled selected>Error load data</option>');
            }
        });
    }
            ...
            },
            {
                ...
                render: function (data, type, row) {
                    if (data != null) {
                        ...
                        // Jika gambar → buka pakai Fancybox
                        if (['jpg', 'jpeg', 'png', 'gif', 'webp'].includes(ext)) {
                            return `
                    <a href="<?= base_url('uploads/pph21/') ?>${data}" 
                       class="text-success" 
                       data-fancybox 
                       data-lightbox="1" 
                       data-caption="${data}">
                ...
                `;
                        ...
                        // Jika jenis lain → tampilkan icon dokument
                        return `
                <a href="<?= base_url('uploads/pph21/') ?>${data}" 
    ...
    function list_verif_pph21() {
        ...
        $('#dt_verif_pph21').DataTable({
            ...
            "ajax": {
                "url": "<?= base_url('pajak/dt_verif_pph21') ?>",
                "dataType": 'JSON',
                "type": "POST"
                // "data": {
                //     start: '<?= date("Y-m-01") ?>',
    ...
    //                 $.ajax({
    //                     method: "POST",
    //                     url: "<?= base_url("pajak/verif_pph21") ?>",
    //                     dataType: "JSON",
    //                     cache: false,
    //                     contentType: false,
    //                     processData: false,
    ...
    // BU FANI GA WAJIB ATTACHMENT
    function verif_pph21() {
    if (USER_ID != 77 && $("#file_verif").val() == "") {
        $.confirm({
            icon: 'fa fa-times-circle',
            title: 'Warning',
            theme: 'material',
        ...
        onOpen: function () {
            $.ajax({
                ...
                success: function (res) {
                    ...
                    let form = document.getElementById("form_verifikasi_pph21");
                    if (form) form.reset();
    ...
    function list_paid_pph21() {
        ...
        $('#dt_paid_pph21').DataTable({
            ...
            "ajax": {
                "url": "<?= base_url('pajak/dt_paid_pph21') ?>",
                "dataType": 'JSON',
                "type": "POST"
            },
            "columns": [{
        ...
        if ($("#file_paid").val() == "") {
            ...
            $.confirm({
                ...
                onOpen: function () {
                    $.ajax({
                        method: "POST",
                        url: "<?= base_url("pajak/save_paid_pph21") ?>",
                        dataType: "JSON",
                        cache: false,
                        contentType: false,
                        processData: false,
