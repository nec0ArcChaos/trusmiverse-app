...
<script type="text/javascript">
    ...
    $("#smartwizard2").on("leaveStep", function(e, anchorObject, currentStepIndex, nextStepIndex, stepDirection) {
      ...
      if (nextStepIndex == 1 && stepDirection == "forward") {        
        if ($("#judul").val() == "") {
          ...
          $.ajax({
            method: "POST",
            url: "<?= base_url("mom_faisal/save_mom") ?>",
            dataType: "JSON",
    ...
    $("#smartwizard3").on("leaveStep", function(e, anchorObject, currentStepIndex, nextStepIndex, stepDirection) {
      ...
      if (nextStepIndex == 1 && stepDirection == "forward") {        
        if ($("#judul_draft").val() == "") {
          ...
          $.ajax({
            method: "POST",
            url: "<?= base_url("mom_faisal/save_mom_draft") ?>",
            dataType: "JSON",
    ...
    var tabel_mom =
      $('#dt_list_mom').DataTable({
        ...
        "ajax": {
          "dataType": 'json',
          "type": "POST",
          "url": "<?= base_url("mom_faisal/get_list_mom") ?>",
          "data": {
            datestart: start,
            dateend: end,
            closed: 1 
  ...
  function cancel()
  {
    ...
    if (id_mom != "") {
      $.ajax({
        method: "GET",
        url: "<?= base_url("mom_faisal/cancel_mom") ?>"+id_mom,
        dataType: "JSON",
        beforeSend: function (res){
  ...
  function finish(closed)
  {
    ...
    if (closing == '' || closing == '<p><br></p>' || closing == '<br>') {
      ...
      $.ajax({
        url: "<?= base_url("mom_faisal/save_closing") ?>",
        method: "POST",
        dataType: "JSON",
        data: {
          id_mom: id_mom,
  ...
  function submit_update(id) {
    ...
    if (input[0] == "issue") {
      $.ajax({
        url: '<?php echo base_url() ?>mom_faisal/save_issue',
        type: 'POST',
        dataType: 'json',
        data: {
          id_mom: id_mom,
      ...
      $.ajax({
        url: '<?php echo base_url() ?>mom_faisal/save_issue_item',
        type: 'POST',
        dataType: 'json',
        data: {
          id_mom: id_mom,
    ...
    var tabel_rekap =
      $('#dt_list_rekap').DataTable({
        ...
        "ajax": {
          ...
          "url": "<?= base_url("mom_faisal/get_list_rekap") ?>"
          // "success": function (res){
          //   console.log(res);
          // },
          // "error": function (jqXHR){
          ...
          },
          {
            ...
            'render': function(data,type,row){
              if (data == 0) {
                ...
                return `<span class="btn btn-sm btn-outline-warning" onclick="detail_rekap('${row['user_id']}','3')">${data}</span>`;
              }
            },
            'className': 'text-center'
          },
          {
            ...
            'render': function(data,type,row){
              if (data == 0) {
                ...
                return `<span class="btn btn-sm btn-outline-success" onclick="detail_rekap('${row['user_id']}','4')">${data}</span>`;
              }
            },
            'className': 'text-center'
          },
          {
            ...
            'render': function(data,type,row){
              if (data == 0) {
                ...
                return `<span class="btn btn-sm btn-outline-success" onclick="detail_rekap('${row['user_id']}','5')">${data}</span>`;
              }
            },
            'className': 'text-center'
          },
          {
            ...
            'render': function(data,type,row){
              if (data == 0) {
                ...
                return `<span class="btn btn-sm btn-outline-danger" onclick="detail_rekap('${row['user_id']}','6')">${data}</span>`;
              }
            },
            'className': 'text-center'
          },
  ...
  function detail_rekap(user_id,tipe) {
    ...
    $('#dt_detail_rekap').DataTable({
      ...
      "ajax": {
        "url": "<?= base_url("mom_faisal/get_detail_rekap") ?>",
        "type": "POST",
        "dataType": 'JSON',
        "data": {
          datestart: $("#start").val(),
  ...
  {    
    $('#dt_mom_result_e').DataTable({
      ...
      "ajax": {
        "dataType": 'JSON',
        "type": "GET",
        "url": "<?= base_url("mom_faisal/get_result_meeting/") ?>"+id,
        // "success": function (res){
        //   console.log(res);
        // },
        // "error": function (jqXHR){
      ...
      "columns": [
        ...
        },
        {
          ...
          'render': function(data,type,row,meta){
            no = meta.row + 1;
            return `<input type="text" class="form-control ${(row['id_kategori'] == 1) ? 'tanggal' : '' }" id="e_deadline_${no}" name="e_deadline_${no}" value="${data}" onfocusout="update_result('${row['id_mom']}','${row['id_issue']}','${row['id_issue_item']}','${row['id_task']}','${row['id_sub_task']}','e_deadline_${no}')">`;
          }
        },
        {
          'data': 'list_pic',
        ...
        formSubmit: {
          ...
          action: function() {
            $.confirm({
              ...
              onOpen: function() {
                $.ajax({                    
                  "url": "<?= base_url("mom_faisal/send_wa_mom") ?>"+id+"/reminder",
                  "type": "GET",
                  "dataType": 'TEXT',
                  "success": function (res){
                    console.log(res);       
  ...
  {    
    ...
    $.ajax({
      "url": "<?= base_url("mom_faisal/update_result") ?>",
      "type": "POST",
      "dataType": 'JSON',
      "data": {
        id_mom: id,
  ...
  {    
    ...
    $.ajax({
      url: "<?= base_url("mom_faisal/get_draft_header/") ?>"+id,
      method: "GET",
      dataType: "JSON",
      beforeSend: function (res){
        $('#btn_draft').attr("disabled",true);  
  ...
  function draft_e(closed)
  {
    ...
    if(id_mom == ""){
      ...
      $.ajax({
        url: "<?= base_url("mom_faisal/save_closing_draft") ?>",
        method: "POST",
        dataType: "JSON",
        data: {
          id_mom: id_mom,
  ...
  function get_issue_result(id_mom) {
    $.confirm({
      ...
      onOpen: function() {
        $.ajax({
          url: '<?php echo base_url() ?>mom_faisal/get_issue_result/'+id_mom,
          type: 'GET',
          dataType: 'json',
        })
        .done(function(data) {
