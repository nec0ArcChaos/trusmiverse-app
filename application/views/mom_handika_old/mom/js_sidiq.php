...
<script type="text/javascript">
    ...
    $("#smartwizard2").on("leaveStep", function(e, anchorObject, currentStepIndex, nextStepIndex, stepDirection) {
      ...
      if (nextStepIndex == 1 && stepDirection == "forward") {
        ...
        if ($("#judul").val() == "") {
          ...
          $.ajax({
            method: "POST",
            url: "<?= base_url("mom_sidiq/save_mom") ?>",
            dataType: "JSON",
      ...
      if (nextStepIndex == 2 && stepDirection == "forward") {
        $("#btn_finish").show();
        $.ajax({
          "url": "<?= base_url('mom_sidiq/check_validasi_result/') ?>" + $("#id_mom_global").val(),
          "type": "GET",
          "dataType": 'JSON',
          "success": function(res) {
            // console.log(res);    
    ...
    $("#smartwizard3").on("leaveStep", function(e, anchorObject, currentStepIndex, nextStepIndex, stepDirection) {
      ...
      if (nextStepIndex == 1 && stepDirection == "forward") {
        if ($("#judul_draft").val() == "") {
          ...
          $.ajax({
            method: "POST",
            url: "<?= base_url("mom_sidiq/save_mom_draft") ?>",
            dataType: "JSON",
      ...
      if (nextStepIndex == 2 && stepDirection == "forward") {
        $("#btn_finish_e").show();
        $.ajax({
          "url": "<?= base_url('mom_sidiq/check_validasi_result/') ?>" + $("#id_mom_global_draft").val(),
          "type": "GET",
          "dataType": 'JSON',
          "success": function(res) {
            // console.log(res);    
  ...
  function add_mom() {
    ...
    $.ajax({
      url: "<?= base_url('mom_sidiq/clear_result'); ?>",
      method: "GET",
      dataType: "JSON",
      success: function(res) {
    ...
    var tabel_mom =
      $('#dt_list_mom').DataTable({
        ...
        "ajax": {
          "dataType": 'json',
          "type": "POST",
          "url": "<?= base_url("mom_sidiq/get_list_mom") ?>",
          "data": {
            datestart: start,
            dateend: end,
            closed: 1
  ...
  function cancel() {
    ...
    if (id_mom != "") {
      $.ajax({
        method: "GET",
        url: "<?= base_url("mom_sidiq/cancel_mom/") ?>" + id_mom,
        dataType: "JSON",
        beforeSend: function(res) {
    ...
    if (closing == '' || closing == '<p><br></p>' || closing == '<br>') {
      ...
      $.confirm({
        ...
        onOpen: function() {
          // Start
          $.ajax({
            url: "<?= base_url("mom_sidiq/save_closing") ?>",
            method: "POST",
            dataType: "JSON",
            data: {
              id_mom: id_mom,
  ...
  function submit_update(id) {
    ...
    if (input[0] == "kategori" && $.inArray(parseInt($('#val_' + id).val()), kat_deadline) == -1) {
      ...
      // Clear Level
      $.ajax({
        url: '<?php echo base_url() ?>mom_sidiq/save_issue_item',
        type: 'POST',
        dataType: 'JSON',
        data: {
          id_mom: id_mom,
    ...
    if (input[0] == "topik") {
      $.ajax({
          url: '<?php echo base_url() ?>mom_sidiq/save_topik',
          type: 'POST',
          dataType: 'json',
          data: {
            id_mom: id_mom,
      ...
      $.ajax({
          url: '<?php echo base_url() ?>mom_sidiq/save_issue',
          type: 'POST',
          dataType: 'json',
          data: {
            id_mom: id_mom,
  ...
  function remove_action(issue_no) {
    ...
    if (no > 1) {
      ...
      $.ajax({
          url: '<?php echo base_url() ?>mom_sidiq/delete_issue_item',
          type: 'POST',
          dataType: 'JSON',
          data: {
            id_mom: id_mom,
    ...
    <tr id="div_issue_${next_no}">
        ...
        <td class="kolom_modif" id="td_level_${next_no}_1" data-id="level_${next_no}_1">
            <select class="form-control" id="val_level_${next_no}_1" onchange="submit_update('level_${next_no}_1')">
                <option>- Choose -</option>
                <?php foreach ($level as $lvl) : ?>
                    <option value="<?php echo $lvl->id ?>"><?php echo $lvl->leveling ?></option>
                <?php endforeach ?>
            </select>
        </td>
        <td class="kolom_modif" id="td_deadline_${next_no}_1" data-id="deadline_${next_no}_1">
            ...
            <input type="text" class="form-control tanggal" id="val_date_deadline_${next_no}_1" 
                onfocusout="submit_update('deadline_${next_no}_1')">
        </td>
  ...
  function list_result_e(id) {
    $('#dt_mom_result_e').DataTable({
      ...
      "ajax": {
        "dataType": 'JSON',
        "type": "GET",
        "url": "<?= base_url("mom_sidiq/get_result_meeting/") ?>" + id,
        // "success": function (res){
        //   console.log(res);
        // },
        // "error": function (jqXHR){
  ...
  function get_issue_result(id_mom) {
    ...
    $.confirm({
      ...
      onOpen: function() {
        $.ajax({
            url: '<?php echo base_url() ?>mom_sidiq/get_issue_result/' + id_mom,
            type: 'GET',
            dataType: 'JSON',
            cache: false,
          })
