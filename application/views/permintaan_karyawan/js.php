...
<script>
    $(document).ready(function() {
        ...
        // Auto open modal & prefill jika redirect dari renewal contract
        var urlParams = new URLSearchParams(window.location.search);
        var renewalEmployeeId = urlParams.get('employee_id');
        console.log('URL employee_id:', renewalEmployeeId);

        if (renewalEmployeeId) {
            // Ambil data prefill via AJAX
            $.ajax({
                url: "<?= base_url('permintaan_karyawan/get_prefill_renewal') ?>",
                method: "POST",
                data: { employee_id: renewalEmployeeId },
                dataType: "JSON",
                success: function(res) {
                    console.log('Prefill AJAX response:', res);
                    if (res.status === true && res.data) {
                        console.log('Prefill data:', res.data);
                        show_add_permintaan_prefill(res.data);
                    } else {
                        console.log('Prefill failed:', res.message);
                    }
                },
                error: function(xhr) {
                    console.log('Prefill AJAX error:', xhr.responseText);
                }
            });
        }
    ...
    function get_permintaan(start, end) {
        let edit = false;
        $.ajax({
            url: "<?= base_url('permintaan_karyawan/get_user_role') ?>",
            method: "get",
            dataType: "json",
            success: function(res) {
                if (res.edit == true) {
        ...
        $('#dt_pk').DataTable({
            ...
            "ajax": {
                "dataType": 'json',
                "method": "POST",
                "url": "<?= base_url(); ?>permintaan_karyawan/list_permintaan",
                "data": {
                    status: status,
                    start: start,
                    end: end
    ...
    function show_detail_modal(id) {
        ...
        $.ajax({
            url: "<?= base_url() ?>/permintaan_karyawan/detail_permintaan",
            type: "POST",
            data: {
                id: id
            },
    ...
    function show_edit_modal(id) {
        ...
        $.ajax({
            url: "<?= base_url() ?>/permintaan_karyawan/edit_permintaan",
            type: "POST",
            data: {
                id: id
            },
            ...
            success: function(res) {
                ...
                res.perusahaan.forEach((value, index) => {
                    perusahaan += `<option value = "${value.user_id} | ${value.company_id}"> ${value.company_name} </option>`;
                })
    ...
    function show_add_permintaan_prefill(prefill) {
        // Buka modal add permintaan
        show_add_permintaan();

        // Tunggu perusahaan options ter-load, lalu set value
        var waitPerusahaan = setInterval(function() {
            if ($('#perusahaan option').length > 1) {
                clearInterval(waitPerusahaan);

                // Set perusahaan
                var perusahaanVal = prefill.employer_id + ' | ' + prefill.company_id;
                $('#perusahaan').val(perusahaanVal);
                perusahaan_select.update();
                console.log('Prefill perusahaan:', perusahaanVal);

                // Load dependent dropdowns
                get_department();
                get_location();

                // Tunggu department & location ter-load
                setTimeout(function() {
                    // Set department
                    if (prefill.department_id) {
                        $('#department').val(prefill.department_id);
                        department_select.update();
                        console.log('Prefill department:', prefill.department_id);
                    }

                    // Set location
                    if (prefill.location_id) {
                        $('#location').val(prefill.location_id);
                        location_select.update();
                        console.log('Prefill location:', prefill.location_id);
                    }

                    // Load posisi berdasarkan department
                    get_posisi();

                    // Tunggu posisi ter-load
                    setTimeout(function() {
                        if (prefill.designation_id && prefill.designation_name) {
                            var posisiVal = prefill.designation_id + '|' + prefill.designation_name;
                            $('#posisi').val(posisiVal);
                            posisi_select.update();
                            console.log('Prefill posisi:', posisiVal);

                            // Load detail posisi (job desc, kpi, etc)
                            get_dt_posisi();
                        }
                    }, 1000);
                }, 1000);
    ...
    function show_add_permintaan() {
        ...
        $.ajax({
            url: "<?= base_url('permintaan_karyawan/get_perusahaan') ?>",
            method: "POST",
            dataType: "JSON",
            success: function(res) {
    ...
    function get_posisi() {
        ...
        if (perusahaan != null) {
            ...
            $.ajax({
                url: "<?= base_url('permintaan_karyawan/get_posisi') ?>",
                method: "POST",
                dataType: "JSON",
                data: {
    ...
    function get_location() {
        ...
        if (perusahaan != null) {
            company = perusahaan.split('|');
            $.ajax({
                url: "<?= base_url('permintaan_karyawan/get_location') ?>",
                method: "POST",
                dataType: "JSON",
                data: {
                    id_perusahaan: company[1]
    ...
    function get_kel_posisi() {
        $.ajax({
            url: "<?= base_url('permintaan_karyawan/get_kel_posisi') ?>",
            method: "POST",
            dataType: "JSON",
            success: function(res) {
    ...
    function get_status_karyawan() {
        $.ajax({
            url: "<?= base_url('permintaan_karyawan/get_status_karyawan') ?>",
            method: "POST",
            dataType: "JSON",
            success: function(res) {
    ...
    function get_tipe_kontrak() {
        $.ajax({
            url: "<?= base_url('permintaan_karyawan/get_tipe_kontrak') ?>",
            method: "POST",
            dataType: "JSON",
            success: function(res) {
                console.log(res);
    ...
    function get_pengganti() {
        ...
        if ($('#permohonan').val() == 'Penggantian Untuk') {
            if (perusahaan == null) {
                ...
                company = perusahaan.split('|');
                $.ajax({
                    url: "<?= base_url('permintaan_karyawan/get_pengganti') ?>",
                    method: "POST",
                    dataType: "JSON",
                    data: {
                        id: company[1]
    ...
    function get_dt_posisi() {
        ...
        if (posisi != '--Pilih Jabatan / Posisi--' && posisi != null && department != null) {
            $.ajax({
                url: "<?= base_url('permintaan_karyawan/get_dt_posisi') ?>",
                method: "POST",
                data: {
                    ...
                    'confirm': {
                        ...
                        action: function() {
                            $.confirm({
                                ...
                                onOpen: function() {
                                    $.ajax({
                                        url: "<?= base_url('permintaan_karyawan/add_permintaan') ?>",
                                        method: "POST",
                                        data: form.serialize(),
                                        dataType: "JSON",
                                        beforeSend: function() {},
    ...
    function get_data_pengganti(nama) {
        $.ajax({
            url: "<?= base_url('permintaan_karyawan/get_data_pengganti') ?>",
            method: "POST",
            data: {
                nama: nama
            },
    ...
    function add_jabatan() {
        ...
        if ($('#jabatan_job_profile').val() == null || $('#jabatan_job_profile').val() == '') {
            ...
            $.ajax({
                url: "<?= base_url('permintaan_karyawan/add_jabatan') ?>",
                method: "POST",
                data: {
