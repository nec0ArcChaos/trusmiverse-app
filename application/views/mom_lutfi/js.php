...
<script type="text/javascript">
  ...
  let slimSelectInstances = {}; // Objek untuk menyimpan semua instance SlimSelect
  let slimSelectDraftInstances = {}; // Objek untuk menyimpan semua instance SlimSelect

  function initSlimSelect(id) {
    if (slimSelectInstances[id]) {
        slimSelectInstances[id].destroy(); // Hancurkan instance lama sebelum membuat yang baru
    }

    slimSelectInstances[id] = new SlimSelect({
        select: `#val_grdsi_${id}`
    });
  }

  function destroySlimSelect(id) {
    if (slimSelectInstances[id]) {
        slimSelectInstances[id].destroy();
        delete slimSelectInstances[id]; // Hapus dari objek global
    }
  }

  function initDraftSlimSelect(id) {
    if (slimSelectDraftInstances[id]) {
        slimSelectDraftInstances[id].destroy(); // Hancurkan instance lama sebelum membuat yang baru
    }

    slimSelectDraftInstances[id] = new SlimSelect({
        select: `#val_grdsi_draft_${id}`
    });
  }

  function destroyDraftSlimSelect(id) {
    if (slimSelectDraftInstances[id]) {
        slimSelectDraftInstances[id].destroy();
        delete slimSelectDraftInstances[id]; // Hapus dari objek global
    }
  }
  ...
  $(document).ready(function() {
    ...
    $("#smartwizard2").on("leaveStep", function(e, anchorObject, currentStepIndex, nextStepIndex, stepDirection) {
      ...
      if (nextStepIndex == 1 && stepDirection == "forward") {
        ...
        if ($("#judul").val() == "") {
          ...
          $.ajax({
            method: "POST",
            url: "<?= base_url("mom_lutfi/save_mom") ?>",
            dataType: "JSON",
      ...
      if (nextStepIndex == 2 && stepDirection == "forward") {
        $("#btn_finish").show();
        $.ajax({
          "url": "<?= base_url('mom_lutfi/check_validasi_result/') ?>" + $("#id_mom_global").val(),
          "type": "GET",
          "dataType": 'JSON',
          "success": function(res) {
            // console.log(res);    
    ...
    $('#dt_mom_result').on('click', '.kolom_modif', function() {
      ...
      if (input[0] == 'deadline' && $.inArray(parseInt($('#val_kategori_' + input[1] + '_' + input[2]).val()), kat_deadline) != -1) {
        ...
        if (input[0] == 'grdsi') {
          if (!slimSelectInstances[`${input[1]}_${input[2]}`]) {
            $('#val_grdsi_' + input[1] + '_' + input[2]).show();
            setTimeout(function () {
              initSlimSelect(`${input[1]}_${input[2]}`);
            }, 1000);
          }
        } else {
          $('#val_' + id).show().focus();
        ...
        if ($('#val_kategori_' + input[1] + '_' + input[2]).val() != 12) {       
          destroySlimSelect(`${input[1]}_${input[2]}`);  
    ...
    $('#dt_mom_result_draft').on('click', '.kolom_modif', function() {
      ...
      if (input[0] == 'deadline' && $.inArray(parseInt($('#val_kategori_draft_' + input[2] + '_' + input[3]).val()), kat_deadline) != -1) {
        ...
        if (input[0] == 'grdsi') {
          if (!slimSelectDraftInstances[`${input[2]}_${input[3]}`]) {
            $('#val_grdsi_draft_' + input[2] + '_' + input[3]).show();
            setTimeout(function () {
              initDraftSlimSelect(`${input[2]}_${input[3]}`);
            }, 1000);
          }
        } else {
          $('#val_' + id).show().focus();
        ...
        if ($('#val_kategori_draft_' + input[2] + '_' + input[3]).val() != 12) {       
          destroyDraftSlimSelect(`${input[2]}_${input[3]}`);  
    ...
    $("#smartwizard3").on("leaveStep", function(e, anchorObject, currentStepIndex, nextStepIndex, stepDirection) {
      ...
      if (nextStepIndex == 1 && stepDirection == "forward") {
        if ($("#judul_draft").val() == "") {
          ...
          $.ajax({
            method: "POST",
            url: "<?= base_url("mom_lutfi/save_mom_draft") ?>",
            dataType: "JSON",
      ...
      if (nextStepIndex == 2 && stepDirection == "forward") {
        $("#btn_finish_e").show();
        $.ajax({
          "url": "<?= base_url('mom_lutfi/check_validasi_result/') ?>" + $("#id_mom_global_draft").val(),
          "type": "GET",
          "dataType": 'JSON',
          "success": function(res) {
            // console.log(res);    
  ...
  function add_mom() {
    ...
    $.ajax({
      url: "<?= base_url('mom_lutfi/clear_result'); ?>",
      method: "GET",
      dataType: "JSON",
      success: function(res) {
  ...
  function get_list_mom(start, end) {
    var tabel_mom = $('#dt_list_mom').DataTable({
      ...
      buttons: [{
        ...
        },
        {
          ...
          action: function(e, dt, node, settings) {
            url = `<?= base_url() ?>mom_lutfi/excel_detail/${start}/${end}`;
      ...
      "drawCallback": function() {
        $('.excel_detail').attr('href', `<?= base_url() ?>mom_lutfi/excel_detail/${start}/${end}`);
      },
      "ajax": {
        "dataType": 'json',
        "type": "POST",
        "url": "<?= base_url("mom_lutfi/get_list_mom") ?>",
        "data": {
          datestart: start,
          dateend: end,
          closed: 1
      ...
      "columns": [{
          ...
          'render': function(data, type, row) {
            ...
            return `<a href="<?= base_url('pr/mom_lutfi/'); ?>${row['id_link']}" target="_blank" class="btn btn-sm btn-primary mb-1"><i class="bi bi-printer"></i></a>
                <a href="<?= base_url('mom_lutfi/excel/'); ?>${row['id_link']}" target="_blank" class="btn btn-sm btn-success mb-1"><i class="bi bi-filetype-xls"></i></a>
  ...
  function cancel() {
    ...
    if (id_mom != "") {
      $.ajax({
        method: "GET",
        url: "<?= base_url("mom_lutfi/cancel_mom/") ?>" + id_mom,
        dataType: "JSON",
        beforeSend: function(res) {
    ...
    if (closing == '' || closing == '<p><br></p>' || closing == '<br>') {
      ...
      $.confirm({
        ...
        onOpen: function() {
          // Start
          $.ajax({
            url: "<?= base_url("mom_lutfi/save_closing") ?>",
            method: "POST",
            dataType: "JSON",
            data: {
              id_mom: id_mom,
  ...
  function submit_update(id) {
    ...
    if (input[0] == "kategori" && $.inArray(parseInt($('#val_' + id).val()), kat_deadline) == -1) {
      ...
      // Clear Level
      $.ajax({
        url: '<?php echo base_url() ?>mom_lutfi/save_issue_item',
        type: 'POST',
        dataType: 'JSON',
        data: {
          id_mom: id_mom,
      ...
      inp_value = $('#val_' + id).val();
      console.log('Masuk ke GRDSI - ');

    } else {
      
      console.log('Selain semua itu - ');
      if (input[0] == 'deadline' && $.inArray(parseInt($('#val_kategori_' + input[1] + '_' + input[2]).val()), kat_deadline) != -1) {
        ...
        console.log('Selain semua itu ya selain - ');
    ...
    if (input[0] == "topik") {
      $.ajax({
          url: '<?php echo base_url() ?>mom_lutfi/save_topik',
          type: 'POST',
          dataType: 'json',
          data: {
            id_mom: id_mom,
      ...
      $.ajax({
          url: '<?php echo base_url() ?>mom_lutfi/save_issue',
          type: 'POST',
          dataType: 'json',
          data: {
            id_mom: id_mom,
  ...
  // function toggleSlimSelect(id, enable) {
  //   let selectElement = document.getElementById(`val_grdsi_${id}`);
    
  //   if (enable) {
  //       selectElement.removeAttribute("aria-hidden");
  //       selectElement.removeAttribute("inert");

  //       grdsi = new SlimSelect({ select: `#val_grdsi_${id}` });
  //   } else {
  //       if (grdsi) {
  //           grdsi.destroy();
  //       }
  //       selectElement.setAttribute("inert", "true");
  //       selectElement.blur();
  //   }
  // }
  ...
  function add_action(issue_no) {
    ...
    data_action = `<tr>
      ...
      <select class="form-control grdsi" id="val_grdsi_${issue_no}_${next_no}" onchange="submit_update('grdsi_${issue_no}_${next_no}')">
      <option>- Choose SI -</option>
    ...
    // grdsi = new SlimSelect({
    //   select: "#val_grdsi_" + issue_no + "_" + next_no
    // });
  ...
  function remove_action(issue_no) {
    ...
    if (no > 1) {
      ...
      $.ajax({
          url: '<?php echo base_url() ?>mom_lutfi/delete_issue_item',
          type: 'POST',
          dataType: 'JSON',
          data: {
            id_mom: id_mom,
  ...
  function add_issue() {
    ...
    <select class="form-control grdsi" id="val_grdsi_${next_no}_1" onchange="submit_update('grdsi_${next_no}_1')">
    <option>- Choose SI -</option>
    <?php foreach ($grdsi as $si) : ?>
      <option value="<?php echo $si->id_si ?>"><?php echo $si->grdsi ?></option>
    <?php endforeach ?>
  ...
  function remove_issue() {
    ...
    if (no > 1) {
      ...
      $.ajax({
          url: '<?php echo base_url() ?>mom_lutfi/delete_issue',
          type: 'POST',
          dataType: 'JSON',
          data: {
            id_mom: id_mom,
  ...
  function add_action_draft(issue_no) {
    ...
    <select class="form-control grdsi" id="val_grdsi_draft_${issue_no}_${next_no}" onchange="submit_update_draft('grdsi_draft_${issue_no}_${next_no}')">
    <option>- Choose SI -</option>
    <?php foreach ($grdsi as $si) : ?>
      <option value="<?php echo $si->id_si ?>"><?php echo $si->grdsi ?></option>
    <?php endforeach ?>
    ...
    // grdsi_draft = new SlimSelect({
    //   select: "#val_grdsi_draft_" + issue_no + "_" + next_no
    // });
  ...
  function add_issue_draft() {
    ...
    <select class="form-control grdsi" id="val_grdsi_draft_${next_no}_1" onchange="submit_update_draft('grdsi_draft_${next_no}_1')">
    <option>- Choose SI -</option>
    <?php foreach ($grdsi as $si) : ?>
      <option value="<?php echo $si->id_si ?>"><?php echo $si->grdsi ?></option>
    <?php endforeach ?>
    ...
    // grdsi_draft = new SlimSelect({
    //   select: "#val_grdsi_draft_" + next_no + '_1'
    // });
    ...
    var tabel_rekap =
      $('#dt_list_rekap').DataTable({
        ...
        "ajax": {
          ...
          "url": "<?= base_url("mom_lutfi/get_list_rekap") ?>"
          // "success": function (res){
          //   console.log(res);
          // },
          // "error": function (jqXHR){
  ...
  function detail_rekap(user_id, tipe) {
    ...
    $('#dt_detail_rekap').DataTable({
      ...
      "ajax": {
        "url": "<?= base_url("mom_lutfi/get_detail_rekap") ?>",
        "type": "POST",
        "dataType": 'JSON',
        "data": {
          datestart: $("#start").val(),
  ...
  function list_result_e(id) {
    $('#dt_mom_result_e').DataTable({
      ...
      "ajax": {
        "dataType": 'JSON',
        "type": "GET",
        "url": "<?= base_url("mom_lutfi/get_result_meeting/") ?>" + id,
        // "success": function (res){
        //   console.log(res);
        // },
        // "error": function (jqXHR){
  ...
  function reminder_whatsapp(id) {
        ...
        formSubmit: {
          ...
          action: function() {
            $.confirm({
              ...
              onOpen: function() {
                $.ajax({
                  "url": "<?= base_url("mom_lutfi/send_wa_mom_lutfi/") ?>" + id + "/reminder",
                  "type": "GET",
                  "dataType": 'TEXT',
                  "success": function(res) {
                    console.log(res);
    ...
    // $.ajax({
    //   "url": "<?= base_url("mom_lutfi/send_wa_mom") ?>"+id+"/reminder",
    //   "type": "GET",
    //   "dataType": 'TEXT',
    //   "success": function (res){
    //     console.log(res);       
  ...
  function update_result(id, issue, item, id_task, id_sub_task, val) {
    ...
    $.ajax({
      "url": "<?= base_url("mom_lutfi/update_result") ?>",
      "type": "POST",
      "dataType": 'JSON',
      "data": {
        id_mom: id,
  ...
  function proses_draft(id) {
    ...
    $('.check-valid').removeClass('is-invalid');
    $.ajax({
      url: "<?= base_url("mom_lutfi/get_draft_header/") ?>" + id,
      method: "GET",
      dataType: "JSON",
      beforeSend: function(res) {
        $('#btn_draft').attr("disabled", true);
  ...
  function get_issue_result(id_mom) {
    ...
    $.confirm({
      ...
      onOpen: function() {
        $.ajax({
            url: '<?php echo base_url() ?>mom_lutfi/get_issue_result/' + id_mom,
            type: 'GET',
            dataType: 'JSON',
            cache: false,
          })
          .done(function(data) {
            ...
            // if (data.result > 0) {
            //   $("#total_issue_draft").val(data.result);
            //   $.each(data.grdsi, function(index, val) {
            //     console.log(val.id)
            //     grdsi_draft = new SlimSelect({
            //       select: "#" + val.id
            //     });

            //     grdsi_draft.setSelected(val.grdsi)
            //   });
            // } else {
            //   grdsi_draft = new SlimSelect({
            //     select: "#val_grdsi_draft_1_1"
            //   });
            // }
  ...
  function save_plan() {
    if ($("#judul_plan").val() == "") {
      ...
      $.ajax({
        url: "<?= base_url("mom_lutfi/save_plan") ?>",
        method: "POST",
        dataType: "JSON",
  ...
  function list_plan_mom() {
    ...
    $('#dt_list_plan').DataTable({
      ...
      "ajax": {
        "dataType": 'JSON',
        "type": "POST",
        "url": "<?= base_url("mom_lutfi/get_list_plan") ?>",
        // "data": {
        //   datestart: null,
        //   dateend: null,
        //   closed: 0
  ...
  function list_plan_bahan(id_plan) {
    ...
    $('#dt_list_plan_bahan').DataTable({
      ...
      "ajax": {
        "dataType": 'JSON',
        "type": "POST",
        "url": "<?= base_url("mom_lutfi/get_list_plan_bahan") ?>",
        "data": {
          id_plan: id_plan
        },
        // "success": function (res){
      ...
      "columns": [{
          ...
          'render': function(data, type, row) {
            ...
            return `<a href="https://trusmiverse.com/apps/bahan_mom_lutfi/${row['id']}" target="_blank" class="btn btn-sm btn-info"><i class="bi bi-box-arrow-up"></i></a>`;
          }
        },
        {
          'data': 'pic_name'
        },
        {
          ...
          'render': function(data, type, row) {
            ...
            if (data == "") {
              ...
              return `<a href="<?= base_url(); ?>uploads/mom_lutfi/${data}" class="text-success" target="_blank"><i class="bi bi-file-medical"></i></a>`;
            }
          },
          'className': 'text-center'
        },
  ...
  function proses_plan(id_plan) {
    ...
    // Get Data Plan
    $.ajax({
      url: "<?= base_url('mom_lutfi/get_data_plan/'); ?>" + id_plan,
      method: "GET",
      dataType: "JSON",
      success: function(res) {
        // console.log(res);
        ...
        formSubmit: {
          ...
          action: function() {
            $.confirm({
              ...
              onOpen: function() {
                $.ajax({
                  url: "<?= base_url('mom_lutfi/hapus_draft/'); ?>" + id_mom,
                  method: "GET",
                  dataType: "JSON",
                  success: function(res) {
                    console.log(res);
  ...
  function list_eskalasi() {
    ...
    var tabel_mom = $('#dt_list_eskalasi').DataTable({
      ...
      "ajax": {
        "dataType": 'json',
        "type": "POST",
        "url": "<?= base_url("mom_lutfi/list_eskalasi") ?>",
        "dataSrc": ""
  ...
  $(document).on('submit', '.verif-form-owner', function(event) {
    ...
    $.ajax({
      url: "<?= base_url('mom_lutfi/save_verified_owner'); ?>", // Ganti dengan URL untuk proses backend
      type: "POST",
      data: formData,
  ...
  $(document).on('submit', '.verif-form-pdca', function(event) {
    ...
    $.ajax({
      url: "<?= base_url('mom_lutfi/save_verified'); ?>", // Ganti dengan URL untuk proses backend
      type: "POST",
      data: formData,
      success: function(response) {
        swal("Berhasil!", "Data Berhasil di update!", "info");
  ...
  function generate_head_resume_v3() {
    ...
    // let end = $('#end').val();
    $.ajax({
      url: '<?= base_url() ?>mom_lutfi/generate_head_resume_v3',
      type: 'POST',
      dataType: 'json',
      // data: {
      //     start: start,
  ...
  function get_pekerjaan(tipe = null, nilai = null) {
    ...
    $.ajax({
      url: "<?= base_url('mom_lutfi/get_pekerjaan') ?>",
      method: "POST",
      dataType: "JSON",
      data: {
  ...
  function get_sub_pekerjaan(tipe = null, nilai = null) {
    ...
    $.ajax({
      url: "<?= base_url('mom_lutfi/get_sub_pekerjaan') ?>",
      method: "POST",
      dataType: "JSON",
      data: {
  ...
  function get_detail_pekerjaan(tipe = null, nilai = null) {
    ...
    $.ajax({
      url: "<?= base_url('mom_lutfi/get_detail_pekerjaan') ?>",
      method: "POST",
      dataType: "JSON",
      data: {
