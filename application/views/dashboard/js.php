...
<script>
    ...
    function resume_absen(periode, employee_id, cutoff) {
        $.ajax({
            ...
            success: function (response) {
                ...
                // animateValue("leave", 0, (parseInt(response.data.absen)), 3000);
                ...
                for (let index = 0; index < response.data.length; index++) {
                    ...
                    if (response.data[index].photo_in != null) {
                        ...
                        <figure class="avatar avatar-20 rounded-circle coverimg vm" style="background-image: url(&quot;https://trusmiverse.com/hr_upload/${response.data[index].photo_in}&quot;);">
                            <img src="https://trusmiverse.com/hr_upload/${response.data[index].photo_in}" alt="" id="userphotoonboarding2" style="display: none;" loading="lazy">
                        </figure></a>`;
                    } else {
                    ...
                    if (response.data[index].photo_out != null) {
                        ...
                        <figure class="avatar avatar-20 rounded-circle coverimg vm" style="background-image: url(&quot;https://trusmiverse.com/hr_upload/${response.data[index].photo_out}&quot;);">
                            <img src="https://trusmiverse.com/hr_upload/${response.data[index].photo_out}" alt="" id="userphotoonboarding2" style="display: none;" loading="lazy">
                        </figure></a>`;
                    } else {
                                                ...
                                                <div class="col-12 col-lg-12 col-xl-4 col-xxl-4">
                                                    <div class="card border-0 mb-4">
                                                        ...
                                                        <div class="coverimg w-100 h-180 position-relative" style="background-image: url('https://trusmicorp.com/e-training/upload/materi/${response[index].image}');">
                                                            ...
                                                            <img src="https://trusmicorp.com/e-training/upload/materi/${response[index].image}" class="mw-100" alt="" style="display: none;" loading="lazy">
                                                        </div>
                                                        <div class="card-body">
    ...
    function show_dont_dos() {
        ...
        const imageUrls = [
            ...
            // `${base_url}5.jpeg`,
            // `${base_url}6.jpeg`,
            `${base_url}burnout.jpg`,
        ];


        // const randomBannerNumber = getRandomBannerNumber(imageUrls, got_it_banner);

        document.getElementById('info_banner').src = imageUrls[Math.floor(Math.random() * imageUrls.length)]
...
function show_dont_dos() {
    ...
    let got_it = "<?= $personal_info->got_it_campaign ?? '0'; ?>";
...
function save_campaign() {
    ...
    const fileTiktok = document.querySelector('input[name="foto"]').files[0];
    const fileIg = document.querySelector('input[name="foto_ig"]').files[0];

    if (!fileTiktok || !fileIg) {
        Swal.fire('Error', 'Foto TikTok dan Instagram wajib diisi', 'error');
        return;
    }
    ...
    reader1.onload = function(e1) {
        ...
        reader2.onload = function(e2) {
            ...
            $.ajax({
                url: "<?= base_url('dashboard/insert_campaign') ?>",
                type: "POST",
                data: {
                    foto_base64: e1.target.result,
                    foto_ig_base64: e2.target.result
                },
                dataType: "json",
        ...
        reader2.readAsDataURL(fileIg);

    };

    reader1.readAsDataURL(fileTiktok);
}

$(document).ready(function () {
    modal_campaign(); // auto muncul pas refresh
});
</script>
<script>
    $(document).ready(function() {
