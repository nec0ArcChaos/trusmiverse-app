...
<script>
    ...
    var popoverTriggerList = [].slice.call(document.querySelectorAll('[data-bs-toggle="popover"]'))
        ...
        document.addEventListener('click', function(event) {
            ...
            var isClickInside = document.querySelector('.notif').contains(event.target);

            popovers.forEach(function(popover) {
                if (!popover.contains(event.target) && !isClickInside) {
                    var popoverInstance = bootstrap.Popover.getInstance(document.querySelector('.notif'));
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                for (let index = 0; index < response.length; index++) {
                    list_employee += `<option value="${response[index].user_id}" ${user_id == response[index].user_id ? 'selected':''}>${response[index].employee_name}</option>`;
                }
            ...
            success: function(response) {
                ...
                for (let index = 0; index < response.data.length; index++) {
                    ...
                    if (response.data[index].office_shift_id != '1165') {
                        shift_comp = `<p class="text-dark small mb-1 col-12 col-md-6">Shift : <span>${response.data[index].shift_in}</span> s/d <span>${response.data[index].shift_out}</span> </p>`;
                    }
                    item_detail_absen += `
    ...
    function show_notif(type){
        if(type == 1){
            $('#modal_change_personal_info').modal('show');
        }
    }
