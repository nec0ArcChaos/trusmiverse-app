...
<script type="text/javascript">
  ...
  function check_issue() {
    $.ajax({
      url: '<?= base_url("ai_problem_solving/check_issue"); ?>', // replace with your actual endpoint
      type: 'POST',
      dataType: 'json',
      data: { id_mom: $("#id_mom").val(), id_issue: $("#id_issue").val() },
      success: function(response) {
        ...
        if (response.status == 'success') {
          $("#input_problem").val(response.data.issue);
        }
      },
  ...
  function next_problem(step) 
  {
    ...
    if (step == "1" && problem != "") {
      $.ajax({
        url: '<?= base_url('ai_problem_solving/next_problem_coaching') ?>',
        type: 'POST',
        dataType: 'json',
        data: {
          step: step,
      ...
      id_mom = $('#id_mom').val();
      id_issue = $('#id_issue').val();
      status = $("#status_solving option:selected").val();
      ...
      $.ajax({
        ...
        success: function(res) {
          ...
          if (status == 'solved') {
            setTimeout(function() {
              location.href = '<?= base_url('ai_problem_solving/coaching') ?>';
            }, 1000);
          } else {
            setTimeout(function() {
              location.reload();
