...
<script>
    ...
    function dt_trusmi_approval(id_tabel, id_status, start, end, modal = '') {
        url = "<?= base_url(); ?>trusmi_approval_dev/dt_trusmi_approval";
        $(id_tabel).DataTable({
            "searching": true,
            "info": true,
            "paging": true,
            ...
            "columns": [{
                ...
                },
                {
                    ...
                    "render": function(data, type, row) {
                        if (data == 1) {
                            if (modal == '1') {
                                status = `<a type="button" onclick="modal_approve('${row['no_app']}','${row['subject']}', '${row['approve_to']}', '${row['description']}', '${row['file_1']}', '${row['file_2']}', '${row['created_by']}')"><span class="badge badge-sm bg-yellow">${row['status']}</span></a>`;
                            } else {
