...
<script type="text/javascript">
  $(document).ready(function() {
    ...
    var kat_deadline = [1, 5, 6, 8, 9, 10, 11];
    ...
    $("#smartwizard2").on("leaveStep", function(e, anchorObject, currentStepIndex, nextStepIndex, stepDirection) {
      ...
      if (nextStepIndex == 1 && stepDirection == "forward") {
        ...
        if ($("#judul").val() == "") {
          ...
          $.ajax({
            method: "POST",
            url: "<?= base_url("mom_lutfie/save_mom") ?>",
            dataType: "JSON",
      ...
      if (nextStepIndex == 2 && stepDirection == "forward") {
        $("#btn_finish").show();
        $.ajax({
          "url": "<?= base_url('mom_lutfie/check_validasi_result/') ?>" + $("#id_mom_global").val(),
          "type": "GET",
          "dataType": 'JSON',
          "success": function(res) {
            // console.log(res);    
    ...
    $('#dt_mom_result').on('click', '.kolom_modif', function() {
      ...
      if (input[0] == 'deadline' && $.inArray(parseInt($('#val_kategori_' + input[1] + '_' + input[2]).val()), kat_deadline) != -1) {
        ...
        $('#val_' + id).show().focus();
      } else if (input[0] == 'ekspektasi' && $('#val_kategori_' + input[1] + '_' + input[2]).val() == 1) {
        $('#val_' + id).show().focus();
      } else {
        if (input[0] != 'ekspektasi') {
          $('#val_' + id).show().focus();
          $('#val_date_' + id).hide();
    ...
    $("#smartwizard3").on("leaveStep", function(e, anchorObject, currentStepIndex, nextStepIndex, stepDirection) {
      ...
      if (nextStepIndex == 1 && stepDirection == "forward") {
        if ($("#judul_draft").val() == "") {
          ...
          $.ajax({
            method: "POST",
            url: "<?= base_url("mom_lutfie/save_mom_draft") ?>",
            dataType: "JSON",
      ...
      if (nextStepIndex == 2 && stepDirection == "forward") {
        $("#btn_finish_e").show();
        $.ajax({
          "url": "<?= base_url('mom_lutfie/check_validasi_result/') ?>" + $("#id_mom_global_draft").val(),
          "type": "GET",
          "dataType": 'JSON',
          "success": function(res) {
            // console.log(res);    
  ...
  function add_mom() {
    ...
    $.ajax({
      url: "<?= base_url('mom_lutfie/clear_result'); ?>",
      method: "GET",
      dataType: "JSON",
      success: function(res) {
    ...
    var tabel_mom =
      $('#dt_list_mom').DataTable({
        ...
        buttons: [{
          ...
          },
          {
            ...
            action: function(e, dt, node, settings) {
              url = `<?= base_url() ?>mom_lutfie/excel_detail/${start}/${end}`;
        ...
        "drawCallback": function() {
          $('.excel_detail').attr('href', `<?= base_url() ?>mom_lutfie/excel_detail/${start}/${end}`);
        },
        "ajax": {
          "dataType": 'json',
          "type": "POST",
          "url": "<?= base_url("mom_lutfie/get_list_mom") ?>",
          "data": {
            datestart: start,
            dateend: end,
            closed: 1
        ...
        "columns": [{
            ...
            'render': function(data, type, row) {
              ...
              return `<a href="<?= base_url('pr/mom_lutfie/'); ?>${row['id_link']}" target="_blank" class="btn btn-sm btn-primary mb-1"><i class="bi bi-printer"></i></a>
                <a href="<?= base_url('mom_lutfie/excel/'); ?>${row['id_link']}" target="_blank" class="btn btn-sm btn-success mb-1"><i class="bi bi-filetype-xls"></i></a>
  ...
  function cancel() {
    ...
    if (id_mom != "") {
      $.ajax({
        method: "GET",
        url: "<?= base_url("mom_lutfie/cancel_mom_lutfie/") ?>" + id_mom,
        dataType: "JSON",
        beforeSend: function(res) {
    ...
    if (closing == '' || closing == '<p><br></p>' || closing == '<br>') {
      ...
      $.confirm({
        ...
        onOpen: function() {
          // Start
          $.ajax({
            url: "<?= base_url("mom_lutfie/save_closing") ?>",
            method: "POST",
            dataType: "JSON",
            data: {
              id_mom: id_mom,
  ...
  function submit_update(id) {
    ...
    if (input[0] == "kategori" && $.inArray(parseInt($('#val_' + id).val()), kat_deadline) == -1) {
      ...
      // Clear Level
      $.ajax({
        url: '<?php echo base_url() ?>mom_lutfie/save_issue_item',
        type: 'POST',
        dataType: 'JSON',
        data: {
          id_mom: id_mom,
    ...
    } else if (input[0] == "kategori" || input[0] == "pic") {
      inp_value = $('#val_' + id).val();
    } else {
      if (input[0] == 'deadline' && $.inArray(parseInt($('#val_kategori_' + input[1] + '_' + input[2]).val()), kat_deadline) != -1) {
    ...
    if (input[0] == "topik") {
      $.ajax({
          url: '<?php echo base_url() ?>mom_lutfie/save_topik',
          type: 'POST',
          dataType: 'json',
          data: {
            id_mom: id_mom,
      ...
      $.ajax({
          url: '<?php echo base_url() ?>mom_lutfie/save_issue',
          type: 'POST',
          dataType: 'json',
          data: {
            id_mom: id_mom,
  ...
  function remove_action(issue_no) {
    ...
    if (no > 1) {
      ...
      $.ajax({
          url: '<?php echo base_url() ?>mom_lutfie/delete_issue_item',
          type: 'POST',
          dataType: 'JSON',
          data: {
            id_mom: id_mom,
  ...
  function add_issue() {
    ...
    // if (meeting == "Owner") {
      ...
      <textarea class="form-control" id="val_topik_${next_no}_1" style="display: none;" class="excel" rows="${next_no}_1" onfocusin="expandTextarea('val_topik_${next_no}_1')" onfocusout="submit_update('topik_${next_no}_1')"></textarea>
      </td>
      ...
      <td class="kolom_modif" id="td_ekspektasi_${next_no}" data-id="ekspektasi_${next_no}_1">
      <input type="hidden" id="total_action_${next_no}" value="1">
      <span id="ekspektasi_${next_no}_1">&nbsp;</span>
      <textarea class="form-control" id="val_ekspektasi_${next_no}_1" style="display: none;" class="excel" rows="${next_no}_1" onfocusin="expandTextarea('val_ekspektasi_${next_no}_1')" onfocusout="submit_update('ekspektasi_${next_no}_1')"></textarea>
      </td>
      ...
      <span class="btn btn-md btn-outline-danger" id="btn_remove_action_${next_no}" onclick="remove_action(${next_no})" style="display:none;"><i class="bi bi-dash-square"></i> Strategy</span>
      </td>
      </tr>`;
    // } else {
    //   data_action = `
  ...
  function remove_issue() {
    ...
    if (no > 1) {
      ...
      $.ajax({
          url: '<?php echo base_url() ?>mom_lutfie/delete_issue',
          type: 'POST',
          dataType: 'JSON',
          data: {
            id_mom: id_mom,
    ...
    var tabel_rekap =
      $('#dt_list_rekap').DataTable({
        ...
        "ajax": {
          ...
          "url": "<?= base_url("mom_lutfie/get_list_rekap") ?>"
          // "success": function (res){
          //   console.log(res);
          // },
          // "error": function (jqXHR){
  ...
  function detail_rekap(user_id, tipe) {
    ...
    tipe_deadline = ['1', '5', '6', '8', '9', '10', '30', '40', '50', '60'];
    ...
    $('#dt_detail_rekap').DataTable({
      ...
      "ajax": {
        "url": "<?= base_url("mom_lutfie/get_detail_rekap") ?>",
        "type": "POST",
        "dataType": 'JSON',
        "data": {
          datestart: $("#start").val(),
  ...
  function list_result_e(id) {
    $('#dt_mom_result_e').DataTable({
      ...
      "ajax": {
        "dataType": 'JSON',
        "type": "GET",
        "url": "<?= base_url("mom_lutfie/get_result_meeting/") ?>" + id,
        // "success": function (res){
        //   console.log(res);
        // },
        // "error": function (jqXHR){
  ...
  function reminder_whatsapp(id) {
        ...
        formSubmit: {
          ...
          action: function() {
            $.confirm({
              ...
              onOpen: function() {
                $.ajax({
                  "url": "<?= base_url("mom_lutfie/send_wa_mom_lutfie/") ?>" + id + "/reminder",
                  "type": "GET",
                  "dataType": 'TEXT',
                  "success": function(res) {
                    console.log(res);
    ...
    // $.ajax({
    //   "url": "<?= base_url("mom_lutfie/send_wa_mom") ?>"+id+"/reminder",
    //   "type": "GET",
    //   "dataType": 'TEXT',
    //   "success": function (res){
    //     console.log(res);       
  ...
  function update_result(id, issue, item, id_task, id_sub_task, val) {
    ...
    $.ajax({
      "url": "<?= base_url("mom_lutfie/update_result") ?>",
      "type": "POST",
      "dataType": 'JSON',
      "data": {
        id_mom: id,
  ...
  function proses_draft(id) {
    ...
    $('.check-valid').removeClass('is-invalid');
    $.ajax({
      url: "<?= base_url("mom_lutfie/get_draft_header/") ?>" + id,
      method: "GET",
      dataType: "JSON",
      beforeSend: function(res) {
        $('#btn_draft').attr("disabled", true);
  ...
  function get_issue_result(id_mom) {
    $.confirm({
      ...
      onOpen: function() {
        $.ajax({
            url: '<?php echo base_url() ?>mom_lutfie/get_issue_result/' + id_mom,
            type: 'GET',
            dataType: 'JSON',
            cache: false,
          })
  ...
  function save_plan() {
    if ($("#judul_plan").val() == "") {
      ...
      $.ajax({
        url: "<?= base_url("mom_lutfie/save_plan") ?>",
        method: "POST",
        dataType: "JSON",
  ...
  function list_plan_mom() {
    ...
    $('#dt_list_plan').DataTable({
      ...
      "ajax": {
        "dataType": 'JSON',
        "type": "POST",
        "url": "<?= base_url("mom_lutfie/get_list_plan") ?>",
        // "data": {
        //   datestart: null,
        //   dateend: null,
        //   closed: 0
  ...
  function list_plan_bahan(id_plan) {
    ...
    $('#dt_list_plan_bahan').DataTable({
      ...
      "ajax": {
        "dataType": 'JSON',
        "type": "POST",
        "url": "<?= base_url("mom_lutfie/get_list_plan_bahan") ?>",
        "data": {
          id_plan: id_plan
        },
        // "success": function (res){
      ...
      "columns": [{
          ...
          'render': function(data, type, row) {
            ...
            return `<a href="https://trusmiverse.com/apps/bahan_mom_lutfie/${row['id']}" target="_blank" class="btn btn-sm btn-info"><i class="bi bi-box-arrow-up"></i></a>`;
          }
        },
        {
          'data': 'pic_name'
        },
        {
          ...
          'render': function(data, type, row) {
            ...
            if (data == "") {
              ...
              return `<a href="<?= base_url(); ?>uploads/mom_lutfie/${data}" class="text-success" target="_blank"><i class="bi bi-file-medical"></i></a>`;
            }
          },
          'className': 'text-center'
        },
  ...
  function proses_plan(id_plan) {
    ...
    // Get Data Plan
    $.ajax({
      url: "<?= base_url('mom_lutfie/get_data_plan/'); ?>" + id_plan,
      method: "GET",
      dataType: "JSON",
      success: function(res) {
        // console.log(res);
        ...
        formSubmit: {
          ...
          action: function() {
            $.confirm({
              ...
              onOpen: function() {
                $.ajax({
                  url: "<?= base_url('mom_lutfie/hapus_draft/'); ?>" + id_mom,
                  method: "GET",
                  dataType: "JSON",
                  success: function(res) {
                    console.log(res);
  ...
  function generate_head_resume_v3() {
    ...
    // let end = $('#end').val();
    $.ajax({
      url: '<?= base_url() ?>mom_lutfie/generate_head_resume_v3',
      type: 'POST',
      dataType: 'json',
      // data: {
      //     start: start,
  ...
  function get_pekerjaan(tipe = null, nilai = null) {
    ...
    // pekerjaan = "<option>-- Pilih Pekerjaan --</option>";
    $.ajax({
      url: "<?= base_url("mom_lutfie/get_pekerjaan") ?>",
      method: "POST",
      dataType: "JSON",
      data: {
      ...
      success: function(res) {
        ...
        if (tipe == 1) {
          res.pekerjaan.forEach(element => {
            newOptions.push({
              text: element['pekerjaan'],
              value: element['id']
            });
          });

          // Update SlimSelect with the new options
  ...
  function get_sub_pekerjaan(tipe = null, nilai = null) {
    ...
    $.ajax({
      url: "<?= base_url("mom_lutfie/get_sub_pekerjaan") ?>",
      method: "POST",
      dataType: "JSON",
      data: {
      ...
      success: function(res) {
        ...
        if (tipe == 1) {
          ...
          // Update SlimSelect with the new options
          sub_pekerjaan.setData(newOptions);
        }
      }
    })
  }
  ...
  function get_detail_pekerjaan(tipe = null, nilai = null) {
    ...
    $.ajax({
      url: "<?= base_url("mom_lutfie/get_detail_pekerjaan") ?>",
      method: "POST",
      dataType: "JSON",
      data: {
      ...
      success: function(res) {
        ...
        if (tipe == 1) {
          ...
          // Update SlimSelect with the new options
          detail_pekerjaan.setData(newOptions);
