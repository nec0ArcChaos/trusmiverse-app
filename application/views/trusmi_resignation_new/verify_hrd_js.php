...
<script>
    ...
    function get_profile_resignation() {
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                $("#head_name").html(response.head_name == null ? '<i class="small">*no data available</i>' : response.head_name);
    ...
    $('#btn_approve_hrd').click(function() {
        $("#modal_approve_hrd").modal("show");
    });

    function updateApprovalHrd() {
        id_resignation = "<?= $_GET['id'] ?? '' ?>";
        $.ajax({
            url: '<?= base_url() ?>trusmi_resignation/update_approval_hrd',
            type: 'POST',
            dataType: 'json',
            data: {
                id_resignation: id_resignation
            ...
            beforeSend: function() {
                $('#btn_save_confirm_hrd').attr('disabled', true);
                $("#btn_save_confirm_hrd").html("Please wait...");
            },
            success: function(response) {
            ...
            complete: function() {
                $('#btn_save_confirm_hrd').attr('disabled', false);
                $("#btn_save_confirm_hrd").html("Yes, Approve");
            },
        });
    }
    ...
    function check_status_resignation() {
        id_resignation = "<?= $_GET['id'] ?? '' ?>";
        $.ajax({
            url: '<?= base_url() ?>trusmi_resignation/check_status_resignation',
            type: 'POST',
            dataType: 'json',
            data: {
                id_resignation: id_resignation
            ...
            success: function(response) {
                console.log(response.status_resignation);
                if (response.status_resignation == true) {
                    $("#btn_print_paklaring").removeClass("d-none");
                    $("#btn_approve_hrd").addClass("d-none");
                } else {
                    $("#btn_print_paklaring").addClass("d-none");
                    $("#btn_approve_hrd").removeClass("d-none");
                }
            },
            error: function(xhr) { // if error occured
