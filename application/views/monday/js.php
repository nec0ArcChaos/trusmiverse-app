...
<script>
    ...
    $('#id_project').change(function(e) {
        ...
        $.ajax({
            type: "GET",
            url: "<?= base_url('monday/get_pekerjaan'); ?>/" + pekerjaan,
            dataType: "json",
    ...
    $('#id_pekerjaan').change(function(e) {
        ...
        $.ajax({
            type: "GET",
            url: "<?= base_url('monday/get_sub_pekerjaan'); ?>/" + pekerjaan,
            dataType: "json",
            success: function(response) {
    ...
    $('#id_sub_pekerjaan').change(function(e) {
        ...
        $.ajax({
            type: "GET",
            url: "<?= base_url('monday/get_det_pekerjaan'); ?>/" + pekerjaan,
            dataType: "json",
            success: function(response) {
                let list_sub = '';
                if (response != null) {
            ...
            "columns": [{
                ...
                },
                {
                    ...
                    "render": function(data, type, row) {
                        if (data != 'IBR Pro') {
                            return data + `<a href="https://trusmiverse.com/apps/pr/mom/${row.enc_mom}" target="_blank" class="badge bg-primary"><i class="bi bi-printer"></i></a>`;
                        }
                        return data
                    }
                },
                ...
                // },
                {
                    "data": "status",
                    "render": function(data, type, row, meta) {
                        let statusButton = `<a role="button" onclick="detail_task('${row['id_task']}')" class="btn btn-sm btn-link text-white" style="cursor:pointer;background-color:${row['status_color']}">${row['status']}</a>`;

                        let revisionButtons = "";
                        if (row['unverified_sub_tasks'] !== null && row['unverified_sub_tasks'] !== "") {
                            let subTasks = row['unverified_sub_tasks'].split(",");
                            revisionButtons = subTasks.map((id_sub_task,index) =>
                                `<a href="https://trusmiverse.com/apps/ibr_update?id=${id_sub_task}&u=<?= $_SESSION['user_id']; ?>" class="badge bg-danger btn-danger ml-1 text-white">Revisi Strategy ${index+1}</a>`
                            ).join(" ");
                        }

                        return statusButton + revisionButtons;
                    },
                    "className": "d-none d-md-table-cell text-left"
                },
    ...
    function list_verif() {
        ...
        $('#list_verif').DataTable({
            ...
            "ajax": {
                "dataType": 'json',
                "type": "GET",
                "url": `<?= base_url('monday'); ?>/list_verif`,
                "dataSrc": ""
            },
            "columns": [{
                    "data": "task"
                ...
                },
                {
                    "data": "status",
                    "render": function(data, type, row, meta) {
                        let statusButton = `<a role="button" class="btn btn-sm btn-link text-white" style="cursor:pointer;background-color:${row['status_color']}">${row['status']}</a>`;
                ...
                },
                {
                    ...
                    "render": function(data) {
                        ...
                        return `<div>
                  <form class="verif_task" data-id="${data}">
                    <input type="hidden" name="id_sub_task" value="${data}">
    ...
    $(document).on('submit', '.verif_task', function(event) {
        ...
        $.ajax({
            url: "<?= base_url('monday/save_verified'); ?>", // Ganti dengan URL untuk proses backend
            type: "POST",
            data: formData,
            success: function(response) {
                $.confirm({
                ...
                $('#list_verif').DataTable().ajax.reload();
            },
            error: function() {
    ...
    // $('.verif_task').submit(function(e) {
    //     e.preventDefault();
    //     let form = $(this);
    //     let id_sub_task = form.data("id"); // Ambil item ID dari data-id
    //     let formData = form.serialize(); // Ambil data form
