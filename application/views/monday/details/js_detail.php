            ...
            "columns": [{
                ...
                "render": function(data, type, row, meta) {
                    ...
                    if (row['id_status'] == 1) {
                        ...
                        if(row['verified_status'] == 2 || row['verified_pdca'] == 2 || row['verified_owner'] == 2){
                            text_button = 'Revisi';
                            class_btn_update = 'bg-danger';
                        }else{
                            text_button = "Update";
                        }
                        button_update_sub = `<a role="button" class="btn btn-sm btn-link ${class_btn_update} text-white" onclick="modal_update_sub_task('${row['id_sub_task']}','${row['id_task']}','${row['id_status']}')">${text_button}</a>`
                    }

                    // if(row['verified_status'] == 2 || row['verified_pdca'] == 2 || row['verified_owner'] == 2){
                    //     button_update_sub += `<a href="https://trusmiverse.com/apps/ibr_update?id=${row['id_sub_task']}&u=<?= $_SESSION['user_id']; ?>" role="button" class="btn btn-sm btn-link bg-danger text-white" >Revisi</a>`;
                    // }
                    ...
                    var verif = ``;
                    if (row['verified_status'] != null) {

                        verif = `
                            <small class="d-block">Status Verif : ${row['verified']}</small> <small>Note : ${row['verified_note']}</small>`;
                ...
                $.ajax({
                    ...
                    if (response.status == true) {
                        setTimeout(() => {
                            ...
                            if (response.detail.ekspektasi != '-') {
                                $('#div_ekspektasi').show();
                                $('#u_history_ekspektasi').val(response.detail.ekspektasi);
                            } else {
                                $('#div_ekspektasi').hide();
                            }
                ...
                onOpen: function() {
                    ...
                    $.ajax({
                        ...
                        if (response.save_sub_task == true) {
                            jconfirm.instances[0].close();
                            $('#modal_update_sub_task').modal('hide');
                            if(response.status_task == true){
                                $.confirm({
                                    icon: 'fa fa-check',
                                    title: 'Success!',
                                    theme: 'material',
                                    type: 'blue',
                                    content: 'Strategy And Goals updated!',
                                    buttons: {
                                        close: {
                                            actions: function() {}
                                        },
