      ...
      $('#dt_list_eaf').DataTable({
        ...
        "columns": [{
          ...
          "render": function(data, type, row) {
            return '<span class="btn btn-primary btn-sm detail_pengajuan" data-id_pengajuan="'+ data +'" style="cursor : pointer;">'+ data +'</span>'
          }
        },
        {
          "data": "company_kode"
          ...
          'render': function(data, type, row) {
            if (row['status'] == 1 || row['status'] == 2 || row['status'] == 6) {
              ...
              if ((row['temp'] != null || row['temp'] == '') && row['temp'].slice(0, 3) == "LPJ") {
                return `<span class="${row['warna']}">${data}</span>&nbsp<span class="badge bg-outline-primary">` + row['temp'] + `</span>`
              } else {
                return `<span class="${row['warna']}">${data}</span>`
              }
            }
        ...
        },
        {
          ...
          render: function(data, type, row) {
            if (row['stt_bawa'] == 1) {
              return `<span class="text-danger f-w-600"> ${data} </span> <a href="<?php echo base_url() ?>print_ba/data_print_ba/${row['id_pengajuan']}" target="_blank" class="btn btn-danger btn-sm" style="cursor : pointer;"><i class="icofont icofont-printer"></i></a>`
            } else {
              return data
            }
          }
