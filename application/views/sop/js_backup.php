    ...
    $('#table_sop').DataTable({
        ...
        "columns": [{
                ...
                "render": function(data, type, row, meta) {
                    return `<a class="btn btn-xs btn-warning sharp shadow" onclick="modal_review('${data}','${row['department']}')"><i class="fa fa-star"></i> </a>`;
...
$('#save_request').on('click',function(){
    validation_input('#select_company_req','company_req','select');
    ...
    var kompetensi = $('[name="kompetensi"]').val();
    var authority = $('[name="authority"]').val();
    // console.log(department+' '+ department_multi);
...
function validation_input(id, input, type){
    ...
    if(type == 'select'){
        if($('[name="'+input+'"]').val() == null || $('[name="'+input+'"]').val() == ''){
            ...
            alert(input +' - '+ $('[name="'+input+'"]').val());
            $(id > 'invalid').attr('hidden', false);
            $(id).focus();
        }
...
function view_internal() {
    ...
    //     // }],
    //     "ajax": {
    //         'url': '<?= base_url("sop_dev/get_internal/") ?>' + no_jp,
    //         'type': 'GET',
    //         'dataType': 'json',
    //     },
    //     "columns": [{
...
$('#pilih_dokumen').on('change', function() {
    ...
    if (pilih == 1) {
        $('.company, .designation, .nama_dokumen, .penjelasan, .draf, .type_dept, .row_dept, .single_department_req').slideDown();
        $('.jobprofile, .multi_department_req').hide();
    } else if (pilih == 2) {
        $('.company, .nama_dokumen, .penjelasan, .jobprofile, .row_dept, .single_department_req').slideDown();
        $('.designation, .draf, .type_dept').slideUp();
        // $('.single_department_req').show();
        $('.multi_department_req').hide();
    } else if (pilih == 3) {
        $('.company, .designation, .nama_dokumen, .penjelasan, .draf, .type_dept, .row_dept, .single_department_req').slideDown();
