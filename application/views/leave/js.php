...
<script>
    // let user_id_ses = "<?= $this->session->userdata('user_id'); ?>"
    // if (user_id_ses == 1) {
    ...
    function get_leave_type() {
        $.ajax({
            ...
            crb_cut_off = '<?= strtotime(date("Y-03-20")) ?>'
            ...
            for (let index = 0; index < response.leave_type.length; index++) {
                ...
                if (parseInt(response.leave_type[index].leave_type_id) == 1 && parseInt(response.leave_type[index].sisa_izin) < 0) {
                    remaining = `(${response.leave_type[index].sisa_izin} Remaining) (Melebihi Batas Ketentuan)`;
                    disable_select = "disabled";
                } else {
                    disable_select = "";
                }

                if (date_now > bali_cut_off && parseInt(response.leave_type[index].leave_type_id) == 19 && parseInt(company_id) == 3) {
                    console.log(response.leave_type[index].leave_type_id)
                } else if (date_now > crb_cut_off && parseInt(response.leave_type[index].leave_type_id) == 19 && parseInt(company_id) != 3) {
                    console.log(response.leave_type[index].leave_type_id)
                } else {
                    opt_leave_type += `<option value="${response.leave_type[index].leave_type_id}" data-warning="${response.leave_type[index].warning}" data-sisa_remaining="${response.leave_type[index].sisa_izin}" ${disable_select}>${response.leave_type[index].type_name} ${remaining}</option>`
                }
            }
    ...
    $('#end_date').on('change', function() {
        ...
        console.log($('#leave_type :selected').attr('data-sisa_remaining'));
    ...
    function add_leave() {
        leave_type_id_temp = $('#leave_type').val();
        if (leave_type_id_temp == 1) {
            sisa_cuti = $('#leave_type :selected').attr('data-sisa_remaining');
            ...
            if (diffDays > parseInt(sisa_cuti)) {
                $.confirm({
                    icon: 'fa fa-close',
                    title: 'Oops!',
                    theme: 'material',
                    type: 'red',
                    content: 'Pengajuan cuti lebih dari Sisa Cuti yang anda milik',
                    buttons: {
                        close: {
                            actions: function() {}
                        },
                    ...
                    action: function() {
                        ...
                        if (!end_date) {
                            $.confirm({
                                ...
                                theme: 'material',
                                type: 'red',
                                content: 'Anda belum memilih end date',
                                buttons: {
                                    close: {
                                        actions: function() {}
                                    },
    ...
    function get_applied_leave() {
        ...
        console.info("txz " + txz);
        console.info("spv " + spv);
            ...
            "columns": [
                ...
                },
                {
                    ...
                    "render": function(data, type, row, meta) {
                        // <p class="text-dinamis text-color-theme mb-0" style="font-size:9pt;">${row['applied_on']}</p>
                ...
                },
                {
                    ...
                    "render": function(data, type, row, meta) {
                        ...
                        if (row['approveable'] == 0 && row['editable'] == 0) {
                            return `
                                <div class="dropdown d-inline-block">
                                        <a class="text-secondary dd-arrow-none" aria-expanded="false" data-bs-display="static" role="button">
                                            <span class="badge badge-sm small ${row['bg_status']} text-white">${row['status']}</span>
                                        </a>
                                </div>`
                        } else {
                ...
                },
                {
                    ...
                    "render": function(data, type, row, meta) {
                        if (row['start_time'] != '' && row['end_time'] != '') {
                            return `${row['start_time']} s/d ${row['end_time']}`
                        } else {
                            return ''
                        }
                    },
