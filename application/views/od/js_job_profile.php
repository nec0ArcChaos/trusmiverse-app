...
<script type="text/javascript">
            ...
            $.ajax({
                ...
                success: function(data) {
                    ...
                    for (i = 0; i < data.length; i++) {
                        set_dat += '<option value="' + data[i].department_id + '">' + data[i]
                ...
                },
                {
                    ...
                    'render': function(data, type, row, meta) {
                        if (row['status_rv'] == null) {
                            ...
                            return `<a href="javascript:void(0);" onclick="modal_review('${data}','${row['departement_id']}')" class="btn btn-xs btn-warning sharp shadow">
                    <i class="fa fa-star"></i> </a> Waiting`;
                        } else {

                            return `<a href="javascript:void(0);" onclick="modal_review('${data}','${row['departement_id']}')" class="btn btn-xs btn-warning sharp shadow">
                    <i class="fa fa-star"></i> </a> <span class="badge badge-default">${row['status_rv']}</span>`;
                        }
    ...
    function send_notifikasi_review(no_jp, designation, no_doc, dept, no_hp, departement, data_pic, prepared) {
        ...
        link = '🔗 <?= 'https://trusmicorp.com/od/review/index/' ?>' + no_jp + '/' + data_pic[0];
        data_review = ``;
        data_review +=
