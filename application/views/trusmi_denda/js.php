...
<script type="text/javascript">
  ...
  $(document).ready(function() {
    ...
    $("#karyawan").change(function() {
      ...
      nominal = $('#karyawan option:selected').data('nominal');

      $("#department_name").val(department_name);
      $("#designation_name").val(designation_name);
      var nomStr = (nominal !== undefined && nominal !== null) ? nominal.toString() : '';
      $("#nominal_denda").val(nomStr ? formatRupiah(nomStr) : '');


    })
    $('#tipe').change(function (e) { 
      e.preventDefault();
      if($(this).val() == 'Konsekuensi Meeting') {
        $('#nominal_denda').attr('readonly', false);
        
      } else {
        $('#nominal_denda').attr('readonly', true);
        // $('#nominal_denda').val('');
      }
    });
    ...
    $("#btn_save").click(function(){
      if ($("#tipe :selected").val() == "#") {
        ...
        $.ajax({
          method: "POST",
          url: "<?= base_url("trusmi_denda/save_denda") ?>",
          dataType: "JSON",
          processData: false,
          contentType: false,
          data: formData,
        ...
        "columns": [
          {
            ...
            'render': function (data, type, row){
              if ([1, 778, 979].includes(parseInt(user_id))) {
                return `<span>${data}</span> <button type="button" class="btn btn-link btn-sm" onclick="delete_denda(${data})"><i class="bi bi-trash"></i></button>`;
              } else {
  ...
  function delete_denda(id) {
      swal({
        ...
        if (willDelete) {
          $.ajax({
            url: "<?= base_url('trusmi_denda/delete_denda') ?>",
            method: "POST",
            data: {
              id: id
            },
  ...
  function open_rekomendasi(data) {
    ...
    $('#nominal_denda_rekom').val(data.denda);
  ...
  function save_denda_rekom() {
    ...
    $('#status_denda_rekom').val() == '1' && ($("#alasan_rekom").val().trim() === "" || $("#alasan_rekom").val().trim().split(/\s+/).length < 3)) {
      swal("Warning","Please enter your reason at least 3 words.","error");
    } else {
