...
<script type="text/javascript">
  ...
  function handle_goals_change(id) {
    var goals_value = $('#val_' + id).val();

    var input_parts = id.split('_');
    var issue_num = input_parts[1];
    var item_num = input_parts[2];

    var topik_textarea_id = 'val_topik_' + issue_num + '_' + item_num;
    var topik_textarea = $('#' + topik_textarea_id);

    if (goals_value == '0') {
      topik_textarea.removeClass('d-none').focus();

      // Pasang event focusout sekali saja
      if (!topik_textarea.data('blur-bound')) {
        topik_textarea.on('focusout', function() {
          submit_update('topik_' + issue_num + '_' + item_num);
        });
        topik_textarea.data('blur-bound', true);
      }
    } else {
      topik_textarea.addClass('d-none');
    }

    // Tetap submit goals meskipun value = 0
    submit_update(id);
  }

  function handle_goals_draft_change(id) {
    var goals_value = $('#val_' + id).val();
    var input_parts = id.split('_');
    var issue_num = input_parts[2];
    var item_num = input_parts[3];

    var topik_textarea_id = 'val_topik_draft_' + issue_num + '_' + item_num;
    var topik_textarea = $('#' + topik_textarea_id);

    if (goals_value == '0') {
      topik_textarea.removeClass('d-none').focus();

      // Pasang event focusout untuk submit topik
      if (!topik_textarea.data('blur-bound')) {
        topik_textarea.on('focusout', function() {
          submit_update_draft('topik_draft_' + issue_num + '_' + item_num);
        });
        topik_textarea.data('blur-bound', true);
      }
    } else {
  ...
  // Start Tab Result  edited by khael
  function submit_update(id) {
    ...
    } else if (input[0] == "kategori" || input[0] == "pic" || input[0] == "grdsi" || input[0] == "goals") {
      inp_value = $('#val_' + id).val();
    } else {
      if (input[0] == 'deadline' && $.inArray(parseInt($('#val_kategori_' + input[1] + '_' + input[2]).val()), kat_deadline) != -1) {
    ...
    } else if (input[0] == "goals") {
      // Jika goals value = 0, ambil juga value topik
      var topik_value = null;
      if (inp_value == '0') {
        var topik_textarea_id = 'val_topik_' + input[1] + '_' + input[2];
        topik_value = $('#' + topik_textarea_id).val();
        // console.log('Goals value = 0, sending topik value:', topik_value);
      } else {
        // Ambil text dari option yang dipilih (goal name)
        var select_id = 'val_goals_' + input[1] + '_' + input[2];
        topik_value = $('#' + select_id + ' option:selected').text();
        // console.log('Goals value != 0, sending topik value from selected option:', topik_value);
      }

      $.ajax({
          url: '<?php echo base_url() ?>mom/save_goals',
          type: 'POST',
          dataType: 'json',
          data: {
            id_mom: id_mom,
        ...
        .always(function() {
          console.log("Goals save complete");
        });
    } else {
      $.ajax({
          url: '<?php echo base_url() ?>mom/save_issue_item',
  ...
  function add_issue() {
    ...
    <select class="form-control goals" id="val_goals_${next_no}_1" onchange="handle_goals_change('goals_${next_no}_1')" onclick="event.stopPropagation();" onfocus="event.stopPropagation();" style="margin-bottom: 5px;">
						<option value="">- Choose Goals -</option>
            <?php foreach ($goals as $goal) : ?>
      <option value="<?php echo $goal->id_goal ?>"><?php echo $goal->goal ?></option>
    <?php endforeach ?>
    </select>
    <span id="topik_${next_no}_1">&nbsp;</span>
            <textarea class="form-control excel" id="val_topik_${next_no}_1" rows="1" 
                onfocusin="expandTextarea('val_topik_${next_no}_1')"></textarea>
    </td>
  ...
  function submit_update_draft(id) {
    ...
    var id_mom = $('#id_mom_global_draft').val();
    ...
    } // Cek jika yang diubah adalah goals
    else if (input[0] === "goals") {
      var goals_value = $('#val_' + id).val();
      var topik_value = null;

      if (goals_value == '0') {
        var topik_textarea_id = 'val_topik_draft_' + input[2] + '_' + input[3];
        topik_value = $('#' + topik_textarea_id).val();
      } else {
        var select_id = 'val_goals_draft_' + input[2] + '_' + input[3];
        topik_value = $('#' + select_id + ' option:selected').text();
      }
  ...
  function add_issue_draft() {
    ...
    <select class="form-control goals_draft" id="val_goals_draft_${next_no}_1" onchange="handle_goals_draft_change('goals_draft_${next_no}_1')">
    ...
    goals_draft = new SlimSelect({
      select: "#val_goals_draft_" + next_no + '_1'
    });
          ...
          },
          {
            ...
            'render': function(data, type, row) {
              if (data == 0) {
                ...
                return `<span class="btn btn-sm btn-outline-theme" onclick="detail_rekap('${row['user_id']}','w6')">${data}</span>`;
              }
            },
            'className': 'text-center'
          },
          ...
          },
          {
            ...
            'render': function(data, type, row) {
              if (data == 0) {
                ...
                return `<span class="btn btn-sm btn-outline-theme" onclick="detail_rekap('${row['user_id']}','12')">${data}</span>`;
              }
            },
            'className': 'text-center'
          },
          ...
          },
          {
            ...
            'render': function(data, type, row) {
              if (data == 0) {
                ...
                return `<span class="btn btn-sm btn-outline-danger" onclick="detail_rekap('${row['user_id']}','90')">${data}</span>`;
              }
            },
            'className': 'text-center'
          },
  ...
  function detail_rekap(user_id, tipe) {
    ...
    tipe_deadline = ['1', '5', '6', '8', '9', '10', '30', '40', '50', '60', '70', '80', '90', 'w1', 'w2', 'w3', 'w4', 'w5'];
          ...
          .done(function(data) {
            ...
            if (data.result > 0) {
              ...
              for (let i = 0; i < data.result; i++) {
                let suffix = (i + 1) + "_1"; // contoh format
                new SlimSelect({
                  select: "#val_goals_draft_" + suffix
                });
              }
            } else {
              pic_draft = new SlimSelect({
  ...
  function list_verif(tipe) {
    ...
    var title = $('#modal_title_list_verif'); // Ini boleh tetap pakai jQuery
    let url, form;
    ...
    console.log(url, form, 'test');


    var filter = $('input[name="filter_verif"]:checked').val();
    $('#input_verif_form').val(form);
    $('#input_verif_url').val(url);
    $('#input_verif_tipe').val(tipe);

    filter_data_verif(filter);
  }


  function list_meeting_owner() {
    $('#modal_list_meeting_owner').modal('show');
    filter_data_meeting_owner();

  }
  $('input[name="filter_verif"]').on('change', function() {
    var selectedValue = $(this).val();
    filter_data_verif(selectedValue);
  });

  function filter_data_verif(filter) {
    console.log(filter);

    var form = $('#input_verif_form').val();
    var url = $('#input_verif_url').val();
    var tipe = $('#input_verif_tipe').val();
    var tabel_mom = $('#dt_list_verif').DataTable({
      ...
      "columns": [{
          'data': 'id_mom',
          'render': function(data, type, row) {
            return `${data} <br><a href="https://trusmiverse.com/apps/pr/mom/${row.id_mom_en}" target="_blank"><button class="btn btn-sm btn-primary"><i class="bi bi-printer"></i></button></a>`;
          }
        },
        {
          'data': 'topik',
              ...
              if (form == 'verif-form-owner') {
                return `<div id="form_verif_owner_${row.id_item_issue}">
                  <form class="${form}" data-id="${row.id_item_issue}">
                    ...
                    <select class="form-control form-control-sm" id="verified_status_${row.id_sub_task}" name="verified_status" onchange="verified_status_owner('${row.id_sub_task}', '${form}')" style="min-width:200px" required>
                      <option value="" selected disabled>- Status -</option>
                      <option value="1">Oke Result</option>
                      <option value="3">Oke Meeting / Diskusi</option>
                      <option value="2">Not Oke</option>
                    </select>
                    <textarea class="form-control form-control-sm" id="verified_note_${row.id_sub_task}" name="verified_note" rows="3" placeholder="Note Here.." required></textarea>
                    <input class="form-control form-control-sm" id="verified_tanggal_${row.id_sub_task}" name="verified_tanggal" type="date" style="display : none;">
  ...
  function filter_data_meeting_owner() {
    var form = $('#input_verif_form').val();
    var meeting_start = $("#start").val();
    var meeting_end = $("#end").val();
    var tabel_mom = $('#dt_list_meeting_owner').DataTable({
      "lengthChange": false,
      "searching": true,
      "info": true,
      "paging": true,
      ...
      "ajax": {
        "dataType": 'json',
        "type": "POST",
        "url": "<?= base_url("mom/list_meeting_owner") ?>",
        "data": {
          start: meeting_start,
          end: meeting_end,
        },
          ...
          'render': function(data, type, row) {
            if (row.id_owner_verified_status == null) {
              ...
              <form class="${form}" data-id="${row.id_item_issue}">
                ...
                <input type="hidden" name="id_task" value="${row.id_task}">
                <input type="hidden" name="id_sub_task" value="${row.id_sub_task}">
                <select class="form-control" id="verified_status_${row.id_sub_task}" name="verified_status" onchange="verified_status_owner('${row.id_sub_task}')" style="min-width:200px" required>
                  <option value="" selected disabled>- Status -</option>
                  <option value="1">Oke Result</option>
                  <option value="3">Oke Meeting / Diskusi</option>
                  <option value="2">Not Oke</option>
                </select>
                <textarea class="form-control" id="verified_note_${row.id_sub_task}" name="verified_note" rows="3" placeholder="Note Here.." required></textarea>
                <input class="form-control" id="verified_tanggal_${row.id_sub_task}" name="verified_tanggal" type="date" style="display : none;">
  ...
  function verified_status_owner(id, tipe) {
    idk = $('#verified_status_' + id).val();

    if (tipe == 'verif-form-owner') {
      if (idk == 3) {
        $('#verified_tanggal_' + id).show();
        $('#verified_tanggal_' + id).attr('required', true);
        $('#verified_note_' + id).hide();
        $('#verified_note_' + id).removeAttr('required');
      } else {
        $('#verified_tanggal_' + id).hide();
        $('#verified_tanggal_' + id).removeAttr('required');
        $('#verified_note_' + id).show();
        $('#verified_note_' + id).attr('required', true);
      }
    } else {
      if (idk == 2) {
        $('#verified_tanggal_' + id).show();
  ...
  function list_eskalasi() {
    ...
    var tabel_mom = $('#dt_list_eskalasi').DataTable({
      ...
      "ajax": {
        "dataType": 'json',
        "type": "POST",
        "url": "<?= base_url("mom/list_eskalasi") ?>",
        "dataSrc": ""
  ...
  $('[id^="verified_status_"]').on('change', function() {
    ...
    const idSubTask = selectId.replace('verified_status_', '');
    if (selectedValue == 2) {
      $('#verified_tanggal_' + idSubTask).show();
      $('#label_tanggal_' + idSubTask).show();
      $('#verified_tanggal_' + idSubTask).attr('required', true);
    } else {
      $('#verified_tanggal_' + idSubTask).hide();
      $('#label_tanggal_' + idSubTask).hide();
      $('#verified_tanggal_' + idSubTask).removeAttr('required');
    }
  ...
  function generate_head_resume_v3() {
    $.ajax({
      url: '<?= base_url() ?>mom/generate_head_resume_v3',
      type: 'POST',
      dataType: 'json',
      beforeSend: function() {
      ...
      // Generate Body - Dynamic approach
      tbody = '';
      for (let index = 0; index < response.body_resume.length; index++) {
        tbody_td = '';

        // Dynamic generation based on actual data length
        for (let weekIndex = 1; weekIndex <= response.data.length; weekIndex++) {
          const weekProperty = `w${weekIndex}`;
          const weekValue = response.body_resume[index][weekProperty];
          tbody_td += `<td>${weekValue > 0 ? '<span class="bi bi-check-lg text-green"></span> Yes' : '<span class="bi bi-x text-red"></span> No'}</td>`;
        }
      ...
      setTimeout(() => {
        ...
        if ($.fn.DataTable.isDataTable('#dt_resume_pembelajar_3')) {
          $('#dt_resume_pembelajar_3').DataTable().destroy();
        }
