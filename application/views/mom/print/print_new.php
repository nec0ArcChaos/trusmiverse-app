...
<head>
  ...
  <style>
    <?= $this->load->view('mom/print/styles.css', '', TRUE); ?>textarea.excel {
      display: block;
...
<body>
  ...
  <main>
        ...
        <?php foreach ($issue as $key) : ?>
          <table width="100%" class="table table-striped" style="margin-top: 8px;">
            ...
            <tr style="background-color: #337ab7; color: #fff;">
              ...
              <td width="13%"><span class="text-white fw-bold">HASIL / TOPIK</span></td>
              <td width="10%"><span class="text-white fw-bold">ISSUE</span></td>
              <td width="1%" align="center"><span class="text-white fw-bold"></span></td>
              <td width="15%"><span class="text-white fw-bold">ACTION</span></td>
              <td width="8%" align="center"><span class="text-white fw-bold">KATEGORI</span></td>
              <td width="7%" align="center"><span class="text-white fw-bold">DEADLINE</span></td>
              <td width="11%" align="center"><span class="text-white fw-bold">PIC</span></td>
              <td width="8%" align="center"><span class="text-white fw-bold">STATUS</span></td>
              <td width="12%" align="center"><span class="text-white fw-bold">EVALUATION</span></td>
              <td width="<?php echo $ekspektasiWidth; ?>" align="center"><span class="text-white fw-bold">EKSPEKTASI</span></td>
              <?php if ($header['meeting'] == 'MEETING OWNER') : ?>
                <td width="12%" align="center" nowrap><span class="text-white fw-bold">VERIFY OWNER</span></td>
              <?php endif; ?>
            </tr>
              ...
              <tr>
                ...
                <?php if ($item->issue !== $current_issue) : ?>
                  <td rowspan="<?= $rowspan_issue[$item->issue]; ?>">
                    ...
                    <a href="<?= base_url('ai_problem_solving/index/' . $header['id_mom'] . '/' . $key->id_issue); ?>" class="btn btn-sm btn-primary" target="_blank"><i class="bi bi-robot"></i> AI 5 Why Analysis</a>
                    <br>
                    <?php if (!empty($key->status_solving) && $key->status_solving == 'solved') : ?>
                      <span class="badge bg-success" style="cursor: pointer;" onclick="show_solved_by_ai('<?= $key->no_ps ?>')"><i class="bi bi-check2"></i> Solved by AI</span>
                    <?php endif; ?>
                  </td>
                ...
                <td align="center">
                  ...
                  <span class="info-data">
                    ...
                    <?php elseif (in_array($item->id_kategori, [1, 5, 6, 8, 9, 10, 11])) : ?>
                      ...
                      <?php if ($item->id_status_verified == 1) : ?>
                        <span class="badge rounded-pill bg-success" style="text-transform: none;"><i class="bi bi-check"></i> Verified by : <?= $item->verified_by ?></span>
                        <?php if ($item->id_owner_verified_status == 1 || $item->id_owner_verified_status == 3) : ?>
                          <span class="badge rounded-pill bg-primary" style="text-transform: none;"><i class="bi bi-check-all"></i> Verified Owner</span>
                        <?php endif; ?>
                      <?php endif; ?>
                ...
                <td align="center">
                  ...
                  <div id="form_verif_<?= $item->id ?>" style="display: none;">
                    <form class="verif-form" data-id="<?= $item->id ?>">
                      ...
                      <select class="form-control form-control-sm mb-1" name="verified_status" id="verified_status_<?= $item->id_sub_task ?>" required>
                        <option value="" selected disabled>- Pilih Status -</option>
                        <option value="1">Oke</option>
                        <option value="3">Oke Meeting</option>
                        <option value="2">Not Oke</option>
                      </select>
                      <small id="label_tanggal_<?= $item->id_sub_task ?>" style="display: none;">Deadline Revisi : </small>
                      <input class="form-control mb-1 form-control-sm" min="<?= date('Y-m-d') ?>" id="pdca_meeting_<?= $item->id_sub_task ?>" value="<?= date('Y-m-d') ?>" name="pdca_meeting" type="date" style="display : none;">
                      <input class="form-control mb-1 form-control-sm" min="<?= date('Y-m-d') ?>" id="verified_tanggal_<?= $item->id_sub_task ?>" value="<?= date('Y-m-d') ?>" name="verified_tanggal" type="date" style="display : none;">

                      <textarea class="form-control form-control-sm " id="verified_note_<?= $item->id_sub_task ?>" name="verified_note" rows="3" placeholder="Note Here.." required></textarea>
                  ...
                  <?php endif; ?>
                  <?php if ($item->id_status_verified == 2) : ?>
                    <br><small>Deadline Revisi : <br><?= $item->deadline ?></small>
                  <?php endif; ?>
                  <?php if ($item->id_status_verified == 3) : ?>
                    <br><small>Tgl Meeting : <br><?= $item->pdca_meeting ?></small>
                  <?php endif; ?>
                </td>
                <?php if ($header['meeting'] == 'MEETING OWNER') : ?>
                  <td>
                    <?php if ($item->owner_verified_status == NULL && $item->id_status_verified == 1) : ?>
                      <small>Waiting Verify</small>
                      <?php
                      $user_allow = [8204, 803, 1];
                      ?>
                      <?php if (in_array($this->session->userdata('user_id'), $user_allow)) : ?>
                        <div id="form_verif_owner_<?= $item->id ?>">
                          <form class="verif-form-owner" data-id="<?= $item->id ?>">
                            <input type="hidden" name="id_item_issue" value="<?= $item->id ?>">
                            <input type="hidden" name="id_task" value="<?= $item->id_task ?>">
                            <input type="hidden" name="id_sub_task" value="<?= $item->id_sub_task ?>">
                            <select class="form-control form-control-sm" name="verified_status" id="owner_verified_status_<?= $item->id_sub_task ?>" style="margin-bottom: 3px;" required>
                              <option value="" selected disabled>- Status -</option>
                              <option value="1">Oke Result</option>
                              <option value="3">Oke Meeting</option>
                              <option value="2">Not Oke</option>
                            </select>
                            <input class="form-control mb-1 form-control-sm" min="<?= date('Y-m-d') ?>" id="owner_verified_tanggal_<?= $item->id_sub_task ?>" value="<?= date('Y-m-d') ?>" name="verified_tanggal" type="date" style="display : none;">
                            <textarea class="form-control form-control-sm" name="verified_note" id="owner_verified_note_<?= $item->id_sub_task ?>" rows="3" placeholder="Note Here.." required></textarea>
                            <button class="label label-primary" id="save_<?= $item->id ?>" onclick="save('<?= $item->id ?>')">Save</button>
                          </form>
                        </div>
                      <?php endif; ?>
    ...
    <!-- Modal Add -->
    <div class="modal fade" id="modal_solved" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1" aria-labelledby="staticBackdropLabel" data-bs-focus="false" aria-hidden="true">
      <div class="modal-dialog modal-fullscreen modal-dialog-scrollable">
        <div class="modal-content">
          <div class="modal-header">
            <h1 class="modal-title fs-5" id="exampleModalLabel">Result from AI 5 Why Analys</h1>
            <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
          </div>
          <div class="modal-body">
            <div class="problem-solving-steps">
              <h2 class="text-center">7 Steps of Problem Solving</h2>
              <ol class="step-list">
                <li class="step-item">
                  <h5>Step 1 - Identify the Problem</h5>
                  <p id="step1_why_user">User Question: /p>
                </li>
                <li class="step-item">
                  <h5>Step 2</h5>
                  <p id="step1_why_ai">AI Answer: </p>
                  <p id="step2_why_user">User Question: </p>
                </li>
                <li class="step-item">
                  <h5>Step 3</h5>
                  <p id="step2_why_ai">AI Answer: </p>
                  <p id="step3_why_user">User Question: </p>
                </li>
                <li class="step-item">
                  <h5>Step 4</h5>
                  <p id="step3_why_ai">AI Answer: </p>
                  <p id="step4_why_user">User Question: </p>
                </li>
                <li class="step-item">
                  <h5>Step 5</h5>
                  <p id="step4_why_ai">AI Answer: </p>
                  <p id="step5_why_user">User Question: </p>
                </li>
                <li class="step-item">
                  <h5>Step 6</h5>
                  <p id="step5_why_ai">AI Answer: </p>
                  <p id="step6_why_user">User Question: </p>
                </li>
                <li class="step-item">
                  <h5>Step 7 - Solving</h5>
                  <p id="step6_why_ai">AI Answer: </p>
                  <p id="step7_why_user">User Question: </p>
                </li>
              </ol>
            </div>
          ...
          <div class="modal-footer">
            <button type="button" class="btn btn-md btn-danger" data-bs-dismiss="modal" aria-label="Close">Close</button>
          </div>
        </div>
      </div>
    </div>
  ...
  <script type="text/javascript">
    ...
    $(".verif-form-owner").submit(function(event) {
      event.preventDefault(); // Mencegah reload halaman
    ...
    $('[id^="verified_status_"]').on('change', function() {
      const selectedValue = $(this).val();
      const idSubTask = this.id.replace('verified_status_', '');

      const $tanggal = $('#verified_tanggal_' + idSubTask);
      const $meeting = $('#pdca_meeting_' + idSubTask);
      const $label = $('#label_tanggal_' + idSubTask);

      const toggleShow = ($el, show) => $el.toggle(!!show);
      const toggleRequired = ($el, required) => $el.prop('required', !!required);

      if (selectedValue == 2) {
        toggleShow($tanggal, true);
        toggleShow($meeting, false);
        toggleShow($label, true);
        $label.text('Deadline Revisi');

        toggleRequired($tanggal, true);
        toggleRequired($meeting, false);
      } else if (selectedValue == 3) {
        toggleShow($tanggal, false);
        toggleShow($meeting, true);
        toggleShow($label, true);
        $label.text('Tgl Meeting');

        toggleRequired($tanggal, false);
        toggleRequired($meeting, true);
      } else {
        toggleShow($tanggal, false);
        toggleShow($label, false);
    ...
    $('[id^="owner_verified_status_"]').on('change', function() {
      const selectedValue = $(this).val();
      const idSubTask = this.id.replace('owner_verified_status_', '');

      const $tanggal = $('#owner_verified_tanggal_' + idSubTask);
      const $note = $('#owner_verified_note_' + idSubTask);

      const toggle = (el, show, required) => {
        el.toggle(!!show).prop('required', !!required);
      };

      if (selectedValue == 3) {
        toggle($tanggal, true, true);
        toggle($note, false, false);
      } else {
        toggle($tanggal, false, false);
        toggle($note, true, true);
      }
    });
    ...
    function add_action(issue_no) {
      ...
      <td class="kolom_modif" id="td_deadline_${issue_no}_${next_no}" data-id="deadline_${issue_no}_${next_no}">
        <input type="text" class="form-control tanggal" id="val_deadline_${issue_no}_${next_no}" name="val_deadline_${issue_no}_${next_no}" value="${deadline}">
      </td>
    ...
    <tr id="div_issue_${next_no}">
      ...
      <td class="kolom_modif" id="td_deadline_${next_no}_1" data-id="deadline_${next_no}_1">
        <input type="text" class="form-control tanggal" id="val_deadline_${next_no}_1" name="val_deadline_${next_no}_1" value="${deadline}">
      </td>
    ...
    function finish() {
      ...
      $.confirm({
        ...
        onOpen: function() {
          // Start
          $.ajax({
            url: "<?= base_url('mom/save_eskalasi'); ?>",
            type: "POST",
    ...
    function show_solved_by_ai(no_ps) {
      $.ajax({
        url: "<?php echo site_url('ai_problem_solving/get_result'); ?>",
        type: "post",
        data: {
          "no_ps": no_ps
        },
        ...
        success: function(res) {
          ...
          dt = res.data;
          $('#modal_solved').modal('show');
          $("#step1_why_user").empty().html('<b>User</b> <i class="bi bi-people"></i> ' + dt.problem);
          $("#step1_why_ai").empty().html('<b>AI</b> <i class="bi bi-robot"></i> ' + dt.rekom_why_1.replace(/\n/g, '<br>'));
          $("#step2_why_user").empty().html('<b>User</b> <i class="bi bi-people"></i> ' + dt.why_1);
          $("#step2_why_ai").empty().html('<b>AI</b> <i class="bi bi-robot"></i> ' + dt.rekom_why_2.replace(/\n/g, '<br>'));
          $("#step3_why_user").empty().html('<b>User</b> <i class="bi bi-people"></i> ' + dt.why_2);
          $("#step3_why_ai").empty().html('<b>AI</b> <i class="bi bi-robot"></i> ' + dt.rekom_why_3.replace(/\n/g, '<br>'));
          $("#step4_why_user").empty().html('<b>User</b> <i class="bi bi-people"></i> ' + dt.why_3);
          $("#step4_why_ai").empty().html('<b>AI</b> <i class="bi bi-robot"></i> ' + dt.rekom_why_4.replace(/\n/g, '<br>'));
          $("#step5_why_user").empty().html('<b>User</b> <i class="bi bi-people"></i> ' + dt.why_4);
          $("#step5_why_ai").empty().html('<b>AI</b> <i class="bi bi-robot"></i> ' + dt.rekom_why_5.replace(/\n/g, '<br>'));
          $("#step6_why_user").empty().html('<b>User</b> <i class="bi bi-people"></i> ' + dt.why_5);
          $("#step6_why_ai").empty().html('<b>AI</b> <i class="bi bi-robot"></i> ' + dt.rekom_solving.replace(/\n/g, '<br>'));
          $("#step7_why_user").empty().html('<b>User</b> <i class="bi bi-people"></i> ' + dt.solving);
        },
        error: function(jqXHR, textStatus, errorThrown) {
          $.confirm({
            icon: 'fa fa-exclamation-triangle',
