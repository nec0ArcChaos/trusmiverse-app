...
<script type="text/javascript">
    ...
    $('#dt_mom_result_e').DataTable({
        ...
        "columns": [
          ...
          },
          {
            ...
            'render': function(data,type,row){
              if (row['id_kategori'] == "") {
              return `<select class="form-control" id="val_kategori_1_1" onchange="submit_update('kategori_1_1')">
                        <option>- Choose -</option>
                        <?php foreach ($kategori as $ktg): ?>
          ...
          },
          {
            ...
            'render': function(data,type,row,meta){
              no = meta.row + 1;
              return `<input type="text" class="form-control ${(row['id_kategori'] == 1) ? 'tanggal' : '' }" id="e_deadline_${no}" name="e_deadline_${no}" value="${data}" onfocusout="update_deadline('${row['id_mom']}','${row['id_issue']}','${row['id_issue_item']}','${row['id_task']}','${row['id_sub_task']}','e_deadline_${no}')">`;
            }
          },
          {
            'data': 'list_pic',
  ...
  function update_deadline(id,issue,item,id_task,id_sub_task,val)
  {    
    deadline = $(`#${val}`).val();
    ...
    $.ajax({
      ...
      "success": function (res){
        ...
        new PNotify({
            title: `Success`,
            text: `Deadline has been updated`,
    ...
    if(id_mom == ""){
      $.ajax({
        ...
        success: function (res){
          ...
          swal("Info!!", "Data has been saved Draft!", "info");
          return true;              
        },
        error: function (jqXHR, textStatus, errorThrown){
          console.log(jqXHR.responseText);
