...
<script type="text/javascript">
  ...
  $(document).ready(function() {
    ...
    $("#smartwizard2").on("leaveStep", function(e, anchorObject, currentStepIndex, nextStepIndex, stepDirection) {
      ...
      if (nextStepIndex == 2 && stepDirection == "forward") {
        ...
        $("#btn_finish").show();
        $.ajax({                    
          "url": "<?= base_url('mom_dev/check_validasi_result/') ?>"+$("#id_mom_global").val(),
          "type": "GET",
          "dataType": 'JSON',
          "success": function (res){
            // console.log(res);    
    ...
    $('#dt_mom_result').on('click', '.kolom_modif', function() {
      ...
      // $.inArray( $('#val_kategori_'+input[1]+'_'+input[2]).val(), kat_deadline ) != -1
      // if (input[0] == 'deadline' && $('#val_kategori_'+input[1]+'_'+input[2]).val() == 1) {
      if (input[0] == 'deadline' && $.inArray( $('#val_kategori_'+input[1]+'_'+input[2]).val(), kat_deadline ) != -1) {
    ...
    $('#dt_mom_result_draft').on('click', '.kolom_modif', function() {
      ...
      if (input[0] == 'deadline' && $.inArray( $('#val_kategori_draft_'+input[2]+'_'+input[3]).val(), kat_deadline ) != -1) {
        $('#val_date_'+id).show().focus();
        $('#val_'+id).hide();
      } else {
        $('#val_'+id).show().focus();
    ...
    $("#smartwizard3").on("leaveStep", function(e, anchorObject, currentStepIndex, nextStepIndex, stepDirection) {
      ...
      if (nextStepIndex == 1 && stepDirection == "forward") {        
        if ($("#judul_draft").val() == "") {
          ...
          $.ajax({
            method: "POST",
            url: "<?= base_url('mom_dev/save_mom_draft') ?>",
            dataType: "JSON",
      ...
      if (nextStepIndex == 2 && stepDirection == "forward") {
        $("#btn_finish_e").show();
        $.ajax({                    
          "url": "<?= base_url('mom_dev/check_validasi_result/') ?>"+$("#id_mom_global_draft").val(),
          "type": "GET",
          "dataType": 'JSON',
          "success": function (res){
            // console.log(res);    
  ...
  function add_mom()
  {
    ...
    $.ajax({
      url: "<?= base_url('mom_dev/clear_result'); ?>",
      method: "GET",
      dataType: "JSON",
      success: function(res){
    ...
    var tabel_mom =
      $('#dt_list_mom').DataTable({
        ...
        "ajax": {
          "dataType": 'json',
          "type": "POST",
          "url": "<?= base_url('mom_dev/get_list_mom') ?>",
          "data": {
            datestart: start,
            dateend: end,
            closed: 1 
        ...
        "columns": [
          {
            ...
            'render': function(data,type,row) {
              // if (<?= $_SESSION['user_id']; ?> == 1 ) {
                return `<a href="<?= base_url('pr/mom_dev/'); ?>${row['id_link']}" target="_blank" class="btn btn-sm btn-primary mb-1"><i class="bi bi-printer"></i></a>
                <a href="<?= base_url('mom_dev/excel/'); ?>${row['id_link']}" target="_blank" class="btn btn-sm btn-success mb-1"><i class="bi bi-filetype-xls"></i></a>
                <button type="button" class="btn btn-sm btn-info mb-1" onclick="reminder_whatsapp('${data}')"><i class="bi bi-whatsapp"></i></button>`;
              // } else {
              //   return `<a href="<?= base_url('_dev/'); ?>${row['id_link']}" target="_blank" class="btn btn-sm btn-primary mb-1"><i class="bi bi-printer"></i></a>
              //   <button type="button" class="btn btn-sm btn-info mb-1" onclick="reminder_whatsapp('${data}')"><i class="bi bi-whatsapp"></i></button>`;
              // }
            },
  ...
  function cancel()
  {
    ...
    if (id_mom != "") {
      $.ajax({
        method: "GET",
        url: "<?= base_url('mom_dev/cancel_mom/') ?>"+id_mom,
        dataType: "JSON",
        beforeSend: function (res){
  ...
  function finish(closed)
  {
    ...
    if (closing == '' || closing == '<p><br></p>' || closing == '<br>') {
      ...
      $.ajax({
        url: "<?= base_url('mom_dev/save_closing') ?>",
        method: "POST",
        dataType: "JSON",
        data: {
          id_mom: id_mom,
  ...
  function submit_update(id) {    
    ...
    if (input[0] == "topik") {
      $.ajax({
        url: '<?php echo base_url() ?>mom_dev/save_topik',
        type: 'POST',
        dataType: 'json',
        data: {
          id_mom: id_mom,
      ...
      $.ajax({
        url: '<?php echo base_url() ?>mom_dev/save_issue',
        type: 'POST',
        dataType: 'json',
        data: {
          id_mom: id_mom,
      ...
      $.ajax({
        url: '<?php echo base_url() ?>mom_dev/save_issue_item',
        type: 'POST',
        dataType: 'json',
        data: {
          id_mom: id_mom,
  ...
  function remove_action(issue_no) {
    ...
    if (no > 1) {
      ...
      $.ajax({
        url: '<?php echo base_url() ?>mom_dev/delete_issue_item',
        type: 'POST',
        dataType: 'JSON',
        data: {
          id_mom: id_mom,
  ...
  function remove_issue() {
    ...
    if (no > 1) {
      ...
      $.ajax({
        url: '<?php echo base_url() ?>mom_dev/delete_issue',
        type: 'POST',
        dataType: 'JSON',
        data: {
          id_mom: id_mom,
    ...
    var tabel_rekap =
      $('#dt_list_rekap').DataTable({
        ...
        "ajax": {
          ...
          "url": "<?= base_url('mom_dev/get_list_rekap') ?>"
          // "success": function (res){
          //   console.log(res);
          // },
          // "error": function (jqXHR){
  ...
  function detail_rekap(user_id,tipe) {
    ...
    $('#dt_detail_rekap').DataTable({
      ...
      "ajax": {
        "url": "<?= base_url('mom_dev/get_detail_rekap') ?>",
        "type": "POST",
        "dataType": 'JSON',
        "data": {
          datestart: $("#start").val(),
  ...
  {    
    $('#dt_mom_result_e').DataTable({
      ...
      "ajax": {
        "dataType": 'JSON',
        "type": "GET",
        "url": "<?= base_url('mom_dev/get_result_meeting/') ?>"+id,
        // "success": function (res){
        //   console.log(res);
        // },
        // "error": function (jqXHR){
      ...
      "columns": [
        ...
        },
        {
          ...
          'render': function(data,type,row,meta){
            no = meta.row + 1;
            return `<input type="text" class="form-control ${(row['id_kategori'] == 1 || row['id_kategori'] == 6 || row['id_kategori'] == 8 || row['id_kategori'] == 9 || row['id_kategori'] == 10) ? 'tanggal' : '' }" id="e_deadline_${no}" name="e_deadline_${no}" value="${data}" onfocusout="update_result('${row['id_mom']}','${row['id_issue']}','${row['id_issue_item']}','${row['id_task']}','${row['id_sub_task']}','e_deadline_${no}')">`;
          }
        },
        {
          'data': 'list_pic',
        ...
        formSubmit: {
          ...
          action: function() {
            $.confirm({
              ...
              onOpen: function() {
                $.ajax({                    
                  "url": "<?= base_url('mom_dev/send_wa_mom/') ?>"+id+"/reminder",
                  "type": "GET",
                  "dataType": 'TEXT',
                  "success": function (res){
                    console.log(res);       
  ...
  {    
    ...
    $.ajax({
      "url": "<?= base_url('mom_dev/update_result') ?>",
      "type": "POST",
      "dataType": 'JSON',
      "data": {
        id_mom: id,
  ...
  {    
    ...
    $('.check-valid').removeClass('is-invalid');
    $.ajax({
      url: "<?= base_url('mom_dev/get_draft_header/') ?>"+id,
      method: "GET",
      dataType: "JSON",
      beforeSend: function (res){
        $('#btn_draft').attr("disabled",true);  
  ...
  function draft_e(closed)
  {
    ...
    if(id_mom == ""){
      ...
      $.ajax({
        url: "<?= base_url('mom_dev/save_closing_draft') ?>",
        method: "POST",
        dataType: "JSON",
        data: {
          id_mom: id_mom,
  ...
  function get_issue_result(id_mom) {
    $.confirm({
      ...
      onOpen: function() {
        $.ajax({
          url: '<?php echo base_url() ?>mom_dev/get_issue_result/'+id_mom,
          type: 'GET',
          dataType: 'json'
        })
        .done(function(data) {
