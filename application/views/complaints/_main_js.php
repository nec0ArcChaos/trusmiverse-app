    ...
    function get_filter_pic() {
        $.ajax({
            ...
            for (let index_t = 0; index_t < response.length; index_t++) {
                if (parseInt(response[index_t].id_pic) == parseInt(my_ticket_id) && parseInt(my_ticket_id) != 4138 && parseInt(my_ticket_id) != 9189 && parseInt(my_ticket_id) != 5504 && parseInt(my_ticket_id) != 3013) {
    ...
    blok_element.addEventListener("change", function() {
        ...
        $.ajax({
            ...
            if (response != '') {
                ...
                $('#konsumen').attr('data-is_akad', response.is_akad)
            } else {
...
<script>
    function get_category() {
        ...
        // get data atribut from konsumen
        is_akad = $('#konsumen').attr('data-is_akad') || "";
        ...
        $.ajax({
            ...
            for (let index = 0; index < response.length; index++) {
                disabled = "";
                disabled_text = "";
                if (is_akad == '' && [1, 2, 4, 5, 6, 7, 8, 9, 10, 12, 19, 20].includes(parseInt(response[index].id_category))) {
                    disabled = "disabled";
                    disabled_text = "(konsumen belum akad)"
                }
                list_category += `<option value="${response[index].id_category}" data-escalation_by="${response[index].escalation_by}" data-escalation_name="${response[index].escalation_name}" ${disabled}>${response[index].category} ${disabled_text}</option>`;
            }
                ...
                $.ajax({
                    ...
                    if (response.status == true) {
                        ...
                        if (
                            ...
                            &&
                            (user_id == '9189' || user_id == '4138' || user_id == '1' || user_id == '5768' || user_id == '10118') // Salma / Farhan / Ceppy / shelya
                        ) {
                        ...
                        if (
                            // user_id == 7804 
                            (user_id == '9189' || user_id == '4138' || user_id == '1' || user_id == '5768') // Salma / Farhan / Ceppy
                            &&
