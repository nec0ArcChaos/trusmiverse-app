...
<script>
    $(document).ready(function() {
        ...
        cb(start, end);
        get_candidates('<?= date('Y-m-01') ?>', '<?= date('Y-m-t') ?>', 3, 'dt_lp');
        get_candidates('<?= date('Y-m-01') ?>', '<?= date('Y-m-t') ?>', 4, 'dt_gp');
        $('#btn_filter').on('click', function() {
            start = $('input[name="startdate"]').val();
            end = $('input[name="enddate"]').val();
            get_candidates(start, end, 3, 'dt_lp');
            get_candidates(start, end, 4, 'dt_gp');
        ...
        $('.range2').on('change', function() {
            ...
            console.log(start);
            console.log(end);
            get_candidates(start, end, '1, 10', 'dt_dp');
        })
        // Job Desc Text_Area
        $('#job_desc').summernote({
    ...
    function show_daftar_psikotes() {
        get_candidates('', '', '1, 10', 'dt_dp');
                ...
                'confirm': {
                    ...
                    action: function() {
                        $.confirm({
                            ...
                            onOpen: function() {
                                $.ajax({
                                    url: "<?= base_url('recruitment/psikotes_ade/save_status') ?>", //intdev 1
                                    method: "POST",
                                    data: form.serialize(),
                                    dataType: "JSON",
                                    success: function(res) {
