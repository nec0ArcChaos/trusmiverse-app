...
<script>
    $(document).ready(function() {
        ...
        // Auto open modal & prefill jika redirect dari renewal contract (?employee_id=xxx)
        if (renewalEmployeeId) {
            $.ajax({
                url: "<?= base_url('recruitment/permintaan_karyawan/get_prefill_renewal') ?>",
                method: "POST",
    ...
    function show_edit_modal(id) {
        ...
        $.ajax({
            ...
            success: function(res) {
                ...
                // status
                let status_approve = ''
                if (user_id == 1 || user_id == 2535 || user_id == 6466) {
    ...
    function show_add_permintaan_prefill(prefill) {
        ...
        // Tunggu perusahaan options ter-load dari AJAX, lalu set value
        var waitPerusahaan = setInterval(function() {
            if ($('#perusahaan option').length > 1) {
                clearInterval(waitPerusahaan);

                // Set perusahaan (format: "employer_id | company_id")
                ...
                // Set jumlah kebutuhan default 1
                $('#jumlah').val(1);
                ...
                setTimeout(function() {
                    if (prefill.department_id) {
                        ...
                        department_select.update();
                        console.log('Prefill department:', prefill.department_id, prefill.department_name);
                    }
                    ...
                    setTimeout(function() {
                        if (prefill.designation_id && prefill.designation_name) {
                            ...
                            // Load detail posisi (job desc, kpi, bawahan, kompetensi)
                            get_dt_posisi();
                        }
                    }, 1200);
                }, 1200);
