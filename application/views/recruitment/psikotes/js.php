        ...
        $.ajax({
            ...
            success: function (res) {
                ...
                res.loker.forEach((value, index) => {
                    let kebutuhan = parseInt(value.kebutuhan) || 0;
                    let pemenuhan = parseInt(value.pemenuhan) || 0;
                    let progress  = `(${pemenuhan}/${kebutuhan})`;
                    let label     = `${value.loker} | ${progress}`;
                    let isFull    = (kebutuhan > 0 && pemenuhan >= kebutuhan);

                    if (isFull) {
                        loker += `<option value="${value.job_id}" disabled data-terpenuhi="1">${label}</option>`;
                    } else {
                        loker += `<option value="${value.job_id}">${label}</option>`;
                    }
                });
                $('#loker').html(loker);
                select_loker.update();

                // Beri warna merah pada opsi yang sudah terpenuhi
                $('#loker').closest('.select_loker')
                    .find('.nice-select-dropdown .list .option.disabled')
                    .css({ 'color': 'red', 'cursor': 'not-allowed' });

                // Alert ketika user klik opsi yang sudah terpenuhi
                $('#loker').closest('.select_loker')
                    .find('.nice-select-dropdown .list .option.disabled')
                    .off('click.terpenuhi')
                    .on('click.terpenuhi', function (e) {
                        e.stopImmediatePropagation();
                        e.preventDefault();
                        error_alert('Loker ini sudah terpenuhi!');
                        return false;
                    });
            },
            complete: function () {
    ...
    function get_psikotes(id) {
        $.ajax({
            ...
            success: function (res) {
                ...
                if (res.psikotes_new) {
                    const data = res.psikotes_new;

                    // ID selector diubah ke #tiu_score_new, #cfit_score_new, dst.
                    $('#tiu_score_new').val(data.tiu_score || '-');
                    $('#cfit_score_new').val(data.cfit_score || '-');
                    $('#mbti_type_new').val(data.mbti_type || '-');
                    $('#disc1_new').val(data.disc1 || '-');
                    $('#disc2_new').val(data.disc2 || '-');
                    $('#disc3_new').val(data.disc3 || '-');

                    // Fungsi helper untuk progress bar diubah untuk menargetkan ID baru
                    function setProgressBar(prefix, value) {
                        const numericValue = parseInt(value) || 0;
                        // Selector diubah menjadi '#...' + '_new'
                        const bar = $('#' + prefix + '_percent_bar_new');

                        if (bar.length) {
                            bar.css('width', numericValue + '%');
                            bar.attr('aria-valuenow', numericValue);
                            bar.text(numericValue + '%');
                        }
                    }

                    setProgressBar('e', data.E_percent);
                    setProgressBar('i', data.I_percent);
                    setProgressBar('s', data.S_percent);
                    setProgressBar('n', data.N_percent);
                    setProgressBar('t', data.T_percent);
                    setProgressBar('f', data.F_percent);
                    setProgressBar('j', data.J_percent);
                    setProgressBar('p', data.P_percent);
                }
            },
        })
    }
