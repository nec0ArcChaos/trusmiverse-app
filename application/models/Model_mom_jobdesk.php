...
class Model_mom_jobdesk extends CI_Model
{
  ...
  function get_list_mom($start, $end, $closed)
  {
    ...
    $super            = [1139, 8892, 803, 4138, 5086, 5385, 329, 4498, 3651, 2903, 5121, 8204, 2735]; // Ittrusmi & Super Admin & Viky & Lintang(Sekdir) & Pak Ibnu, Farhan, alfin3333, ratih, sandi2696, od rsp, muhammad5956(hilal)
    ...
    if ($role_id == 1 || in_array($user_id, $super) || in_array($designation_id, $list_designation)) {
      if ($closed == 0) { // Draft
        ...
        if ($user_id != 1 && $user_id != 5121 && $user_id != 803 && $user_id != 323 && $user_id != 8204) { // Jika Super Admin bukan akun Pak Ibnu & Pak Hendra, & hilal
          $kondisi_new = "WHERE NOT FIND_IN_SET(68,final.list_dep_pic)";
        }
      }
    ...
    } else if ($user_id == 70 || $user_id == 2729 || $user_id == 321 || $user_id == 7838) { // Nining27, saeroh1425, andre22, hrbatik
      if ($closed == 0) { // Draft
        $kondisi = "AND created.user_id = $user_id";
      ...
      if ($closed == 0) { // Draft
        ...
        $kondisi = "AND mom.tgl BETWEEN '$start' AND '$end'";
        $kondisi_new = "WHERE final.company_id IN (1,3,5) OR final.user_id = $user_id";
      }
      ...
      if ($closed == 0) { // Draft
        ...
        $kondisi = "AND mom.tgl BETWEEN '$start' AND '$end'";
        $kondisi_new = "WHERE FIND_IN_SET($user_id,final.list_peserta_pic) OR final.user_id = $user_id OR FIND_IN_SET(106,final.list_dep_pic) OR final.department_id IN( 106,142,207,204,210,205,206,211,161)";
      }
      ...
      if ($closed == 0) { // Draft
        ...
        $kondisi = "AND mom.tgl BETWEEN '$start' AND '$end'";
        $kondisi_new = "WHERE FIND_IN_SET($user_id,final.list_peserta_pic) OR final.user_id = $user_id OR FIND_IN_SET(106,final.list_dep_pic) OR final.department_id IN( 106,142,207,204,210,205,206,211)";
      }
    ...
    if ($user_id == 2505) { // puput1261
      if ($closed == 0) { // Draft
        ...
        $kondisi = "AND mom.tgl BETWEEN '$start' AND '$end'";
        $kondisi_new  = "WHERE final.user_id = $user_id OR FIND_IN_SET(68,final.list_dep_pic) OR FIND_IN_SET(25,final.list_dep_pic) OR FIND_IN_SET(136,final.list_dep_pic) OR final.department_id IN (68,25,136)";
      }
    }
    ...
    if ($user_id == 7731) { // Dimas Nurullah
      if ($closed == 0) { // Draft
        ...
        $kondisi = "AND mom.tgl BETWEEN '$start' AND '$end'";
        $kondisi_new  = "WHERE final.user_id = $user_id OR FIND_IN_SET(106,final.list_dep_pic) OR FIND_IN_SET(142,final.list_dep_pic) OR FIND_IN_SET(183,final.list_dep_pic) OR final.department_id IN (106,142,183)";
      }
    }
    ...
    if ($user_id == 5510) { // idris3450
      if ($closed == 0) { // Draft
        ...
        $kondisi = "AND mom.tgl BETWEEN '$start' AND '$end'";
        $kondisi_new  = "WHERE FIND_IN_SET($user_id,final.list_peserta_pic) OR FIND_IN_SET(476,final.list_peserta_pic) OR final.user_id = $user_id";
      }
    }
                            ...
                            FROM
                              ...
                              LEFT JOIN (
                                ...
                                FROM
                                  ...
                                  WHERE
                                    mom.closed = $closed AND mom.meeting IN ('Jobdesk')
                                    $kondisi
                                  GROUP BY
                                    item.id_mom,
                                    em.user_id 
  ...
  {
    return $this->db->query("SELECT * FROM mom_kategori WHERE id IN (1,8,9,10,12)")->result();
  }
  ...
  function get_list_rekap($start, $end)
  {
    ...
    $list_designation = [1217, 1218]; // HRBP Manager, Officer
    $list_super       = [1139, 8892, 4138, 5385, 329, 4498, 5121]; // alfin3333, ratih, sandi2696
    ...
    return $this->db->query("SELECT
                              ...
                              COUNT(IF(ibr.`status` = 3 AND item.verified_status = 2,1,NULL)) AS revisi,
                              COUNT(IF(ibr.`status` = 3 AND item.verified_status <> 2,1,NULL)) AS done,
                            ...
                            FROM
                              ...
                              WHERE
                                ...
                                AND trusmi_history_lock.created_at BETWEEN '2025-03-01' 
                                AND '2025-03-31' 
                              GROUP BY
                                trusmi_history_lock.employee_id) AS kunci ON kunci.employee_id = pic.user_id
                              LEFT JOIN (SELECT
                                all_dates.date - INTERVAL 6 DAY AS start_mom,
                                all_dates.date AS end_mom,
                                CEIL((DAY(all_dates.date) + WEEKDAY(DATE_FORMAT('$start', '%Y-%m-%d'))) / 7) AS `week`
                            FROM
                                all_dates 
                            WHERE
                                WEEKDAY( all_dates.date ) = 6 
                                AND SUBSTR( all_dates.date, 1, 7 ) = SUBSTR( '$start', 1, 7 )) AS weekly ON item.deadline BETWEEN weekly.start_mom AND weekly.end_mom
                            WHERE
                              mom.closed = 1 AND mom.meeting IN ('Jobdesk')
  ...
  function get_detail_rekap($start, $end, $user, $tipe)
  {
    if ($tipe == 30) {
      ...
      $kondisi = "AND ibr.`status` = 3 AND item.verified_status <> 2";
    } else if ($tipe == 50) {
    ...
    return $this->db->query("SELECT
                              ...
                              CASE
                                WHEN item.deadline BETWEEN '2024-01-05' AND CURDATE() AND COALESCE(sub.progress,0) < 100 AND ibr.`status` < 3 AND item.verified_status <> 2  THEN
                                  'Locked'
                                WHEN item.deadline BETWEEN '2024-01-05' AND CURDATE() AND ibr.`status` IN(2,3) AND item.verified_status = 2  THEN
                                  'Locked'
                            ...
                            WHERE
                              (
                                mom.tgl BETWEEN '$start' AND '$end' OR
                                ibr.due_date BETWEEN '$start' AND '$end' OR
                                item.deadline BETWEEN '$start' AND '$end'
                                ) 
                              AND pic.user_id = '$user'
                              AND mom.closed = 1 AND mom.meeting IN ('Jobdesk')
                              $kondisi")->result();
  }
...
FROM 
	td_sub_task subt
JOIN mom_issue_item AS item ON FIND_IN_SET( subt.id_sub_task, item.id_sub_task ) AND item.is_eskalasi = 1
    ...
    $query = "SELECT 
                ...
                FROM(
                    SELECT tm.user_id, tm.employee_name, tm.company_name, tm.jabatan, th.w, th.tgl_awal, th.tgl_akhir, SUM(IF(th.w IS NOT NULL,1,0)) AS input FROM
                      ...
                      LEFT JOIN mom_plan b ON FIND_IN_SET(tm.user_id, peserta) AND b.meeting IN ('Jobdesk')
                      LEFT JOIN (
                                          SELECT
                                              @rank := @rank + 1 AS w,
                                              calendar_week.* 
...
FROM
	...
	JOIN mom_issue AS moms ON moms.id_mom = momi.id_mom 
	AND moms.id_issue = momi.id_issue
	JOIN mom_kategori AS momk ON momk.id = momi.kategori
	JOIN mom_level AS moml ON moml.id = momi.`level`
  JOIN xin_employees crt ON crt.user_id = mom.created_by
	JOIN xin_employees AS emp ON FIND_IN_SET( emp.user_id, momi.pic ) 
...
WHERE
	...
	AND mom.meeting = 'Owner' AND mom.meeting IN ('Jobdesk')
...
WHERE
	...
	AND task.`status` = 3
	AND DATE(mom.created_at) > '2025-03-01'
  AND mom.created_by = '$user'
-- 	AND DATE( momi.verified_at ) > '2025-03-10'
...
GROUP BY
  ...
  public function list_eskalasi(){
    ...
    $query = "SELECT
      ...
      WHERE item.is_eskalasi = 1 AND mom.meeting IN ('Jobdesk') $kondisi
