...
class Model_mom_khael extends CI_Model
{
  ...
  function get_list_mom($start, $end, $closed)
  {
    ...
    $super            = [1139, 8892, 8970, 803, 4138, 5086, 5385, 329, 4498, 3651, 2903, 5121, 8204, 2735]; // Ittrusmi & Super Admin & Viky & Lintang(Sekdir) & Pak Ibnu, Farhan, alfin3333, ratih, sandi2696, od rsp, muhammad5956(hilal)
    ...
    if ($user_id == 7731) { // Dimas Nurullah
      if ($closed == 0) { // Draft
        ...
        $kondisi = "AND mom.tgl BETWEEN '$start' AND '$end'";
        $kondisi_new  = "WHERE final.user_id = $user_id OR FIND_IN_SET(106,final.list_dep_pic) OR FIND_IN_SET(142,final.list_dep_pic) OR FIND_IN_SET(183,final.list_dep_pic) OR final.department_id IN (106,142,183)  OR FIND_IN_SET($user_id,final.cc)";
        $kondisi_jobdesk = "AND mom.meeting NOT IN ('Jobdesk')";
      }
    }
  ...
  function get_list_rekap($start, $end)
  {
    ...
    $list_designation = [1217, 1218]; // HRBP Manager, Officer
    $list_super       = [1139, 8892, 8970, 4138, 5385, 329, 4498, 5121]; // alfin3333, ratih, sandi2696
    ...
    return $this->db->query("SELECT 
      ...
      -- Waktu
      COUNT(IF(DATEDIFF(IF(ibr.`status` = 3, DATE(ibr.done_date),NULL), item.deadline) < 1, 1, NULL)) AS ontime,
      COUNT(IF(DATEDIFF(IF(ibr.`status` = 3, DATE(ibr.done_date),NULL), item.deadline) > 0, 1, NULL)) AS late,
  ...
  function get_detail_rekap($start, $end, $user, $tipe)
  {
    if ($tipe == 30) {
      ...
      $kondisi = "AND DATEDIFF(IF(ibr.`status` = 3, DATE(ibr.done_date),NULL), item.deadline) < 1";
    } else if ($tipe == 60) {
      $kondisi = "AND DATEDIFF(IF(ibr.`status` = 3, DATE(ibr.done_date),NULL), item.deadline) > 0";
...
ORDER BY tas.id_task")->result();
 ...
 GROUP BY
  ...
  {
    $kondisi = (in_array($user_id, [1, 803])) ? "" : "AND momi.pic = $user_id";
    $query = "SELECT
        momi.id_mom,
        md5(momi.id_mom) AS id_mom_en,
        momi.id_issue,
