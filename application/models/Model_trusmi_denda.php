...
class Model_trusmi_denda extends CI_Model
{
  ...
  function get_dt_denda($periode)
  {
    ...
    if (in_array($id_user, array(1,78,1161,1139,778,979))) {
      $kondisi = "";
    } else {
    ...
    // if (in_array($id_hr, array(340, 272, 4303))) {
      ...
      $query = "SELECT
                  ...
                  CONCAT(xin_companies.`name`, ' | ', xin_departments.department_name) department_name,
									xin_designations.designation_name,
                  CASE
                    WHEN xin_employees.user_role_id IN (1, 2, 10) THEN 150000
                    WHEN xin_employees.user_role_id = 3 THEN 75000
                    WHEN xin_employees.user_role_id = 5 THEN 50000
                    ELSE 20000 
  END AS nominal
                FROM
                  hris.xin_employees 
  ...
  function dt_rekomendasi_denda($periode, $company, $department)
  {
    ...
    $kondisi = "";
    $kondisi2 = "";
    if ($id_user != 1 && $id_user != 1139) {
      $kondisi = "AND denda.head_id = '$id_user'";
      $kondisi2 = "WHERE denda.head_id = '$id_user'";
    }

    if ($department != null) {
      $sub_query = "WHERE company_id = '$company' AND department_id = '$department' $kondisi";
    } else if ($company != null) {
      $sub_query = "WHERE company_id = '$company' $kondisi";
    } else {
      $sub_query = "$kondisi2";
    }
    ...
    $query = "SELECT 
      ...
      END AS profil,
      CASE
                    WHEN emp.user_role_id IN (1, 2, 10) THEN 150000
                    WHEN emp.user_role_id = 3 THEN 75000
                    WHEN emp.user_role_id = 5 THEN 50000
                    ELSE 20000 
  END AS nominal
  FROM 
  ...
  JOIN 
      ...
      AND denda.periode = '$periode' -- addnew";
    return $this->db->query($query)->result();
  }
}
