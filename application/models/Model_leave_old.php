...
class Model_leave extends CI_Model
{
    ...
    public function get_leave_type()
    {
        ...
        $last_pool_year = $pool_year - 1;

        $cek_masa_kerja = $this->db->query("SELECT TIMESTAMPDIFF(MONTH,e.date_of_joining,CURRENT_DATE) AS masa_kerja, YEAR(DATE_ADD(e.date_of_joining, INTERVAL 12 MONTH)) AS eligible_year FROM xin_employees e WHERE user_id = '$user_id'")->row_array();
        $cond = "";
        if ($cek_masa_kerja['masa_kerja'] < 12) {
            $leave_custom = '1,19';
            $cond = "AND lt.leave_type_id NOT IN ($leave_custom)";
        }
        else if ($pool_year <= $cek_masa_kerja['eligible_year']) {
            $leave_custom = '19';
            $cond = "AND lt.leave_type_id NOT IN ($leave_custom)";
        }
        if ($user_id) {
            $query = "SELECT
                            ...
                            SELECT
                                ...
                                (
                                        ...
                                        WHERE
                                            ...
                                            YEAR(IF(DAY(from_date) > 20, DATE_ADD(from_date, INTERVAL 11 DAY), from_date)) = $pool_year AND la.`status` = 2 
                    ...
                    LEFT JOIN (
						...
						LEFT JOIN
                            ...
                            WHERE
                                a.employee_id = '$user_id' AND leave_type_id = 1 AND YEAR(IF(DAY(a.from_date) > 20, DATE_ADD(a.from_date, INTERVAL 11 DAY), a.from_date)) = $last_pool_year AND a.`status` = 2
                            ...
                            WHERE
                                b.employee_id = '$user_id' AND b.leave_type_id = 19 AND YEAR(IF(DAY(b.from_date) > 20, DATE_ADD(b.from_date, INTERVAL 11 DAY), b.from_date)) = $pool_year AND b.`status` = 2
