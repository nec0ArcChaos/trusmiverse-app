...
class Model_trusmi_resignation_new extends CI_Model
{
    ...
    public function dt_trusmi_resignation($start, $end, $user_role_id, $user_id)
    {
        ...
        $cond = "AND r.employee_id = $user_id";
        $super_admin = [1, 2063, 61, 2903, 979, 3388, 2951, 778];
        $pdca = ['329'];
        ...
        IF
            (
                COALESCE ( atasan_rsp.nama_mng, '' ) = 'IT Trusmi Group',
                '',
            COALESCE ( atasan_rsp.nama_mng, '' )) AS nama_mng,
            r.resignation_id,
            r.company_id,
        ...
        CASE
                ...
                'Approve GM' ELSE 'Not Approved' 
            END AS status_resignation,
            r.created_at,
...
CASE
		...
		WHEN u.id_divisi = 2 
		OR u.id_divisi = 20 THEN
			mng.employee_name ELSE CONCAT( mngr.first_name, ' ', mngr.last_name ) 
		END nama_mng 
FROM
	...
	LEFT JOIN xin_employees emp ON emp.user_id = u.join_hr 
	OR emp.user_id = u.id_hr
	LEFT JOIN xin_departments dp ON dp.department_id = emp.department_id
	LEFT JOIN xin_employees mngr ON mngr.user_id = dp.head_id 
WHERE
	u.id_hr IS NOT NULL 
	AND u.id_user != 1
            ) AS atasan_rsp ON atasan_rsp.id_hr = r.employee_id 
            WHERE
        resignation_date BETWEEN '$start' 
        AND '$end' $cond
        GROUP BY
            resignation_id";
    ...
    public function get_data_reason($category_id)
    {
        $query = "SELECT
                    ...
                    reason AS `text`
                FROM
                    `trusmi_resign_reason_new` 
                WHERE
                    id_category = $category_id";
        return $this->db->query($query)->result();
    }

    public function get_data_detail_reason($reason_id)
    {
        $query = "SELECT
                    id_detail_reason AS `value`,
                    detail_reason AS `text`
                FROM
                    `trusmi_resign_detail` 
                WHERE
                    id_reason = $reason_id";
        return $this->db->query($query)->result();
    }
    ...
    public function get_my_resignation($id_resignation)
    {
        ...
        if ($user_role_id == 1 || $user_id == 2063 || $user_id == 61 || $user_id == 323) {
            $condition = "";
        } else {
    ...
    public function get_pic_clearance($type, $department_id = null)
    {
        if ($type == 1) {
            $id = $department_id;
        } else {
        ...
        $query = "SELECT
                ...
                FROM
                    ...
                    LEFT JOIN xin_departments d ON d.department_id = '$id'
    ...
    function get_company($id = null)
    {
        ...
        $user_id = $this->session->userdata("user_id");
        if ($user_id == 3013) { // ade himawan
            $this->db->select('company_id AS `value`, name AS `text`')->from('xin_companies')->where_in('company_id', [$id, 1]);
        } else if ($id == null) {
            $this->db->select('company_id AS `value`, name AS `text`')->from('xin_companies');
        } else {
            $this->db->select('company_id AS `value`, name AS `text`')->from('xin_companies')->where('company_id', $id);
        }
        return $this->db->get()->result();
    }

    function get_department($id, $id_department = null)
    {
        $user_id = $this->session->userdata("user_id");
        if ($user_id == 3013) { // ade himawan
            $this->db->select('department_id AS `value`, department_name AS `text`')->from('xin_departments')->where('company_id', $id)->where_in('department_id', [232, 120]);
        } else if ($user_id == 1721) {
            $this->db->select('department_id AS `value`, department_name AS `text`')->from('xin_departments')->where('company_id', $id)->where_not_in('department_id', [168, 169, 116, 47]);
        } elseif ($id_department == null) {
            $this->db->select('department_id AS `value`, department_name AS `text`')->from('xin_departments')->where('company_id', $id);
        } else {
            $this->db->select('department_id AS `value`, department_name AS `text`')->from('xin_departments')->where('department_id', $id_department);
        }
    ...
    {
        return $this->db->select('designation_id AS `value`, designation_name AS `text`')->from('xin_designations')->where('department_id', $id)->get()->result();
    }

    function get_employee($id)
    {
        return $this->db->select('user_id AS `value`, CONCAT(first_name, " ", last_name) AS `text`')->from('xin_employees')->where('designation_id', $id)->where('is_active', 1)->order_by('text', 'ASC')->get()->result();
    }

    function get_info_employee($id)
    {
        return $this->db->select('CONCAT(first_name, " ", last_name) AS `nama`')->from('xin_employees')->where('user_id', $id)->get()->row();
    }

    function get_category_reason()
    {
        $query = "SELECT
                    id_category AS `value`,
                    category AS `text`
                FROM
                    `trusmi_resign_category_new`";
        return $this->db->query($query)->result();
    }
}
