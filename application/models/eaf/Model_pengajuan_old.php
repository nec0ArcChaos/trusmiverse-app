...
class Model_pengajuan extends CI_Model
{
    ...
    function get_list_eaf($id_user, $id_div, $start, $end, $tipe)
    {
        ...
        if (
            $this->session->userdata('user_id') == 1 || $_SESSION['user_id'] == 237 || $_SESSION['user_id'] == 61 || $this->session->userdata('user_id') == 73
            || $this->session->userdata('user_id') == 1358
            || $this->session->userdata('user_id') == 22348
        ) {
            if ($tipe == 1) {
    ...
    function jenis_biaya_by_company($company_id, $nominal)
    {
        ...
        $query = "SELECT
            ...
            e_biaya.bulan,
            e_biaya.minggu,
            IF($nominal>e_biaya.budget2 AND emp2.user_id IS NOT NULL AND e_biaya.budget2 IS NOT NULL,e_jenis_biaya.id_user_approval2,e_jenis_biaya.id_user_approval) as id_user_approval,
            e_jenis_biaya.project,
            COALESCE(e_jenis_biaya.blok,'') AS blok,
            COALESCE(e_jenis_biaya.id_user_verified, '') AS id_user_verified,
            e_jenis_biaya.ba,
            0 AS id_divisi,
            IF($nominal>e_biaya.budget2 AND emp2.user_id IS NOT NULL AND e_biaya.budget2 IS NOT NULL,COALESCE(REPLACE(CONCAT(TRIM(emp2.first_name), ' ', TRIM(emp2.last_name)),',',', '),''),COALESCE(REPLACE(CONCAT(TRIM(emp.first_name), ' ', TRIM(emp.last_name)),',',', '),'')) as employee
        FROM
            e_eaf.e_jenis_biaya
            JOIN e_eaf.e_biaya ON e_jenis_biaya.id_budget = e_biaya.id_budget 
    ...
    function get_list_lpj($id_user)
    {
        ...
        // if ($this->session->userdata('id_user') == 1 || $_SESSION['id_user'] == 237 || $_SESSION['id_user'] == 61 || $this->session->userdata('id_user') == 73 || $this->session->userdata('id_user') == 1358) {
        //     $where = "";
        // } else {
    ...
    {
        return $this->db->query("SELECT MAX(id_pengajuan) as id_pengajuan FROM e_eaf.e_pengajuan WHERE id_kategori <> 19")->row_array();
    }
    ...
    function get_pengajuan_for_wa($id, $level)
    {
        return $this->db->query("SELECT
                                    ...
                                    IF ( e_approval.id_user_approval = 78, '6282316041423', approval.contact_no ) AS approve_contact,
                                    e_pengajuan.pengaju,
