...
class Model_approval extends CI_Model
{
    ...
    {
        if ($status == 2) {
            // 73 iduser rsp farhan | 68 id hr farhan
            // if ($this->session->userdata('user_id') == 1 || $this->session->userdata('user_id') == 73) {
            if ($this->session->userdata('user_id') == 1 || $this->session->userdata('user_id') == 68) {
            ...
            $cek_param_su_all = $this->get_e_parameter(6, $id_user)->row_array();
            $cek_param_su_tkb = $this->get_e_parameter(11, $id_user)->row_array();

            if ($this->session->userdata('user_id') == 1 || $this->session->userdata('user_id') == 68 || $cek_param_su_all['id'] != Null) {
                $kondisi = "AND e_pengajuan.`status` IN ($status) AND e_approval.id_user_approval != $id_user";
            } else if ($cek_param_su_tkb['id'] != Null) {
                $kondisi = "AND e_pengajuan.`status` IN ($status) AND (e_approval.id_user_approval = $id_user OR e_company.company_id = 3)";
            } else {
                $kondisi = "AND e_pengajuan.`status` IN ($status) AND e_approval.id_user_approval = $id_user";
            }
        } else {
        ...
        $query = $this->db->query("SELECT
                                    ...
                                    CASE
                                        ...
                                        FROM
                                            ...
                                            LEFT JOIN e_eaf.e_jenis_biaya ON e_pengajuan.jenis = e_jenis_biaya.id_jenis
                                            LEFT JOIN e_eaf.e_biaya ON e_jenis_biaya.id_budget = e_biaya.id_budget
                                            LEFT JOIN e_eaf.e_detail_keperluan ON e_pengajuan.id_pengajuan = e_detail_keperluan.id_pengajuan 
                                            LEFT JOIN e_eaf.e_tipe_pembayaran ON e_pengajuan.id_pengajuan = e_tipe_pembayaran.id_pengajuan
                                            LEFT JOIN e_eaf.e_approval ON e_approval.id_pengajuan = e_pengajuan.id_pengajuan
                                            LEFT JOIN ( SELECT MAX( id_approval ) AS id_approval FROM e_eaf.e_approval WHERE id_user_approval IS NOT NULL GROUP BY id_pengajuan ) AS approve ON e_approval.id_approval = approve.id_approval
                                            LEFT JOIN e_eaf.e_photo_acc ON e_pengajuan.id_pengajuan = e_photo_acc.id_pengajuan
                                            LEFT JOIN e_eaf.e_header_lpj ON e_pengajuan.id_pengajuan = e_header_lpj.id_lpj
                                            LEFT JOIN (
                                            SELECT
                                                e_biaya.id_budget,
                                                e_biaya.budget 
    ...
    {
        // if ($this->session->userdata('user_id') == 1 && ($this->session->userdata('user_id') == 1 || $this->session->userdata('user_id') == 61 || $this->session->userdata('user_id') == 1358 || $this->session->userdata('user_id') == 73)) {
        //     $where = "";
        // } else {
        ...
        if ($cek_param_su_all['id'] != Null) {
            $where = "";
        } else if ($cek_param_su_tkb['id'] != Null) {
            $where = "AND (e_approval.id_user_approval = '$id_user' OR e_company.company_id = 3)";
        } else {
            $where = "AND e_approval.id_user_approval = '$id_user'";
        }
        ...
        $query = $this->db->query("SELECT
                                        ...
                                        e_approval.id_approval,
                                        COALESCE(e_verified.note_approve,e_detail_keperluan.note) AS note_approve,
                                        e_photo_acc.photo_acc,
                                        e_jenis_biaya.id_tipe_biaya,
                                        ...
                                        usr_desg.designation_name AS adm_desg_name,
                                        usr_dept.department_name AS adm_dept_name,
                                        usr_comp.name AS adm_comp_name,

                                        pengaju_desg.designation_name,
                                        pengaju_dept.department_name,
                                        pengaju_comp.name AS company_name,
                                        COALESCE(REPLACE(CONCAT(TRIM(pengaju.first_name), ' ', TRIM(pengaju.last_name)),',',', '),'') AS pengaju,
                                        
                                        COALESCE(REPLACE(CONCAT(TRIM(usr_apr.first_name), ' ', TRIM(usr_apr.last_name)),',',', '),'') AS user_approval,
                                    ...
                                    FROM
                                        ...
                                        LEFT JOIN ( SELECT id_approval, id_pengajuan, id_user_approval, note_approve FROM e_eaf.e_approval WHERE `level` = 1 AND `status` IS NULL ) AS e_approval ON e_approval.id_pengajuan = e_pengajuan.id_pengajuan
                                        LEFT JOIN ( SELECT id_approval, id_pengajuan, id_user_approval, note_approve, update_approve FROM e_eaf.e_approval WHERE `level` = 10 AND `status` = 'Approve' ) AS e_verified ON e_verified.id_pengajuan = e_pengajuan.id_pengajuan
                                        ...
                                        LEFT JOIN hris.xin_designations AS usr_desg ON usr_desg.designation_id = usr.designation_id 
                                        LEFT JOIN hris.xin_departments AS usr_dept ON usr_dept.department_id = usr.department_id
                                        LEFT JOIN hris.xin_companies AS usr_comp ON usr_comp.company_id = usr.company_id
                                    ...
                                    FROM
                                        ...
                                        JOIN ( SELECT id_approval, id_pengajuan, id_user_approval, note_approve FROM e_eaf.e_approval WHERE `level` = 1 AND `status` IS NULL ) AS e_approval ON e_approval.id_pengajuan = e_pengajuan.id_pengajuan
    ...
    function get_list_eaf_all($id_user, $id_div, $status, $datestart, $dateend)
    {
        ...
        if ($cek_param_su_all['id'] != Null) {
            ...
            $where = "AND DATE(e_pengajuan.tgl_input) BETWEEN '$datestart' AND '$dateend' AND 
            (e_approval.id_user_approval = '$id_user' OR e_company.company_id = 3)";
        } else {
        ...
        $query = $this->db->query("SELECT
                                        ...
                                        IF(e_detail_keperluan.id_project IS NULL,e_detail_keperluan.nama_keperluan,CONCAT(e_detail_keperluan.nama_keperluan,' ',m_project.project,' ',COALESCE(e_detail_keperluan.blok,''))) as nama_keperluan,
                                        e_detail_keperluan.note,
                                        COALESCE(e_detail_keperluan.nominal_uang,0) AS nominal_uang,
                                        e_detail_keperluan.id_project,
                                        m_project.project,
                                        e_detail_keperluan.blok,
                                        e_approval.id_approval,
                                    ...
                                    FROM
                                        ...
                                        LEFT JOIN e_eaf.e_detail_keperluan ON e_detail_keperluan.id_pengajuan = e_pengajuan.id_pengajuan
                                        LEFT JOIN ( SELECT id_approval, id_pengajuan, id_user_approval, note_approve FROM e_eaf.e_approval WHERE `level` = 1 AND `status` = 'Approve' ) AS e_approval ON e_approval.id_pengajuan = e_pengajuan.id_pengajuan
                                        LEFT JOIN ( SELECT id_approval, id_pengajuan, id_user_approval, note_approve FROM e_eaf.e_approval WHERE `level` = 10 AND `status` = 'Approve' ) AS e_verified ON e_verified.id_pengajuan = e_pengajuan.id_pengajuan
                                        LEFT JOIN e_eaf.e_photo_acc ON e_photo_acc.id_pengajuan = e_pengajuan.id_pengajuan
                                        LEFT JOIN e_eaf.e_jenis_biaya ON e_jenis_biaya.id_jenis = e_pengajuan.jenis
                                    ...
                                    FROM
                                        ...
                                        LEFT JOIN rsp_project_live.m_project ON e_detail_keperluan.id_project = m_project.id_project
                                        JOIN ( SELECT id_approval, id_pengajuan, id_user_approval FROM e_eaf.e_approval WHERE id_user_approval = 737 AND `status` IS NULL ) AS e_approval ON e_approval.id_pengajuan = e_pengajuan.id_pengajuan
    ...
    function get_detail_approval($id_pengajuan)
    {
        ...
        FROM
            ...
            LEFT JOIN e_eaf.e_detail_keperluan ON e_detail_keperluan.id_pengajuan = e_pengajuan.id_pengajuan
            LEFT JOIN ( SELECT id_approval, id_pengajuan, id_user_approval FROM e_eaf.e_approval WHERE id_user_approval <> 737 AND `status` IS NULL ) AS e_approval ON e_approval.id_pengajuan = e_pengajuan.id_pengajuan
            LEFT JOIN e_eaf.e_photo_acc ON e_photo_acc.id_pengajuan = e_pengajuan.id_pengajuan
            LEFT JOIN e_eaf.e_jenis_biaya ON e_jenis_biaya.id_jenis = e_pengajuan.jenis
    ...
    function get_ba($id)
    {
        return $this->db->query("SELECT
                                    ...
                                    COALESCE(REPLACE(CONCAT(TRIM(usr.first_name), ' ', TRIM(usr_ver.first_name)),',',', '),'') AS `employee_name`
                                FROM
                                    e_eaf.e_pengajuan
                                    LEFT JOIN e_eaf.e_header_lpj ON e_header_lpj.id_lpj = e_pengajuan.id_pengajuan
                                    LEFT JOIN ( SELECT id_pengajuan, id_user_approval FROM e_eaf.e_approval WHERE id_pengajuan = '$id' AND `level` = 1 AND `status` = 'Approve' ) AS approval ON approval.id_pengajuan = e_pengajuan.id_pengajuan
                                    -- LEFT JOIN `user` AS usr ON usr.id_user = approval.id_user_approval
                                    LEFT JOIN hris.xin_employees AS usr ON usr.user_id = approval.id_user_approval
                                    LEFT JOIN ( SELECT id_pengajuan, id_user_approval FROM e_eaf.e_approval WHERE id_pengajuan = '$id' AND `level` = 10 AND `status` = 'Approve' ) AS verified ON verified.id_pengajuan = e_pengajuan.id_pengajuan
                                    -- LEFT JOIN `user` AS usr_ver ON usr_ver.id_user = verified.id_user_approval 
                                    LEFT JOIN hris.xin_employees AS usr_ver ON usr_ver.user_id = verified.id_user_approval
                                WHERE
                                    e_pengajuan.id_pengajuan = '$id'")->row_array();
    }
    ...
    function get_approval_for_wa($id)
    {
        return $this->db->query("SELECT
                                ...
                                FROM
                                    ...
                                    FROM
                                        e_eaf.e_approval
                                        JOIN ( SELECT MAX( id_approval ) AS id_approval FROM e_eaf.e_approval WHERE `level` IN (1,4) GROUP BY id_pengajuan ) AS max_apr ON e_approval.id_approval = max_apr.id_approval AND `status` IS NOT NULL
    ...
    {
        return $this->db->query("SELECT * FROM e_eaf.e_approval WHERE id_pengajuan = '$id' AND `level` = 1 AND `status` = 'Approve'");
    }
    ...
    function cek_sisa_budget($id_pengajuan)
    {
        return $this->db->query("SELECT
                                ...
                                FROM
                                    ...
                                    JOIN e_eaf.e_jenis_biaya AS jb ON jb.id_budget = aju.budget 
