...
class Model_lock extends CI_Model
{
    ...
    function get_list_eaf_lock_lpj2($id_user)
    {
        ...
        $query = $this->db->query("SELECT * FROM (SELECT
                        ...
                        SELECT
                            c.status_lock AS tipe,
                            a.created_by AS created_by,
                            a.approve_to AS id_hr,
                            b.username AS username,
                            count( a.no_app ) AS jumlah,
                            c.id_lock AS id,
                            c.status_lock AS status_lock,
                            c.warning_lock AS warning_lock 
                        FROM
                            ((
                                    trusmi_approval a
                                    JOIN xin_employees b ON ((
                                            b.user_id = a.approve_to 
                                        )))
                                JOIN trusmi_m_lock c ON ((
                                        c.id_lock = 192 
                                    ))) 
                        WHERE
                            ((
                                a.status NOT IN ( 3, 2, 7 )) 
            AND ( substr( curtime(), 1, 2 ) > 12 ))
            ) dt $kondisi");
