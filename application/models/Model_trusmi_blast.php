...
class Model_trusmi_blast extends CI_Model
{
    ...
    {
        // $allowed_user = [1, 61, 62, 323, 979, 63, 64, 1161, 3388, 2041, 2063, 2969, 2969, 2070, 2903, 778, 2774, 2843, 6486, 3325, 6466, 6183, 2378, 5073, 2535, 6408, 5738, 5336, 1139, 7731, 9621];
    ...
    function dt_wa_blast($start, $end, $id = null)
    {
        // $allowed_user = [1, 61, 62, 323, 979, 63, 64, 1161, 2041, 2063, 2969, 2969, 2070, 2903, 778, 1139, 8892, 6486, 2843, 2516, 3325, 6466, 6183, 1139];
        $this->load->model('Model_auth', 'auth');
        $user_id = $this->session->userdata("user_id");
        $check_hak_akses = $this->auth->check_hak_akses('trusmi_blast', $user_id);
        ...
        if ($id == NULL) {
            if ($user_id == 2843 || $user_id == 2516) {
                $sekdir = "AND trusmi_blast.created_by = $user_id";
            } else {
                $sekdir = "";
            }
            $kondisi = "WHERE DATE(trusmi_blast.created_at) BETWEEN '$start' AND '$end' 
                        $sekdir";
        } else {
            $kondisi = "WHERE trusmi_blast.id = $id";
        }
        $query = "SELECT
                    trusmi_blast.id,
                    trusmi_blast.title,
                    trusmi_blast.employees AS employee_id,
                    REPLACE(
                        REPLACE(
                            GROUP_CONCAT(
                                CONCAT(xin_employees.first_name,' ',xin_employees.last_name) 
