...
class Model_grd extends CI_Model
{
	...
	function data_goal($divisi, $start, $end)
	{
		...
		$sql = "SELECT
					...
					COALESCE(ROUND(SUM(si.actual_si_p) / COUNT(si.id_si),1),0) AS progress_si,

					
					COALESCE(SUM(progress.total_tasklist),0) AS total_tasklist,
					COALESCE(ROUND(SUM(progress.total_actual) / SUM(progress.total_tasklist),1),0) AS progress_tasklist,
					...
					ROUND(COALESCE(
						CASE 
							...
							ELSE 100 - COALESCE(ROUND(((goal.actual / goal.target) * 100),1),0)
						END
	...
	function data_so($divisi, $start, $end)
	{
		...
		$sql = "SELECT
					...
					so.updated_at, 
					so.updated_by,
					COALESCE(so.evidence,'') AS evidence_file, COALESCE(so.evidence_link,'') AS evidence_link,
					COALESCE(MIN(DATE_FORMAT(`task`.`start`, '%d-%m-%Y %H:%i')),'-') AS start,
					COALESCE(MAX(DATE_FORMAT(`task`.`end`, '%d-%m-%Y %H:%i')),'-') AS end,
					COALESCE(CONCAT(
						TIMESTAMPDIFF(DAY, MIN(`task`.`start`), MAX(`task`.`done_at`)), ' h, ',
	...
	function data_si($divisi, $start, $end)
	{
		...
		$sql = "SELECT
					...
					si.updated_at, 
					si.updated_by,
					COALESCE(si.evidence,'') AS evidence_file, COALESCE(si.evidence_link,'') AS evidence_link,
	...
	function data_tasklist($divisi, $start, $end)
	{
		...
		$sql = "SELECT
					...
					COALESCE(task.evidence,'') AS evidence, 
					COALESCE(task.evidence_link,'') AS evidence_link,
	...
	public function data_persen_bt($divisi, $start, $end)
	{
		...
		if($divisi == null || $divisi == "") {
			$kond = "AND divisi = 'Operasional'";
		}else{
			$kond = "AND divisi = '$divisi'";
		}
	...
	public function get_budget_vs_actual($start, $end, $divisi)
	{
		...
		$query2 = "SELECT
					...
					e_biaya.id_biaya,
					e_biaya.nama_biaya,
					COALESCE(e_biaya.budget_awal,0) AS awal_budget,
					COALESCE(e_biaya.budget_awal,0) + COALESCE ( tambahan.nominal_tambah, 0 ) AS budget_awal,
					COALESCE(m_0.sudah_lpj + m_0.rembers,0) AS act_budget,
					ROUND(((COALESCE(m_0.sudah_lpj + m_0.rembers,0)) / (COALESCE(e_biaya.budget_awal,0) + COALESCE ( tambahan.nominal_tambah, 0 ))) * 100) budget_all_p,
					100 - ROUND(((COALESCE(m_0.sudah_lpj + m_0.rembers,0)) / (COALESCE(e_biaya.budget_awal,0) + COALESCE ( tambahan.nominal_tambah, 0 ))) * 100) budget_all_p_sisa,
					
					COALESCE(budget_last.budget_awal,0) AS last_budget,
					COALESCE(budget_last.budget_awal,0) + COALESCE ( last_tambahan.nominal_tambah, 0 ) AS budget_last,
					COALESCE(last_month.sudah_lpj + last_month.rembers,0) AS act_budget_last,
						CASE
						WHEN COALESCE(budget_last.budget_awal,0) > COALESCE(e_biaya.budget_awal,0) THEN 'bi bi-arrow-down'
						WHEN COALESCE(budget_last.budget_awal,0) < COALESCE(e_biaya.budget_awal,0) THEN 'bi bi-arrow-up'
						ELSE 'bi bi-dash' -- Jika sama, gunakan ikon netral
					END AS icon_budget,
						CASE
						WHEN COALESCE(budget_last.budget_awal,0) > COALESCE(e_biaya.budget_awal,0)  THEN 'text-danger'
						WHEN COALESCE(budget_last.budget_awal,0) < COALESCE(e_biaya.budget_awal,0) THEN 'text-success'
						ELSE 'text-secondary' -- Jika sama, gunakan warna netral
					END AS text_color_budget,
						CASE
						WHEN COALESCE(last_month.sudah_lpj + last_month.rembers,0) > COALESCE(m_0.sudah_lpj + m_0.rembers,0) THEN 'bi bi-arrow-down'
						WHEN COALESCE(last_month.sudah_lpj + last_month.rembers,0) < COALESCE(m_0.sudah_lpj + m_0.rembers,0) THEN 'bi bi-arrow-up'
						ELSE 'bi bi-dash' -- Jika sama, gunakan ikon netral
					END AS icon_budget_act,
						CASE
						WHEN COALESCE(last_month.sudah_lpj + last_month.rembers,0) > COALESCE(m_0.sudah_lpj + m_0.rembers,0)  THEN 'text-danger'
						WHEN COALESCE(last_month.sudah_lpj + last_month.rembers,0) < COALESCE(m_0.sudah_lpj + m_0.rembers,0) THEN 'text-success'
						ELSE 'text-secondary' -- Jika sama, gunakan warna netral
					END AS text_color_budget_act
				FROM
					rsp_project_live.e_biaya
					LEFT JOIN (
					SELECT
						id_biaya,
					...
					FROM
						...
						FROM
							rsp_project_live.e_pengajuan
							LEFT JOIN ( SELECT id_pengajuan AS id_lpj, total_pengajuan, created_at FROM rsp_project_live.e_pengajuan WHERE id_kategori = 19 ) AS lpj ON e_pengajuan.temp = lpj.id_lpj
							LEFT JOIN ( SELECT id_pengajuan, update_approve FROM rsp_project_live.e_approval WHERE id_user_approval = 737 AND e_approval.level = 5) AS approval ON approval.id_pengajuan = e_pengajuan.id_pengajuan 
							LEFT JOIN rsp_project_live.e_biaya ON e_biaya.id_biaya = e_pengajuan.id_biaya
						WHERE
							substr( approval.update_approve, 1, 7 ) = '2025-05' AND 
							-- e_biaya.tahun_budget = 2025 AND e_biaya.bulan = 05 AND
					...
					FROM
						rsp_project_live.e_biaya_penambahan 
					WHERE
						e_biaya_penambahan.bulan = 05
						AND e_biaya_penambahan.tahun = 2025
					GROUP BY
						e_biaya_penambahan.id_biaya,
						e_biaya_penambahan.bulan,
						e_biaya_penambahan.tahun 
					) AS tambahan ON tambahan.id_biaya = e_biaya.id_biaya
					LEFT JOIN rsp_project_live.e_biaya budget_last ON budget_last.bulan = 04 AND budget_last.tahun_budget = 2025 AND budget_last.id_budget = 406 
					LEFT JOIN (
					SELECT
						id_biaya,
						SUM( IF ( id_kategori = 17, total_pengajuan_eaf, 0 ) ) AS rembers,
					...
					FROM
						...
						FROM
							...
							LEFT JOIN rsp_project_live.e_biaya ON e_biaya.id_biaya = e_pengajuan.id_biaya
						WHERE
							-- substr( approval.update_approve, 1, 7 ) = '04onth' AND 
							e_biaya.tahun_budget = 2025 AND e_biaya.bulan = 04 AND
							e_pengajuan.status = 3 
						) x 
					GROUP BY
						id_biaya 
					)   AS last_month ON last_month.id_biaya = budget_last.id_biaya
					LEFT JOIN (
					SELECT
						e_biaya_penambahan.id_biaya,
						SUM( e_biaya_penambahan.nominal_tambah ) AS nominal_tambah 
					FROM
						rsp_project_live.e_biaya_penambahan 
					WHERE
						e_biaya_penambahan.bulan = 04
						AND e_biaya_penambahan.tahun = 2025
					GROUP BY
						e_biaya_penambahan.id_biaya,
						e_biaya_penambahan.bulan,
						e_biaya_penambahan.tahun 
					) AS last_tambahan ON last_tambahan.id_biaya = budget_last.id_biaya 
							
					WHERE
					e_biaya.bulan = 05
					AND e_biaya.tahun_budget = 2025
					AND e_biaya.id_budget = 406
					GROUP BY id_biaya

		";

		if($divisi == 'Soft Launch' || $divisi == 'Grand Opening'){
			return $this->db->query($query)->row_object();			
		} else if($divisi == 'Marketing Communication') {
			return $this->db->query($query2)->row_object();
		}
	}
