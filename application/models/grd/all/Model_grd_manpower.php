...
class Model_grd_manpower extends CI_Model
{
	...
	public function get_warning_manpower($start, $end, $id_company){
		// $periode = DateTime::createFromFormat('m-Y', $month)->format('Y-m');
		if(empty($start) && empty($end)){
			$start = date('Y-m-01');
			$end = date('Y-m-t');
...
";
	...
	public function get_warning_manpower_kehadiran($start, $end, $id_company){
		...
		(
			...
			WHERE
				xin_employees.company_id = $id_company 
				AND (
	...
	public function get_warning_manpower_undone($start, $end, $id_company){
		// $periode = DateTime::createFromFormat('m-Y', $month)->format('Y-m');
		if(empty($start) && empty($end)){
			$start = date('Y-m-01');
			$end = date('Y-m-t');
	...
	public function get_warning_manpower_detail($start, $end, $id_company){
		// $periode = DateTime::createFromFormat('m-Y', $month)->format('Y-m');
		// $start = date('Y-m', strtotime('-1 month', strtotime($periode))) . '-21';
		// $end = $periode . '-20';
		if(empty($start) && empty($end)){
