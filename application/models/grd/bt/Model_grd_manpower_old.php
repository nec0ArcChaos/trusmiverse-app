...
class Model_grd_manpower extends CI_Model
{
	...
	public function get_support($month){
		...
		$query = "SELECT
				...
				FROM
					...
					WHERE
						LEFT ( td_task.created_at, 7 ) = '$periode' 
					GROUP BY
						td_task.divisi 
					...
					WHERE
						LEFT ( mom_header.created_at, 7 ) = '$periode' 
					GROUP BY
						mom_header.divisi 
					...
					WHERE
						LEFT ( chat.created_at, 7 ) = '$periode' 
					GROUP BY
						chat.divisi 
					...
					WHERE
						LEFT ( gemba.created_at, 7 ) = '$periode' 
					GROUP BY
						gemba.divisi 
					...
					WHERE
						LEFT ( ps_task.created_at, 7 ) = '$periode' 
					GROUP BY
						ps_task.divisi 
					...
					WHERE
						LEFT ( t_sharing_leader.created_at, 7 ) = '$periode' 
					GROUP BY
						t_sharing_leader.divisi 
					...
					WHERE
						LEFT ( one_header.created_at, 7 ) = '$periode' 
					GROUP BY
						one_header.divisi 
					...
					WHERE
						LEFT ( briefing.created_at, 7 ) = '$periode' 
					GROUP BY
						briefing.divisi 
	...
	public function get_warning_manpower($month){
		$periode = DateTime::createFromFormat('m-Y', $month)->format('Y-m');
		$query = "SELECT 
			...
			FROM (
				...
				FROM (
					...
					WHERE
						emp.company_id = 5
						AND LEFT(his.created_at, 7) = '$periode'
				...
				WHERE 
					...
					AND LEFT(warning.created_at, 7) = '$periode'
				...
				WHERE
					emp.company_id = 5 
					AND LEFT ( trusmi_denda.created_at, 7 ) = '$periode'
					
			) AS combined_data;
";
		return $this->db->query($query)->row_object();
	}
	public function get_warning_manpower_kehadiran($month){
...
";
	...
	public function get_warning_manpower_undone($month){
		$periode = DateTime::createFromFormat('m-Y', $month)->format('Y-m');
		$query = "SELECT 
			...
			LEFT JOIN (
				...
				AND (LEFT(grd_t_tasklist.start,7) = '$periode' OR LEFT(grd_t_tasklist.end,7) = '$periode')
			) undone ON grd_t_tasklist.id_tasklist = undone.id_tasklist
			WHERE LEFT(grd_t_tasklist.start,7) = '$periode' OR LEFT(grd_t_tasklist.end,7) = '$periode'
	...
	public function get_warning_manpower_detail($month){
		$periode = DateTime::createFromFormat('m-Y', $month)->format('Y-m');
		$start = date('Y-m', strtotime('-1 month', strtotime($periode))) . '-21';
		$end = $periode . '-20';
		$query = "SELECT
