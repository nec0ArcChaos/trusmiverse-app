...
class Model_okr extends CI_Model
{
	...
	{
		$query = "SELECT divisi FROM okr_m_coorporate WHERE id_company = $id_company GROUP BY id_company, divisi";
		return $this->db->query($query)->result();
	}
	...
	public function get_tasklist_milestone($id_milestone)
	{
		$query = "SELECT 
				...
				FROM okr_t_tasklist_milestone task
	...
	function data_goal($divisi, $start, $end)
	{
		...
		$sql = "SELECT
				...
				FROM
					okr_m_coorporate AS goal
				LEFT JOIN xin_companies AS comp ON goal.id_company = comp.company_id

				LEFT JOIN okr_m_project AS so ON so.id_goal = goal.id_goal
				LEFT JOIN okr_t_objective AS si ON si.id_so = so.id_so
	...
	// 			FROM
	// 				okr_m_project AS so
	// 			LEFT JOIN okr_m_coorporate AS goal ON so.id_goal = goal.id_goal
	// 			LEFT JOIN okr_t_objective AS si ON si.id_so = so.id_so
	// 			LEFT JOIN okr_t_tasklist AS task ON task.id_si = si.id_si
	...
	// 				COUNT(okr_t_objective.id_si) AS total_actual_si,
	// 				SUM(okr_t_objective.actual_si_p) AS total_actual_si_p
	// 				FROM okr_t_tasklist
	// 				JOIN okr_t_objective ON okr_t_tasklist.id_si = okr_t_objective.id_si
	// 				GROUP BY id_so
	// 			) AS progress ON progress.id_so = so.id_so
	// 			LEFT JOIN (
	// 				SELECT okr_t_objective.id_si, id_so, 
	// 				COALESCE(SUM(actual_si_p) / COUNT(okr_t_objective.id_si),0) AS overall_so
	...
	function data_so($divisi, $start, $end)
	{
		...
		$sql = "SELECT
					...
					COALESCE(so.target_so, 0) AS target,
					COALESCE(so.actual_so, 0) AS actual,
					COALESCE(so.actual_so_type, 'up') AS actual_type,
					COALESCE(so.output, '-') AS output,
					...
					ROUND(COALESCE(so.actual_so_p, 0), 1) AS progress_so_satuan,
					-- CASE 
					-- 	WHEN COALESCE(so.actual_so_type, 'up') = 'up' 
					-- 		THEN CASE 
					-- 			WHEN ROUND(COALESCE(so.actual_so_p, 0), 1) < 70 THEN 'danger'
					-- 			WHEN ROUND(COALESCE(so.actual_so_p, 0), 1) BETWEEN 70 AND 85 THEN 'warning'
					-- 			ELSE 'success'
					-- 		END
					-- 	WHEN COALESCE(so.actual_so_type, 'up') = 'down'
					-- 		THEN CASE 
					-- 			WHEN ROUND(COALESCE(so.actual_so_p, 0), 1) < 50 THEN 'success'
					-- 			WHEN ROUND(COALESCE(so.actual_so_p, 0), 1) BETWEEN 50 AND 70 THEN 'warning'
					-- 			ELSE 'danger'
					-- 		END
					-- END AS warna_so,
					...
					-- 	WHEN COALESCE(so.actual_so_type, 'up') = 'down'
					-- 		THEN CASE 
					-- 			WHEN ROUND(COALESCE(overall_so, 0), 1) < 50 THEN 'success'
					-- 			WHEN ROUND(COALESCE(overall_so, 0), 1) BETWEEN 50 AND 70 THEN 'warning'
					-- 			ELSE 'danger'
					-- 		END
					-- END AS warna
					CASE 
	...
	public function get_project_progress_chart($start = null, $end = null)
{
    if (empty($start) && empty($end)) {
        $start = date('Y-m-01');
        $end   = date('Y-m-t');
    ...
    $sql = "SELECT
                goal.divisi AS divisi,
                COUNT(so.id_so) AS total_project,
                ROUND(AVG(so.actual_so_p), 2) AS avg_progress,  -- rata-rata progress %
                SUM(so.actual_so) AS total_actual
            FROM okr_m_project AS so
            LEFT JOIN okr_m_coorporate AS goal ON so.id_goal = goal.id_goal
            WHERE DATE(so.start) <= '$end' AND DATE(so.end) >= '$start'
            GROUP BY goal.divisi
            ORDER BY FIELD(goal.divisi, 'INFRASTRUKTUR PREMIUM','PRODUK','PELAYANAN','MARKETING')";
	...
	function data_si($divisi, $start, $end)
	{
		...
		$sql = "SELECT
					...
					so.so,
					si.si, 
					si.target_output AS target_output,
					si.target_outcome AS target_outcome,
					...
					COALESCE(si.actual_si_type, 'up') AS actual_type,
					...
					CASE 
						WHEN COALESCE(si.actual_si_type, 'up') = 'up' 
							THEN CASE 
								WHEN ROUND(COALESCE(total_actual_si_p, 0), 1) < 70 THEN 'danger'
								WHEN ROUND(COALESCE(total_actual_si_p, 0), 1) BETWEEN 70 AND 85 THEN 'warning'
								ELSE 'success'
							END
						WHEN COALESCE(si.actual_si_type, 'up') = 'down'
							THEN CASE 
								WHEN ROUND(COALESCE(total_actual_si_p, 0), 1) < 50 THEN 'success'
								WHEN ROUND(COALESCE(total_actual_si_p, 0), 1) BETWEEN 50 AND 70 THEN 'warning'
								ELSE 'danger'
							END
					END AS warna_si,
				...
				FROM
					okr_t_objective AS si
				LEFT JOIN okr_m_project AS so ON si.id_so = so.id_so
				LEFT JOIN okr_m_coorporate AS goal ON so.id_goal = goal.id_goal
				LEFT JOIN okr_t_tasklist AS task ON task.id_si = si.id_si
				...
				LEFT JOIN (
					...
					JOIN hris.okr_t_tasklist task ON FIND_IN_SET(task.id_tasklist,chat.tasklist)
				...
				LEFT JOIN (
					...
					FROM mom_header
					JOIN hris.okr_t_tasklist task ON FIND_IN_SET(task.id_tasklist,mom_header.tasklist)
				...
				LEFT JOIN (
					...
					FROM cm_task
					JOIN hris.okr_t_tasklist task ON FIND_IN_SET(task.id_tasklist,cm_task.tasklist)
				...
				LEFT JOIN (
					...
					FROM gemba
					JOIN hris.okr_t_tasklist task ON FIND_IN_SET(task.id_tasklist,gemba.tasklist)
				...
				LEFT JOIN (
					...
					FROM coaching
					JOIN hris.okr_t_tasklist task ON FIND_IN_SET(task.id_tasklist,coaching.tasklist)
				...
				LEFT JOIN (
					...
					JOIN hris.okr_t_tasklist task ON FIND_IN_SET(task.id_tasklist,td_task.tasklist)
...
SELECT 
    ...
    task.id_si, 
    task.pic, 
    COALESCE(CONCAT(emp.first_name, ' ', emp.last_name), '-') AS nama_pic,
    task.jenis_tasklist, 
    ...
    COALESCE(st.warna, 'bg-secondary') AS warna,
	COALESCE(task.evidence, '-') AS evidence

FROM okr_t_tasklist task
LEFT JOIN xin_employees emp ON emp.user_id = task.pic
LEFT JOIN okr_m_task_status st ON st.id = task.status
where task.status = 3
";
		...
		$query = "SELECT
					...
					FROM
						(((((((
												...
												LEFT JOIN `okr_t_objective` `si` ON ((
														`so`.`id_so` = `si`.`id_so` 
													)))
											LEFT JOIN `okr_t_tasklist` `task` ON ((
													`task`.`id_si` = `si`.`id_si` 
												)))
								...
								LEFT JOIN okr_m_coorporate goal ON (
	...
	public function get_history_det($id)
	{
		$query = "SELECT
					...
					st_before.warna AS st_before_warna
				FROM
					`okr_t_tasklist_milestone_history` his
					JOIN m_pekerjaan_status st ON st.id = his.`status`
					JOIN m_pekerjaan_status st_before ON st_before.id = his.status_before
				WHERE his.id_tasklist = '$id' 
				ORDER BY his.created_at DESC 
	...
	public function data_poin_check_bt($divisi, $start, $end)
	{
		$query = "SELECT
					...
					(
						...
						FROM
							(
								...
								WHERE SUBSTR(periode,1,7) BETWEEN SUBSTR(DATE_ADD('$start', INTERVAL -1 MONTH),1,7) AND SUBSTR(LAST_DAY(DATE_ADD('$start', INTERVAL -1 MONTH)),1,7)
						-- 		WHERE DATE_FORMAT(periode, '%d-%m-%Y') BETWEEN '01-02-2025' AND '10-02-2025'
								AND company = 5
								AND divisi = '$divisi'
				...
				ON SUBSTR(sales.periode,1,7) = SUBSTR(l_sales.periode,1,7)
				LEFT JOIN
					(
						SELECT
							MIN(x.periode) AS periode,
						...
						FROM
							(
								...
								WHERE SUBSTR(periode,1,7) BETWEEN SUBSTR(DATE_ADD('$start', INTERVAL -1 YEAR),1,7) AND SUBSTR(LAST_DAY(DATE_ADD('$start', INTERVAL -1 YEAR)),1,7)
						-- 		WHERE DATE_FORMAT(periode, '%d-%m-%Y') BETWEEN '01-02-2025' AND '10-02-2025'
								AND company = 5
								AND divisi = '$divisi'
				...
				ON SUBSTR(sales.periode,1,4) = SUBSTR(y_sales.periode,1,4)
				LEFT JOIN
					(
						SELECT
							MIN(x.periode) AS periode,
	...
	function kanban_data($divisi = null, $start, $end)
	{
		...
		$query = "SELECT
				...
				FROM
					`okr_t_tasklist` task
					LEFT JOIN okr_m_task_status m_status ON m_status.id = task.`status`
					...
					LEFT JOIN okr_t_objective si ON si.id_si = task.id_si
					WHERE 
...
FROM
	...
	LEFT JOIN okr_m_task_status st ON st.id = so.`status` 
WHERE
	so.id_so = $id_so 
GROUP BY
	id_so";
