...
class Model_grd extends CI_Model
{
	...
	function data_tasklist($divisi, $start, $end)
	{
		...
		$sql = "SELECT
                ...
                WHERE
                    task.divisi = '$divisi'
					" . (!in_array($this->session->userdata('user_id'), [1,61,118,321, 323, 476, 8259]) && !in_array($this->session->userdata('user_role_id'),[2,3,10]) ? ' AND pic.user_id =\'' . $this->session->userdata('user_id').'\'' : '') . " 	
