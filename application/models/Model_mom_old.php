...
class Model_mom extends CI_Model
{
  ...
  {
    if ($closed == 0) {
      $kondisi = "AND created.user_id <> 2041";
    } else {
      $kondisi = "AND tgl BETWEEN '$start' AND '$end'AND created.user_id <> 2041";
    }
    return $this->db->query("SELECT
                              ...
                              mom.agenda,
                              GROUP_CONCAT( peserta.employee_name ) AS peserta,
                              GROUP_CONCAT(
                              CASE
                                  ...
                                  AND peserta.gender = 'Male' THEN
                                    ...
                                    AND peserta.gender = 'Female' THEN
                                      'default_female.jpg' ELSE peserta.profile_picture 
                                    END 
                                    ) AS pp_peserta,
  ...
  function get_list_rekap($start,$end)
  {
    return $this->db->query("SELECT
                              ...
                              COUNT(IF(item.kategori IN (1,6),1,NULL)) AS tasklist,
  ...
  {
    if ($tipe == 1) {
      $kondisi = "AND item.kategori IN (1,6)";
    } else if ($tipe == 2) {
    ...
    return $this->db->query("SELECT
                            ...
                            FROM
                              ...
                              SELECT
                                ...
                                CONCAT( mom.peserta, ',', item.pic ) AS pic,
                                mom.pembahasan,
                                mom.closing_statement 
                              FROM
                                mom_header AS mom
  ...
  function get_issue_item_new($id_mom, $id_issue)
  {
    return $this->db->query("SELECT
                            ...
                            WHERE
                              issue.id_mom = '$id_mom' 
                              AND item.id_issue = '$id_issue' 
                              AND item.kategori IN (1,6)")->result();
  }
