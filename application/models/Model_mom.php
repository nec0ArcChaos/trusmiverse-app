...
class Model_mom extends CI_Model
{
  ...
  function is_owner($id_mom, $user_id)
  {
    $this->db->where('id_mom', $id_mom);
    $this->db->where('created_by', $user_id);
    return $this->db->get('mom_header')->num_rows() > 0;
  }
  ...
  function get_list_mom($start, $end, $closed)
  {
    ...
    $super = [1139, 8892, 8970, 803, 4138, 5086, 5385, 329, 4498, 3651, 2903, 5121, 8204, 2735, 9645, 10214, 10127, 7731, 10404, 11664, 11665]; // Ittrusmi & Super Admin & Viky & Lintang(Sekdir) & Pak Ibnu, Farhan, alfin3333, ratih, sandi2696, od rsp, muhammad5956(hilal), cita7321, moch5508, pujaannicha8053
    ...
    if ($role_id == 1 || in_array($user_id, $super) || in_array($designation_id, $list_designation)) {
      if ($closed == 0) { // Draft
        ...
        if ($user_id != 1 && $user_id != 5121 && $user_id != 803 && $user_id != 323 && $user_id != 8204 && $user_id != 7731) { // Jika Super Admin bukan akun Pak Ibnu & Pak Hendra, & hilal
      ...
      $user_id == 70 || $user_id == 2729
      // || $user_id == 321 
      || $user_id == 7838
    ) { // Nining27, saeroh1425, andre22, hrbatik
      if ($closed == 0) { // Draft
        $kondisi = "AND created.user_id = $user_id";
        ...
        $kondisi_new = "WHERE final.company_id IN (1,4,5) OR final.user_id = $user_id OR FIND_IN_SET($user_id,final.cc)";
        $kondisi_jobdesk = "AND mom.meeting NOT IN ('Jobdesk')";
      }
    ...
    return $this->db->query("SELECT * FROM (SELECT
                              ...
                              -- CONCAT_WS(',', mom.peserta, GROUP_CONCAT(item.pic)) AS list_peserta_pic,
                              -- CONCAT_WS(',', GROUP_CONCAT(peserta.dep_id), GROUP_CONCAT(item.dep_pic)) AS list_dep_pic,
                              -- CONCAT_WS(',', GROUP_CONCAT(peserta.comp_id), GROUP_CONCAT(item.comp_pic)) AS list_comp_pic,
                              mom.cc,
  ...
  function print_issue_item($id_mom, $id_issue)
  {
    return $this->db->query("SELECT
                              ...
                              td_status.`status`,
                              td_status.`label`,
                              COALESCE(td_sub_task.evaluasi, td_task.evaluation) AS evaluasi,
...
GROUP BY
  ...
  function notif_reminder()
  {
    return $this->db->query("SELECT * FROM view_notif_reminder_mom")->result();
  }
}
