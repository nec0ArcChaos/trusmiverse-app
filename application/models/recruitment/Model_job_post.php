...
class Model_job_post extends CI_Model
{
	...
	function get_job_question(){
		$query = "SELECT * FROM trusmi_jobs_question WHERE is_active = '1'";
	...
	function dt_list_permintaan($job_id = null, $start = null, $end = null)
	{
		...
		$query = "SELECT
			...
			trusmi_jobs_request.question
		FROM
			trusmi_jobs_request
			-- LEFT JOIN trusmi_jobs_question qt ON FIND_IN_SET(trusmi_jobs_request.question,qt.id)
	...
	function get_pic_recruiter()
	{
		$query = "SELECT
				...
				(SELECT
					...
					FROM xin_employees 
					WHERE (user_role_id = 11 OR user_id = 9398)
					AND is_active = 1
