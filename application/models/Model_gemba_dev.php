...
class Model_gemba_dev extends CI_Model
{
    ...
    function get_tipe()
    {
        ...
        if (in_array($this->session->userdata('user_id'), [1, 476, 9909, 1139])) {
...
FROM
  gemba
  --   JOIN (SELECT id_gemba, status FROM gemba_item GROUP BY id_gemba) AS item ON item.id_gemba = gemba.id_gemba
...
WHERE
  EXISTS (SELECT 1 FROM gemba_item gi WHERE gi.id_gemba = gemba.id_gemba AND gi.STATUS IS NULL)
--   AND gemba.cek IS NULL
  $kondisi")->result();
    }
...
WHERE
  ...
  AND DATE(gemba.created_at) > '2026-02-01'
    ...
    GROUP_CONCAT(foto.file ORDER BY foto.file SEPARATOR ',') AS foto_temuan
FROM gemba
JOIN gemba_item AS item 
    ON item.id_gemba = gemba.id_gemba
JOIN gemba_ceklis AS ceklis 
    ON ceklis.id = item.id_gemba_ceklis
LEFT JOIN gemba_foto AS foto
    ON foto.id_gemba = item.id_gemba
    AND foto.id_gemba_ceklis = item.id_gemba_ceklis
WHERE
    gemba.id_gemba = ?
...
GROUP BY
    ...
    public function get_project_location()
    {
        return $this->db->query("
            SELECT
                id_project,
                ...
                latitude,
                longitude
            FROM rsp_project_live.m_project
    ...
    function list_gemba($start, $end)
    {
        ...
        $list_super = [1139, 8892, 8970, 70, 2, 5197, 2951, 5086, 5385, 2729, 68, 5286, 3651, 476, 9645, 4498, 7731, 11664, 11665]; // Viky, Lintang, Nining27, Ali95, hience3160, alfin3333, saeroh, Farhan63,farid3241, Dimas Nurullah
        ...
        if ($user_id == 11372) { // soniyanto8949
            $kondisi = "AND (gemba.created_by = $user_id OR gemba.created_by IN (9621, 10176, 10168))";
        }
  ...
  gemba.peserta,
  gemba.created_at,
  gemba.created_by AS created_by_id,
  CONCAT(em.first_name, ' ', em.last_name) AS created_by,
  gemba.updated_at,
  CONCAT(up.first_name, ' ', up.last_name) AS updated_by,
...
FROM
  gemba
  JOIN (SELECT id_gemba, COUNT(*) AS total_progres FROM gemba_item GROUP BY id_gemba) AS item ON item.id_gemba = gemba.id_gemba
...
--   JOIN td_status_strategy AS sts ON sts.id = gemba.`status`
    ...
    function get_result_gemba($id_gemba)
    {
        ...
        SELECT
            ...
            GROUP_CONCAT(gemba_foto.file) AS file,
        ...
        JOIN gemba_ceklis cek ON cek.id = item.id_gemba_ceklis
        LEFT JOIN gemba_foto 
            ON gemba_foto.id_gemba = item.id_gemba 
            AND gemba_foto.id_gemba_ceklis = item.id_gemba_ceklis
        LEFT JOIN xin_employees up ON up.user_id = item.updated_by
        LEFT JOIN user u ON u.id_user = item.pic
        WHERE item.id_gemba = '$id_gemba'
        GROUP BY item.id_gemba, item.id_gemba_ceklis
    ")->result();
    }
    ...
    // Tambahan: update tgl_plan (hanya jika created_by = user login)
    function update_tgl_plan($id_gemba, $tgl_plan, $user_id)
    {
        $this->db->where('id_gemba', $id_gemba);
        $this->db->where('created_by', $user_id);
        return $this->db->update('gemba', [
            'tgl_plan' => $tgl_plan,
    ...
    // Tambahan: hapus gemba beserta item & foto (hanya jika created_by = user login)
    function delete_gemba($id_gemba, $user_id)
    {
        $gemba = $this->db->select('id_gemba')
            ->from('gemba')
        ...
        $this->db->where('id_gemba', $id_gemba);
        $this->db->delete('gemba_item');

        $this->db->where('id_gemba', $id_gemba);
        $this->db->where('created_by', $user_id);
        $this->db->delete('gemba');
