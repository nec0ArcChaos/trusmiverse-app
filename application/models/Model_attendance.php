...
class Model_attendance extends CI_Model
{
    ...
    function dt_attendance($company_id, $department_id, $employee_id, $start, $end){
        ...
        // }else
        if ($_SESSION['user_id'] == 1139) {
            $kondisi = "";
        } else if($employee_id != '0'){ // 0: All
            if($this->session->userdata('user_id') == '2535'){ // 0: nani
                $kondisi = "AND hris.xin_employees.department_id = 120";
            } else if($this->session->userdata('user_id') == '1709'){ // 0: nani
                $kondisi = "AND hris.xin_employees.department_id = 186";
            } else {
    ...
    {
        if ($this->session->userdata('user_id') == 8446) {
            $query = "SELECT
