...
class Model_wajs_notif extends CI_Model
{
    ...
    {
        $query = "SELECT user_id, CONCAT(first_name, ' ', last_name) AS full_name, COALESCE(wa_notif_reg, 0) AS registered
    ...
    function get_error_executions($from, $to)
    {
        $this->db->where('status', 'pending');
        $this->db->where('created_at >=', $from);
        $this->db->where('created_at <=', $to);
        $query = $this->db->get('wa_notif_log');
        return $query->result();
    }
