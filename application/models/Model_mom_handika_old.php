...
class Model_mom_handika_old extends CI_Model
{
  ...
  function get_list_mom($start, $end, $closed)
  {
    ...
    if ($role_id == 1 || in_array($user_id, $super) || in_array($designation_id, $list_designation)) {
      if ($closed == 0) { // Draft
        ...
        if ($user_id != 803 && $user_id != 323 && $user_id != 8204) { // Jika Super Admin bukan akun Pak Ibnu & Pak Hendra, & hilal
    ...
    return $this->db->query("SELECT
                            ...
                            FROM
                              ...
                              LEFT JOIN (SELECT
                                ...
                                CEIL((DAY(all_dates.date) + WEEKDAY(DATE_FORMAT('2025-02-01', '%Y-%m-%d'))) / 7) AS `week`
                            FROM
                                all_dates 
                            WHERE
                                WEEKDAY( all_dates.date ) = 6 
                                AND SUBSTR( all_dates.date, 1, 7 ) = '2025-02') AS weekly ON item.deadline BETWEEN weekly.start_mom AND weekly.end_mom
                            WHERE
                              mom.closed = 1
  ...
  function print_issue_eskalasi_item($id_mom, $id_issue)
  {
    return $this->db->query("SELECT 
	issue.id_mom,
    issue.topik,
...
LEFT JOIN mom_issue AS issue ON issue.id_mom = item.id_mom AND item.id_issue = issue.id_issue
...
LEFT JOIN td_sub_task_history AS hs ON hs.id_sub_task = item.id_sub_task AND hs.progress > 0
WHERE 
	issue.id_mom = '$id_mom' AND item.id_issue = '$id_issue'
GROUP BY id_task")->result();
  }
  ...
  function get_grd_si()
  {
    return $this->db->query("SELECT
                                  si.id_si,
                                  so.id_so,
                                  CONCAT(si.si,' | ',so.so,' | ',so.divisi) AS grdsi
                                FROM
                                  grd_t_si AS si
...
WHERE
	...
	AND DATE( momi.verified_at ) > '2025-03-13'
