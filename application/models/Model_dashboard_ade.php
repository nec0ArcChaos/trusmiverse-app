...
class Model_dashboard_ade extends CI_Model
{
    ...
    public function personal_info()
    {
        ...
        $query = "SELECT
			...
			IF(e.profile_background = '', 'bg-10.jpg', COALESCE(e.profile_background,'bg-10.jpg')) AS profile_background,
			e.email,
			e.company_id,
    ...
    public function detail_absen($periode,  $employee_id, $designation_id = null, $cutoff) // detdev addnew cutof
    {
        ...
        if ($cutoff == 1) { // 21-20
            if ($month == date("m") && date("d") <= 20) {
                ...
                $start          = date("Y-m-21", strtotime("-2 month", strtotime(date($periode . '-t'))));
            } else {
                $start          = date("Y-m-21", strtotime("-1 month", strtotime(date($periode . '-t'))));
            }
            $end            = date($periode . "-20");
            ...
            if ($month == date("m") && date("d") <= 20) {
                ...
                $start          = date("Y-m-16", strtotime("-2 month", strtotime(date($periode . '-t'))));
            } else {
                $start          = date("Y-m-16", strtotime("-1 month", strtotime(date($periode . '-t'))));
            }
            $end            = date($periode . "-15");
