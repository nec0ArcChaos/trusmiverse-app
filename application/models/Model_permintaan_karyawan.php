...
class Model_permintaan_karyawan extends CI_Model
{
    ...
    public function list_permintaan($start, $end, $status, $user_id = null)
    {
		...
		FROM
			...
			JOIN xin_companies ON xin_companies.company_id = xin_users.company_id
    ...
    {
        $query = "SELECT xu.user_id, xu.company_id, xc.`name` AS company_name
                    FROM xin_users xu
                    JOIN xin_companies xc ON xc.company_id = xu.company_id
                    WHERE xu.is_active = 1;";
        ...
        return $this->db->query("SELECT
								...
								FROM
									trusmi_jobs_request AS job
											...
											JOIN xin_companies AS xc ON xc.company_id = comp.company_id
