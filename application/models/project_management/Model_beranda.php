...
class Model_beranda extends CI_Model
{
    ...
    {
        // FIX: set updated_at saat insert supaya tracking langsung aktif
        $data['updated_at'] = date('Y-m-d H:i:s');
        $this->db->insert('t_pm_projects', $data);
        return $this->db->insert_id();
    }

    // FIX: update_project selalu set updated_at = NOW() supaya conflict detection akurat
    ...
    {
        // FIX: set updated_at saat insert
        $data['updated_at'] = date('Y-m-d H:i:s');
        $this->db->insert('t_pm_tasklist', $data);
        return $this->db->insert_id();
    }

    // FIX: update_task selalu set updated_at = NOW()
    ...
    // ─────────────────────────────────────────────────────────────────────
    // FIX: method baru — cek apakah record sudah diubah user lain
    //      Mengembalikan true jika konflik (updated_at di DB lebih baru dari yang dikirim frontend)
    // ─────────────────────────────────────────────────────────────────────
    public function check_conflict($id, $type, $client_updated_at)
    {
        $table = ($type == 'project') ? 't_pm_projects' : 't_pm_tasklist';
        $row   = $this->db->select('updated_at')->where('id', $id)->get($table)->row();

        if (!$row || empty($row->updated_at) || empty($client_updated_at)) {
            return false; // Tidak bisa cek, lewati saja
        }

        // Konflik jika DB lebih baru dari snapshot yang dikirim frontend
        return strtotime($row->updated_at) > strtotime($client_updated_at);
    }

    // ─────────────────────────────────────────────────────────────────────
    // FIX: method baru — ambil updated_at terbaru setelah save
    //      Dikirim kembali ke frontend supaya snapshot-nya selalu fresh
    // ─────────────────────────────────────────────────────────────────────
    public function get_updated_at($id, $type)
    {
        $table = ($type == 'project') ? 't_pm_projects' : 't_pm_tasklist';
        $row   = $this->db->select('updated_at')->where('id', $id)->get($table)->row();
    ...
    public function delete_item($id, $type)
    {
        ...
        if ($type == 'project') {
            $this->db->where('id', $id)->delete('t_pm_projects');
            $this->db->where('project_id', $id)->delete('t_pm_tasklist');
        } else {
            $this->db->where('id', $id)->delete('t_pm_tasklist');
        }
    ...
    {
        $res = $this->db->select('company_id')->from('xin_companies')->where('alias', $name)->get()->row();
        return $res ? $res->company_id : null;
    }

    public function resolve_status_id($name)
    {
        $res = $this->db->select('id')->from('m_pm_status')->where('status_name', $name)->get()->row();
        return $res ? $res->id : null;
    }

    public function resolve_category_id($name)
    {
        $res = $this->db->select('id')->from('m_pm_categories')->where('category', $name)->get()->row();
        return $res ? $res->id : null;
    }
    ...
    public function get_lookup_options()
    {
        return [
            ...
            'employees'      => $this->db->select('user_id, first_name, last_name, profile_picture')
                ->group_start()->where('designation_id IN (405,459,570,984,1431)')->or_where('department_id IN (68)')->group_end()
                ->where('is_active', 1)->get('xin_employees')->result_array(),
            'product_owners' => $this->db->select('user_id, first_name, last_name, profile_picture')
                ->where('user_role_id > 2')->where('user_role_id IS NOT NULL')->where('is_active', 1)
    ...
    public function insert_progress_history($data)
    {
        return $this->db->insert('t_pm_progress_history', $data);
    }
    ...
    {
        $this->db->select('id, start_date, deadline, end_date, status, week');
        ...
        foreach ($tasks as $t) {
            ...
            if (!empty($t['deadline'])) {
                $wk = !empty($t['week']) ? $t['week'] : $this->_calculate_week_num($t['deadline']);
                if (!isset($stats['weeks'][$wk])) {
                    $stats['weeks'][$wk] = ['total' => 0, 'done' => 0];
                }
                $stats['weeks'][$wk]['total']++;
                if ($st === 3) $stats['weeks'][$wk]['done']++;
            }
        }
