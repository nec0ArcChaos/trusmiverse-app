...
class Model_mom_lutfi extends CI_Model
{
  ...
  function get_list_rekap($start, $end)
  {
    ...
    return $this->db->query("SELECT
                            ...
                            WHERE
                              mom.closed = 1 AND mom.meeting NOT IN ('Jobdesk')
  ...
  function get_detail_rekap($start, $end, $user, $tipe)
  {
    ...
    return $this->db->query("SELECT
                              ...
                              CASE
                                WHEN item.deadline BETWEEN '2024-01-05' AND CURDATE() AND COALESCE(sub.progress,0) < 100 AND ibr.`status` < 3 AND item.verified_status IS NULL  THEN
                                  'Locked'
  ...
  function print_issue_item($id_mom, $id_issue)
  {
    return $this->db->query("SELECT
                            ...
                            FROM
                              ...
                              LEFT JOIN td_sub_task_history AS hs ON hs.id_sub_task = item.id_sub_task AND hs.progress IN ('0', '100')
                            WHERE
                              issue.id_mom = '$id_mom'
