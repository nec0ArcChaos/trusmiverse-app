...
class Model_api_sp3k extends CI_Model
{
                        ...
                        FROM
                            (
                                SELECT
                                    ...
                                    inv.id_inv,
                                    `int`.tgl_sp3k,
                                    COALESCE(`int`.jadwal_sp3k_1,`int`.jadwal_sp3k_2) AS jadwal_sp3k,
                                    ...
                                    gci.id_konsumen,
                                    CASE 
                                        WHEN gci.manager IN (19,167) THEN 18
                                        WHEN gci.id_gm IN (238) THEN 838
                                        WHEN gci.manager IN (106,111) THEN gci.manager
                                    ELSE gci.id_gm
                                    END AS id_gm,
                                    gci.id_project,
                                    gci.blok,
                                ...
                                WHERE LEFT(COALESCE(`int`.tgl_sp3k,COALESCE(`int`.jadwal_sp3k_1, `int`.jadwal_sp3k_2)), 7) = '$periode'
                                -- AND (gci.manager IN (19,167,111,106)
                                -- OR (gci.id_gm IN (18,238,838,24099)))
                                ORDER BY inv.id_inv DESC
                            ) AS x
                        LEFT JOIN rsp_project_live.`user` AS gm ON gm.id_user = x.id_gm
                    ) AS berkas

                LEFT JOIN
                    (
                        SELECT
                            LEFT(COALESCE(jadwal_sp3k_1, jadwal_sp3k_2), 7) AS periode,
                            COUNT(id_int) AS `target`
                        FROM 
                            rsp_project_live.t_interview
                        WHERE 
                            LEFT(COALESCE(jadwal_sp3k_1, jadwal_sp3k_2), 7) = '$periode'
                        GROUP BY 
                            LEFT(COALESCE(jadwal_sp3k_1, jadwal_sp3k_2), 7)
    ...
    function kpi_rm($id_rm, $periode)
    {
        ...
        $query = "SELECT
                    ...
                    COALESCE(CASE 
                        WHEN ROUND( ((COUNT(IF(berkas.hasil_inv = 1, 1, NULL)) / tgt.target) * 100), 2) >= 100 THEN 100
                    ELSE ROUND( ((COUNT(IF(berkas.hasil_inv = 1, 1, NULL)) / tgt.target) * 100), 2)
                    END,0) AS achieve_persentase,

                    COUNT(IF(berkas.hasil_inv = 1 AND berkas.sts_ld = 'Ontime', 1, NULL)) AS hasil_ontime,
                    COALESCE(ROUND( ((COUNT(IF(berkas.hasil_inv = 1 AND berkas.sts_ld = 'Ontime', 1, NULL)) / COUNT(IF(berkas.hasil_inv = 1, 1, NULL))) * 100), 2),0) AS persentase_ontime,
                    
                    COUNT(IF(berkas.hasil_inv = 1 AND berkas.sts_ld = 'Late', 1, NULL)) AS hasil_late,
                    COALESCE(ROUND( ((COUNT(IF(berkas.hasil_inv = 1 AND berkas.sts_ld = 'Late', 1, NULL)) / COUNT(IF(berkas.hasil_inv = 1, 1, NULL))) * 100), 2),0) AS persentase_late
    ...
    function data_berkas($id_rm, $periode)
    {
        ...
        $query = "SELECT
                    ...
                    COALESCE(COUNT(berkas.id_bic),0) AS jml_berkas,
                    ...
                    COALESCE(COUNT(IF(berkas.hasil_inv = 1 AND berkas.sts_ld = 'Ontime', 1, NULL)),0) AS ld_lengkap_ontime,
                    ROUND( ((COUNT(IF(berkas.hasil_inv = 1 AND berkas.sts_ld = 'Ontime', 1, NULL)) / COUNT(berkas.id_inv)) * 100), 2) AS p_ld_lengkap_ontime,
                    
                    COALESCE(COUNT(IF(berkas.hasil_inv = 1 AND berkas.sts_ld = 'Late', 1, NULL)),0) AS ld_lengkap_late,
    ...
    function data_dp_customer($id_rm, $periode)
    {
        ...
        $query = "SELECT
                    ...
                    COUNT(dp.id_bic) AS jml_data,
                    
                    COUNT(IF( (dp.sts_sisa_ar = 1 AND dp.hasil_inv = 1), 1, NULL)) AS jml_ar_lunas,
                    ROUND( ((COUNT(IF( (dp.sts_sisa_ar = 1 AND dp.hasil_inv = 1), 1, NULL)) / COUNT(dp.id_bic)) * 100), 2) AS p_jml_ar_lunas,
                    
                    COUNT(IF( (dp.sts_sisa_ar = 2), 1, NULL)) AS jml_ar_blunas,
                    ROUND( ((COUNT(IF( (dp.sts_sisa_ar = 2), 1, NULL)) / COUNT(dp.id_bic)) * 100), 2) AS p_jml_ar_blunas
                ...
                FROM
                    (
                        ...
                        LEFT JOIN rsp_project_live.t_gci AS gci ON bic.id_gci = gci.id_gci
                        LEFT JOIN rsp_project_live.t_inventory AS inv ON bic.id_bic = inv.id_bic
                        LEFT JOIN rsp_project_live.`view_spr` AS v_spr ON v_spr.id_gci= bic.id_gci
                        LEFT JOIN rsp_project_live.m_project AS mp ON mp.id_project = gci.id_project
                        LEFT JOIN rsp_project_live.user AS marketing ON marketing.id_user = gci.created_by
                ...
                LEFT JOIN rsp_project_live.`user` AS gm ON gm.id_user = dp.id_gm
                WHERE dp.id_gm = '$id_rm'
                GROUP BY dp.id_gm
                ";
                ...
                FROM
                    (
                        SELECT
                            ...
                            -- CASE 
                            --     WHEN a.manager IN (19,167) THEN 18
                            --     WHEN a.id_gm IN (238) THEN 838
                            --     WHEN a.manager IN (106,111) THEN a.manager
                            -- ELSE a.id_gm
                            -- END AS id_gm,

                            CASE
                                WHEN a.manager IN (838,2029) AND a.id_gm = 838 THEN 838
                                WHEN a.manager IN (19,24327,167,2003) AND a.id_gm = 18 THEN 18
                                WHEN a.manager IN (111) AND a.id_gm = 18 THEN 111
                                WHEN a.manager IN (106) AND a.id_gm = 18 THEN 106
                                WHEN a.manager IN (2029) AND a.id_gm = 24099 THEN 24099
                            END AS id_gm,
                        ...
                        AND (a.manager IN (19,167,111,106)
                        OR (a.id_gm IN (18,238,838,24099)))
                    ) AS profilling
                LEFT JOIN rsp_project_live.`user` AS gm ON gm.id_user = profilling.id_gm
                WHERE profilling.id_gm = '$id_rm'
                        ...
                        FROM
                            (
                                SELECT
                                    ...
                                    CASE
                                        WHEN gci.manager IN (838,2029) AND gci.id_gm = 838 THEN 838
                                        WHEN gci.manager IN (19,24327,167,2003) AND gci.id_gm = 18 THEN 18
                                        WHEN gci.manager IN (111) AND gci.id_gm = 18 THEN 111
                                        WHEN gci.manager IN (106) AND gci.id_gm = 18 THEN 106
                                        WHEN gci.manager IN (2029) AND gci.id_gm = 24099 THEN 24099
                                    END AS id_gm,
                                ...
                                WHERE SUBSTR(t_bic.created_at, 1, 7) = '$periode'
                                AND t_bic.hasil_bic IN (1,2)
                    --               AND inv.hasil_inv = 1
    ...
    function data_bangun_30($id_rm, $periode)
    {
        ...
        $query = "SELECT
                ...
                FROM
                    (
                        SELECT
                            ...
                            CASE
                                WHEN t_gci.manager IN (838,2029) AND t_gci.id_gm = 838 THEN 838
                                WHEN t_gci.manager IN (19,24327,167,2003) AND t_gci.id_gm = 18 THEN 18
                                WHEN t_gci.manager IN (111) AND t_gci.id_gm = 18 THEN 111
                                WHEN t_gci.manager IN (106) AND t_gci.id_gm = 18 THEN 106
                                WHEN t_gci.manager IN (2029) AND t_gci.id_gm = 24099 THEN 24099
                            END AS id_gm,
                        ...
                        LEFT JOIN rsp_project_live.t_bic AS t_bic ON t_bic.id_gci = t_gci.id_gci
                ...
                LEFT JOIN rsp_project_live.`user` AS gm ON gm.id_user = bangun.id_gm
                WHERE bangun.id_gm = '$id_rm'
