...
class Model_api_akad extends CI_Model
{
        ...
        $query = "SELECT
                  ...
                  FROM
                    (
                      ...
                      FROM rsp_project_live.t_akad AS intv
                      ...
                      WHERE intv.hasil_akad = 1
                      AND LEFT(intv.jadwal_akad, 7) = '$periode'
                      -- AND tgci.id_gm IN (61, 18, 238, 23609)
                    ) AS akad
                  LEFT JOIN
                    (
                      SELECT
    ...
    function kpi_rm($id_rm, $periode)
    {
        ...
        $query = "SELECT
                    ...
                    COALESCE(COUNT(akad.hasil_akad),0) AS actual,
                    ROUND(100, 1) as target_persentase,
                    COALESCE(ROUND(COALESCE(COUNT(akad.hasil_akad),0) / COALESCE(tgt.target,1) * 100, 1), 0) AS achieve_persentase,
                    COALESCE(COUNT(IF(akad.leadtime < 16 AND akad.hasil_akad IS NOT NULL, 1, NULL)),0) AS hasil_ontime,
                    COALESCE(COUNT(IF(akad.leadtime > 15 AND akad.hasil_akad IS NOT NULL, 1, NULL)),0) AS hasil_late,
                    COALESCE(ROUND(
                      COUNT(IF(akad.leadtime < 16 AND akad.hasil_akad IS NOT NULL, 1, NULL)) / COALESCE(tgt.target,1) * 100,
                      1
                    ),0) AS persentase_ontime,
                    COALESCE(ROUND(
                      COUNT(IF(akad.leadtime > 15 AND akad.hasil_akad IS NOT NULL, 1, NULL)) / COALESCE(tgt.target,1) * 100,
                      1
                    ),0) AS persentase_late
                  FROM
                    (
                      ...
                      FROM
                        (
                          ...
                          LEFT JOIN rsp_project_live.t_spk tspk ON tspk.id_spk = intv.id_spk
                          JOIN rsp_project_live.t_gci tgci ON tgci.id_gci = tspk.id_gci
                          LEFT JOIN rsp_project_live.`user` AS mkt ON mkt.id_user = tgci.id_user
                          LEFT JOIN rsp_project_live.t_lpa lpa ON lpa.project = tgci.id_project
                          AND lpa.blok = tgci.blok
                          WHERE intv.hasil_akad = 1
                          AND LEFT(intv.jadwal_akad, 7) = '$periode'
                  ...
                  FROM
                    (
                      ...
                      SELECT
                        ...
                        k.nama_konsumen,
                        CASE
                          WHEN sales.id_manager IN (838,2029) AND sales.id_gm = 838 THEN 838
                          WHEN sales.id_manager IN (19,24327,167,2003) AND sales.id_gm = 18 THEN 18
                          WHEN sales.id_manager IN (111) AND sales.id_gm = 18 THEN 111
                          WHEN sales.id_manager IN (106) AND sales.id_gm = 18 THEN 106
                          WHEN sales.id_manager IN (2029) AND sales.id_gm = 24099 THEN 24099
                        END AS id_gm,
                      ...
                      ON kalendar.tgl BETWEEN DATE(b.tgl_bic) AND DATE(COALESCE(i.tgl_input_berkas,CURRENT_DATE))
                      LEFT JOIN
                        (
                          SELECT 
                            tgl,
                      ...
                      ON kalendar_berkas.tgl BETWEEN DATE(b.tgl_bic) AND DATE(COALESCE(COALESCE(inv.updated_at,inv.created_at),CURRENT_DATE))
    ...
    function jumlah_sp3k($id_rm, $periode)
    {
        $query = "SELECT
                    ...
                    ROUND(100, 1) AS target_persentase,
                    sp3k.id_gm,
                    COALESCE(COUNT(sp3k.actual),0) AS actual,
                    COALESCE(ROUND(COALESCE(COUNT(sp3k.actual),0) / COALESCE(tgt.target,1) * 100), 0) AS achieve_persentase
                  FROM
                    (
                      SELECT
                        x.actual,
                      ...
                      FROM
                        (
                          SELECT
                            ROUND(100, 1) as target_persentase,
                            COALESCE((intv.hasil_int),0) AS actual,
                            CASE
                              WHEN mkt.id_manager IN (838,2029) AND mkt.id_gm = 838 THEN 838
                              WHEN mkt.id_manager IN (19,24327,167,2003) AND mkt.id_gm = 18 THEN 18
                              WHEN mkt.id_manager IN (111) AND mkt.id_gm = 18 THEN 111
                              ...
                              WHEN mkt.id_manager IN (2029) AND mkt.id_gm = 24099 THEN 24099
                            END AS id_gm
                          FROM rsp_project_live.t_interview AS intv
                    ...
                    (
                      ...
                      FROM
                        (
                          ...
                          LEFT JOIN rsp_project_live.`user` AS mkt ON mkt.id_user = t_gci.id_user
                          ...
                          WHERE SUBSTR(intv.tgl_sp3k, 1, 7) = '$periode'
                        ...
                        (
                          SELECT
                            ...
                            CASE
                              ...
                              WHEN mkt.id_manager IN (2029) AND mkt.id_gm = 24099 THEN 24099
                            END AS id_gm
                          FROM rsp_project_live.view_spr v_spr
                          ...
                          LEFT JOIN rsp_project_live.`user` AS mkt ON mkt.id_user = tgci.id_user
                          WHERE LEFT(intv.tgl_sp3k, 7) = '$periode'
                        ...
                        (
                          SELECT
                            ...
                            CASE
                              ...
                              WHEN mkt.id_manager IN (2029) AND mkt.id_gm = 24099 THEN 24099
                            END AS id_gm
                          FROM rsp_project_live.t_gci AS tgci
