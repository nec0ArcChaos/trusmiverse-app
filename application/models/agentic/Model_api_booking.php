...
class Model_api_booking extends CI_Model
{
    ...
    function produktivitas_mpp($id_gm, $periode)
    {
        ...
        $query = "SELECT 
                    ...
                    ROUND(COALESCE(SUM(rm.booking) / COUNT(IF(rm.booking > 0, 1, NULL)),0), 1) AS prod,
                    ROUND((COUNT(IF(rm.booking > 0, 1, NULL)) / COUNT(rm.gm_fix)) * 100) AS persen_produktivitas
                FROM
                    (
                        SELECT
                            mkt.join_hr,
    ...
    function rasio_follow_up_database($id_gm, $periode)
    {
        ...
        $query = "SELECT
                    gci_fu.id_gm,
                    COALESCE(COUNT(gci_fu.id_gci),0) AS total_database,
                    COALESCE(COUNT(gci_fu.total_fu),0) AS total_fu,
                    gci_fu.persen_target,
                    COALESCE(ROUND((COUNT(gci_fu.total_fu) / COUNT(gci_fu.id_gci)) * 100),0) AS persen_achieve
                FROM
                    (
                        SELECT
    ...
    function rasio_hot_prospect_ke_leads($id_gm, $periode)
    {
        ...
        $query = "SELECT
                    COALESCE(COUNT(fu_mm.id_gci),0) AS total_hot_prospect,
                    COALESCE(COUNT(fu_mm.total_leads),0) AS total_leads,
                    COALESCE(ROUND( (COUNT(fu_mm.total_leads) / COUNT(fu_mm.id_gci)) * 100),0) persen_achieve,
                    100 AS persen_target
                FROM
                    (
                        SELECT
    ...
    function rasio_leads_ke_booking($id_gm, $periode)
    {
        ...
        $query = "SELECT
                    ...
                    COALESCE(ROUND( (COUNT(ghis.total_booking) / COUNT(ghis.id_gci)) * 100 ),0) AS persen_achieve,
                    ghis.persen_target
                FROM
                    (  
                        SELECT
    ...
    function produktivitas_grab_posting($id_gm, $periode)
    {
        ...
        $query = "SELECT
                    ...
                    COALESCE(SUM(gci_ps.total_posting),0) AS total_postingan_ke_sosmed,
                    COALESCE(ROUND(AVG(gci_ps.total_posting)),0) AS average_posting_per_sales,
                    COALESCE(ROUND( (COUNT(gci_ps.total_user) / COUNT(gci_ps.id_user) ) * 100 ),0) AS persen_achieve,
                    100 AS persen_target
                FROM
                    (
                        SELECT
    ...
    function keterlaksanaan_canvasing($id_gm, $periode)
    {
        ...
        $query = "SELECT
                    COALESCE(COUNT(canvasing.id_activity),0) AS total_canvasing_spv,
                    COALESCE(COUNT(IF(canvasing.ach = 1, 1, NULL)),0) AS total_canvasing_sesuai_target_database,
                    COALESCE(COUNT(IF(canvasing.ach = 0, 1, NULL)),0) AS total_canvasing_tidak_sesuai_target_database,
                    COALESCE(ROUND((COUNT(IF(canvasing.ach = 1, 1, NULL)) / COUNT(canvasing.id_activity)) * 100),0) AS persen_achieve,
                    100 AS persen_target
                FROM
                    (
                        SELECT
