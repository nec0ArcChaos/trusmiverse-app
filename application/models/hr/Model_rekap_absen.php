...
class Model_rekap_absen extends CI_Model
{
    ...
    public function get_department($company_id)
    {
        ...
        UNION SELECT
        'grade_sales' AS `value`,
        'Grade Sales (RSP)' AS `text`
        FROM
        xin_designations
        WHERE xin_designations.designation_id = 731";
        } else {
            $union = "UNION SELECT
            xin_departments.department_id AS `value`,
            CONCAT(xin_departments.department_name, ' (Bali)') AS `text` 
    ...
    public function get_employees($company_id, $department_id)
    {
        ...
        if ($department_id == 'grade_sales') {
            $where = "WHERE xin_employees.designation_id = 731 AND xin_employees.is_active = 1 AND xin_employees.ctm_grade IS NOT NULL AND xin_employees.ctm_grade != ''";
        } else {
            $where = "WHERE
    ...
    {
        if ($department_id == 'grade_sales') {
            $kondisi = "xin_employees.designation_id = 731 AND xin_employees.ctm_grade IS NOT NULL AND xin_employees.ctm_grade != ''";
        } else if ($this->session->userdata('user_id') == 1426) {
            $kondisi = "xin_employees.designation_id IN (509,810,1241)";
        } else {
            $kondisi = "xin_employees.department_id = $department_id";
        }
        $query = "SELECT
            TRIM(TRAILING ',' FROM GROUP_CONCAT(x.user_id SEPARATOR ',')) AS id 
        FROM
            (
            SELECT
    ...
    public function get_absensi($start, $end, $company_id, $department_id, $employee_id, $sub_emp, $periode = NULL, $cutoff_id)
    {
        $bali = array(175, 177, 180);
        ...
        if (in_array($department_id, $bali)) {
            ...
            if ($department_id == 0 && $department_id != 'grade_sales') {
                $company = "xin_employees.company_id = $company_id";
            } else {
                if ($sub_emp != 0 && $department_id != 'grade_sales') {
                    $company = "xin_employees.company_id = $company_id";
                } else {
        ...
        if ($department_id == 'grade_sales') {
            $department = "xin_employees.designation_id = 731 AND xin_employees.ctm_grade IS NOT NULL AND xin_employees.ctm_grade != '' AND xin_employees.is_active = 1";
        ...
        // addnew
        // if ($cutoff_id == 1) {
        $cutoff = "AND xin_employees.ctm_cutoff = $cutoff_id";
        // }
        ...
        FROM
            (SELECT
                    ...
                    CASE
                        ...
                        WHEN xin_leave_applications.leave_id IS NOT NULL AND xin_leave_applications.leave_type_id IN (7,24) THEN
                            CASE
                        ...
                        WHEN (CASE
                                ...
                                -- WHEN xin_attendance_time.clock_in IS NOT NULL AND (xin_employees.office_shift_id = 170 OR xin_designations.trusmi_shift = 35) THEN 720
                                ELSE NULL 
                            END) < TIMESTAMPDIFF(MINUTE, xin_attendance_time.clock_in, xin_attendance_time.clock_out) THEN 'P'
    ...
    public function get_absensi_resume($start, $end, $company_id, $department_id, $employee_id, $sub_emp, $periode = NULL, $cutoff_id)
    {
        $bali = array(175, 177, 180);
