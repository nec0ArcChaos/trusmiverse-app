...
class Model_rekap_absen extends CI_Model
{
    ...
    function get_sub_emp($department_id, $posisi)
    {
        ...
        $query = "SELECT
            GROUP_CONCAT(xin_employees.user_id) AS id
        FROM
            xin_employees 
        WHERE
            $kondisi 
            ...
            AND xin_employees.ctm_posisi IN ( $posisi )";
    ...
    public function get_absensi($start, $end, $company_id, $department_id, $employee_id, $sub_emp)
    {
        if ($department_id == 0) {
            $department = "";
        } else {
        ...
        $query = "SELECT
            ...
            SUM(IF(rekap.`status` = 'T' OR rekap.`status` = 'T', rekap.telat, 0)) + SUM(IF(rekap.`status` = 'NP' OR rekap.`status` = 'T', rekap.telat_pc, 0)) AS menit_telat,
            -- off
            ...
            (SELECT
                ...
                WHERE
                    ...
                    AND (
                        ...
                        AND
                        IF
                            ( xin_employees.date_of_leaving = '' OR xin_employees.date_of_leaving IS NULL, '$end' + INTERVAL 1 DAY, xin_employees.date_of_leaving ) BETWEEN '$start' 
                            AND '$end' 
                        ) 
                    )
