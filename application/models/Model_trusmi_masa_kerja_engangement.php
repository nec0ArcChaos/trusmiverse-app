...
class Model_trusmi_masa_kerja_engangement extends CI_Model
{
	...
	{
        // $list_head = $this->db->query("SELECT
        ...
        //                             GROUP BY
        //                                 xin_departments.head_id")->result();
        ...
        // if($user_id == '1' || $user_id == '979' || $user_id == '323'){ // 979 : personalia, 323 : pak hendra
        //     $kondisi_user = "";
        // }else if(in_array($user_id, $array_head)){
        //     // $kondisi_user = "AND xin_employees.user_id = $user_id";
        //     $kondisi_user = "AND usr.user_id = $user_id";
        // }else{
        //     // $kondisi_user = "AND head.user_id = $user_id";
        //     $kondisi_user = "AND hd.user_id = $user_id";
        // }
        ...
        if ($type == 1) {
			if ($start == 0 && $end == 0) {
				...
				$kondisi = "AND TIMESTAMPDIFF( MONTH, usr.date_of_joining, CURDATE( ) ) >= $start 
				AND TIMESTAMPDIFF( MONTH, usr.date_of_joining, CURDATE( ) ) <= $end";
			}
		} else {
			// $kondisi = "AND employee_contract.to_date BETWEEN '".date('Y-m-d', strtotime($start))."' AND '".date('Y-m-d', strtotime($end))."'";
			$kondisi = "AND kon.to_date BETWEEN '".date('Y-m-d', strtotime($start))."' AND '".date('Y-m-d', strtotime($end))."'";
		}
        ...
        //             IF ( renewal_new.id IS NOT NULL, '', IF(CURDATE() > DATE_ADD(renewal_old.contract_end,INTERVAL 1 MONTH),'', renewal_old.`status`) ) AS status_renewal,
        //             IF ( renewal_new.id IS NOT NULL, '', IF(CURDATE() > DATE_ADD(renewal_old.contract_end,INTERVAL 1 MONTH),'', renewal_old.`renewal`) ) AS renewal,
        //             IF ( renewal_new.id IS NOT NULL, '', IF(CURDATE() > DATE_ADD(renewal_old.contract_end,INTERVAL 1 MONTH),'', renewal_old.`feedback`) ) AS feedback,
        //             CASE 
        ...
        //                         MAX( CASE WHEN xin_employee_contract.to_date = '' THEN DATE_ADD( CURDATE( ), INTERVAL 10 YEAR ) ELSE xin_employee_contract.to_date END ) AS max_date 
        //                     FROM
        ...
        //                     AND max_id.max_date =
        //                 CASE
        //                         WHEN xin_employee_contract.to_date = '' THEN
        //                         DATE_ADD( CURDATE( ), INTERVAL 10 YEAR ) ELSE xin_employee_contract.to_date 
        //                     END 
        //                     GROUP BY
        //                         max_id.employee_id 
        ...
        //                 AND employee_contract.contract_type_id = 5
        //             $kondisi
        //             $kondisi_user
        ...
        $query = "SELECT
                        usr.employee_id,
                        CONCAT(usr.first_name,' ',usr.last_name) AS nama,
                        usr.company_id,
                        ...
                        tp.`name` AS status_kontrak,
                        CONCAT(hd.first_name,' ',hd.last_name) AS head,
                        usr.date_of_joining AS tgl_gabung,
                        kon.to_date AS habis_kontrak,
                        CONCAT(
                            ROUND(( TIMESTAMPDIFF( DAY, CURDATE(), kon.to_date ) + 1 ) / 30 ),
                            ' Month ',
                            ROUND(( TIMESTAMPDIFF( DAY, CURDATE(), kon.to_date ) + 1 ) % 30 ),
                            ' Days' 
                        ) AS remaining,
                        ROUND( ( TIMESTAMPDIFF( MONTH, usr.date_of_joining, CURDATE( ) ) / 12 ), 1 ) AS masa_kerja,
                        '' AS status_renewal,
                        '' AS renewal,
                        '' AS feedback,
                        renew.id AS id_renewal
                    FROM
                        xin_employee_contract AS kon
                        JOIN xin_employees AS usr ON usr.user_id = kon.employee_id
                        JOIN xin_companies AS comp ON comp.company_id = usr.company_id
                        JOIN xin_departments AS dep ON dep.department_id = usr.department_id
                        JOIN xin_designations AS dg ON dg.designation_id = usr.designation_id
                        JOIN xin_employees AS hd ON hd.user_id = dep.head_id
                        JOIN xin_contract_type AS tp ON tp.contract_type_id = kon.contract_type_id
                        LEFT JOIN trusmi_renewal_contract AS renew ON renew.employee_id = usr.user_id AND renew.`status` IS NULL
                    WHERE
                    	(( kon.title LIKE '%Engagement%' AND kon.contract_type_id = 4 ) OR kon.contract_type_id = 5 ) 
                        AND kon.is_active = 1
                        $kondisi";
    ...
    public function dt_trusmi_resignation($start, $end)
    {
        ...
        if ($department_id == 68 || $user_id == 1 || $user_id == 323 || $user_id == 1139) {
            $cond = "";
        } else {
