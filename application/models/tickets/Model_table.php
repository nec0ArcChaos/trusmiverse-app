...
class Model_table extends CI_Model
{
    ...
    public function dt_tickets($start, $end, $filter_type, $filter_pic, $filter_status)
    {
        ...
        if ($filter_pic == "all") {
            $cond_pic = "";
        } else if ($filter_pic == 476) {
            $cond_pic = "AND (FIND_IN_SET( '$filter_pic', t.pic ) OR t.created_by = '$filter_pic' OR t.type = 3)";
        } else {
        ...
        // 2521 najmatul laela, 5648 haryadi3543, 2765 rizal aburakman, 2505 Puput, moch5508, pujaannicha8053
        if ($user_role_id == 1 || in_array($user_id, [1, 61, 62, 323, 979, 63, 64, 1161, 2041, 2063, 2969, 2070, 2903, 2521, 5648, 2765, 2505, 8970, 8298, 10214, 9645, 10404, 4498, 10127, 7731, 10404, 11664, 11665]) == 1 || in_array($department_id, [68, 83, 117, 230]) == 1) {
            ...
            $cond = "WHERE substr(t.created_at,1,10) BETWEEN '$start' AND '$end' AND (FIND_IN_SET( '$user_id', t.pic ) OR t.created_by = '$user_id' OR t.type = 3) $cond_status";
        } else if ($user_id == 76) {
            $cond = "WHERE substr(t.created_at,1,10) BETWEEN '$start' AND '$end' AND (FIND_IN_SET( '$user_id', t.pic ) OR t.created_by = '$user_id' OR d.department_id = $department_id) $cond_status";
        } else {
        ...
        $query = "SELECT
                    t.created_by,
                    t.description,
                    GROUP_CONCAT(DISTINCT COALESCE(im.impact,'') ORDER BY im.impact ASC) AS impact,
                    ...
                    COALESCE(SUBSTR(t.done_date,1,10),'') AS `tgl_selesai`,
	...
	ELSE CONCAT(DATE_FORMAT( t.START, '%d %b %y' ), ' - ', DATE_FORMAT( t.END, '%d %b %y' )) END,'') AS timeline,
                    ...
                    t.uat_leadtime as deadline_uat,
                    uat.status_uat,
                    uat.note as note_uat,
                    uat.files,
                    uat.created_at as tgl_uat,
                    uat.id_uat,
                    review.ekspektasi,
                    review.rating_kesesuaian,
                    review.rating_uiux,
                    COALESCE(review.kecepatan_akses,'') AS kecepatan_akses,
                    COALESCE(review.impact,'') AS impact_review,
                    -- tikdev by Ade
                    t.development,
                    t.week
                FROM
                    `ticket_task` t
                    LEFT JOIN ticket_impact im ON FIND_IN_SET(im.id,t.impact)
                    ...
                    LEFT JOIN (
                                ...
                                FROM
                                    ticket_uat t
                                    JOIN(SELECT MAX(id_uat) as id_uat FROM  ticket_uat GROUP BY id_task) as uat ON uat.id_uat = t.id_uat 
                                ) as uat ON uat.id_task = t.id_task 
                    LEFT JOIN (
                        SELECT
                            rv.id_task,
                            rv.ekspektasi,
                            rv.rating_kesesuaian,
                            rv.rating_uiux,
                            rv.kecepatan_akses,
                            GROUP_CONCAT(ip.impact) AS impact
                        FROM
                            ticket_review AS rv
                            LEFT JOIN ticket_impact AS ip ON FIND_IN_SET(ip.id,rv.impact)
                        WHERE
                            LEFT(id_task,1) <> 'X'
                        GROUP BY
                            rv.id_task
                    ) AS review ON review.id_task = t.id_task
                    $cond
