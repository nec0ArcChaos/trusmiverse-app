...
class Model_gantt extends CI_Model
{
    ...
    public function dt_gantt_type_dev($start, $end, $filter_type, $filter_pic, $filter_status)
    {
        ...
        if ($filter_type == "all") {
            ...
            $cond_type = "WHERE ty.id = '$filter_type'";
        }
        ...
        if ($filter_pic == "all") {
            ...
            $cond_pic = "AND (FIND_IN_SET( '$filter_pic', t.pic ))";
            $cond_user = "AND e.user_id = $filter_pic";
        }

        if ($user_role_id == 1 || in_array($user_id, [1, 61, 62, 323, 979, 63, 64, 1161, 2041, 2063, 2969, 2969, 2070, 2903]) == 1 || in_array($department_id, [68, 83]) == 1) {
            ...
            $cond = "WHERE (substr(t.created_at,1,10) BETWEEN '$start' AND '$end' $cond_status AND (FIND_IN_SET( '$user_id', t.pic )) OR (substr(t.created_at,1,10) BETWEEN '$start' AND '$end' $cond_status t.created_by = '$user_id'))";
        }
        $sql = "SELECT
                    ty.id AS id_type,
                    ty.type,
                    COALESCE(st.id,'99') AS id_sub_type,
                    COALESCE(st.sub_type, 'Tickets') AS sub_type,
                    COUNT(DISTINCT t.id_task ) AS jml_task,
                    '' AS status_color,
                    DATE_FORMAT( COALESCE ( MIN( t.`start` ), CURRENT_DATE ), '%d %b' ) AS `start_date_text`,
                    DATE_FORMAT( COALESCE ( MIN( t.`start` ), CURRENT_DATE ), '%d-%m-%Y' ) AS `start_date`,
                    DATE_FORMAT( COALESCE(t.done_date, COALESCE ( MAX( t.`end` ), CURRENT_DATE )), '%d-%m-%Y' ) AS `end_date`,
                    DATE_FORMAT( COALESCE(t.done_date, COALESCE ( MAX( t.`end` ), CURRENT_DATE )), '%d %b' ) AS `end_date_text`,
                    COALESCE ( TIMESTAMPDIFF( DAY, MIN( t.`start` ), MAX( t.`end` )), 0 ) AS duration,
                    0 AS progress
                FROM
                    ticket_type ty
                    LEFT JOIN ticket_category c ON c.type = ty.id
                    LEFT JOIN ticket_sub_type st ON st.id = c.sub_type
                    LEFT JOIN ticket_task t ON DATE( t.created_at ) BETWEEN '$start' AND '$end' AND t.category = c.id $cond_pic
                    $cond
                    GROUP BY ty.id";
        return $this->db->query($sql)->result();
    }

    public function dt_gantt_sub_type_dev($start, $end, $filter_type, $filter_pic, $filter_status)
    {
        $user_id = $this->session->userdata("user_id");
        $user_role_id = $this->session->userdata("user_role_id");
        $department_id = $this->session->userdata("department_id");
        ...
        if ($user_role_id == 1 || in_array($user_id, [1, 61, 62, 323, 979, 63, 64, 1161, 2041, 2063, 2969, 2969, 2070, 2903]) == 1 || in_array($department_id, [68, 83]) == 1) {
            ...
            $cond = "WHERE substr(t.created_at,1,10) BETWEEN '$start' AND '$end' $cond_status AND ((FIND_IN_SET( '$user_id', t.pic ) OR t.created_by = '$user_id'))";
        }
        $sql = "SELECT
                    ty.id AS id_type,
                    ty.type,
                ...
                FROM
                    ...
                    LEFT JOIN ticket_task t ON DATE( t.created_at ) BETWEEN '$start' AND '$end' AND t.category = c.id $cond_pic
                    $cond
                    GROUP BY st.id, ty.id";
        return $this->db->query($sql)->result();
    }


    public function dt_gantt_tickets_dev($start, $end, $filter_type, $filter_pic, $filter_status)
    {
        $user_id = $this->session->userdata("user_id");
        $user_role_id = $this->session->userdata("user_role_id");
        $department_id = $this->session->userdata("department_id");
        ...
        $sql = "SELECT
                    ...
                    GROUP_CONCAT(DISTINCT e.username) AS pic_name,
                    DATE_FORMAT(t.due_date,'%d %b') AS due_date,
                    COUNT(DISTINCT t.id_task ) AS jml_task,
                    DATE_FORMAT( COALESCE ( MIN( t.`start` ), CURRENT_DATE ), '%d-%m-%Y 00:00:00' ) AS `start_date`,
                    DATE_FORMAT( COALESCE( t.done_date, COALESCE ( MAX( t.`end` ), CURRENT_DATE )), '%d-%m-%Y 23:59:59' ) AS `end_date`,
                    DATE_FORMAT( COALESCE ( MIN( t.`start` ), CURRENT_DATE ), '%d %b' ) AS `start_date_text`,
                    DATE_FORMAT( COALESCE( t.done_date, COALESCE ( MAX( t.`end` ), CURRENT_DATE )), '%d %b' ) AS `end_date_text`,
                    COALESCE ( TIMESTAMPDIFF( DAY, MIN( t.`start` ), MAX( t.`end` )), 0 ) AS duration,
                    0 AS progress
                FROM
                    ...
                    $cond
                    GROUP BY st.id, ty.id, t.id_task
                    ORDER BY COALESCE(t.start, CURRENT_TIME)";
        return $this->db->query($sql)->result();
    }

    public function dt_gantt($start, $end, $filter_type, $filter_pic)
    {
        $user_id = $this->session->userdata("user_id");
        $user_role_id = $this->session->userdata("user_role_id");
        $department_id = $this->session->userdata("department_id");
        ...
        if ($user_role_id == 1 || in_array($user_id, [1, 61, 62, 323, 979, 63, 64, 1161, 2041, 2063, 2969, 2969, 2070, 2903]) == 1 || in_array($department_id, [68, 83]) == 1) {
            $cond = "WHERE substr(t.created_at,1,10) BETWEEN '$start' AND '$end' $cond_pic $cond_type OR (t.due_date BETWEEN '$start' AND '$end' AND t.type IN (1,2) $cond_pic $cond_type)";
        } else {
            $cond = "WHERE substr(t.created_at,1,10) BETWEEN '$start' AND '$end' AND (FIND_IN_SET( '$user_id', t.pic ) OR t.created_by = '$user_id')";
        }
        $query = "SELECT
                    t.created_by,
                    t.description,
                    ...
                    GROUP_CONCAT(DISTINCT CONCAT(' ',e.username)) AS team_name,
