...
class Model_mom_dev extends CI_Model
{
  ...
  function get_list_mom($start, $end, $closed)
  {
    ...
    if ($role_id == 1 || $user_id == 1139 || $user_id == 8892 || $user_id == 803 || $user_id == 2951 || $user_id == 4138 ) { // Ittrusmi & Super Admin & Viky & Lintang(Sekdir) & Pak Ibnu
      ...
      if ($closed == 0) { // Draft
        ...
        $kondisi = "AND tgl BETWEEN '$start' AND '$end'";
        $kondisi_new = "WHERE final.list_dep_pic LIKE '%$department_id%' OR final.department_id = $department_id";
      }
      ...
      if ($closed == 0) { // Draft
        ...
        $kondisi_new = "WHERE final.list_peserta_pic LIKE '%$user_id%' OR final.user_id = $user_id";
      }
    }
    ...
    return $this->db->query("SELECT * FROM (SELECT
                            ...
                            FROM
                              ...
                              LEFT JOIN ( SELECT user_id, CONCAT( first_name, ' ', last_name ) AS employee_name, contact_no, profile_picture, gender, department_id AS dep_id FROM xin_employees ) AS peserta ON FIND_IN_SET( peserta.user_id, mom.peserta )
  ...
  {
    return $this->db->query("SELECT COUNT(id) AS total FROM mom_kategori WHERE id NOT IN (6)")->row_array();
  }
  ...
  function get_list_rekap($start, $end)
  {
    ...
    } else if ($user_id == 1139 || $user_id == 8892 || $department_id == 117 || $role_id == 1) { // Viky, Lintang Sekdir, Div PDCA, Super Admin
      $cond = "";
    } else if ($role_id == 2) { // Head
      $cond = "AND pic.department_id = $department_id";
    } else if ($role_id == 3) { // Manager
      $cond = "AND pic.department_id = $department_id AND pic.user_role_id <> 2";
    } else if ($user_id == 4499 ) { // elia2700
      $join = "LEFT JOIN rsp_project_live.`user` AS rsp ON rsp.id_hr = pic.user_id";
      $cond = "AND (pic.department_id = 106 OR rsp.id_divisi = 6)"; // Project
    } else if ($user_id == 3650 || $user_id == 3651) { // supri & andre
      $join = "LEFT JOIN rsp_project_live.`user` AS rsp ON rsp.id_hr = pic.user_id";
      $cond = "AND (pic.department_id = 120 OR rsp.id_divisi = 2)"; // Project
    } else {
    ...
    return $this->db->query("SELECT
                            ...
                            WHERE
                              -- mom.tgl BETWEEN '$start' AND '$end' AND 
                              mom.closed = 1 $cond
                            GROUP BY
                              pic.user_id")->result();
  }
  ...
  function get_issue_new($id_mom)
  {
    return $this->db->query("SELECT
                            ...
                            FROM
                              ...
                              WHERE
                                issue.id_mom = '$id_mom' 
                                AND item.kategori IN (1,5,6,8,9,10) 
                              GROUP BY
                                issue.id_issue,
                                em.user_id 
                              ) AS issue 
