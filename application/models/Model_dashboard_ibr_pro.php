...
class Model_dashboard_ibr_pro extends CI_Model
{
    ...
    public function ibr_pro_profile($user_id)
    {
        $query = "SELECT 
                        ...
                        COUNT(IF(t.id_task IS NOT NULL,1,NULL)) AS goal,
    ...
    public function ibr_pro_list_data()
    {
        ...
        if ($user_id == 803) {
            ...
            // 76 = Yeyen Nuryenti 
            $cond = "AND e.user_id IN ('$user_id','786','1434','2','1449','820','77','331','2397','323','1186','90','355','68','76','1139')";
            // } else if ($user_id == 1) { // Akun IT All Akses
            //     $cond = "";
            //     $condIn = "";
        } else if (!in_array($user_role_id, [6, 7, 8])) { // Selain Officer, Staff, Helper or SPV Up
            $cond = "AND e.department_id = $department_id";

            if ($user_id == '476') {
                // Pak Angga
                // (72,156,157) HR RSP
                //  Leader UP Company 3 TKB
                $cond = "AND (e.department_id = $department_id OR e.department_id IN (72,156,157) OR (e.company_id = 3 AND role.level_sto >= 4))";
            }
        } else { // Other
        ...
        $query = "SELECT 
                        ...
                        COALESCE(strategy.jumlah,0) AS strategy
                    FROM xin_employees e
                    LEFT JOIN xin_user_roles role ON role.role_id = e.user_role_id
                    LEFT JOIN td_task t ON FIND_IN_SET(e.user_id, t.pic) 
                            AND YEAR(t.due_date) = YEAR(CURDATE())
                            AND MONTH(t.due_date) = MONTH(CURDATE())
                    LEFT JOIN xin_designations d ON d.designation_id = e.designation_id
