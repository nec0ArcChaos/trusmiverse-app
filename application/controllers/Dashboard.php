...
class Dashboard extends CI_Controller
{
    ...
    public function index()
    {
        ...
        $campaign = $this->db->query("SELECT id FROM trusmi_campaign WHERE DATE(trusmi_campaign.created_at) BETWEEN '2026-03-06' AND '2026-03-07' AND created_by = $id_user LIMIT 1")->row_array();
    ...
    public function index_bahrul()
    {
        ...
        $data['cek_pin_slip_pass']    = $this->model_dashboard->cek_pin_slip_pass($this->session->userdata('user_id'));
        $data['pageTitle']        = "Dashboard";
        $data['js']               = "dashboard/js_bahrul";
        $data['css']              = "dashboard/css_bahrul";
        $data['content']          = "dashboard/index_bahrul";
        $this->load->view('layout/main', $data);
    }
    ...
    public function update_personal_info()
    {
        ...
        $email_corporate = strip_tags($this->input->post('email_corporate', TRUE));
    ...
    public function resume_absen()
    {
        ...
        $cutoff = $this->input->post('cutoff'); // addnew
        ...
        // if ($company_id == 3 && $month == date("m") && date("d") <= 20) {
        //     $end_harus_hadir = date("Y-m-16", strtotime($periode . "-01"));
        // }
        ...
        // $today = date("Y-m-01", strtotime($periode));
        // $start = date("Y-m-21", strtotime($today . " -1 months"));
        // // $start = date("Y-m-01", strtotime($periode)); // test
        // $end = date("Y-m-20", strtotime($periode));
        // // $end = date("Y-m-t", strtotime($periode)); // test
        ...
        //     // $start          = date("2025-03-01"); // test
        //     $end            = date($periode . "-t");
        //     // $end            = date("2025-03-31"); // test
        //     $response['end_cok'] = substr(date("Y-m-t"), 8, 2);
        // } else if ($company_id == 3) {
        //     if ($month == date("m") && date("d") <= 20) {
        //         $start          = date("Y-m-16", strtotime("-1 month", strtotime(date($periode . '-d'))));
        //     } else if ($month > date("m") && date("d") > 21) {
        //         $start          = date("Y-m-21", strtotime("-2 month", strtotime(date($periode . '-d'))));
        //     } else {
        ...
        $periode_cutoff = $this->periode_cutoff($periode, $cutoff, $start, $end); // addnew
        // $periode_cutoff = $this->periode_cutoff($periode); // addnew
    ...
    public function periode_cutoff($periode, $cutoff, $start, $end) // addnew cutoff
    {
        $user_id = $this->session->userdata("user_id");
        $allowed_akses = ['2063', '1', '979'];
        if (in_array($user_id, $allowed_akses)) {
        ...
        $sub_emp = 0;
        // MERAPIHKAN LOGIC PERIODE CUTOFF !!!!
        $time = strtotime($periode);
        // if ($company_id == 3) {
        //     $start = date("Y-m-16", strtotime("-1 month", $time));
        //     $end = date("Y-m-15",  $time);
        // } else if ($designation_id == 731) {
        ...
        // $data_absen = $this->model_rekap_absen->get_absensi($start, $end, $company_id, $department_id, $employee_id,  $sub_emp)->row(0);
        $data_detail_absen = $this->model_dashboard->new_detail_absen($start, $end, $employee_id, $designation_id);
        $response['start'] = $start;
        $response['end'] = $end;
    ...
    public function detail_absen()
    {
        ...
        $time = strtotime($this->input->post('periode') ?? date("Y-m"));

        // di komen by ade karena tidak digunakan
        ...
        // detdev addnew cutoff
        $cutoff = $this->input->post('cutoff');

        $data_detail_absen = $this->model_dashboard->detail_absen($this->input->post('periode') ?? date("Y-m"), $employee_id, $designation_id, $cutoff); // detdev
        $data['data'] = $data_detail_absen['data_detail_absen'];
        $data['start_detail'] = $data_detail_absen['start_detail'];
        $data['end_detail'] = $data_detail_absen['end_detail'];
            ...
            if (!empty($_FILES['attachment']['tmp_name'])) {
                if (is_uploaded_file($_FILES['attachment']['tmp_name'])) {
                    ...
                    if (in_array($ext, $allowed)) {
                        ...
                        $profile = "/var/www/trusmiverse/hr/uploads/leave/"; //New path after using cloud
...
{
    $base64      = $this->input->post('foto_base64');
    $base64_ig   = $this->input->post('foto_ig_base64');

    if (!$base64 || !$base64_ig) {
        echo json_encode([
            'status' => false,
            'message' => 'Foto TikTok dan IG wajib'
        ]);
        return;
    }

    $path = '/var/www/trusmiverse/apps/uploads/campaign/';
    if (!is_dir($path)) {
        mkdir($path, 0777, true);
    }

    function saveBase64($base64, $path){
    ...
    $foto_tiktok = saveBase64($base64,$path);
    $foto_ig     = saveBase64($base64_ig,$path);
    ...
    $this->db->insert('trusmi_campaign',$insert);
    ...
    function update_ttd_digital(){
        ...
        $folderPath = 'uploads/ttd_digital/';
        $fileName = 'ttd_' . uniqid() . '.png'; // Buat nama file unik
        ...
        if (file_put_contents($file, $image_base64)) {
            ...
            $this->output
                ->set_content_type('application/json')
                ->set_output(json_encode(['success' => true, 'file' => $file]));
        } else {
            // Jika gagal, kirim response error
            $this->output
                ->set_content_type('application/json')
                ->set_output(json_encode(['success' => false, 'message' => 'Gagal menyimpan file']));
        }
    }
}
