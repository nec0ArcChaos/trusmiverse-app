...
class Grd_dept extends CI_Controller
{
    ...
    public function index()
    {
        ...
        $data['department_id'] = (empty($_GET['dept_id'])) ? 0 : $_GET['dept_id'];
        ...
        if($data['department_id'] == 0){
            ...
            $data['department_name'] = $this->db->query("SELECT `department_name` FROM xin_departments WHERE department_id = $data[department_id] LIMIT 1")->row()->department_name;
        }

        $data['company_name'] = $data['company_name'] . ' | ' . $data['department_name'];
