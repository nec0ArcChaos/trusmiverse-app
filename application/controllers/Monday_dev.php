...
class Monday_dev extends CI_Controller
{
    ...
    public function index()
    {
        ...
        $data['css']              = "monday_dev/css";
        $data['content']          = "monday_dev/index";
        $data['js']               = "monday_dev/js";
    ...
    public function update_sub_task()
    {
        ...
        if (!empty($_FILES['history_file_sub']['name'])) {
            ...
            $config['allowed_types'] = 'gif|jpg|png|jpeg|png|pdf|xls|xlsx';
            $new_name = $id_sub_task . '_' . time();
            $config['file_name']     = $new_name;
            $this->load->library('upload', $config);
    ...
    function get_pekerjaan($divisi)
    {
        ...
        $query = "SELECT id_so AS id, so AS pekerjaan FROM `grd_m_so` WHERE divisi = '$divisi'";
