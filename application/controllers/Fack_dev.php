...
class Fack_dev extends CI_Controller
{
    ...
    public function index()
    {
        if ($this->session->userdata('user_id') != "") {
            $data['pageTitle']        = "Formulir Registrasi Calon Karyawan DEV";
            $data['css']              = "fack/css";
            $data['js']               = "fack/js";
    ...
    {
        $data = $this->model_fack->get_designation();
...
Mohon segera melengkapi data pribadi dan kami akan melanjutkan pada tahap offering. Terimakasih 🙏🏻",
    ...
    {
        $applicantion_id = $this->input->post('application_id');
        $query = "SELECT id, application_id, employee_id, nama_perusahaan, lokasi, posisi, tahun_masuk, tahun_keluar, COALESCE(salary_awal,0) AS salary_awal, COALESCE(salary_akhir,0) AS salary_akhir, alasan_keluar FROM fack_work_experience WHERE application_id = '$applicantion_id'";
        $data = $this->db->query($query)->result();
        $response['data'] = $data;
        echo json_encode($response);
    }
    ...
    {
        $applicantion_id = $this->input->post('application_id');
        $query = "SELECT id, application_id, employee_id, nama_organisasi, lokasi, posisi, jenis_kegiatan, masa_aktif FROM fack_organization_experience WHERE application_id = '$applicantion_id'";
        $data = $this->db->query($query)->result();
        $response['data'] = $data;
        echo json_encode($response);
    }
    ...
    public function insert_administrasi()
    {
        ...
        $get_employee_id = $this->db->query("SELECT CONCAT(DATE_FORMAT('$date_of_joining', '%y%m%d'),',',(MAX(SUBSTRING_INDEX(employee_id, ',', -1)) + 1)) AS employee_id, (MAX(SUBSTRING_INDEX(employee_id, ',', -1)) + 1) max_id FROM xin_employees WHERE employee_id LIKE '%,%'")->row_array();
        ...
        if ($this->input->post('status_hasil') == 7) {
            ...
            if ($pas_foto != '' && $pas_foto != null) {
                ...
                if (file_exists($imagePath) == 1) {
                    $newPath = "/opt/lampp/htdocs/hr/uploads/profile/" . $pas_foto;
