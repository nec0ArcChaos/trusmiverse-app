...
class Mom_ade extends CI_Controller
{
	...
	function index()
	{
		...
		$data['project']	= $this->db->query("SELECT designation_name AS project, company_id, divisi as id_project FROM `xin_designations`")->result();;
...
🌐 Link Approve : " . $apr->link_approve . "";
	...
	function get_pekerjaan()
	{
		...
		$query = "SELECT id_so AS id, so AS pekerjaan FROM `grd_m_so` WHERE divisi = '$id_project'";
	...
	function save_verified()
	{
		...
		if ($status == 2) {
			$update = $this->db->query("UPDATE td_sub_task SET progress = 0, status = null WHERE id_sub_task = '$id_sub_task';");
	...
	{
		$data = $this->model->get_detail_ibr($id);
		if ($data->file != NULL && $data->link == NULL) {
			$lampiran = $data->file;
		} else if ($data->file == NULL && $data->link != NULL) {
			$lampiran = $data->link;
		} else if ($data->file != NULL && $data->link != NULL) {
			$lampiran = "1. " . $data->file . "
            2. " . $data->link;
		}
		$msg = "📢 *Notifikasi Revisi Tasklist IBR PRO (MOM)* 📢

Tasklist *IBR PRO* telah diverifikasi dan terdapat revisi di task :
🏆 Goals / Issue : *$data->task*
💡 Strategi : *$data->sub_task*
👤 PIC : *$data->pic*
🌐 Lampiran : $lampiran

📑 Status : *Tidak Oke*
💬 Note : *$data->verified_note*
📝 Verify by : *$data->pdca*
📌 Verify at : *" . date('Y-m-d H:i:s') . "*

🔗 Detail : https://trusmiverse.com/apps/pr/mom/$data->mom
...
Mohon revisi maksimal 1x 24 jam, jika tidak akan di berlakukan Lock Absen";
		...
		$this->whatsapp_lib->send_single_msg('rsp', $data->contact_no, $msg);
