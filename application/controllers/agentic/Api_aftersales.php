...
class Api_aftersales extends CI_Controller
{
    function __construct()
    {
        ...
        $this->load->model("agentic/Model_api_aftersales", 'model');
    ...
    function aftersales()
    {
        ...
        if ($api_key !== 'n8nAgenticAftersales$123') {
        ...
        $response['detail'] = [
            // 'akad_vs_aftersales' => $this->model->akad_vs_aftersales($project,$periode),
            'leadtime_perbaikan' => $this->model->leadtime_perbaikan($project, $periode),
            'leadtime_respon_perbaikan' => $this->model->leadtime_respon_perbaikan($project, $periode),
            'rating_aftersales' => $this->model->rating_aftersales($project, $periode),
            'leadtime_perbaikan_web' => $this->model->leadtime_perbaikan_web($project, $periode),
        ];
    ...
    function update_data_agentic()
    {
        ...
        $note = "Aftersales (Berat) : " . $resume['actual'] . "/" . $resume['target'] . " (" . $resume['actual_persentase'] . "%/" . $resume['target_persentase'] . "%)";

        $this->db->trans_begin();

        // 1 KPI Data
        // Cek apakah data dengan id_project sudah ada
        $query = $this->db->query("SELECT * FROM agentic.1_kpi_data WHERE type_agentic = 'aftersales' AND project_id = '$id_project' AND LEFT(periode, 7) = '$data[periode]'");
        if ($query->num_rows() > 0) {
        ...
        $indikator_name = [
            // 'Jumlah Aftersales berat berbading total akad',
            'Leadtime Ontime Perbaikan Aftersales',
            'Leadtime Ontime Respon Perbaikan Aftersales',
            'Rating Perbaikan Aftersales',
            'Leadtime Perbaikan dari web complain',
        ];
        if ($query_kpi_health->num_rows() > 0) {
            ...
            for ($i = 0; $i < COUNT($indikator_name); $i++) {
                ...
                // if ($indikator_name[$i] == 'Jumlah Aftersales berat berbading total akad') {
                //     $target_value = @$detail['akad_vs_aftersales']['target_persentase'] ?? 5;
                //     $actual_value = $detail['akad_vs_aftersales']['achieve_persentase'] ?? 0;
                //     $condition_rule = '=' . @$detail['akad_vs_aftersales']['target_persentase'] ?? 5;
                //     $note_rule = "(".$detail['akad_vs_aftersales']['actual']."/".$detail['akad_vs_aftersales']['target'].")";
                // } else 
                if ($indikator_name[$i] == 'Leadtime Ontime Perbaikan Aftersales') {
                    $target_value = @$detail['leadtime_perbaikan']['target_persentase'] ?? 0;
                    $actual_value = $detail['leadtime_perbaikan']['achieve_persentase'] ?? 0;
                    $condition_rule = '=' . @$detail['leadtime_perbaikan']['target_persentase'] ?? 0;
                    $note_rule = "(" . $detail['leadtime_perbaikan']['actual'] . "/" . $detail['leadtime_perbaikan']['target'] . ")";
                }
                else if ($indikator_name[$i] == 'Leadtime Ontime Respon Perbaikan Aftersales') {
                    $target_value = @$detail['leadtime_respon_perbaikan']['target_persentase'] ?? 0;
                    $actual_value = $detail['leadtime_respon_perbaikan']['achieve_persentase'] ?? 0;
                    $condition_rule = '=' . @$detail['leadtime_respon_perbaikan']['target_persentase'] ?? 0;
                    $note_rule = "(" . $detail['leadtime_respon_perbaikan']['actual'] . "/" . $detail['leadtime_respon_perbaikan']['target'] . ")";
                }
                else if ($indikator_name[$i] == 'Rating Perbaikan Aftersales') {
                    $target_value = @$detail['rating_aftersales']['target_persentase'] ?? 0;
                    $actual_value = $detail['rating_aftersales']['achieve_persentase'] ?? 0;
                    $condition_rule = '=' . @$detail['rating_aftersales']['target_persentase'] ?? 0;
                    $note_rule = "(" . $detail['rating_aftersales']['actual'] . "/" . $detail['rating_aftersales']['target'] . ")";
                }
                else if ($indikator_name[$i] == 'Leadtime Perbaikan dari web complain') {
                    $target_value = @$detail['leadtime_perbaikan_web']['target_persentase'] ?? 0;
                    $actual_value = $detail['leadtime_perbaikan_web']['achieve_persentase'] ?? 0;
                    $condition_rule = '=' . @$detail['leadtime_perbaikan_web']['target_persentase'] ?? 0;
                    $note_rule = "(" . $detail['leadtime_perbaikan_web']['actual'] . "/" . $detail['leadtime_perbaikan_web']['target'] . ")";
                }
