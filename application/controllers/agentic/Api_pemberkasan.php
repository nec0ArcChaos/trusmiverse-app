...
class Api_pemberkasan extends CI_Controller
{
    function __construct()
    {
        ...
        $this->load->model("agentic/model_api_pemberkasan", 'model');
    ...
    function pemberkasan()
    {
        $api_key = $this->input->get_request_header('x-api-key', TRUE);
        if ($api_key !== 'n8nAgenticPemberkasan$123') {
        ...
        $data_bank = $this->model->hasil_feedback($id_gm, $periode);

        $response['detail'] = [
            'pemberkasan' => [
                'progres_berkas' => $this->model->progres_berkas($id_gm, $periode),
                'berkas_tidak_lengkap' => $this->model->berkas_tidak_lengkap($id_gm, $periode),
                'berkas_real' => $this->model->berkas_real($id_gm, $periode),
                'leadtime_pemberkasan' => $this->model->leadtime_pemberkasan($id_gm, $periode),
                'dp_customer' => $this->model->dp_customer($id_gm, $periode),
                'hasil_feedback' => explode(',', $data_bank['history_bank']),
    ...
    function update_data_agentic()
    {
        ...
        $query = $this->db->query("SELECT * FROM agentic.1_kpi_data WHERE type_agentic = 'pemberkasan' AND id_gm = '$id_gm' AND LEFT(periode, 7) = '$data[periode]'");
        if ($query->num_rows() > 0) {
            ...
            $update_data_kpi = [
                ...
                'note' => 'Target : ' . $resume['sp3k']['target'] . ' | Actual : ' . $resume['sp3k']['actual'],
                'unit' => '',
        ...
        $indikator_name = [
            ...
            'Berkas Real / Wayang',
            'Leadtime Pemberkasan',
            'DP Konsumen',
            // 'Hasil Feedback / Interview Bank',
        ];
        if ($query_kpi_health->num_rows() > 0) {
            ...
            for ($i = 0; $i < COUNT($indikator_name); $i++) {
                ...
                if ($indikator_name[$i] == 'Progres Kelengkapan Berkas') {
                    $target_value = $detail['pemberkasan']['progres_berkas']['target_persentase'];
                    $actual_value = $detail['pemberkasan']['progres_berkas']['persentase'];
                    $note = '(' . $detail['pemberkasan']['progres_berkas']['actual'] . '/' . $detail['pemberkasan']['progres_berkas']['target'] . ')';
                    $condition_rule = '=' . $detail['pemberkasan']['progres_berkas']['target_persentase'];
                }
                else if ($indikator_name[$i] == 'Berkas Tidak Lengkap / Revisi') {
                    $target_value = $detail['pemberkasan']['berkas_tidak_lengkap']['target_persentase'];
                    $actual_value = $detail['pemberkasan']['berkas_tidak_lengkap']['persentase'];
                    $condition_rule = '=' . $detail['pemberkasan']['berkas_tidak_lengkap']['target_persentase'];
                    $note = '(' . $detail['pemberkasan']['berkas_tidak_lengkap']['actual'] . '/' . $detail['pemberkasan']['berkas_tidak_lengkap']['target'] . ')';
                }
                else if ($indikator_name[$i] == 'Berkas Real / Wayang') {
                    $target_value = $detail['pemberkasan']['berkas_real']['target_persen'];
                    $actual_value = $detail['pemberkasan']['berkas_real']['persentase'];
                    $condition_rule = '=' . $detail['pemberkasan']['berkas_real']['target_persen'];
                    $note = '(' . $detail['pemberkasan']['berkas_real']['jumlah'] . '/' . $detail['pemberkasan']['berkas_real']['target'] . ')';
                }
                else if ($indikator_name[$i] == 'Leadtime Pemberkasan') {
                    $target_value = $detail['pemberkasan']['leadtime_pemberkasan']['target_persentase'];
                    $actual_value = $detail['pemberkasan']['leadtime_pemberkasan']['persentase'];
                    $condition_rule = '= ' . $detail['pemberkasan']['leadtime_pemberkasan']['target_persentase'];
                    $note = '(' . $detail['pemberkasan']['leadtime_pemberkasan']['actual'] . '/' . $detail['pemberkasan']['leadtime_pemberkasan']['target'] . ')';
                }
                else if ($indikator_name[$i] == 'DP Konsumen') {
                    $target_value = $detail['pemberkasan']['dp_customer']['target_persentase'];
                    $actual_value = $detail['pemberkasan']['dp_customer']['achieve_persentase'];
                    $condition_rule = '= ' . $detail['pemberkasan']['dp_customer']['target_persentase'];
                    $note = '(' . $detail['pemberkasan']['dp_customer']['actual'] . '/' . $detail['pemberkasan']['dp_customer']['target'] . ')';
                }
                // else if ($indikator_name[$i] == 'Hasil Feedback / Interview Bank') {
                //     $target_value = $detail['pemberkasan']['hasil_feedback'];
                //     $actual_value = $detail['pemberkasan']['hasil_feedback'];
                //     $condition_rule = '= ' . $detail['pemberkasan']['hasil_feedback'];
                // }
