...
class Api_project_pm extends CI_Controller
{
    ...
    function __construct()
    {
        ...
        $this->load->model("agentic/model_api_project_pm", 'model');
    ...
    public function project()
    {
        ...
        $project_tipe = $data['project_tipe'] ?? 'subsidi';
        ...
        if (!$id_pm || !$start_date || !$end_date) {
            echo json_encode(['status' => false, 'message' => 'Missing parameters']);
            return;
        }
        $project_manager = $this->model->get_pm($id_pm);
        $project = $this->model->get_project($id_pm, $project_tipe);
        $resume_leadtime = $this->model->kpi_pm($id_pm, $periode, $project_tipe);
        $kpi_corporate = $this->model->kpi_corporate($periode, $project_tipe);
        $leadtime = $this->model->get_leadtime($id_pm, $periode, $project_tipe);
        $produktivitas = $this->model->get_produktivitas($id_pm, $periode, $project_tipe);
        $ketersediaan_material = $this->model->get_ketersediaan_material($id_pm, $periode, $project_tipe);
        $keterlambatan = $this->model->get_data_keterlambatan($id_pm, $periode, $project_tipe);

        $response['periode'] = $periode;
        $response['project_manager'] = $project_manager;
        $response['project'] = is_array($project)
            ? (count($project)
            ? implode(', ', array_filter(array_map(function ($x) {
            return is_object($x) ? ($x->project ?? '') : ($x['project'] ?? '');
        }, $project)))
            : 'N/A')
            : ($project->project ?? 'N/A');
        $response['kpi_corporate'] = $kpi_corporate;

        $response['resume'] = [
            'periode' => $periode,
            'corporate_kpi' => $project[0]->kpi_project_housing ?? 'N/A',
            'target' => $resume_leadtime->target ?? '0',
            'actual' => $resume_leadtime->actual ?? '0',
            'target_persentase' => $resume_leadtime->target_persentase ?? '0',
            'achieve_persentase' => $resume_leadtime->achieve_persentase ?? '0',
        ];

        $response['detail']['keterlambatan'] = $keterlambatan;
        $response['detail']['leadtime_pembangunan'] = [
            'target' => $resume_leadtime->target_persentase ?? '0',
            'aktual' => $resume_leadtime->achieve_persentase ?? '0',
            'status' => $resume_leadtime->status ?? 'N/A',
            'data_agregasi_leadtime' => $leadtime,
            'data_5_blok_leadtime_terendah' => $this->model->get_5_blok_leadtime_terburuk($id_pm, $periode, $project_tipe),
        ];
        $response['detail']['produktivitas_tukang'] = [
            'target' => '100',
            'produktivitas' => $produktivitas->persen_produktif ?? '0',
            '5_blok_produktivitas_terendah' => $this->model->get_5_blok_produktivitas_terendah($id_pm, $periode, $project_tipe),
        ];
        $response['detail']['ketersediaan_material'] = [
            'target' => '100',
            'ketersediaan_material' => $ketersediaan_material->persen_ketersediaan_material ?? '0',
            '5_blok_paling_sering_material_kosong' => $this->model->get_5_blok_paling_sering_material_kosong($id_pm, $periode, $project_tipe),
        ];
        $gangguan_cuaca = $this->model->get_gangguan_cuaca($id_pm, $periode, $project_tipe);
        $response['detail']['gangguan_cuaca'] = [
            'target' => '100',
            'persen_gangguan_cuaca' => $gangguan_cuaca->avg_gangguan_cuaca ?? '0',
        ];
        $response['detail']['problem'] = $this->model->get_problem($id_pm, $periode, $project_tipe);
    ...
    function update_data_agentic()
    {
        ...
        $produktivitas_tukang = $detail['produktivitas_tukang'];
        $ketersediaan_material = $detail['ketersediaan_material'];


        // echo json_encode(['resume' => $resume, 'produktivitas_tukang' => $produktivitas_tukang, 'ketersediaan_material' => $ketersediaan_material]);
        // return;

        $gangguan_cuaca = $detail['gangguan_cuaca'];
        ...
        $id_pm = $data['data_input']['project_manager']['id'];
        $created_at = date('Y-m-d');
        $periode = $data['periode'];
        $type_agentic = $data['type_agentic'];

        $this->db->trans_begin();

        // 1 KPI Data
        // Cek apakah data dengan id_pm sudah ada
        $query = $this->db->query("SELECT * FROM agentic.1_kpi_data WHERE type_agentic = '$type_agentic' AND id_pm = '$id_pm' AND SUBSTR(periode,1,7) = SUBSTR('$periode',1,7)");
        if ($query->num_rows() > 0) {
            $data_existing = $query->row_array();
            $id_kpi = $data_existing['id'];

            $note = "SPK Ontime: " . $resume['actual'] . "/" . $resume['target'] . "";
            // Update
            $update_data_kpi = [
                'type_agentic' => $type_agentic,
        ...
        if ($query_kpi_health->num_rows() > 0) {
            ...
            for ($i = 0; $i < COUNT($indikator_name); $i++) {
                ...
                if ($indikator_name[$i] == 'Ontime') {
                    ...
                    $actual_value = $resume['achieve_persentase'];
                    $condition_rule = '>=' . 80;
                    $note = '(' . $resume['actual'] . '/' . $resume['target'] . ')';
                }
        ...
        if ($query_ringkasan_eks->num_rows() > 0) {
            ...
            foreach ($ringkasan_esklusif as $key => $value) {
                ...
                $insert_data_ringkasan_eks = [
                    ...
                    'status_value' => $resume['achieve_persentase'],
