...
class Api_qc extends CI_Controller
{
    function __construct()
    {
        ...
        $this->load->model("agentic/Model_api_qc", 'model');
    ...
    function qc()
    {
        ...
        $response['detail'] = [
            'leadtime_qc' => $this->model->leadtime_qc($project, $periode),
            'jumlah_temuan_qc' => $this->model->jumlah_temuan_qc($project, $periode),
            // 'leadtime_perbaikan_qc' => $this->model->leadtime_perbaikan_qc($project,$periode),
            'qc_vs_aftersales' => $this->model->qc_vs_aftersales($project, $periode),
            'temuan_berulang' => $this->model->temuan_berulang($project, $periode),
            'vendor_temuan_berulang' => $this->model->vendor_temuan_berulang($project, $periode),
        ];
    ...
    function update_data_agentic()
    {
        ...
        $note = "Product Unit Complaint Rate : " . $resume['target_persentase'] . "/" . $resume['achieve_persentase'] . "";

        $this->db->trans_begin();

        // 1 KPI Data
        // Cek apakah data dengan id_project sudah ada
        $query = $this->db->query("SELECT * FROM agentic.1_kpi_data WHERE type_agentic = 'qc' AND project_id = '$id_project' AND LEFT(periode, 7) = '$data[periode]'");
        if ($query->num_rows() > 0) {
        ...
        $indikator_name = [
            'Leadtime QC',
            'Jumlah Temuan QC',
            // 'Leadtime Perbaikan QC',
            'QC VS Aftersales',
            'Jumlah Temuan QC Berulang',
            'Jumlah Vendor dengan Temuan QC Berulang'
        ];
        if ($query_kpi_health->num_rows() > 0) {
            ...
            for ($i = 0; $i < COUNT($indikator_name); $i++) {
                ...
                if ($indikator_name[$i] == 'Leadtime QC') {
                    $target_value = 100;
                    $actual_value = $detail['leadtime_qc']['achieve_persentase'] ?? 0;
                    $condition_rule = '=' . 100;
                    $note_rule = "(" . $detail['leadtime_qc']['actual'] . "/" . $detail['leadtime_qc']['target'] . ")";
                }
                else if ($indikator_name[$i] == 'Jumlah Temuan QC') {
                    $target_value = @$detail['jumlah_temuan_qc']['target_persentase'] ?? 5;
                    $actual_value = $detail['jumlah_temuan_qc']['persentase_temuan_tidak_sesuai'] ?? 0;
                    $condition_rule = '=' . @$detail['jumlah_temuan_qc']['target_persentase'] ?? 5;
                    $note_rule = "(" . $detail['jumlah_temuan_qc']['temuan_tidak_sesuai'] . "/" . $detail['jumlah_temuan_qc']['target'] . ")";
                }
                // else if ($indikator_name[$i] == 'Leadtime Perbaikan QC') {
                //     $target_value = 100;
                //     $actual_value = $detail['leadtime_perbaikan_qc']['achieve_persentase'] ?? 0;
                //     $condition_rule = '=' . 100;
                //     $note_rule = "(".$detail['leadtime_perbaikan_qc']['actual']."/".$detail['leadtime_perbaikan_qc']['target'].")";
                // } 
                else if ($indikator_name[$i] == 'QC VS Aftersales') {
                    $target_value = 100;
                    $actual_value = $detail['qc_vs_aftersales']['achieve_persentase'] ?? 0;
                    $condition_rule = '=' . 100;
                    $note_rule = "(" . $detail['qc_vs_aftersales']['actual'] . "/" . $detail['qc_vs_aftersales']['target'] . ")";
                }
                else if ($indikator_name[$i] == 'Jumlah Temuan QC Berulang') {
                    $target_value = 100;
                    $actual_value = $detail['temuan_berulang']['achieve_persentase'] ?? 0;
                    $condition_rule = '=' . 100;
                    $note_rule = "(" . $detail['temuan_berulang']['actual'] . "/" . $detail['temuan_berulang']['target'] . ")";
                }
                else if ($indikator_name[$i] == 'Jumlah Vendor dengan Temuan QC Berulang') {
                    $target_value = 100;
                    $actual_value = $detail['vendor_temuan_berulang']['achieve_persentase'] ?? 0;
                    $condition_rule = '=' . 100;
                    $note_rule = "(" . $detail['vendor_temuan_berulang']['actual'] . "/" . $detail['vendor_temuan_berulang']['target'] . ")";
                }
