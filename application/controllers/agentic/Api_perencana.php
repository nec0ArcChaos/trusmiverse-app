...
class Api_perencana extends CI_Controller
{
    function __construct()
    {
        ...
        $this->load->model("agentic/Model_api_perencana", 'model');
    ...
    function perencana()
    {
        ...
        if ($api_key !== 'n8nAgenticPerencana$123') {
        ...
        $response['detail'] = [
            'boq_housing' => $this->model->boq_housing($project, $periode),
            'boq_infra' => $this->model->boq_infra($project, $periode),
            'siteplan' => $this->model->penyelesaian_siteplan($project, $periode),
            'plan_actual_gatek' => $this->model->plan_actual_gatek($project, $periode),
            'leadtime_plan_mco' => $this->model->leadtime_plan_mc0($project, $periode),
            'tepat_plan_mco' => $this->model->tepat_plan_mco($project, $periode)
        ];
    ...
    function update_data_agentic()
    {
        ...
        $note = "penyelesaian Gambar Teknik dan BoQ : " . $resume['actual'] . "/" . $resume['target'] . "";

        $this->db->trans_begin();

        // 1 KPI Data
        // Cek apakah data dengan id_project sudah ada
        $query = $this->db->query("SELECT * FROM agentic.1_kpi_data WHERE type_agentic = 'perencana' AND project_id = '$id_project' AND LEFT(periode, 7) = '$data[periode]'");
        if ($query->num_rows() > 0) {
            ...
            $update_data_kpi = [
                ...
                'corporate_kpi_name' => $resume['nama'],
                ...
                'target_value' => $resume['target'],
                'actual_value' => $resume['achieve'] ?? 0,
                'note' => $note,
                'unit' => '%',
        ...
        $indikator_name = [
            'Penyelesaian Gambar Teknik & BOQ Housing',
            'Penyelesaian Gambar Teknik & BOQ Infra',
            'Penyelesaian Siteplan',
            'Ketersediaan Gambar Teknik  BOQ Housing & Infra Berdasarkan Plan Bangun',
            'Lead Time Ketersediaan Plan MC 0 & MC 100',
            'Tepat dalam pengukuran plan MC0 dan MC100 Vs Aktual'
        ];
        if ($query_kpi_health->num_rows() > 0) {
            ...
            for ($i = 0; $i < COUNT($indikator_name); $i++) {
                ...
                if ($indikator_name[$i] == 'Penyelesaian Gambar Teknik & BOQ Housing') {
                    $target_value = @$detail['boq_housing']['target'] ?? 5;
                    $actual_value = $detail['boq_housing']['achieve'] ?? 0;
                    $condition_rule = '=' . @$detail['boq_housing']['target'] ?? 5;
                    $note_rule = "(" . $detail['boq_housing']['actual'] . "/" . $detail['boq_housing']['target'] . ")";
                }
                if ($indikator_name[$i] == 'Penyelesaian Gambar Teknik & BOQ Infra') {
                    $target_value = @$detail['boq_infra']['target'] ?? 5;
                    $actual_value = $detail['boq_infra']['achieve'] ?? 0;
                    $condition_rule = '=' . @$detail['boq_infra']['target'] ?? 5;
                    $note_rule = "(" . $detail['boq_infra']['actual'] . "/" . $detail['boq_infra']['target'] . ")";
                }
                if ($indikator_name[$i] == 'Penyelesaian Siteplan') {
                    $target_value = @$detail['siteplan']['target'] ?? 5;
                    $actual_value = $detail['siteplan']['achieve'] ?? 0;
                    $condition_rule = '=' . @$detail['siteplan']['target'] ?? 5;
                    $note_rule = "(" . $detail['siteplan']['actual'] . "/" . $detail['siteplan']['target'] . ")";
                }
                if ($indikator_name[$i] == 'Lead Time Ketersediaan Plan MC 0 & MC 100') {
                    $target_value = @$detail['leadtime_plan_mco']['target'] ?? 5;
                    $actual_value = $detail['leadtime_plan_mco']['achieve'] ?? 0;
                    $condition_rule = '=' . @$detail['leadtime_plan_mco']['target'] ?? 5;
                    $note_rule = "(" . $detail['leadtime_plan_mco']['actual'] . "/" . $detail['leadtime_plan_mco']['target'] . ")";
                }
                if ($indikator_name[$i] == 'Tepat dalam pengukuran plan MC0 dan MC100 Vs Aktual') {
                    $target_value = @$detail['tepat_plan_mco']['target'] ?? 5;
                    $actual_value = $detail['tepat_plan_mco']['achieve'] ?? 0;
                    $condition_rule = '=' . @$detail['tepat_plan_mco']['target'] ?? 5;
                    $note_rule = "(" . $detail['tepat_plan_mco']['actual'] . "/" . $detail['tepat_plan_mco']['target'] . ")";
                }
        ...
        if ($query_ringkasan_eks->num_rows() > 0) {
            ...
            foreach ($ringkasan_esklusif as $key => $value) {
                ...
                $insert_data_ringkasan_eks = [
                    'id' => $id_analysis,
                    'kpi_id' => $id_kpi,
                    'status_kpi' => $resume['achieve'] >= $resume['target'] ? 'Ontime' : 'Delay',
                    'status_value' => $resume['achieve'] ?? 0,
