...
class Api_drbm extends CI_Controller
{
    function __construct()
    {
        ...
        $this->load->model("agentic/Model_api_drbm", 'model');
    ...
    function drbm()
    {
        ...
        $response['id_rm'] = $this->model->get_rm($id_rm);
        $response['kpi_corporate'] = $this->model->kpi_corporate($periode);
        $response['resume'] = $this->model->kpi_rm($id_rm, $periode);

        $response['detail']['jumlah_drbm'] = $this->model->jumlah_drbm($id_rm, $periode);
        $response['detail']['jumlah_sph'] = $this->model->jumlah_sph($id_rm, $periode);
        $response['detail']['credit_scoring'] = $this->model->credit_scoring($id_rm, $periode);
        // $response['detail']['leadtime']['waktu_spk_mulai_vs_waktu_mulai_pelaksana'] = $this->model->spk_vs_pelaksana($id_rm,$periode);
        // $response['detail']['leadtime']['perencana_infra'] = $this->model->leadtime_perencana_infra($id_rm,$periode);
    ...
    function update_data_agentic()
    {
        ...
        $resume = $data['data_input']['data']['resume'];
        $detail = $data['data_input']['data']['detail'];
        ...
        $id_gm = $data['data_input']['data']['id_rm']['id'];
        $created_at = date('Y-m-d');
        $note = "Rasio DRBM vs Akad : " . $resume['total_drbm'] . "/" . $resume['total_akad'] . "";

        $this->db->trans_begin();

        // 1 KPI Data
        // Cek apakah data dengan id_gm sudah ada
        $query = $this->db->query("SELECT * FROM agentic.1_kpi_data WHERE type_agentic = 'drbm' AND id_gm = '$id_gm' AND LEFT(periode, 7) = '$data[periode]'");
        if ($query->num_rows() > 0) {
            ...
            $update_data_kpi = [
                ...
                'id_gm' => $id_gm,
                'target_corporate' => $data['data_input']['data']['kpi_corporate']['presentase_target'],
                'actual_corporate' => $data['data_input']['data']['kpi_corporate']['presentase'],
                'unit_corporate' => '%',
                'target_value' => $resume['presentase_target'],
                'actual_value' => $resume['presentase'],
                'unit' => '%',
                'note' => $note,
        ...
        $indikator_name = [
            'Jumlah DRBM Lunas',
            'Jumlah SPH Lunas',
            'Credit Scoring Konsumen Layak',
            'Sisa Gaji Konsumen > 800.000'
        ];
        if ($query_kpi_health->num_rows() > 0) {
            ...
            for ($i = 0; $i < COUNT($indikator_name); $i++) {
                ...
                if ($indikator_name[$i] == 'Jumlah DRBM Lunas') {
                    $target_value = $detail['jumlah_drbm']['target'];
                    $actual_value = $detail['jumlah_drbm']['persen_lunas'];
                    $condition_rule = '=' . $detail['jumlah_drbm']['target'];
                    $note_rule = "(" . $detail['jumlah_drbm']['total_lunas'] . "/" . $detail['jumlah_drbm']['total_drbm'] . ")";
                }
                else if ($indikator_name[$i] == 'Jumlah SPH Lunas') {
                    $target_value = $detail['jumlah_sph']['target'];
                    $actual_value = $detail['jumlah_sph']['persen_lunas'];
                    $condition_rule = '=' . $detail['jumlah_sph']['target'];
                    $note_rule = "(" . $detail['jumlah_sph']['total_lunas'] . "/" . $detail['jumlah_sph']['total_sph'] . ")";
                }
                else if ($indikator_name[$i] == 'Credit Scoring Konsumen Layak') {
                    $target_value = $detail['credit_scoring']['target'];
                    $actual_value = $detail['credit_scoring']['persen_konsumen_layak'];
                    $condition_rule = '=' . $detail['credit_scoring']['target'];
                    $note_rule = "(" . $detail['credit_scoring']['total_konsumen_layak'] . "/" . $detail['credit_scoring']['total_konsumen_loan'] . ")";
                }
                else if ($indikator_name[$i] == 'Sisa Gaji Konsumen > 800.000') {
                    $target_value = $detail['credit_scoring']['target'];
                    $actual_value = $detail['credit_scoring']['persen_sisa_gaji_standar_up'];
                    $condition_rule = '=' . $detail['credit_scoring']['target'];
                    $note_rule = "(" . $detail['credit_scoring']['total_sisa_gaji_standar_up'] . "/" . $detail['credit_scoring']['total_konsumen_loan'] . ")";
                }
        ...
        if ($query_ringkasan_eks->num_rows() > 0) {
            ...
            foreach ($ringkasan_esklusif as $key => $value) {
                ...
                $insert_data_ringkasan_eks = [
                    'id' => $id_analysis,
                    'kpi_id' => $id_kpi,
                    'status_kpi' => $resume['presentase'] <= $resume['presentase_target'] ? 'Ontime' : 'Delay',
                    'status_value' => $resume['presentase'],
