...
class Api_plan_housing extends CI_Controller
{
    function __construct()
    {
        ...
        $this->load->model("agentic/Model_api_plan_housing", 'model');
    ...
    function plan()
    {
        ...
        if ($api_key !== 'n8nAgenticPlanHousing$123') {
        ...
        $response['detail']['sp3k_vs_lahan'] = $this->model->sp3k_vs_lahan($project, $periode);
        $response['detail']['sp3k_vs_spk'] = $this->model->sp3k_vs_spk($project, $periode);
        $response['detail']['spk_vs_pelaksana'] = $this->model->spk_vs_pelaksana($project, $periode);
    ...
    function update_data_agentic()
    {
        ...
        $note = "Plan Housing : " . $resume['actual'] . "/" . $resume['target'] . "";

        $this->db->trans_begin();

        // 1 KPI Data
        // Cek apakah data dengan id_project sudah ada
        $query = $this->db->query("SELECT * FROM agentic.1_kpi_data WHERE type_agentic = 'plan_housing' AND project_id = '$id_project' AND LEFT(periode, 7) = '$data[periode]'");
        if ($query->num_rows() > 0) {
        ...
        $indikator_name = [
            'Jumlah SP3K, DP Konsumen, Booking Cash vs Kesiapan Lahan',
            'Jumlah SP3K, DP Konsumen, Booking Cash vs SPK Sudah Terbit',
        ...
        if ($query_kpi_health->num_rows() > 0) {
            ...
            for ($i = 0; $i < COUNT($indikator_name); $i++) {
                ...
                if ($indikator_name[$i] == 'Jumlah SP3K, DP Konsumen, Booking Cash vs Kesiapan Lahan') {
                    $target_value = $detail['sp3k_vs_lahan']['target_persentase'] ?? 0;
                    $actual_value = $detail['sp3k_vs_lahan']['achieve_persentase'] ?? 0;
                    $condition_rule = '=' . $detail['sp3k_vs_lahan']['target_persentase'] ?? 0;
                    $note_rule = "(" . $detail['sp3k_vs_lahan']['actual'] . "/" . $detail['sp3k_vs_lahan']['target'] . ")";
                }
                else if ($indikator_name[$i] == 'Jumlah SP3K, DP Konsumen, Booking Cash vs SPK Sudah Terbit') {
                    $target_value = $detail['sp3k_vs_spk']['target_persentase'] ?? 0;
                    $actual_value = $detail['sp3k_vs_spk']['achieve_persentase'] ?? 0;
                    $condition_rule = '=' . $detail['sp3k_vs_spk']['target_persentase'] ?? 0;
                    $note_rule = "(" . $detail['sp3k_vs_spk']['actual'] . "/" . $detail['sp3k_vs_spk']['target'] . ")";
                }
                else if ($indikator_name[$i] == 'SPK Awal vs Tanggal Mulai Pelaksana') {
                    $target_value = $detail['spk_vs_pelaksana']['target_persentase'] ?? 0;
                    $actual_value = $detail['spk_vs_pelaksana']['achieve_persentase'] ?? 0;
                    $condition_rule = '= ' . $detail['spk_vs_pelaksana']['target_persentase'] ?? 0;
                    $note_rule = "(" . $detail['spk_vs_pelaksana']['actual'] . "/" . $detail['spk_vs_pelaksana']['target'] . ")";
                }
