        ...
        if ($status == 2) {
            ...
            try {
                ...
                if (!is_null($get_sub->id_ps)) {
                    ...
                    $get_ps = $this->db->query("SELECT * FROM ps_task WHERE id_ps = '$get_sub->id_ps'")->row();
                    $detail_pic = $this->db->query("SELECT
                    user_id,
                  ...
                  WHERE
                    user_id = $get_ps->created_by")->row_array();
...
✅ *Solusi :* " . htmlspecialchars(strip_tags($get_ps->resume)) . "
