...
class Permintaan_karyawan extends CI_Controller //Formulir Dokumen Karyawan
{
    ...
    function index()
    {
        ...
        $data['css']              = "recruitment/permintaan_karyawan/css";
        $data['js']               = "recruitment/permintaan_karyawan/js";
        $data['content']          = "recruitment/permintaan_karyawan/index";
        ...
        // Prefill data dari renewal contract (jika redirect dengan employee_id)
        $data['prefill'] = null;
        ...
        if ($employee_id && $employee_id != '') {
            $data['prefill'] = $this->model->get_employee_prefill($employee_id);
        }

        $this->load->view('layout/main', $data);
    }

    // Endpoint untuk prefill FPK dari renewal contract
    function get_prefill_renewal()
    {
        $employee_id = $this->input->post('employee_id');
        if (!$employee_id) {
            echo json_encode(['status' => false, 'message' => 'employee_id required']);
            return;
        }
        $prefill = $this->model->get_employee_prefill($employee_id);

        if ($prefill) {
            echo json_encode(['status' => true, 'data' => $prefill]);
        } else {
...
👤User :" . $user . "",
    ...
    function get_user_role()
    {
        ...
        if (in_array($user_info['user_role_id'], array(1, 2, 3, 10, 11, 12))) {
            ...
            $data['edit'] = true;
        } elseif (in_array($user_info['user_id'], array(1, 2535, 6466))) {
            $data['edit'] = true;
        }
        echo json_encode($data);
    }
