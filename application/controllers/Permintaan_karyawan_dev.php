...
class Permintaan_karyawan_dev extends CI_Controller //Formulir Dokumen Karyawan
{
    ...
    function index()
    {
        ...
        $data['js']               = "permintaan_karyawan/js_dev";
        $data['content']          = "permintaan_karyawan/index";
    ...
    function get_dt_posisi()
    {
        ...
        $posisi = $_POST['posisi'];
        $job_profil = $this->model->get_job_profil($department, $posisi)->row_array();
    ...
    public function send_wa_fpk($job_id)
    {
        ...
        $kontak[] = '6283148391521';
        $kontak[] = '6285861312912';
...
🕐Requested At : " . $fpk['created_at'] . "",
    ...
    function save_edit_permintaan()
    {
        ...
        $long_description = htmlspecialchars(addslashes($job_desc), ENT_QUOTES);
        $posisi = explode('|', $_POST['posisi']);
        // Split the long description into sentences
        $sentences = explode('.', $job_desc);
        // Filter out empty sentences
        $non_empty_sentences = array_filter($sentences, function ($sentence) {
            return trim($sentence) !== "";
        });
        // Count the number of non-empty sentences
        $sentence_count = count($non_empty_sentences);
        $kpi_count = str_word_count($_POST['kpi']);
        if (substr($_POST['perusahaan'], 0, 1) == 3 && $_POST['jumlah'] > 3) {
            $data['error'] = 'Jumlah Kebutuhan maksimal 3 per FPK';
        } else if ($kpi_count < 10) {
            $data['error'] = 'KPI minimal terdiri dari 10 kata.';
        } else if ($sentence_count <= 2) {
            $data['error'] = 'The Job Description must be more than 2 sentences. (Gunakan titik di akhir kalimat)';
        } else {
            $permintaan = [
                "employer_id" => substr($_POST['perusahaan'], 0, 1),
                "department_id" => $_POST['department'],
