...
class Mom_sidiq extends CI_Controller
{
	function __construct()
	{
		...
		$this->load->model('Model_mom_sidiq', 'model');
...
🌐 Link Approve : " . $apr->link_approve . "";
	...
	function save_verified()
	{
		...
		if ($status == 2) {
			$update = $this->db->query("UPDATE mom_issue_item SET freq_revisi = freq_revisi + 1 WHERE id_sub_task = '$id_sub_task';");
	...
	function save_eskalasi()
	{
		...
		for ($i = 1; $i <= $totalIssue; $i++) {
			...
			$dataIssue = array(
				...
				'id_eskalasi' => $i,
				'id_issue' => $i,
				'reff_id_sub_task' => $id_sub_task
			);

			// $result['issue_save'] = $this->db->insert('mom_issue', $dataIssue);
			...
			for ($iAction = 1; $iAction <= $countAction; $iAction++) {
				...
				// $result['issue_item_save'] = $this->db->insert('mom_issue_item', $dataIssueItem);
				$result['issue_item_save'][] = $dataIssueItem;

				for ($iPic = 1; $iPic <= $countPic; $iPic++) {
					$listPic['pic_' . $i] = array();
					$no = 0;
					foreach ($_POST['val_pic_' . $i . '_' . $iPic] as $pic) {
						$listPic['pic_' . $i][$no] = $pic;
						$no++;
					}
				}

				$mom_issue_item = array(
					'pic' => (count($listPic['pic_' . $i]) > 1) ? implode(",", $listPic['pic_' . $i]) : $listPic['pic_' . $i][0],
				);

				$result['pic'][] = $mom_issue_item;
				// $this->db->where('id_mom', $id_mom);
				// $this->db->where('id_issue', $i);
				// $this->db->where('id_issue_item', $iAction);
				// $this->db->update('mom_issue_item', $mom_issue_item);
			}
		}
