...
class Work_on_holiday_ade extends CI_Controller //Formulir Dokumen Karyawan
{
    function __construct()
    {
        ...
        $this->load->model('Model_work_on_holiday_ade', 'model');
        $this->load->model("model_profile");
    }
    function index(){
        $user = $this->session->userdata('user_id');
        $data['pageTitle']        = "Work On Holiday";
        $data['css']              = "work_on_holiday/css_ade";
        $data['js']               = "work_on_holiday/js_ade";
        $data['content']          = "work_on_holiday/index_ade";
    ...
    public function no_pk()
	{
		$q = $this->db->query("SELECT
			MAX( RIGHT ( pk_job.id_pk, 3 ) ) AS rv_max 
			FROM
			pk_job 
			WHERE
			SUBSTR( pk_job.created_at, 1, 7 ) = LEFT(CURDATE(),7)");
		$kd = "";
		if ($q->num_rows() > 0) {
    ...
    function save(){
        ...
        // testing addnew revnew
        // $id_karyawan = $this->input->post('list_karyawan');
...
Demikian Perintah Kerja ini dibuat agar dapat di laksanakan.";
            $this->whatsapp_lib->send_single_msg('rsp',$pic->contact_no,$msg);
            $this->whatsapp_lib->send_single_msg('rsp','6283195851323',$msg);//comben 
...
";
        $hasil = $this->whatsapp_lib->send_single_msg('rsp',$no_hp,$msg);
        echo json_encode($hasil);
