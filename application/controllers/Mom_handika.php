...
class Mom_handika extends CI_Controller
{
	function __construct()
	{
		...
		$this->load->model('Model_mom_handika', 'model');
	...
	function index()
	{
		$data['content'] 	= "mom_handika/index";
		$data['pageTitle'] 	= "Minutes of Meeting | New";
		$data['css'] 		= "mom_handika/css";
		$data['js'] 		= "mom_handika/js";
		...
		$query = "SELECT 
                ...
                FROM(
                    ...
                    --   WHERE a.is_active = 1 AND (user_id IN (118,637,116,998,4201) OR user_id IN (2108,2733,1294,1733,1369,2127,1844))
                      ...
                      LEFT JOIN mom_header b ON FIND_IN_SET(tm.user_id, peserta) AND b.meeting NOT IN ('Jobdesk')
                      LEFT JOIN (
                                          SELECT
                                              @rank := @rank + 1 AS w,
                                              calendar_week.* 
