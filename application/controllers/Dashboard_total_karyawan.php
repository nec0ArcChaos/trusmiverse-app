...
class Dashboard extends CI_Controller
{
    ...
    public function update_ctm_slip()
    {
        ...
        if ($get_ctm_pin_slip->ctm_pin_slip == "") {
            ...
            $response['password_verify'] = password_verify($old_ctm_pin_slip,  $get_ctm_pin_slip->ctm_pin_slip);
            if (password_verify($old_ctm_pin_slip,  $get_ctm_pin_slip->ctm_pin_slip)) {
                $hash_ctm_pin_slip = password_hash($new_ctm_pin_slip, PASSWORD_BCRYPT, $options);
                $data = [
                    'ctm_pin_slip' => $hash_ctm_pin_slip
                ];
    ...
    public function mkt()
    {
        ...
        $data['content']          = "dashboard/mkt/index_v2";
        $this->load->view('layout/main', $data);
    }
    ...
    public function resume_absen()
    {
        ...
        if ($month == date("m") && date("d") <= 20) {
            ...
            if ($company_id == 3) {
                $end_harus_hadir = date("Y-m-16", strtotime($periode));
            } else {
                $end_harus_hadir = date("Y-m-20", strtotime($periode));
            }
        }
    ...
    {
        if ($this->session->userdata("user_id") == '2063' || $this->session->userdata("user_id") == '1') {
            $employee_id = $this->input->post('employee_id');
        } else {
            $employee_id = $this->session->userdata("user_id");
        }
