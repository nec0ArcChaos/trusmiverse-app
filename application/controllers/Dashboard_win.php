...
class Dashboard_win extends CI_Controller
{
    ...
    public function resume_absen()
    {
        ...
        if ($designation_id == 731) {
            // if ($month == date("m") && date("d") <= substr(date("Y-m-t"), 8, 2)) {
            //     $start          = date("Y-m-01", strtotime("-1 month", strtotime(date($periode . '-d'))));
            // } else {
            //     $start          = date("Y-m-01", strtotime("-1 month", strtotime(date($periode . substr(date("Y-m-t"), 7, 3)))));
            // }
