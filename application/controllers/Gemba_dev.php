...
if (!empty($_FILES['file_item']['name'][0])) {
    ...
    // Load library sekali di luar loop
    $this->load->library('upload');

    for ($i = 0; $i < $count; $i++) {

        // Skip jika file kosong / tidak dipilih
        ...
        $config = [
            'upload_path'   => './uploads/gemba/',
            'allowed_types' => 'jpg|jpeg|png|gif|pdf|xls|xlsx|doc|docx',
            'file_name' => $id_gemba . '_' . $id_gemba_ceklis,
            'overwrite'     => true,
            'max_size' => 2048, //2mb
        ];
        ...
        if ($this->upload->do_upload('temp_file')) {
            ...
            // PDF, Excel, Word → langsung simpan nama file
            $file_names[] = $uploaded['file_name'];
            $file_link[]  = base_url('uploads/gemba/' . $uploaded['file_name']);

        } else {
            // Log error upload per file (tidak stop keseluruhan proses)
            log_message('error', 'Gemba upload error file-' . $i . ': ' . $this->upload->display_errors('', ''));
        }
    }
}
		...
		// Hanya jalan kalau ada file yang berhasil diupload
		// =========================
		if (!empty($file_names)) {

			// Replace foto lama → hapus dulu baru insert baru
			$this->db->where('id_gemba', $id_gemba);
			$this->db->where('id_gemba_ceklis', $id_gemba_ceklis);
	...
	public function update_user_location()
{
	...
	public function edit_tgl_plan()
	{
		...
		if (!$id_gemba || !$tgl_plan) {
			echo json_encode(['status' => false, 'message' => 'Data tidak lengkap']);
			return;
		}

		$updated = $this->model->update_tgl_plan($id_gemba, $tgl_plan, $user_id);

		if ($this->db->affected_rows() > 0 || $updated) {
			echo json_encode(['status' => true, 'message' => 'Tanggal plan berhasil diupdate']);
		} else {
			echo json_encode(['status' => false, 'message' => 'Gagal update atau Anda tidak memiliki akses']);
		}
	}

	public function delete_gemba()
	{
		header('Content-Type: application/json');
		$id_gemba = $this->input->post('id_gemba');
		$user_id  = $_SESSION['user_id'];
		...
		if (!$id_gemba) {
			echo json_encode(['status' => false, 'message' => 'ID Gemba tidak ditemukan']);
			return;
		}

		$deleted = $this->model->delete_gemba($id_gemba, $user_id);

		if ($deleted) {
			echo json_encode(['status' => true, 'message' => 'Data Gemba berhasil dihapus']);
		} else {
			echo json_encode(['status' => false, 'message' => 'Gagal menghapus atau Anda tidak memiliki akses']);
		}
	}
