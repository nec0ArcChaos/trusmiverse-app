...
class Mom_jobdesk extends CI_Controller
{
	function __construct()
	{
		...
		$this->load->model('Model_mom_jobdesk', 'model');
	...
	function index()
	{
		$data['content'] 	= "mom_jobdesk/index";
		$data['pageTitle'] 	= "MoM Jobdesk | New";
		$data['css'] 		= "mom_jobdesk/css";
		$data['js'] 		= "mom_jobdesk/js";
	...
	function resume()
	{
		$data['content'] 	= "mom_jobdesk/resume/index";
		$data['pageTitle'] 	= "MoM Jobdesk";
		$data['css'] 		= "mom_jobdesk/resume/css";
		$data['js'] 		= "mom_jobdesk/resume/js";
		$data['karyawan']	= $this->model->get_employee();
		$data['pic']		= $this->model->get_employee();
		$data['kategori']	= $this->model->get_kategori_new2();
		...
		$query = "SELECT 
                ...
                FROM(
                    ...
                    --   WHERE a.is_active = 1 AND (user_id IN (118,637,116,998,4201) OR user_id IN (2108,2733,1294,1733,1369,2127,1844))
                      ...
                      LEFT JOIN mom_header b ON FIND_IN_SET(tm.user_id, peserta) AND b.meeting IN ('Jobdesk')
                      LEFT JOIN (
                                          SELECT
                                              @rank := @rank + 1 AS w,
                                              calendar_week.* 
