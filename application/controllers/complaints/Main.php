...
class Main extends CI_Controller
{
    ...
    public function test_nomor()
    {
        ...
        // Jika nomor telepon dimulai dengan "628", ubah menjadi "628"
        if (substr($nomor_telepon_bersih, 0, 2) == "628") {
    ...
    public function view()
    {
        ...
        $data['content'] = "complaints/main";
        $data['js'] = "complaints/_main_js";
    ...
    public function dika()
    {
        ...
        $data['pageTitle'] = "Complaints";
        $data['css'] = "complaints/_main_css";
        $data['content'] = "complaints/main_dika";
        $data['js'] = "complaints/_main_dika_js";
        $this->load->view('layout/main', $data);
    }
    ...
    public function get_category()
    {
        $id_project = $this->input->post('id_project') ?? '';
        if ($id_project != '') {
            $project = $this->db->query("SELECT COALESCE(luar_kota,0) AS luar_kota FROM rsp_project_live.m_project p WHERE p.id_project = '$id_project'")->row();
            if ($project->luar_kota == 1) {
                $condition = " WHERE c.`id` != '15' ";
            } else {
                $condition = " WHERE c.`id` != '16' ";
            }
            $type = $this->db->query("SELECT c.id AS id_category, c.`category`, c.head_id AS escalation_by, CONCAT(e.first_name,' ',e.last_name) AS escalation_name FROM cm_category c LEFT JOIN xin_employees e ON e.user_id = c.head_id $condition ORDER BY c.category")->result();
        } else {
    ...
    public function get_status()
    {
        ...
        $kondisi = "";
        if ($status_input != 1) { // dari internal
            $kondisi = "AND id NOT IN (3,5)";
        }
        $status = $this->db->query("SELECT id AS id_status, `status`, `color` FROM cm_status WHERE id != 7 $kondisi")->result();
    ...
    public function get_pic()
    {
        ...
        t.id ='$id_category' AND 
        e.user_id != 1 AND e.is_active = 1
    ...
    public function get_pic_ticket()
    {
        ...
        if (in_array($department_id, [68, 83,120]) == 1) {
            $cond = " AND e.user_id != '$user_id'";
        }
        ...
        FROM
            `cm_category` t
        LEFT JOIN xin_employees e ON FIND_IN_SET( e.designation_id, t.designation_id )
        WHERE e.user_id NOT IN (1, 10118) AND e.is_active = 1
        GROUP BY e.user_id
        ) AS m
    ...
    public function get_detail_task_new()
    {
        $id_task = $this->input->post('id_task') ?? "";
        $response['detail'] = "";
        $response['status'] = false;
        if ($id_task != "") {
            $response['detail'] = $this->model_main->get_detail_task_new($id_task);
            $response['status'] = true;
        }
    ...
    public function save_task()
    {
        ...
        $id_category = $this->input->post('id_category') ?? '';
        $id_status = 1; // bypass langsung ke level 2
        // if ($id_category == '12') {
        //     $head_request_id    = 1;
        ...
        // $this->send_cm_notif_to_head_pic($id_task); Udah di bypass
    ...
    {
        // ----------------------------------------------------
        // DUMMY CASE 1 — TEPAT WAKTU
        // ----------------------------------------------------
        $complaints1 = (object)[
            'id_task' => 'TEST001',
            'id_status' => 6,
            'requester_name' => 'Dummy Requester',
            'pic_name' => 'Dummy PIC',
            'head_pic_name' => 'Dummy Head',
            'category' => 'Progres Bangunan',
            'project' => 'RN Getasan',
            'blok' => 'A18',
            'konsumen' => 'Dummy Konsumen',
            'task' => 'Dummy Task',
            'description' => 'Dummy Description',
            'verified_at' => '2025-09-23 17:00',
            'verified_note' => 'Verified note dummy',
            'priority' => 'Ringan',
            'start_timeline' => '2025-09-24',
            'end_timeline' => '2025-10-01',
            'pic_note' => 'PIC note dummy',
            'done_date' => '2025-10-01 10:00', // TEPAT WAKTU
            'escalation_name' => 'Dummy Escalation',
            'escalation_at' => '2025-09-22 10:00',
            'escalation_note' => 'Escalation note dummy',
            'requester_contact_no' => '62882000489612',
            'escalation_contact_no' => '62882000489612',
            'created_at' => '2025-09-22 11:31'
        ];

        // ----------------------------------------------------
        // DUMMY CASE 2 — TERLAMBAT
        // ----------------------------------------------------
        $complaints2 = clone $complaints1;
        $complaints2->id_task = 'TEST002';
        $complaints2->done_date = '2025-10-02 17:00'; // TERLAMBAT 1 HARI


        // ====================================================
        // FUNCTION BUILD MESSAGE (DIREVISI)
        // ====================================================
        $build_message = function ($complaints) {

            // STATUS DONE / UNSOLVED
            ...
            // KATEGORI TELAT / TEPAT WAKTU
            $end_date = date_create($complaints->end_timeline);
            $done_date = date_create($complaints->done_date);

            $diff = date_diff($end_date, $done_date);
            $selisih_hari = (int)$diff->format("%r%a");

            if ($selisih_hari <= 0) {
                $kategori_waktu = "Tepat Waktu";
                $header_kondisi = "_Kondisi Tepat Waktu_";
            } else {
                $kategori_waktu = "Terlambat " . $selisih_hari . " Hari";
                $header_kondisi = "_Kondisi Terlambat_";
            }
...
🚨 Hello *" . trim(ucwords($complaints->requester_name)) . "*, there is an update regarding your complaint!
...
🕒 Kategori : *" . $kategori_waktu . "*

🔗 link Review / Feedback :
https://trusmiverse.com/apps/customer-complaints/" . hashids_encode($complaints->id_task) . "
...
🚨 There is an update regarding *" . trim(ucwords($complaints->requester_name)) . "* complaint!
...
" . $complaints->created_at;
            ...
            return [$msg_to_requester, $msg_to_head_pic];
        };



        // ====================================================
        // KIRIM DUMMY TEPAT WAKTU
        // ====================================================
        list($msg_req1, $msg_head1) = $build_message($complaints1);

        $this->send_wa('62882000489612', $msg_req1, '9884');
        $this->send_wa('62882000489612', $msg_head1, '9884');


        // ====================================================
        // KIRIM DUMMY TERLAMBAT
        // ====================================================
        list($msg_req2, $msg_head2) = $build_message($complaints2);

        $this->send_wa('62882000489612', $msg_req2, '9884');
        $this->send_wa('62882000489612', $msg_head2, '9884');


        echo "DUMMY NOTIFICATION SENT.";
    }
    ...
    public function update_verifikasi_new()
    {
        $id_task = $this->input->post('id_task') ?? '';
        $id_category = @$_POST['id_category'] ?? '';
        $update_task = "";
        ...
        if ($id_task != "") {
            ...
            if (
                $id_category == 18
                // && $user_id == 7804
                && ($user_id == 10118 || $user_id == 4138) // shelya / Farhan
            ) {
                $data['id_project'] = $this->input->post('id_project') ?? '';
                $data['project'] = $this->input->post('project') ?? '';
                $data['blok'] = $this->input->post('blok') ?? '';
    ...
    public function update_pengerjaan()
    {
        ...
        $response = [];
        if ($id_status == '6' && empty($_FILES['evidence']['name'])) {
            // Siapkan response error dalam format JSON
            $response = [
                'status' => 'error',
                'message' => 'Gagal! Untuk menyelesaikan tugas (status Done), Anda wajib mengunggah file bukti (evidence).'
            ];
        ...
        if ($id_task != "") {
            ...
            if (!empty($_FILES['evidence']['name'])) {
                ...
                if (!$this->upload->do_upload('evidence')) {
                    $response['evidence_error'] = $this->upload->display_errors();
                } else {
                    $data = array('upload_data' => $this->upload->data());
                    ...
                    $hasil = $this->db->insert('cm_task_attach', $data_upload);
                    $response['evidence_success'] = [
                        'file_name' => $fileName,
                        'size' => $fileSize,
                        'ext' => $fileExt,
                        'db_status' => $hasil
    ...
    public function upload_evidence($id_task)
    {
        $id_task = $this->input->post('id_task');
...
" . $complaints->created_at . "";
        $this->send_wa('6285324409384', $msg, '2063');
        $this->send_wa('6281573533735', $msg, '10118'); // shelya
    }
...
" . $complaints->created_at . "";
        // $this->whatsapp_lib->send_single_msg('rsp', '62896561628701', $msg);
        $this->send_wa($complaints->head_pic_contact_no, $msg);
        $this->send_wa('6285324409384', $msg, '2063');
        $this->send_wa('62882000489612', $msg, '9884');
    }
...
" . $complaints->created_at . "";
        $this->send_wa($complaints->escalation_contact_no, $msg);
        $this->send_wa('6285324409384', $msg, '2063');
        // $this->send_wa('6281321464138', $msg, '8257');
        $this->send_wa('6281573533735', $msg, '10118');
        $this->send_wa('6281394292055', $msg, '4138');
        // $this->whatsapp_lib->send_single_msg('rsp', '6285324409384', $msg_to_requester, '2063'); // aris
        // $this->whatsapp_lib->send_single_msg('rsp', '6281321464138', $msg, '8257'); // Dika
        // $this->whatsapp_lib->send_single_msg('rsp', '6281573533735', $msg, '10118'); // shelya
        // $this->whatsapp_lib->send_single_msg('rsp', '6281394292055', $msg, '4138'); // Farhan Firdaus
...
" . $complaints->created_at . "";
        $this->send_wa_eksternal($complaints->requester_contact_no, $msg);
        $this->send_wa('6285324409384', $msg, '2063');
        $this->send_wa('6281573533735', $msg, '10118');
        // $this->whatsapp_lib->send_single_msg('rsp', '6285324409384', $msg_to_requester, '2063'); // complain testing
        // $this->whatsapp_lib->send_single_msg('rsp', '6281573533735', $msg, '10118'); // shelya
    }
...
" . $complaints->created_at . "";
        ...
        $this->send_wa_eksternal($complaints->requester_contact_no, $msg);
        $this->send_wa('6285324409384', $msg, '2063');
        $this->send_wa('6285220777223', $msg, '2397');
        $this->send_wa('6281573533735', $msg, '10118');
        // $this->whatsapp_lib->send_single_msg('rsp', '6285324409384', $msg_to_requester, '2063');
        // $this->whatsapp_lib->send_single_msg('rsp', '6285220777223', $msg, '2397'); // Pak Firman
        // $this->whatsapp_lib->send_single_msg('rsp', '6281573533735', $msg, '10118'); // shelya
    }
...
" . $row->created_at . "";
            $this->send_wa($row->pic_contact_no, $msg);
            $this->send_wa('6281573533735', $msg, '10118');
            // $this->whatsapp_lib->send_single_msg('rsp', '6281573533735', $msg, '10118'); // shelya
            // $this->whatsapp_lib->send_single_msg('rsp', '62896561628701', $msg);
        }
    ...
    //         // $this->whatsapp_lib->send_single_msg('rsp', '6285324409384', $msg_to_head_pic, '2063');
    //         $this->send_wa_eksternal($complaints->escalation_contact_no, $msg_to_head_pic);
    //         // $this->whatsapp_lib->send_single_msg('rsp', '62896561628701', $msg_to_head_pic);

    //         // head requester or verified by
    //         // $this->whatsapp_lib->send_single_msg('rsp', '6285324409384', $msg_to_head_pic, '2063');
    //         // $this->whatsapp_lib->send_single_msg('rsp', $complaints->head_pic_contact_no, $msg_to_head_pic);
    //         // $this->send_wa('6281120217656', $msg_to_head_pic);
    //         // $this->whatsapp_lib->send_single_msg('rsp', '6281120217656', $msg_to_head_pic);

    //         // requester or created by or konsumen
    //         // $this->whatsapp_lib->send_single_msg('rsp', '6285324409384', $msg_to_requester, '2063');
    //         $this->send_wa('6285324409384', $msg_to_requester, '2063');
    //         $this->send_wa_eksternal($complaints->requester_contact_no, $msg_to_requester);
    //         // $this->whatsapp_lib->send_single_msg('rsp', '6285220777223', $msg_to_requester, '2397'); // Pak Firman
    //         $this->send_wa('6285220777223', $msg_to_requester, '2397');
    //         // $this->whatsapp_lib->send_single_msg('rsp', '62896561628701', $msg_to_requester);
    //         // $this->whatsapp_lib->send_single_msg('rsp', '6281573533735', $msg_to_requester, '10118'); // shelya
    //         $this->send_wa('6281573533735', $msg_to_requester, '10118');
    ...
    public function send_notif_done($id_task)
    {
        ...
        // ----------------------------------------------------
        // HITUNG KATEGORI TEPAT WAKTU / TERLAMBAT
        // ----------------------------------------------------
        // ======================================================================
        // ======================================================================
        // HITUNG SELISIH HARI
...
✅ Verified By : *" . trim(ucwords($complaints->head_pic_name)) . "*
📆 Verified At : *" . $complaints->verified_at . "*
📄 Verification Notes : *" . trim(strip_tags(str_replace('<', ' <', $complaints->verified_note))) . "*

🗂️ Priority : *" . trim(ucwords($complaints->priority)) . "*
📆 Timeline : *" . $complaints->start_timeline . " s/d " . $complaints->end_timeline . "*

✅ Pic By : *" . trim(ucwords($complaints->pic_name)) . "*
📆 Pic At : *" . $complaints->done_date . "*
📄 Pic Notes : *" . trim(strip_tags(str_replace('<', ' <', $complaints->pic_note))) . "*
...
" . $complaints->created_at;
        ...
        // ======================================================================
        // PESAN UNTUK HEAD PIC / ESCALATION
        // ======================================================================
        $msg_to_head_pic =
            "🚨 There is an update regarding *" . trim(ucwords($complaints->requester_name)) . "* complaint!
...
" . $complaints->created_at;
        ...
        // Escalation → ke Head PIC
        $this->send_wa($complaints->escalation_contact_no, $msg_to_head_pic);
...
✅ Pic By : *" . trim(ucwords($complaints->pic_name)) . "*
📆 Done Date : *" . trim(ucwords($complaints->done_date)) . "*
📄 Alasan Reschedule : *" . trim(strip_tags(str_replace('<', ' <', $complaints->pic_note))) . "*
...
" . $complaints->created_at . "";
        ...
        // head pic or escalation by
        // $this->whatsapp_lib->send_single_msg('eaf', '6285324409384', $msg_to_head_pic, '2063');
        $this->send_wa('6285324409384', $msg_to_head_pic, '2063');
        $this->send_wa($complaints->escalation_contact_no, $msg_to_head_pic);
        // $this->whatsapp_lib->send_single_msg('eaf', '62896561628701', $msg_to_head_pic);
        // head requester or verified by
        // $this->whatsapp_lib->send_single_msg('eaf', '6285324409384', $msg_to_head_pic, '2063');
        // $this->whatsapp_lib->send_single_msg('eaf', $complaints->head_pic_contact_no, $msg_to_head_pic);
        // $this->whatsapp_lib->send_single_msg('eaf', '62896561628701', $msg_to_head_pic);
        // requester or created by or konsumen
        // $this->whatsapp_lib->send_single_msg('eaf', '6285324409384', $msg_to_requester, '2063');
        $this->send_wa('6285324409384', $msg_to_requester, '2063');
        $this->whatsapp_lib->send_single_msg('eaf', $complaints->requester_contact_no, $msg_to_requester);
        $this->send_wa('6285220777223', $msg_to_requester, '2397');
        $this->send_wa('62896601628022', $msg_to_requester, '118');
        $this->send_wa('6285624444554', $msg_to_requester, '803');
        // $this->send_wa('62896561628701', $msg_to_requester);
        $this->send_wa('6281573533735', $msg_to_requester, '10118');
        // $this->whatsapp_lib->send_single_msg('eaf', '6285220777223', $msg_to_requester, '2397'); // Pak Firman
        // $this->whatsapp_lib->send_single_msg('eaf', '62896601628022', $msg_to_requester, '118'); // Pak Andyka05
        // $this->whatsapp_lib->send_single_msg('eaf', '6285624444554', $msg_to_requester, '803'); // Pak Ibnu
        // $this->whatsapp_lib->send_single_msg('eaf', '62896561628701', $msg_to_requester);
        // $this->whatsapp_lib->send_single_msg('eaf', '6281573533735', $msg_to_head_pic); // shelya
    }
    ...
    public function resend_notif_request()
    {
        ...
        //             $data_text = array(
        //                 "channelID" => "22256282380", // Channel Trusmi Group
        //                 // "phone" => '6285324409384',
    ...
    public function resend_notif_done()
    {
        $id_task = $this->input->post('id_task');
        $notif_ulang = $this->send_notif_done($id_task);
        echo json_encode(['status' => 'success', 'messsage' => 'Berhasil Mengirim Ulang Notifikasi']);
    }
    ...
    function follow_up()
    {
        ...
        if ($user_id == '10118') {
            ...
            $return['follow_up'] = $this->db->update('cm_task', $fu);
        } else {
    ...
    function change_category()
    {
        ...
        $this->db->where('id_task', $id_task);
        $update = $this->db->update('cm_task', ['id_category' => $id_category]);
    ...
    function send_wa_eksternal($contact_number = '', $msg = '', $type = 'text', $domain = 'trusmiverse', $url_image = '')
    {
        try {
            $this->load->library('WAJS_eksternal');

            return $this->wajs_eksternal->send_wajs_notif_eksternal(
                $contact_number,
                $msg,
                $type,
                $domain,
            ...
            return "Server WAJS Eksternal Error: " . $th->getMessage();
        }
    }
}
