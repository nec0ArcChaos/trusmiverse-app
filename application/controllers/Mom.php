...
class Mom extends CI_Controller
{
	...
	private function _check_ownership($id_mom)
	{
		if (!$this->model->is_owner($id_mom, $this->session->userdata('user_id'))) {
			$result['status'] = 'error';
			$result['message'] = 'Unauthorized: You are not the owner of this MoM.';
	...
	function save_mom()
	{
		...
		if ($_POST['id_mom'] == "") {
			...
			$this->_check_ownership($_POST['id_mom']);
	...
	function save_closing()
	{
		...
		if ($closed == 1) { // Finish
			...
			// $result['send_email'] = $this->send_email_mom2($id_mom); 2026-02-26
		}
...
🔗 Detail : " . $mom['link'] . "";
	...
	{
		$id_mom_draft = $_POST['id_mom_draft'];
		$this->_check_ownership($id_mom_draft);
		$id_mom = $this->model->generate_id_mom();
...
Jangan lupa cek detail dan selesaikan tepat waktu ya! Semangat terus, kamu pasti bisa!💪✨
	...
	public function notif_reminder_wa()
	{
		try {
        $items = $this->model->notif_reminder();
        if (empty($items)) {
            echo json_encode(['status' => 'no_data', 'msg' => 'Tidak ada item yang perlu direminder']);
            return;
        }

        $results = [];
        foreach ($items as $item) {
            //$phone = trim($item->pic_contact ?? '');
            $phone = '083824955357'; // Nomor WA untuk testing
            $uid   = trim($item->pic_user_id ?? '');
            $pic   = trim($item->pic ?? '');
            if (empty($phone)) continue;

            $action_details_raw = array_values(array_filter(array_map('trim', explode('|||', $item->action_details ?? ''))));
            if (empty($action_details_raw)) continue;

            $action_list = '';
            foreach ($action_details_raw as $no => $raw) {
                [$action, $status, $deadline, $id_link] = array_pad(explode('§', $raw), 4, '-');
                $link = 'https://trusmiverse.com/apps/pr/mom/' . $id_link;

                $action_list .= ($no + 1) . ". ✅ *{$action}*\n"
                             . "   📊 Status   : {$status}\n"
                             . "   📅 Deadline : *{$deadline}*\n"
                             . "   🔗 Detail   : {$link}\n\n";
            }

            $msg = "⏰ *Reminder IBR PRO*\n\n"
                 . "👤 PIC : *{$pic}*\n\n"
                 . "Berikut action Anda yang belum selesai:\n\n"
                 . $action_list
                 . "Mohon segera update progress tasklist dan selesaikan sebelum deadline.";

            $wa_result = $this->send_wa($phone, $msg, $uid);
            $results[] = [
                'phone'   => $phone,
                'pic'     => $pic,
                // 'actions' => array_column(array_map(fn($r) => explode('§', $r), $action_details_raw), 0),
        ...
        echo json_encode(['status' => 'ok', 'sent' => count($results), 'detail' => $results]);
    } catch (\Throwable $th) {
