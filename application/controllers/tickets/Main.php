...
class Main extends CI_Controller
{
    ...
    function __construct()
    {
        ...
        $this->load->model('Model_wajs_notif', 'model_wajs');
    ...
    // oddev by Ade
    public function get_sub_type_od()
    {
        $type = $this->db->query("SELECT st.id AS id_sub_type, st.`sub_type` FROM ticket_sub_type st 
        LEFT JOIN ticket_category tc ON tc.sub_type = st.id 
        WHERE tc.type = 7 GROUP BY tc.sub_type ORDER BY sub_type")->result();
        echo json_encode($type);
    }
    ...
    public function get_status()
    {
        ...
        if ($id_type != 1) {
            $status = $this->db->query("SELECT id AS id_status, `status`, `color` FROM ticket_status WHERE id IN (1,2,3,4,5,6,7,8,12) ORDER BY no_urut")->result();
        } else {
    ...
    public function get_pic()
    {
        ...
        // AND e.is_active = 1 WHERE t.id ='$id_type' AND e.user_id = 1434 AND e.is_active = 1
        ...
        if ($id_type == 7) {
            $pic =  $this->db->query("SELECT
                                        ...
                                        COUNT(
                                        IF
                                        ( tt.`id_task` IS NOT NULL, 1, NULL )) AS ticket 
                                    FROM
                                        `ticket_type` t
                                    	LEFT JOIN xin_employees e ON FIND_IN_SET( e.designation_id, t.designation_id )
                                        -- LEFT JOIN xin_employees e ON e.department_id = 72
                                        LEFT JOIN ticket_task tt ON FIND_IN_SET( e.user_id, tt.pic ) 
                                        AND tt.`status` < 3 
                                        AND e.is_active = 1 
                                    WHERE
                                        t.id = '$id_type' 
                                    ...
                                    GROUP BY
                                        e.user_id 
                                    ORDER BY
                                        CONCAT( e.first_name, ' ', e.last_name )")->result();
        } else {
    ...
    // oddev
    public function det_get_pic_od()
    {
        $id_type = $this->input->post('id_type');
        ...
        $pic =  $this->db->query("SELECT
                                    ...
                                    FROM
                                        `ticket_type` t
                                    	-- LEFT JOIN xin_employees e ON FIND_IN_SET( e.designation_id, t.designation_id )
                                        LEFT JOIN xin_employees e ON e.department_id = 72 OR e.user_id = 329
                                        LEFT JOIN ticket_task tt ON FIND_IN_SET( e.user_id, tt.pic ) 
                                        AND tt.`status` < 3 
                                        AND e.is_active = 1 
                                    WHERE
    ...
    public function save_task()
    {
        ...
        $data_history2 = [
            ...
            'status' => $id_status,
            'status_before' => $id_status,
            'note' => 'Tickets Received',
            'created_at' => date('Y-m-d H:i:s'),
            'created_by' => $id_pic ?? $this->session->userdata('user_id')
        ];
        sleep(1);
        $this->db->insert('ticket_task_history', $data_history2);

        $response['notif'] = $this->send_notif_request($id_task);
    ...
    public function var_dumps()
    {
        ...
        // $response['wa'][] = $this->wajs->var_dumps();
        $this->load->library('WAJS');
        $response['wa'][] = $this->wajs->send_wajs_notif('6285322418200', 'test', 'text', 'trusmiverse', 8259);
        // $response['wa'] = $this->model_wajs->check_mpm();
        // var_dump($response['wa']->total);
    }
...
👤 Requester : *" . trim(ucwords($row->requester_name)) . "* 
👤 Contact : *" . trim(ucwords($row->requester_contact_no)) . "*    
 
" . $row->created_at . "";
            try {
                $this->load->library('WAJS');
                $response['wa'][] = $this->wajs->send_wajs_notif($row->contact_no, $message, 'text', 'trusmiverse', $row->user_id);
            } catch (\Throwable $th) {
                // throw $th;
            }
            ...
            $response['tele'][] = $this->telegram_api->send(
                $row->contact_no, // nomor telepon
                urlencode($message), // pesan yang akan dikirim
                1 // 1 untuk format pesan sebagai teks, 2 untuk format pesan sebagai Foto
            );

            $response['text'][] = json_decode($result_text, true);
        }
...
https://trusmiverse.com/apps/my-tickets/" . $row_ticket->id_task;
        ...
        try {
            $this->load->library('WAJS');
            $response['wa'][] = $this->wajs->send_wajs_notif($contact, $message, 'text', 'trusmiverse', $user_id);
        } catch (\Throwable $th) {
            // throw $th;
        }
    ...
    {
        $dt_ticket = $this->db->query("SELECT * FROM ticket_task WHERE id_task = '$id_task'")->row_array();
        if ($dt_ticket['type'] == 3) {
            $row_ticket = $this->model_main->get_ticket_uat($id_task, 476);
        } else {
            $row_ticket = $this->model_main->get_ticket_uat($id_task, 't.created_by');
        }
        $url = "https://onetalk-api.taptalk.io/api/integration/v1/inbox/send_message_whatsapp";
        $data_text = array(
            "channelID" => "2225082380", // Channel Trusmi Group
...
Terima kasih atas kerjasama dan dukungannya.",
    ...
    public function check_lsa()
    {
        ...
        $sql = "SELECT
            x.user_id,
            t.id_task,
            IF( COALESCE(MIN(t.`end`), CURRENT_TIME) < CURRENT_TIME, CURRENT_TIME, COALESCE(MIN(t.`end`), CURRENT_TIME))  AS `end` 
        FROM
            (
            SELECT
                e.user_id,
    ...
    // tikdev by Ade
    public function update_date_task()
    {
        $id_task = $this->input->post('id_task') ?? '';
        ...
        if ($id_task != "") {
            $data = [
                ...
                'end' => $end_date,
                'due_date' => $due_date,
                'done_date' => ($done_date != '') ? $done_date : null
            ];
    ...
    public function assign_develop_to_task()
    {
        $id_task = $this->input->post('id_task') ?? '';
        $develop_period = $this->input->post('develop_period') ?? '';
        $develop_week = $this->input->post('develop_week') ?? '';

        $update_task = "";
        if ($id_task != "") {
            $data = [
                'development' => $develop_period,
                'week' => $develop_week,
            ];
