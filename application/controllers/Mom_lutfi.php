...
class Mom_lutfi extends CI_Controller
{
	function __construct()
	{
		...
		$this->load->model('Model_mom_lutfi', 'model');
	...
	function index()
	{
		$data['content'] 	= "mom_lutfi/index";
		$data['pageTitle'] 	= "Minutes of Meeting | New";
		$data['css'] 		= "mom_lutfi/css";
		$data['js'] 		= "mom_lutfi/js";
	...
	function resume()
	{
		$data['content'] 	= "mom_lutfi/resume/index";
		$data['pageTitle'] 	= "Minutes of Meeting";
		$data['css'] 		= "mom_lutfi/resume/css";
		$data['js'] 		= "mom_lutfi/resume/js";
		$data['karyawan']	= $this->model->get_employee();
		$data['pic']		= $this->model->get_employee();
		$data['kategori']	= $this->model->get_kategori_new2();
...
🔗 Detail : " . $mom['link'] . "";
	...
	// $this->load->view("mom_lutfi/print/print", $data);
			...
			foreach ($issue as $val) {
				...
				foreach ($issue_item as $val_item) {
					...
					$data .= '</select>
										...
										<td class="kolom_modif" id="td_grdsi_draft_' . $val_item->id_issue . '_' . $val_item->id_issue_item . '">
											<select class="form-control grdsi" id="val_grdsi_draft_' . $val_item->id_issue . '_' . $val_item->id_issue_item . '" onchange="submit_update_draft(&apos;grdsi_draft_' . $val_item->id_issue . '_' . $val_item->id_issue_item . '&apos;)">
												<option data-placeholder="true">- Choose SI -</option>';
	...
	function excel($id)
	{
		...
		$this->load->view("mom_lutfi/print/excel", $data);
	}
...
" . $plan['link'];
	...
	function excel_detail($start, $end)
	{
		...
		$this->load->view("mom_lutfi/print/excel_detail", $data);
	}
...
🌐 Link Approve : " . $apr->link_approve . "";
	...
	function save_verified()
	{
		...
		if ($status == 2) {
			$update = $this->db->query("UPDATE mom_issue_item SET freq_revisi = COALESCE(freq_revisi,0) + 1 WHERE id_sub_task = '$id_sub_task';");
...
🔗 Detail : https://trusmiverse.com/apps/pr/mom_lutfi/$value->mom
...
Mohon di verifikasi kembali, jika tidak akan di berlakukan Lock Absen";
	...
	public function list_verif_owner(){
		$data = $this->model->list_verif_owner();
		echo json_encode($data);
	}
	public function list_verif_pdca(){
		$data = $this->model->list_verif_pdca();
		echo json_encode($data);
	}
