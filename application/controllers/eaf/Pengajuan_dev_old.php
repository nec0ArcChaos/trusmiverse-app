...
class Pengajuan_dev extends CI_Controller
{
	function __construct()
	{
		...
		$this->load->model('eaf/model_pengajuan_dev', 'model_pengajuan');
	...
	function insert_pengajuan()
	{
		...
		if ($result['warning'] == '') {
			...
			$result['pengajuan'] = $this->db->insert('e_eaf.e_pengajuan_dev', $pengajuan);
			...
			$result['tipe_pembayaran'] = $this->db->insert('e_eaf.e_tipe_pembayaran_dev', $pembayaran);
			...
			$result['approval'] = $this->db->insert('e_eaf.e_approval_dev', $approval);
			...
			$result['detail_keperluan'] = $this->db->insert('e_eaf.e_detail_keperluan_dev', $entries);

			// ===== UPLOAD FOTO NOTA =====
			if (!empty($_POST['nota'])) {
				define('UPLOAD_DIR', './uploads/eaf_dev/');
				...
				$result['photo_acc'] = $this->db->insert('e_eaf.e_photo_acc_dev', $foto);
			}
			...
			if ($pilihan_ba == 'ba') {
				...
				$result['photo_acc'] = $this->db->insert('e_eaf.e_photo_acc_dev', $data_ba);
			}
