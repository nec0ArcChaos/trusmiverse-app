...
class Pengajuan_fel extends CI_Controller
{
	function __construct()
	{
		...
		$this->load->model('eaf/model_pengajuan_fel', 'model_pengajuan');
	...
	function index()
	{
		...
		$user_id			= $this->session->userdata('user_id');
		if (isset($user_id)) {
			$data['content'] 	= "eaf/pengajuan_fel/index";
			$data['pageTitle'] 	= "Pengajuan EAF";
			$data['css'] 		= "eaf/pengajuan_fel/css";
			$data['js'] 		= "eaf/pengajuan_fel/js";
	...
	function insert_pengajuan()
	{
		...
		if ($this->session->userdata('user_id') != "") {
			...
			if ($id_kategori == 20) {
				...
				if ($ck['masa_kerja'] < 4) {
					...
					$result['pengajuan'] = $this->db->insert('e_eaf.e_pengajuan_copy1', $pengajuan);
				...
				$entries = array(
					...
					'nama_keperluan'    => $nkeperluan[2],
					// 'nominal_uang'      => str_replace('.', '', $_POST['total']),
					'nominal_uang' => str_replace('.', '', $_POST['total'] ?? 0),
					'note'              => str_replace("'", "`", $note_clear1),
					'tgl_nota'          => $_POST['tgl_nota'],
					// 'id_project' 		=> Null,
					// 'blok' 				=> Null
		...
		// 	$upload_path = '/var/www/trusmiverse/apps/assets/uploads/nota_eaf/';
		// 	// Cek folder ada atau bisa dibuat
		// 	if (!is_dir($upload_path)) {
		// 		if (!mkdir($upload_path, 0775, true)) {
		// 			echo json_encode(['status'=>'error', 'msg'=>"Folder upload tidak bisa dibuat: $upload_path"]);
		// 			return;
		// 		}
		// 	}
		...
		// 		echo json_encode(['status'=>'error', 'msg'=>"Folder tidak bisa ditulis: $upload_path"]);
		// 		return;
		// 	}
		...
		// 		CURLOPT_URL => 'https://n8n.trustcore.id/webhook-test/felisa-nota',
		...
		function upload_nota()
		{
			...
			curl_setopt_array($ch, [
				CURLOPT_URL => 'https://n8n.trustcore.id/webhook/eaf-struk',
				CURLOPT_RETURNTRANSFER => true,
				CURLOPT_POST => true,
				CURLOPT_POSTFIELDS => $postFields,
			]);
			...
			$this->db->insert('e_eaf.temp_nota', $nota);
			$insert_id = $this->db->insert_id();   // << ini yang akan jadi temp_nota_id

			// --- response ke AJAX ---
			echo json_encode([
				'status' => 'success',
				'id'     => $insert_id,  // WAJIB → supaya #temp_nota_id keisi
				'total'  => $nota['total'],
				'msg'    => 'Upload sukses'
			]);
		}
	...
	// 	curl_setopt_array($ch, [
	// 		CURLOPT_URL => 'https://n8n.trustcore.id/webhook-test/felisa-nota', // ⚠️ pakai production URL
	// 		CURLOPT_RETURNTRANSFER => true,
	// 		CURLOPT_POST => true,
	// 		CURLOPT_POSTFIELDS => $postFields,
	// 	]);
	...
	// 	// Decode hasil dari n8n
	// 	$data = json_decode($response, true);

	// 	// if (json_last_error() !== JSON_ERROR_NONE) {
	// 	// 	echo json_encode(['status'=>'error','msg'=>'Response n8n tidak valid','raw'=>$response]);
	// 	// 	return;
	// 	// }
	...
	// 	// Cari total pakai regex
	// 	// $total = null;
	// 	// if ($ocrText && preg_match('/(TOTAL|JUMLAH)(?:\s+BAYAR)?\s*[:\-]?\s*(RP)?\s*([0-9\.\,]+)/i', $ocrText, $m)) {
	// 	// 	$total = preg_replace('/[^\d]/', '', $m[3]);
	// 	// }
	...
	// 		// 'total'  => $data['total'],
	// 		// 'ocr'    => $ocrText,      // teks hasil OCR
	// 		// 'raw'    => $response      // response asli dari n8n
	// 	]);
	// }
