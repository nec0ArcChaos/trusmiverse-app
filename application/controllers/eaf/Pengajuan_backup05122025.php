...
class Pengajuan extends CI_Controller
{
        ...
        if ($this->session->userdata('user_id') != '') {
            ...
            if ($id_kategori == 20) {
                ...
                if ($ck['masa_kerja'] < 4) {
                    ...
                    $result['tipe_pembayaran'] = $this->db->insert(
                        'e_eaf.e_tipe_pembayaran',
                        $pembayaran
                    );
                    ...
                    $result['detail_keperluan'] = $this->db->insert(
                ...
                // Integrasi tour leader - Batik group
                ...
                if (
                    ...
                    // Handle file upload Foto Buku Rekening
                    $foto_buku_rekening = $this->input->post('foto_buku_rekening');
                    if (!empty($foto_buku_rekening)) {
                        define('UPLOAD_DIR_FOTO', './uploads/integrasi_tl/foto_buku_rekening/');

                        $foto_buku_rekening = explode(',', $foto_buku_rekening);
                        $img_foto = str_replace(' ', '+', $foto_buku_rekening[1]);
                        $data_foto = base64_decode($img_foto);
                        $name_foto = uniqid() . '.' . $foto_buku_rekening[0];
                        $file_foto = UPLOAD_DIR_FOTO . $name_foto;
                        $success_foto = file_put_contents($file_foto, $data_foto);

                        if ($success_foto) {
                            $integrasi_tl_data['buku_rekening'] = $name_foto;
                        }
                    }

                    // Handle file upload Lampiran Reservasi 
                    $lampiran_reservasi = $this->input->post('lampiran_reservasi');
                    if (!empty($lampiran_reservasi)) {
                        define('UPLOAD_DIR_LAMPIRAN', './uploads/integrasi_tl/lampiran_reservasi/');

                        $lampiran_reservasi = explode(',', $lampiran_reservasi);
                        $img_lampiran = str_replace(' ', '+', $lampiran_reservasi[1]);
                        $data_lampiran = base64_decode($img_lampiran);
                        $name_lampiran = uniqid() . '.' . $lampiran_reservasi[0];
                        $file_lampiran = UPLOAD_DIR_LAMPIRAN . $name_lampiran;
                        $success_lampiran = file_put_contents($file_lampiran, $data_lampiran);

                        if ($success_lampiran) {
                            $integrasi_tl_data['lampiran_reservasi'] = $name_lampiran;
                        }
                    }

                    $result['integrasi_tl'] = $this->db->insert('e_eaf.integrasi_tl', $integrasi_tl_data);

                    $no_reservasi = $this->input->post('no_reservasi');
                    if (!empty($no_reservasi)) {
                        $db_tour_leader = $this->load->database('db_tour_leader', TRUE);
                        
                        $update_reservasi_data = [
                            'id_pengajuan' => $id_pengajuan,
                        ];
                        
                        $db_tour_leader->where('no_reservasi', $no_reservasi);
                        $result['update_reservasi'] = $db_tour_leader->update('tbl_reservasi', $update_reservasi_data);
                    }
                }
    ...
    {
        $data = $this->model_pengajuan->get_no_reservasi();
