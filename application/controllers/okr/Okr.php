...
class Okr extends CI_Controller
{
    ...
    function __construct()
    {
        ...
        $this->load->database();
        $this->load->model('grd/bt/Model_okr', 'model');
    ...
    public function index()
    {
        ...
        $data['pageTitle'] = "Dashboard OKR";
        $data['css'] = "okr/bt/css";
        $data['js'] = "okr/bt/js_okr";
        $data['content'] = "okr/bt/okr";
        ...
        // $data['goals']  = $this->model->data_goal($divisi, $month, $start, $end);        
    ...
    public function goal_pic()
    {
        ...
        $data = [
            ...
            'pic' => $this->db->query("SELECT
                    ...
                    -- $cond 
                    -- AND e.user_role_id IN (3,4,5,6)
                    AND (e.company_id IN (5)
                    OR (e.company_id IN (1) AND e.department_id = 10))
    ...
    public function save_task()
    {
        ...
        if (!empty($_FILES['evidence']['name'])) {
            $config['upload_path'] = './uploads/okr/evidence/';
            ...
            if ($this->upload->do_upload('evidence')) {
                ...
                $evidence = $fileData['file_name'];
            } else {
                // Kalau gagal upload bisa return error
                $response['save_task'] = false;
        ...
        if ($jenis == 'Weekly') {
            ...
            for ($i = 0; $i < $week; $i++) {
                ...
                $data = [
                    ...
                    'output' => $output[$i],
                    'target' => $target[$i],
                    'evidence' => $evidence, // ✅ masuk DB
                    'created_at' => date("Y-m-d H:i:s"),
                    'created_by' => $this->session->userdata('user_id'),
                    'status' => $id_status,
                ];
                $save_task[] = $this->db->insert('okr_t_tasklist', $data);
                ...
                $this->db->insert('okr_t_tasklist_history', $data_history);
            }
    ...
    public function save_task_milestone()
    {
        ...
        $save_task = $this->db->insert('okr_t_tasklist_milestone', $data);
        ...
        $save_history = $this->db->insert('okr_t_tasklist_milestone_history', $data_history);
    ...
    {
        $config['upload_path'] = 'uploads/okr//evidence/'; // Direktori untuk menyimpan file
        ...
        $hasil['okr_t_tasklist_milestone'] = $this->db->update('okr_t_tasklist_milestone', $data);
        ...
        $hasil['okr_t_tasklist_milestone_history'] = $this->db->insert('okr_t_tasklist_milestone_history', $data_history);
    ...
    public function getMilestones()
    {
        $milestones = $this->db->query("SELECT 
                                            ...
                                            CASE 
                                                WHEN  ROUND(COALESCE(SUM(prg.persen),0) / COALESCE(COUNT(mile.id),0)) < 70 THEN 'FD97A4'
                                                WHEN  ROUND(COALESCE(SUM(prg.persen),0) / COALESCE(COUNT(mile.id),0)) BETWEEN 70 AND 85 THEN 'FFE97B'
                                            ELSE 'BFEC78'
                                            END AS warna,
                                            -- COALESCE(SUM(prg.persen),0) AS progress
                                            ROUND(COALESCE(SUM(prg.persen),0) / COALESCE(COUNT(mile.id),0)) AS progress
    ...
    public function data_grd()
    {
        ...
        $start_last_month = $this->input->post('start_last_month');
        $end_last_month = $this->input->post('end_last_month');

        if (!$start_last_month || !$end_last_month) {
            // fallback: hitung di PHP kalo dari AJAX ga dikirim
            if ($start) {
                $start_last_month = date('Y-m-01', strtotime("-1 month", strtotime($start)));
                $end_last_month = date('Y-m-t', strtotime("-1 month", strtotime($end)));
            } else {
                $start_last_month = date('Y-m-01', strtotime("-1 month"));
                $end_last_month = date('Y-m-t', strtotime("-1 month"));
            }
        }
        ...
        //okr last month
        $data_goal_last_month = $this->model->data_goal($divisi, $start_last_month, $end_last_month);
        $data_so_last_month = $this->model->data_so($divisi, $start_last_month, $end_last_month);
        $data_si_last_month = $this->model->data_si($divisi, $start_last_month, $end_last_month);
        $data_tasklist_last_month = $this->model->data_tasklist($divisi, $start_last_month, $end_last_month);
        ...
        foreach ($data_goal as $dt_goal) {
            ...
            //START LAST MONTH
            $get_goal_so_last = array_filter($data_so_last_month, function ($so) use ($dt_goal) {
                return $so->id_goal === $dt_goal->id_goal;
            });

            foreach ($get_goal_so_last as $main) {
                $main->data_si = array_values(array_filter($data_si_last_month, function ($si) use ($main) {
                    return $si->id_so === $main->id_so;
                }));

                foreach ($main->data_si as $si) {
                    $si->tasklist = array_values(array_filter($data_tasklist_last_month, function ($tasklist) use ($si) {
                        return $tasklist->id_si === $si->id_si;
                    }));
                }
            }

            $total_progress_so_last = array_sum(array_column($get_goal_so_last, 'progress_so_satuan'));
            $count_so_last = count($get_goal_so_last);
            $average_so_last = $count_so_last > 0 ? $total_progress_so_last / $count_so_last : 0;
            $dt_goal->average_so_last_month = round($average_so_last, 1);
            //END LAST MONTH
            ...
            $result[] = [
                ...
                // LAST MONTH
                'total_progress_so_last' => $total_progress_so_last,
                'count_so_last' => $count_so_last,
                'progress_so_last' => min(100, $dt_goal->average_so_last_month) ?? 0,
                'progress_si_last_month' => min(100, $dt_goal->progress_si_last_month) ?? 0,

                // HISTORY buat frontend
                'history' => [
                    [
                        'progress_goal' => $dt_goal->prs ?? 0, // corporate (bulan ini)
                        'avg_project' => min(100, $dt_goal->average_so) ?? 0, // project (bulan ini)
                        'avg_objective' => min(100, $dt_goal->progress_si) ?? 0, // objective (bulan ini)
                    ],
                    [
                        'progress_goal' => $dt_goal->progress_goal_last_month ?? 0, // corporate (bulan lalu)
                        'avg_project' => min(100, $dt_goal->average_so_last_month) ?? 0, // project (bulan lalu)
                        'avg_objective' => min(100, $dt_goal->progress_si_last_month) ?? 0, // objective (bulan lalu)
                    ]
                ]
            ];
    ...
    public function progress()
    {
        $sql = "SELECT 
    task.id_tasklist,
    task.detail AS task_name,
    COALESCE(task.target, 0) AS target,
    COALESCE(task.actual, 0) AS actual,
    CASE
        WHEN COALESCE(task.target,0)=0 THEN 0
        WHEN ROUND((COALESCE(task.actual,0)/task.target)*100,1) > 100 THEN 100
        WHEN ROUND((COALESCE(task.actual,0)/task.target)*100,1) <= 0 THEN 0
        ELSE ROUND((COALESCE(task.actual,0)/task.target)*100,1)
    END AS acv,
    task.status
FROM okr_t_tasklist task
ORDER BY task.id_tasklist";

        $data['tasks'] = $this->db->query($sql)->result_array();
        $this->load->view('okr/bt/_content/progress_view', $data);
    ...
    public function update_task()
    {
        ...
        $this->db->where('id_tasklist', $id_tasklist);
        $hasil = $this->db->update('okr_t_tasklist', $data);
    ...
    public function update_si()
    {
        ...
        $hasil = $this->db->update('okr_t_objective', $data);
        ...
        $this->db->insert('okr_t_objective_history', $data_history);
    ...
    public function update_so()
    {
        ...
        $hasil = $this->db->update('okr_m_project', $data);
        ...
        $this->db->insert('okr_m_project_history', $data_history);
    ...
    public function data_warning_manpower()
    {
        ...
        $data = [
            ...
            'header' => $this->model_manpower->get_warning_manpower($start, $end),
            'kehadiran' => $this->model_manpower->get_warning_manpower_kehadiran($start, $end),
            'task_undone' => $this->model_manpower->get_warning_manpower_undone($start, $end),
            'detail' => $this->model_manpower->get_warning_manpower_detail($start, $end),
        ];
        echo json_encode($data);
    }
    ...
    public function data_persen_bt_budget()
    {
        ...
        $data = [
            ...
            'prs_bt_budget' => $this->model->get_budget_vs_actual($start, $end)
        ];
    ...
    function hitungPencapaian($target, $actual, $actual_tipe, $target_tipe)
    {
        ...
        if ($actual_tipe === 'up') {
            ...
            if ($actual == 0) {
                $pencapaian = round($target * 100);
            } else {
                $pencapaian = round(($target / $actual) * 100);
            }
    ...
    public function save_so()
    {
        ...
        $actual_so_type = $this->input->post('so_actual_so_type') ?? '';
        $id_status = 1;
        ...
        $data = [
            ...
            'actual_so_type' => $actual_so_type,
            'created_at' => date("Y-m-d H:i:s"),
            'created_by' => $this->session->userdata('user_id'),
            'status' => $id_status,
        ];

        $save_m_so = $this->db->insert('okr_m_project', $data);
    ...
    public function save_si()
    {
        ...
        $target_output = $this->input->post('si_target_output') ?? '';
        $target_outcome = $this->input->post('si_target_outcome') ?? '';
        ...
        $actual_si_type = $this->input->post('si_actual_si_type') ?? '';
        $id_status = 1;
        ...
        $data = [
            ...
            'actual_si_type' => $actual_si_type,
            'created_at' => date("Y-m-d H:i:s"),
            'created_by' => $this->session->userdata('user_id'),
            'status' => $id_status,
        ];

        $save_m_si = $this->db->insert('okr_t_objective', $data);
    ...
    public function update_tasklist_grd()
    {
        ...
        $response['update'] = $this->db->update('okr_t_tasklist', $data_update);
        ...
        $response['insert'] = $this->db->insert('okr_t_tasklist_history', $data_insert);
