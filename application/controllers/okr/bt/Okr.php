...
class Okr extends CI_Controller
{
    ...
    public function data_grd()
    {
        ...
        foreach ($data_goal as $dt_goal) {
            ...
            /** ================= THIS MONTH (Objective) ================= **/
            $totalSI = 0;
            $totalProgressSI = 0;

            foreach ($get_goal_so as $so) {
                if (!empty($so->data_si) && is_array($so->data_si)) {
                    foreach ($so->data_si as $si) {
                        $totalProgressSI += $si->acv ?? 0;
                        $totalSI++;
                    }
                }
            }
            $avgObjectiveThis = $totalSI > 0 ? $totalProgressSI / $totalSI : 0;

            /** ================= LAST MONTH (Objective) ================= **/
            /** ==================== LAST MONTH ==================== **/
            $get_goal_so_last = array_filter($data_so_last_month, function ($so) use ($dt_goal) {
                return $so->id_goal === $dt_goal->id_goal;
            });

            // Hitung Objective Last Month
            $totalSI_last = 0;
            $totalProgressSI_last = 0;

            foreach ($get_goal_so_last as $so_last) {
                if (!empty($so_last->data_si) && is_array($so_last->data_si)) {
                    foreach ($so_last->data_si as $si_last) {
                        $totalProgressSI_last += $si_last->acv ?? 0;
                        $totalSI_last++;
                    }
                }
            }

            $avgObjectiveLast = $totalSI_last > 0 ? $totalProgressSI_last / $totalSI_last : 0;
            ...
            $result[] = [
                ...
                'progress_si' => round($avgObjectiveThis, 1),  // pakai hasil hitungan barusan
                'progress_tasklist' => min(100, $dt_goal->progress_tasklist) ?? 0,

                // LAST MONTH
                'sos_last_month' => array_values($get_goal_so_last),
                'progress_so_last' => round($average_so_last, 1),
                'progress_si_last' => round($avgObjectiveLast, 1) // tambahin juga
            ];
 ...
 public function update_task()
{
    ...
    // Ambil task lama
    $task = $this->db->get_where('okr_t_tasklist', ['id_tasklist' => $id_tasklist])->row();
    if (!$task) {
    ...
    if ($task->status == 3) {
        $status_final = 3; // tetap Done
    } else {
        $status_final = $status_post ?? $task->status; // update kalau dikirim
    }
    ...
    // Jika status final Done, set done_at
    if ($status_final == 3 && $task->status != 3) {
        $data['done_at'] = date('Y-m-d H:i:s');
    }
    ...
    $data_history = [
        ...
        'progress' => $this->input->post('actual'),
        'status' => $status_final,
        'status_before' => $task->status, // ambil status lama
        'note' => $this->input->post('note'),
        'created_at' => date('Y-m-d H:i:s'),
        'created_by' => $this->session->userdata('user_id'),
    ];
